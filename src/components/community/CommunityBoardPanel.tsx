...
function normalizeRealtimePost(
  raw: Record<string, unknown>,
): CommunityPostRow | null {
...
export function CommunityBoardPanel() {
  ...
  const loadPage = useCallback(
    async (start: number, append: boolean) => {
      ...
      try {
        ...
        const { counts, mine } = buildReactionMaps(
          ids,
          (reactRows || []) as {
            post_id: string;
            type: string;
        ...
        const commentCounts = aggregateComments(
          ids,
          (commentRows || []) as { post_id: string }[],
        );
        const next = rowsToFeedPosts(
          list,
          authors,
          counts,
          mine,
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "common", table: "posts" },
        async (payload) => {
          const row = normalizeRealtimePost(
      ...
      .on(
        "postgres_changes",
        { event: "DELETE", schema: "common", table: "posts" },
        (payload) => {
  ...
  const onUpdateFeedPost = useCallback(
    (postId: string, updater: (fp: FeedPost) => FeedPost) => {
      setPosts((prev) =>
        prev.map((p) => (p.post.id === postId ? updater(p) : p)),
      );
    },
    [],
  );
  ...
  const onIncrementCommentCount = useCallback((postId: string) => {
    setPosts((prev) =>
      prev.map((p) =>
        p.post.id === postId ? { ...p, commentCount: p.commentCount + 1 } : p,
      ),
    );
  }, []);
