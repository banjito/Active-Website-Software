...
-- Modified with IF NOT EXISTS for policies to prevent errors
...
DROP POLICY IF EXISTS "Allow authenticated users to view equipment" ON neta_ops.equipment;
DROP POLICY IF EXISTS "Allow users to insert equipment" ON neta_ops.equipment;
DROP POLICY IF EXISTS "Allow users to update equipment" ON neta_ops.equipment;

DROP POLICY IF EXISTS "Allow authenticated users to view calibrations" ON neta_ops.calibrations;
DROP POLICY IF EXISTS "Allow users to insert calibrations" ON neta_ops.calibrations;
DROP POLICY IF EXISTS "Allow users to update calibrations" ON neta_ops.calibrations;

DROP POLICY IF EXISTS "Allow authenticated users to view procedures" ON neta_ops.procedures;
DROP POLICY IF EXISTS "Allow users to insert procedures" ON neta_ops.procedures;
DROP POLICY IF EXISTS "Allow users to update procedures" ON neta_ops.procedures;

DROP POLICY IF EXISTS "Allow authenticated users to view certificates" ON neta_ops.certificates;
DROP POLICY IF EXISTS "Allow users to insert certificates" ON neta_ops.certificates;
DROP POLICY IF EXISTS "Allow users to update certificates" ON neta_ops.certificates;

DROP POLICY IF EXISTS "Allow authenticated users to view quality_metrics" ON neta_ops.quality_metrics;
DROP POLICY IF EXISTS "Allow users to insert quality_metrics" ON neta_ops.quality_metrics;
DROP POLICY IF EXISTS "Allow users to update quality_metrics" ON neta_ops.quality_metrics;

DROP POLICY IF EXISTS "Allow authenticated users to view equipment_assignments" ON neta_ops.equipment_assignments;
DROP POLICY IF EXISTS "Allow users to insert equipment_assignments" ON neta_ops.equipment_assignments;
DROP POLICY IF EXISTS "Allow users to update equipment_assignments" ON neta_ops.equipment_assignments;

DROP POLICY IF EXISTS "Allow authenticated users to view maintenance_records" ON neta_ops.maintenance_records;
DROP POLICY IF EXISTS "Allow users to insert maintenance_records" ON neta_ops.maintenance_records;
DROP POLICY IF EXISTS "Allow users to update maintenance_records" ON neta_ops.maintenance_records;

DROP POLICY IF EXISTS "Allow authenticated users to view vehicles" ON neta_ops.vehicles;
DROP POLICY IF EXISTS "Allow users to insert vehicles" ON neta_ops.vehicles;
DROP POLICY IF EXISTS "Allow users to update vehicles" ON neta_ops.vehicles;
...
DROP TRIGGER IF EXISTS update_equipment_timestamp ON neta_ops.equipment;
CREATE TRIGGER update_equipment_timestamp
BEFORE UPDATE ON neta_ops.equipment
FOR EACH ROW
EXECUTE FUNCTION update_timestamp();

DROP TRIGGER IF EXISTS update_calibrations_timestamp ON neta_ops.calibrations;
CREATE TRIGGER update_calibrations_timestamp
BEFORE UPDATE ON neta_ops.calibrations
FOR EACH ROW
EXECUTE FUNCTION update_timestamp();

DROP TRIGGER IF EXISTS update_procedures_timestamp ON neta_ops.procedures;
CREATE TRIGGER update_procedures_timestamp
BEFORE UPDATE ON neta_ops.procedures
FOR EACH ROW
EXECUTE FUNCTION update_timestamp();

DROP TRIGGER IF EXISTS update_certificates_timestamp ON neta_ops.certificates;
CREATE TRIGGER update_certificates_timestamp
BEFORE UPDATE ON neta_ops.certificates
FOR EACH ROW
EXECUTE FUNCTION update_timestamp();

DROP TRIGGER IF EXISTS update_quality_metrics_timestamp ON neta_ops.quality_metrics;
CREATE TRIGGER update_quality_metrics_timestamp
BEFORE UPDATE ON neta_ops.quality_metrics
FOR EACH ROW
EXECUTE FUNCTION update_timestamp();

DROP TRIGGER IF EXISTS update_equipment_assignments_timestamp ON neta_ops.equipment_assignments;
CREATE TRIGGER update_equipment_assignments_timestamp
BEFORE UPDATE ON neta_ops.equipment_assignments
FOR EACH ROW
EXECUTE FUNCTION update_timestamp();

DROP TRIGGER IF EXISTS update_maintenance_records_timestamp ON neta_ops.maintenance_records;
CREATE TRIGGER update_maintenance_records_timestamp
BEFORE UPDATE ON neta_ops.maintenance_records
FOR EACH ROW
EXECUTE FUNCTION update_timestamp();

DROP TRIGGER IF EXISTS update_vehicles_timestamp ON neta_ops.vehicles;
