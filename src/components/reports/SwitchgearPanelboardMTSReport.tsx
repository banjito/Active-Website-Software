...
const SwitchgearPanelboardMTSReport: React.FC = () => {
  ...
  const persistReport = React.useCallback(async () => {
      ...
      .upsert(
        {
          ...
          job_id: jobId,
          user_id: user.id,
          report_data: formDataRef.current,
        },
        { onConflict: "id" },
      );
    if (error) throw error;

    // Idempotent: creates the asset + job link the first time and repairs a
    // report that lost it, so it is safe to call on every save.
    await ensureReportAsset(reportId);
    ...
    return reportId;
  }, [jobId, user?.id, reportSlug, ensureReportAsset]);
