...
const LowVoltageCircuitBreakerElectronicTripATSSecondaryInjectionReport: React.FC = () => {
  ...
  const loadReport = async () => {
    ...
    try {
      // First try generic normalized JSONB table
      ...
      if (generic && generic.data) {
        ...
        setFormData(prev => ({
          ...
          // Trip Testing (secondaryInjection)
          tripTesting: d.secondaryInjection ? {
            ...prev.tripTesting,
            testedSettings: {
              longTime: { ...prev.tripTesting.testedSettings.longTime, ...(d.secondaryInjection.testedSettings?.longTime || {}) },
              shortTime: { ...prev.tripTesting.testedSettings.shortTime, ...(d.secondaryInjection.testedSettings?.shortTime || {}) },
              instantaneous: { ...prev.tripTesting.testedSettings.instantaneous, ...(d.secondaryInjection.testedSettings?.instantaneous || {}) },
              groundFault: { ...prev.tripTesting.testedSettings.groundFault, ...(d.secondaryInjection.testedSettings?.groundFault || {}) },
            },
            secondaryInjection: {
              longTime: { ...prev.tripTesting.secondaryInjection.longTime, ...(d.secondaryInjection.secondaryInjection?.longTime || {}) },
              shortTime: { ...prev.tripTesting.secondaryInjection.shortTime, ...(d.secondaryInjection.secondaryInjection?.shortTime || {}) },
              instantaneous: { ...prev.tripTesting.secondaryInjection.instantaneous, ...(d.secondaryInjection.secondaryInjection?.instantaneous || {}) },
              groundFault: { ...prev.tripTesting.secondaryInjection.groundFault, ...(d.secondaryInjection.secondaryInjection?.groundFault || {}) },
            }
          } : prev.tripTesting,
        ...
        <div className="max-w-7xl w-full space-y-6">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-x-4 gap-y-2 print:hidden">
              ...
              <div className="md:col-span-2"><label htmlFor="address" className="form-label">Address:</label><input id="address" type="text" value={formData.address} onChange={(e) => handleChange('address', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
              ...
              <table className="w-full border-collapse border border-gray-300 dark:border-gray-600 table-fixed visual-mechanical-table">
                ...
                <thead className="bg-gray-50 dark:bg-dark-150">
                  <tr>
                    ...
                    <th className="border border-gray-300 dark:border-gray-600 px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Result</th>
                  </tr>
                </thead>
          ...
          <div className="mb-6 print:hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div><label className="form-label">Megohmmeter:</label><input type="text" value={formData.testEquipment.megohmmeter.name} onChange={(e) => handleChange('testEquipment.megohmmeter.name', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
              ...
              <div><label className="form-label">Low-Res Ohmmeter:</label><input type="text" value={formData.testEquipment.lowResistanceOhmmeter.name} onChange={(e) => handleChange('testEquipment.lowResistanceOhmmeter.name', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
