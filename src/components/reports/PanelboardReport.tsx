...
const PanelboardReport: React.FC = () => {
  ...
  useEffect(() => {
    setFormData(prev => {
      ...
      if (!updated.insulationResistanceTests || updated.insulationResistanceTests.length === 0) {
        ...
        updated.insulationResistanceTests = updated.insulationResistanceTests.map(test => ({
          values: {
            ag: test.values?.ag || '',
            bg: test.values?.bg || '',
            cg: test.values?.cg || '',
      ...
      if (!updated.temperatureCorrectedTests || updated.temperatureCorrectedTests.length === 0) {
        updated.temperatureCorrectedTests = [{
          values: { ag: '', bg: '', cg: '', ab: '', bc: '', ca: '', an: '', bn: '', cn: '' }
        }];
      } else {
        updated.temperatureCorrectedTests = updated.temperatureCorrectedTests.map(test => ({
          values: {
            ag: test.values?.ag || '',
            bg: test.values?.bg || '',
            cg: test.values?.cg || '',
      ...
      if (!updated.contactResistanceTests || updated.contactResistanceTests.length === 0) {
        ...
        updated.contactResistanceTests = updated.contactResistanceTests.map(test => ({
          busSection: test.busSection || '',
          values: {
            aPhase: test.values?.aPhase || '',
            bPhase: test.values?.bPhase || '',
  ...
  const calculateCorrectedValue = (value: string) => {
    ...
    if (typeof value === 'string' && (value.includes('>') || value.includes('<') || value.toUpperCase() === 'N/A')) {
      return value;
    }
    ...
    <ReportWrapper isPrintMode={isPrintMode}>
      ...
      <div className={`flex justify-between items-center mb-6 ${isPrintMode ? 'hidden' : ''} print:hidden`}>
        ...
        <div className="flex gap-2 items-center">
          ...
          {/* Locked state: hide Edit, still allow Print */}
          {currentReportId && !isEditing && locked && (
            <button
              onClick={() => window.print()}
              className="px-4 py-2 text-sm text-white bg-gray-600 hover:bg-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-4 print:hidden nameplate-onscreen">
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">Catalog No.</label>
              <input
                type="text"
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">System Voltage (V)</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">Rated Voltage (V)</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">Rated Current (A)</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">Phase Configuration</label>
              <input
                type="text"
      ...
      <div className="mb-6">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 visual-mechanical-table table-fixed">
            ...
            <thead>
              <tr>
                <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">ID</th>
            ...
            <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-200 dark:divide-gray-700">
              {formData.visualInspectionItems.map((item, index) => (
                <tr key={item.id}>
                  <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-white text-center">{item.id}</td>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (!newTests[0].values) {
                            newTests[0].values = { ag: '', bg: '', cg: '', ab: '', bc: '', ca: '', an: '', bn: '', cn: '' };
                          }
                ...
                {['ag', 'bg', 'cg', 'ab', 'bc', 'ca', 'an', 'bn', 'cn'].map((key) => (
                  <td key={key} className="px-3 py-2">
                    <div className="print:hidden">
                      <input
                        type="text"
                        value={calculateCorrectedValue(formData.insulationResistanceTests?.[0]?.values?.[key] || '')}
                        readOnly
                    ...
                    <div className="hidden print:block text-center">{calculateCorrectedValue(formData.insulationResistanceTests?.[0]?.values?.[key] || '')}</div>
                  </td>
                ))}
                <td className="px-3 py-2">
                  <div className="print:hidden">
      ...
      <div className="mb-6">
        ...
        <div className="flex justify-end mb-2">
          <div className="flex items-center space-x-2">
            <span className="text-sm font-medium text-gray-700 dark:text-white">Test Amperage:</span>
            <select
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (!newTests[index].values) {
                              newTests[index].values = { aPhase: '', bPhase: '', cPhase: '', neutral: '', ground: '' };
                            }
      ...
      <div className="mb-6">
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 print:hidden test-eqpt-onscreen">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white mb-2">Megohmmeter</label>
              <EquipmentAutocomplete
                value={formData.testEquipment.megohmmeter.name}
                onChange={(value) => handleNestedChange('testEquipment', 'megohmmeter', 'name', value)}
                onSelect={(equipment) => {
                  ...
                  handleNestedChange('testEquipment', 'megohmmeter', 'name', equipment.equipment_name);
                  handleNestedChange('testEquipment', 'megohmmeter', 'serialNumber', equipment.serial_number || '');
                  handleNestedChange('testEquipment', 'megohmmeter', 'ampId', equipment.amp_id || '');
                  handleNestedChange('testEquipment', 'megohmmeter', 'calDate', formatLocalDateShort(equipment.calibration_date));
                }}
                readOnly={!isEditing}
                className="mt-1"
              />
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white mb-2">Serial Number</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white mb-2">AMP ID</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white mb-2">Cal Date</label>
              <input
                type="text"
                value={formData.testEquipment.megohmmeter.calDate}
                onChange={(e) => handleNestedChange('testEquipment', 'megohmmeter', 'calDate', e.target.value)}
                readOnly={!isEditing}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 print:hidden test-eqpt-onscreen mt-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white mb-2">Low-Resistance Ohmmeter</label>
              <EquipmentAutocomplete
                value={(formData.testEquipment as any).lowResistanceOhmmeter?.name || ''}
                onChange={(value) => setFormData(prev => ({
                  ...prev,
                  testEquipment: {
                    ...prev.testEquipment,
                    lowResistanceOhmmeter: { ...((prev.testEquipment as any).lowResistanceOhmmeter || { serialNumber: '', ampId: '', calDate: '' }), name: value } as any
                  }
                }))}
                onSelect={(equipment) => {
                  const formatDate = (dateString: string | null): string => {
            ...
            <div>
              ...
              <input
                type="text"
                value={(formData.testEquipment as any).lowResistanceOhmmeter?.serialNumber || ''}
                onChange={(e) => setFormData(prev => ({
                  ...prev,
                  testEquipment: {
                    ...prev.testEquipment,
                    lowResistanceOhmmeter: { ...((prev.testEquipment as any).lowResistanceOhmmeter || { name: '', ampId: '', calDate: '' }), serialNumber: e.target.value } as any
                  }
                }))}
                readOnly={!isEditing}
                placeholder="Enter Serial Number"
            ...
            <div>
              ...
              <input
                type="text"
                value={(formData.testEquipment as any).lowResistanceOhmmeter?.ampId || ''}
                onChange={(e) => setFormData(prev => ({
                  ...prev,
                  testEquipment: {
                    ...prev.testEquipment,
                    lowResistanceOhmmeter: { ...((prev.testEquipment as any).lowResistanceOhmmeter || { name: '', serialNumber: '', calDate: '' }), ampId: e.target.value } as any
                  }
                }))}
                readOnly={!isEditing}
                placeholder="Enter AMP ID"
            ...
            <div>
              ...
              <input
                type="text"
                value={(formData.testEquipment as any).lowResistanceOhmmeter?.calDate || ''}
                onChange={(e) => setFormData(prev => ({
                  ...prev,
                  testEquipment: {
                    ...prev.testEquipment,
                    lowResistanceOhmmeter: { ...((prev.testEquipment as any).lowResistanceOhmmeter || { name: '', serialNumber: '', ampId: '' }), calDate: e.target.value } as any
                  }
                }))}
                readOnly={!isEditing}
                placeholder="Enter Cal Date"
          ...
          <div className="hidden print:block">
            <table className="w-full table-fixed border-collapse border border-gray-300 print:border-black print:border text-[0.85rem] test-eqpt-print">
              ...
              <tbody>
                <tr>
                  <td className="p-2 align-middle border border-gray-300 print:border-black print:border text-center">
                    <div className="font-semibold text-center">Megohmmeter:</div>
                    <div className="mt-0 text-center">{formData.testEquipment.megohmmeter.name || ''}</div>
                  </td>
                  <td className="p-2 align-middle border border-gray-300 print:border-black print:border text-center">
                    <div className="font-semibold text-center">Serial Number:</div>
                    <div className="mt-0 text-center">{formData.testEquipment.megohmmeter.serialNumber || ''}</div>
                  </td>
                  <td className="p-2 align-middle border border-gray-300 print:border-black print:border text-center">
                    <div className="font-semibold text-center">AMP ID:</div>
                    <div className="mt-0 text-center">{formData.testEquipment.megohmmeter.ampId || ''}</div>
                  </td>
                  <td className="p-2 align-middle border border-gray-300 print:border-black print:border text-center">
                    <div className="font-semibold text-center">Cal Date:</div>
                    <div className="mt-0 text-center">{formData.testEquipment.megohmmeter.calDate || ''}</div>
                  </td>
                </tr>
                <tr>
                  <td className="p-2 align-middle border border-gray-300 print:border-black print:border text-center">
                    <div className="font-semibold text-center">Low-Resistance Ohmmeter:</div>
                    <div className="mt-0 text-center">{((formData.testEquipment as any).lowResistanceOhmmeter?.name) || ''}</div>
                  </td>
                  <td className="p-2 align-middle border border-gray-300 print:border-black print:border text-center">
                    <div className="font-semibold text-center">Serial Number:</div>
                    <div className="mt-0 text-center">{((formData.testEquipment as any).lowResistanceOhmmeter?.serialNumber) || ''}</div>
                  </td>
                  <td className="p-2 align-middle border border-gray-300 print:border-black print:border text-center">
                    <div className="font-semibold text-center">AMP ID:</div>
                    <div className="mt-0 text-center">{((formData.testEquipment as any).lowResistanceOhmmeter?.ampId) || ''}</div>
                  </td>
                  <td className="p-2 align-middle border border-gray-300 print:border-black print:border text-center">
                    <div className="font-semibold text-center">Cal Date:</div>
                    <div className="mt-0 text-center">{((formData.testEquipment as any).lowResistanceOhmmeter?.calDate) || ''}</div>
                  </td>
                </tr>
              </tbody>
            </table>
