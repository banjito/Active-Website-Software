...
export function TableFooter({ className = "", ...props }: TableFooterProps) {
  return (
    <tfoot
      className={`border-t bg-dark-primary/5 dark:bg-dark-150 font-medium [&>tr]:last:border-b-0 ${className}`}
      {...props}
    />
  );
}
...
export function TableContainer({
  ...
  return (
    <div
      className={`w-full overflow-hidden rounded-none border-2 border-dark-accent/30 dark:border-dark-accent/20 bg-white dark:bg-dark-150 shadow-sm ${className}`}
      {...props}
    >
      {children}
    </div>
...
export function EmptyTableRow({ colSpan = 1 }: { colSpan?: number }) {
  return (
    <tr>
      <td
        colSpan={colSpan}
        className="h-24 text-center text-dark-primary/60 dark:text-dark-secondary/60"
      >
        No results.
      </td>
    </tr>
