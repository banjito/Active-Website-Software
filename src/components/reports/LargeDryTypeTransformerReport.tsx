            ...
            <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-x-4 gap-y-2 print:hidden job-info-onscreen">
              ...
              <div className="flex items-center space-x-1">
                <div>
                  ...
                  <input id="temperature.ambient" type="number" value={formData.temperature.ambient} onChange={(e) => handleTemperatureChange(Number(e.target.value))} readOnly={!isEditing} className={`form-input w-20 ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
              ...
              <div><label htmlFor="humidity" className="form-label">Humidity %:</label><input id="humidity" type="number" value={formData.temperature.humidity || ''} onChange={(e) => handleChange('temperature', 'humidity', e.target.value === '' ? null : Number(e.target.value))} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} placeholder="Optional" /></div>
            </div>
  ...
  style.textContent = `
    ...
    @media print {
      * { 
      ...
      /* Force hide test equipment on-screen form in print with higher specificity */
      .section-test-equipment .test-eqpt-onscreen { display: none !important; }
      .section-test-equipment .print\\:hidden { display: none !important; }
      
      /* Nuclear option - hide ALL test equipment on-screen elements in print */
      .section-test-equipment div[class*="grid"] { display: none !important; }
      .section-test-equipment div[class*="md:grid-cols-3"] { display: none !important; }
      .section-test-equipment .space-y-6 > div:first-child { display: none !important; }
      
      /* Ensure only the print table shows in test equipment section */
      .section-test-equipment .hidden.print\\:block { display: table !important; }
      
      /* Force-hide on-screen Job Information grid in print to avoid duplication */
      .job-info-onscreen, .job-info-onscreen * { display: none !important; }
      /* Extra guard: hide any grid/inputs/labels inside the Job Information section */
      .section-job-info + .grid { display: none !important; }
      .section-job-info ~ .grid { display: none !important; }
      .section-job-info ~ div input, .section-job-info ~ div label { display: none !important; }
      
      /* Ensure JobInfoPrintTable (hidden print:block) is visible when printing */
