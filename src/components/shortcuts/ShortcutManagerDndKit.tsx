...
export const ShortcutManagerDndKit: React.FC<ShortcutManagerProps> = ({ isOpen, onClose }) => {
  ...
  const handleSaveShortcut = async (e: React.FormEvent) => {
    ...
    if (!isEditing && shortcuts.length >= MAX_SHORTCUTS) {
      setError(`You can have at most ${MAX_SHORTCUTS} shortcuts. Remove one to add another.`);
      return;
    }
  ...
  const quickAddItems = React.useMemo(() => {
    ...
    BUILTIN_PORTALS.forEach(p => {
      p.options.forEach(opt => {
        ...
        if (opt.label.toLowerCase().includes('drag to reorder') || opt.label.toLowerCase().includes('reorder your shortcuts')) return;
        const label = `${p.label} • ${opt.label}`;
        items.push({ key: `${p.key}:${opt.path}`, label, path: opt.path });
      });
    });
  ...
  const handleQuickAdd = async () => {
    ...
    if (slotsLeft <= 0) {
      setError(`You can have at most ${MAX_SHORTCUTS} shortcuts. Remove one to add more.`);
      setQuickAddOpen(false);
      return;
    }
    ...
    try {
      setLoading(true);
      setError(null);
      await ShortcutService.bulkCreateShortcuts(user.id, toAdd.map(c => ({ title: c.label, url: c.path })));
    } finally {
      setLoading(false);
      setQuickAddOpen(false);
      setSelectedQuick({});
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-dark-150 rounded-lg shadow-lg w-full max-w-[500px] max-h-[85vh] flex flex-col overflow-hidden">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <Button
                ...
                title={shortcuts.length >= MAX_SHORTCUTS ? `Maximum ${MAX_SHORTCUTS} shortcuts allowed` : undefined}
              >
        ...
        {/* Scrollable Content Area - when Quick Add or Edit modal is open, don't render the main list so "Drag to reorder" never appears inside the modal */}
        <div className="flex-1 overflow-y-auto px-4 py-3 min-h-0" style={{ scrollBehavior: 'smooth' }}>
          {quickAddOpen || isFormOpen ? (
            <div className="py-6 text-center text-sm text-gray-500 dark:text-gray-400">
              {quickAddOpen ? 'Search and select shortcuts above, then click Add Selected.' : 'Fill in the form to add or edit a shortcut.'}
            </div>
          ) : loading ? (
