...
export async function submitReportForApproval(id: string, userId: string, comments?: string) {
  try {
    ...
    // Allow resubmission from draft, rejected, approved, sent, or archived
    const allowedToSubmit = ['draft', 'rejected', 'approved', 'sent', 'archived'];
    if (!allowedToSubmit.includes(currentReport.status)) {
      throw new Error('Only draft, rejected, approved, sent, or archived reports can be submitted for approval');
    }
    
    // Check if this is a resubmission and increment version if needed
    // Increment version on resubmission from any non-draft state
    const shouldBumpVersion = ['rejected', 'approved', 'sent', 'archived'].includes(currentReport.status);
    const newVersion = shouldBumpVersion
    ...
    // Update linked asset status to 'ready_for_review' and stamp submitted_at
    try {
      ...
      if (assetLinks && assetLinks.length > 0) {
        ...
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('assetStatusChanged', {
            detail: { assetIds, newStatus: 'ready_for_review' }
          }));
        }
      }
    } catch (assetError) {
      console.warn('Failed to update linked asset status:', assetError);
...
export async function reviewReport(approvalRequest: ReportApprovalRequest) {
  try {
    ...
    // If approved, stamp approved_at and issued_at (approved/issued at same time here)
    if (status === 'approved') {
...
export async function markReportAsSent(id: string, userId: string, comments?: string) {
  try {
    ...
    // Update linked assets to sent status with sent_at timestamp
    try {
      const { data: assetLinks, error: linkError } = await supabase
        .schema('neta_ops')
        .from('asset_reports')
      ...
      if (!linkError && assetLinks && assetLinks.length > 0) {
        ...
        if (updateError) {
          console.warn('Warning: Failed to update linked asset status to sent:', updateError);
        }
      }
    } catch (assetError) {
      console.warn('Warning: Error updating linked asset status to sent:', assetError);
    }
...
/**
 * Update the sent_at timestamp for a report (for editing sent date)
 */
export async function updateReportSentDate(reportId: string, newSentDate: string) {
  try {
    const { data, error } = await supabase
      .schema('neta_ops')
      .from('technical_reports')
      .update({ sent_at: newSentDate, updated_at: new Date().toISOString() })
      .eq('id', reportId)
      .select()
      .single();
    ...
    console.error('Error updating report sent date:', error);
    return { data: null, error };
  }
}
