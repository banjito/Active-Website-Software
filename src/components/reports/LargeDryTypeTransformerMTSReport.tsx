...
const LARGE_TRANSFORMER_MTS_TABLE =
  "large_dry_type_transformer_mts_reports" as const;
...
const calculateTurnsRatio = (
  nameplateVoltage: string,
  secondaryVoltage: string,
): string => {
  // If nameplate voltage is empty, 0, or "-", return empty string
  if (!nameplateVoltage || nameplateVoltage === "0" || nameplateVoltage === "-")
    return "";
...
const LargeDryTypeTransformerMTSReport: React.FC = () => {
  ...
  const initialVisualInspectionState = {
    ...
    "7.2.1.2.A.1_comments": "",
    "7.2.1.2.A.2": "Select One",
    "7.2.1.2.A.2_comments": "",
    "7.2.1.2.A.3*": "Select One",
    "7.2.1.2.A.3*_comments": "", // Note asterisk
    "7.2.1.2.A.4": "Select One",
    "7.2.1.2.A.4_comments": "",
    "7.2.1.2.A.5*": "Select One",
    "7.2.1.2.A.5*_comments": "", // Note asterisk
    "7.2.1.2.A.6": "Select One",
    "7.2.1.2.A.6_comments": "",
    "7.2.1.2.A.7": "Select One",
    "7.2.1.2.A.7_comments": "",
    "7.2.1.2.A.8": "Select One",
    "7.2.1.2.A.8_comments": "",
    "7.2.1.2.A.9": "Select One",
    "7.2.1.2.A.9_comments": "",
    "7.2.1.2.A.10": "Select One",
    "7.2.1.2.A.10_comments": "",
    "7.2.1.2.A.11": "Select One",
    "7.2.1.2.A.11_comments": "",
  };
  ...
  const getVisualInspectionDescription = (id: string): string => {
    const descriptions: { [key: string]: string } = {
      ...
      "7.2.1.2.A.5*":
        "Verify that control and alarm settings on temperature indicators are as specified.", // Note asterisk
  ...
  const handleTemperatureChange = (fahrenheit: number) => {
    const closestMatch = tempConvTable.reduce((prev, curr) =>
      Math.abs(curr[0] - fahrenheit) < Math.abs(prev[0] - fahrenheit)
        ? curr
        : prev,
    );
  ...
  const handleChange = (
    ...
    setFormData((prev) => {
      if (section) {
        ...
        return {
          ...prev,
          [section]: { ...(currentSection as object), [field]: value },
        };
      } else {
  ...
  const handleNestedChange = (
    ...
    setFormData((prev) => {
      ...
      return {
        ...prev,
        [section]: { ...(currentSection as object), [subsection]: value },
      };
    });
  };
  ...
  const handleTurnsRatioChange = (
    index: number,
    field: keyof FormData["turnsRatio"]["taps"][0],
    value: string,
  ) => {
    setFormData((prev) => {
      const newTaps = [...prev.turnsRatio.taps];
      const calculatedRatio = calculateTurnsRatio(
        prev.nameplateData.tapConfiguration.voltages[index] || "",
  ...
  useEffect(() => {
    ...
    const updateCalculatedValues = (
      ...
      const dielectricAbsorption = calculateDAPRatio(
        corrected.oneMinute,
        corrected.halfMinute,
      );
      const polarizationIndex = calculateDAPRatio(
        corrected.tenMinute,
        corrected.oneMinute,
      );
    ...
    if (formData.insulationResistance) {
      setFormData((prev) => {
        ...
        if (
          !_.isEqual(
            prev.insulationResistance.primaryToGround.corrected,
            primaryCalcs.corrected,
          ) ||
          !_.isEqual(
            prev.insulationResistance.secondaryToGround.corrected,
            secondaryCalcs.corrected,
          ) ||
          !_.isEqual(
            prev.insulationResistance.primaryToSecondary.corrected,
  ...
  const loadReport = async () => {
    ...
    try {
      const { data, error } = await supabase
        .schema("neta_ops")
        .from(LARGE_TRANSFORMER_MTS_TABLE)
        .select("*")
        .eq("id", reportId)
        .single();
      if (error) {
      ...
      if (data) {
        setFormData((prev) => ({
          ...
          visualInspection: {
            ...initialVisualInspectionState,
            ...(data.visual_inspection?.items ?? {}),
          }, // Merge with initial to ensure all keys
          insulationResistance: {
            ...prev.insulationResistance,
      ...
      const err = error as SupabaseError;
      console.error(
        `Error loading report from ${LARGE_TRANSFORMER_MTS_TABLE}:`,
        err,
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(`Error saving to ${LARGE_TRANSFORMER_MTS_TABLE}:`, error);
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6 dark:text-white">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-4 mt-4 print:hidden nameplate-onscreen">
            <div>
              <label className="form-label">KVA:</label>
              <input
                type="text"
            ...
            <div>
              <label className="form-label">Temp. Rise (°C):</label>
              <input
                type="text"
            ...
            <div>
              <label className="form-label">Impedance (%):</label>
              <input
                type="text"
            ...
            <div className="grid grid-cols-[100px_1fr_1fr_1fr] gap-4 items-center">
              ...
              <div className="flex justify-center items-center space-x-2">
                <input
                  ...
                  readOnly={!isEditing}
                  className={`w-24 text-center form-input ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
                />
          ...
          <div className="mt-6 border-t dark:border-zinc-700 pt-4 print:hidden nameplate-onscreen">
                  ...
                  {formData.nameplateData.tapConfiguration.voltages.map(
                    (voltage, index) => (
                      <input
                        ...
                        readOnly={!isEditing}
                        className={`w-full text-center form-input ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
                ...
                <div className="flex items-center space-x-8">
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      readOnly={!isEditing}
                      className={`w-16 text-center form-input ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
                    />
                    <span className="text-zinc-500 dark:text-white">/</span>
                    <input
                      type="text"
            ...
            <table className="min-w-full border-collapse border border-zinc-300 dark:border-zinc-600">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td className="px-3 py-2 border border-zinc-300 dark:border-zinc-600 text-sm text-zinc-900 dark:text-white text-center">
                    {formData.nameplateData.secondary.material === "Aluminum"
                      ? "☒"
                      : "☐"}{" "}
                    Aluminum
                  </td>
                ...
                {Object.keys(initialVisualInspectionState)
                  ...
                  .map((id) => (
                    <tr key={id}>
                      ...
                      <td className="px-3 py-2 text-sm text-zinc-700 dark:text-white whitespace-normal break-words">
                      ...
                      <td className="px-3 py-2">
                        <div className="print:hidden">
                          <select
                            value={
                              formData.visualInspection[id] || "Select One"
                            }
                            onChange={(e) =>
        ...
        <div className="mb-6">
          ...
          <div className="mb-6">
            <h3 className="text-lg font-semibold mb-4 text-zinc-800 dark:text-white border-b dark:border-zinc-700 pb-2">
              Insulation Resistance Values
            </h3>
            <div className="overflow-x-auto">
                  ...
                  {[
                    ...
                    <tr key={testItem.id}>
                      ...
                      <td className="px-6 py-4">
                        <select
                          value={
                            formData.insulationResistance[testItem.id]
                              ?.testVoltage || ""
                          }
                          onChange={(e) =>
                            handleNestedChange(
                      ...
                      <td className="px-6 py-4">
                        <input
                          type="text"
                          value={
                            formData.insulationResistance[testItem.id]?.readings
                              ?.halfMinute || ""
                          }
                          onChange={(e) =>
                      ...
                      <td className="px-6 py-4">
                        <select
                          value={
                            formData.insulationResistance[testItem.id]?.unit ||
                            "MΩ"
                          }
                          onChange={(e) =>
                            handleNestedChange(
          ...
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                <thead className="bg-zinc-50 dark:bg-dark-150">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-white uppercase tracking-wider w-1/3">
                      Calculated Values
                    </th>
                    <th className="px-6 py-3 text-center text-xs font-medium text-zinc-500 dark:text-white uppercase tracking-wider">
                      Primary
        ...
        <div className="mb-6">
          <div className="w-full h-1 bg-[#f26722] mb-4"></div>
          <h2 className="text-xl font-semibold mb-4 text-zinc-900 dark:text-white border-b dark:border-zinc-700 pb-2 print:text-black print:border-black print:font-bold section-turns-ratio">
            Electrical Tests - Turns Ratio
          </h2>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center">
                ...
                <input
                  ...
                  readOnly={!isEditing}
                  className={`w-20 text-center form-input ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
                />
                <span className="ml-2 text-sm font-medium text-zinc-700 dark:text-white">
                  V
                </span>
              </div>
            </div>
            <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700 turns-ratio-table">
              <thead className="bg-zinc-50 dark:bg-dark-150">
                <tr>
                  <th className="px-6 py-3 text-center text-xs font-medium text-zinc-500 dark:text-white uppercase tracking-wider">
                    Tap
              ...
              <tbody className="bg-white dark:bg-dark-150 divide-y divide-zinc-200 dark:divide-zinc-700">
                {formData.turnsRatio.taps.map((tapData, index) => {
                  ...
                  const phaseA_Dev = calculateDeviation(
                    calculatedRatio,
                    tapData.phaseA_TTR,
                  );
                  const phaseB_Dev = calculateDeviation(
                    calculatedRatio,
                    tapData.phaseB_TTR,
                  );
                  const phaseC_Dev = calculateDeviation(
                    calculatedRatio,
                    tapData.phaseC_TTR,
                  );
                  ...
                  return (
                    <tr key={index}>
                      <td className="px-6 py-4 text-center text-sm text-zinc-900 dark:text-white">
                        {tapData.tap}
                      </td>
                      <td className="px-6 py-4">
                        <input
                          ...
                          readOnly
                          className="w-full text-center form-input bg-zinc-100 dark:bg-dark-150"
                        />
                      </td>
                      <td className="px-6 py-4">
                        <input
