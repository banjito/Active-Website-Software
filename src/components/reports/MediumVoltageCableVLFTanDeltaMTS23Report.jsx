...
const getTCF = (celsius) => {
  ...
  const exactMatch = tempFactors.find((tf) => tf.temp === c);
  if (exactMatch) return exactMatch.factor;

  // Interpolate between closest values
  const lowerFactor = tempFactors.filter((tf) => tf.temp < c).pop();
  const upperFactor = tempFactors.find((tf) => tf.temp > c);
...
const MediumVoltageCableVLFTanDeltaMTS23Report = () => {
  ...
  const isMTSRoute = true;
  const reportSlug = "medium-voltage-cable-vlf-tan-delta-mts23";
  ...
  const [formData, setFormData] = useState({
    ...
    visualInspection: {
      ...
      inspectFireproofing: InspectionResult.SELECT, // 7.3.3.A.6
      inspectCurrentTransformers: InspectionResult.SELECT, // 7.3.3.A.7
      compareCableData: InspectionResult.SELECT, // 7.3.3.A.8
    },
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (!error) {
          ...
          setError(
            `Error loading data: ${(err && err.message) || "Unknown error"}`,
          );
        }
      } finally {
  ...
  const loadJobInfo = async (currentJobId) => {
    ...
    try {
      ...
      let customerName = "";
      let customerAddress = "";
      // Prefer site address from the job; fallback to customer address if missing
      let siteAddress = jobData.site_address || "";
      ...
      if (jobData?.customer_id) {
        ...
        try {
          ...
          if (customerError) {
            ...
            if (!siteAddress) siteAddress = customerData.address || "";
          }
        } catch (customerErr) {
      ...
      setError(
        `Failed to load job info: ${(error && error.message) || "Unknown error"}`,
      );
      toast.error(
        `Failed to load job info: ${(error && error.message) || "Unknown error"}`,
      );
    ...
    const chartData =
      tanDeltaValues?.map((value) => {
        ...
        const kV =
          value?.kV !== undefined && value?.kV !== ""
            ? parseFloat(value.kV)
            : parseFloat(formData.tanDeltaTest?.systemVoltageL2G || 14.4) *
              stepFactor;
  ...
  const handleWithstandTestChange = (index, field, value, phase, subfield) => {
    setFormData((prev) => {
      ...
      const withstandTest = JSON.parse(
        JSON.stringify(prev.withstandTest || { readings: [] }),
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (error.message && error.message.includes("permission denied")) {
        ...
        toast.error(
          `Failed to save report: ${(error && error.message) || "Unknown error"}`,
        );
      }
    } finally {
      setIsSaving(false);
  ...
  useEffect(() => {
    ...
    // Correct with the factor we just computed, in the same update. Calling
    // updateCorrectedValues() here read the tcf from the render before this
    // one, so every corrected value used the PREVIOUS temperature's factor.
    setFormData((prev) => {
      const insulationTest = prev.insulationTest;
      if (!insulationTest) {
        return { ...prev, temperature: { ...prev.temperature, tcf } };
      }
      const correct = (row) => ({
        ag: calculateCorrectedValue(row?.ag, tcf),
        bg: calculateCorrectedValue(row?.bg, tcf),
        cg: calculateCorrectedValue(row?.cg, tcf),
      });
      return {
        ...prev,
        ...
        insulationTest: {
          ...insulationTest,
          preTestCorrected: correct(insulationTest.preTest),
          postTestCorrected: correct(insulationTest.postTest),
        },
      };
    });
    ...
    if (typeof document !== "undefined") {
      ...
      style.textContent = `
        ...
        @media print {
          ...
          /* Additional specific styling for Visual & Mechanical Inspection table */
          .vm-standard {
            table-layout: fixed !important;
            width: 100% !important;
          }

          .vm-standard th:first-child,
          .vm-standard td:first-child {
            width: 10% !important;
            min-width: 60px !important;
          }

          .vm-standard th:nth-child(2),
          .vm-standard td:nth-child(2) {
            width: 45% !important;
            min-width: 180px !important;
          }

          .vm-standard th:nth-child(3),
          .vm-standard td:nth-child(3) {
            width: 45% !important;
            min-width: 180px !important;
          }

          /* Force the Result column to have adequate space */
          .vm-standard td:last-child {
            min-width: 180px !important;
            padding-right: 16px !important;
            word-wrap: break-word !important;
          ...
          /* Prevent select text clipping in print */
          .vm-standard td:last-child select {
            width: 100% !important;
            padding: 0 !important;
            font-size: 9px !important; /* Make font smaller so "Not Applicable" fits */
          ...
          * { color: black !important; }
          ...
          #mv-vlf-td-status {
            ...
            border-color: var(--mv-status-border) !important;
  ...
  const loadReport = async () => {
    ...
    try {
      console.log(
        `Attempting to load report from 'neta_ops.medium_voltage_cable_vlf_tan_delta_mts23_reports' table`,
      );
      ...
      if (error) {
        console.error(
          `Error loading from medium_voltage_cable_vlf_tan_delta_mts23_reports:`,
          error,
        );
      ...
      if (data && data.report_data) {
        ...
        // One jsonb blob, like the other MTS 23 report tables.
        const reportData = data.report_data;

        setFormData((prev) => ({
          ...prev,
          ...reportData, // Spread all the report data
          // Preserve fresh job info loaded earlier
          customerName: prev.customerName || reportData.customerName,
          siteAddress: prev.siteAddress || reportData.siteAddress,
          // Ensure specific nested objects are properly handled and merged
          cableInfo: { ...prev.cableInfo, ...reportData.cableInfo },
          terminationData: {
            ...prev.terminationData,
            ...reportData.terminationData,
          },
          visualInspection: {
          ...
          shieldContinuity: {
            ...prev.shieldContinuity,
            ...reportData.shieldContinuity,
          },
          insulationTest: {
            ...prev.insulationTest,
            ...reportData.insulationTest,
          },
          ...
          tanDeltaTest: (() => {
            const raw = reportData.tanDeltaTest;
            const filterOut2Uo = (arr) =>
              (arr || []).filter(
                (v) =>
                  (v?.voltageStep || v?.voltageLabel || "").trim() !== "2.0 Uo",
              );
            const filterPointsOut2Uo = (arr) =>
              (arr || []).filter((p) => p.kV < 1.9);
            const defaultTanDelta = {
              systemVoltageL2G: "14.4",
              testVoltage: "",
              frequency: "0.1",
              values: [
            ...
            if (!raw) return defaultTanDelta;
            const filteredValues = filterOut2Uo(raw.values);
            const filteredPoints = filterPointsOut2Uo(raw.points);
            return {
              ...raw,
              values: filteredValues.length
                ? filteredValues
                : defaultTanDelta.values,
              points:
                raw.points && filteredPoints.length
                  ? filteredPoints
                  : defaultTanDelta.points,
            };
          })(),
        }));
  ...
  return (
    <ReportWrapper isPrintMode={isPrintMode}>
      ...
      <div className="p-6 flex justify-center bg-neutral-50 dark:bg-dark-200">
        <div className="max-w-7xl w-full space-y-2">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                <div className="flex items-center">
                  ...
                  <input
                    ...
                    readOnly={!isEditMode}
                    className={`w-1/2 rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
                  />
                </div>
                <div className="flex items-center">
                  <label className="w-1/2 text-sm font-medium text-neutral-700 dark:text-neutral-300"></label>
                </div>
                <div className="flex items-center">
                  <label className="w-1/2 text-sm font-medium text-neutral-700 dark:text-neutral-300">
                    Manufacturer
          ...
          <section className="mb-6 visual-mechanical-section">
            ...
            <div className="grid grid-cols-1 gap-4 print:hidden">
              <div className="flex items-center">
                <label className="w-3/4 text-sm font-medium text-neutral-700 dark:text-neutral-300">
                  7.3.3.A.1 Inspect exposed sections of cables and connectors
                  for physical damage and evidence of degradation and corona.
                </label>
                <select
                  value={
                    formData.visualInspection?.inspectCablesAndConnectors ||
                    InspectionResult.SELECT
                  }
                  onChange={(e) =>
                    handleChange("visualInspection", {
                      ...formData.visualInspection,
                      inspectCablesAndConnectors: e.target.value,
                    })
                  }
                  disabled={!isEditMode}
                  className={`w-1/4 rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
                >
              ...
              <div className="flex items-center">
                ...
                <select
                  value={
                    formData.visualInspection?.inspectTerminationsAndSplices ||
                    InspectionResult.SELECT
                  }
                  onChange={(e) =>
                    handleChange("visualInspection", {
                      ...formData.visualInspection,
                      inspectTerminationsAndSplices: e.target.value,
                    })
                  }
                  disabled={!isEditMode}
                  className={`w-1/4 rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
              ...
              <div className="flex items-center">
                ...
                <select
                  value={
                    formData.visualInspection?.inspectShieldGrounding ||
                    InspectionResult.SELECT
                  }
                  onChange={(e) =>
                    handleChange("visualInspection", {
                      ...formData.visualInspection,
                      inspectShieldGrounding: e.target.value,
                    })
                  }
                  disabled={!isEditMode}
                  className={`w-1/4 rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
              ...
              <div className="flex items-center">
                ...
                <select
                  value={
                    formData.visualInspection?.verifyBendRadius ||
                    InspectionResult.SELECT
                  }
                  onChange={(e) =>
                    handleChange("visualInspection", {
                      ...formData.visualInspection,
                      verifyBendRadius: e.target.value,
                    })
                  }
                  disabled={!isEditMode}
                  className={`w-1/4 rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
              ...
              <div className="flex items-center">
                <label className="w-3/4 text-sm font-medium text-neutral-700 dark:text-neutral-300">
                  7.3.3.A.6 *Inspect fireproofing in common cable areas.
                </label>
                <select
                  value={
                    formData.visualInspection?.inspectFireproofing ||
                    InspectionResult.SELECT
                  }
                  onChange={(e) =>
                    handleChange("visualInspection", {
                      ...formData.visualInspection,
                      inspectFireproofing: e.target.value,
                    })
                  }
                  disabled={!isEditMode}
                  className={`w-1/4 rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
              ...
              <div className="flex items-center">
                ...
                <select
                  value={
                    formData.visualInspection?.inspectCurrentTransformers ||
                    InspectionResult.SELECT
                  }
                  onChange={(e) =>
                    handleChange("visualInspection", {
                      ...formData.visualInspection,
                      inspectCurrentTransformers: e.target.value,
                    })
                  }
                  disabled={!isEditMode}
                  className={`w-1/4 rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
              ...
              <div className="flex items-center">
                <label className="w-3/4 text-sm font-medium text-neutral-700 dark:text-neutral-300">
                  7.3.3.A.8 Compare cable data with drawings and cable schedule.
                </label>
                <select
                  value={
                    formData.visualInspection?.compareCableData ||
                    InspectionResult.SELECT
                  }
                  onChange={(e) =>
                    handleChange("visualInspection", {
                      ...formData.visualInspection,
                      compareCableData: e.target.value,
                    })
                  }
                  disabled={!isEditMode}
                  className={`w-1/4 rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
              ...
              <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700 vm-standard table-fixed">
                ...
                <thead className="bg-neutral-50 dark:bg-dark-200">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-center text-xs font-medium text-neutral-500 dark:text-neutral-300 uppercase tracking-wider">
                      Result
                    </th>
                  </tr>
                </thead>
                <tbody className="bg-white dark:bg-dark-150 divide-y divide-neutral-200 dark:divide-neutral-700">
                  <tr>
                    ...
                    <td className="px-3 py-2 text-center">
                      <select
                        ...
                        disabled={!isEditMode}
                        className={`w-full rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
                      >
                        {Object.values(InspectionResult).map((result) => (
                          <option key={result} value={result}>
                            {result}
                  ...
                  <tr>
                    ...
                    <td className="px-3 py-2 text-sm text-neutral-700 dark:text-neutral-300">
                      Compare cable data with drawings and cable schedule.
                    </td>
                    <td className="px-3 py-2 text-center">
                      <select
                        value={
          ...
          <section className="mb-6 insulation-resistance-section">
            ...
            <div className="mb-4 flex items-center">
              ...
              <select
                ...
                disabled={!isEditMode}
                className={`w-32 rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
              >
            ...
            <div className="overflow-x-auto print:hidden ir-screen">
              <table className="min-w-full border-collapse border border-neutral-300 dark:border-neutral-700">
                <thead>
                  <tr>
                    <th className="border border-neutral-300 dark:border-neutral-700 px-3 py-2 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase"></th>
                    <th
                      className="border border-neutral-300 dark:border-neutral-700 px-3 py-2 text-center text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider"
                      colSpan={3}
                    >
                  ...
                  <tr>
                    ...
                    <th className="border border-neutral-300 dark:border-neutral-700 px-3 py-2 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider"></th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="border border-neutral-300 dark:border-neutral-700 px-3 py-2 whitespace-nowrap text-sm text-neutral-900 dark:text-white">
                      Pre-Test
                    </td>
                    <td className="border border-neutral-300 dark:border-neutral-700 px-3 py-2 whitespace-nowrap">
                      <input
                    ...
                    <td className="border border-neutral-300 dark:border-neutral-700 px-3 py-2 whitespace-nowrap">
                      <input
                        ...
                        readOnly={true}
                        className="w-full rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm dark:bg-dark-100 dark:text-white bg-neutral-100 dark:bg-dark-200"
                      />
                    </td>
                    <td className="border border-neutral-300 dark:border-neutral-700 px-3 py-2 whitespace-nowrap">
                      <input
            ...
            <div className="hidden print:block overflow-x-auto ir-print">
              <table className="min-w-full border-collapse border border-neutral-300 dark:border-neutral-700 table-fixed">
                ...
                <thead>
                  <tr>
                    <th className="border border-neutral-300 dark:border-neutral-700 px-1 py-1 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase"></th>
                    <th
                      className="border border-neutral-300 dark:border-neutral-700 px-2 py-1 text-center text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider"
                      colSpan={3}
                    >
                      Insulation Resistance
                    </th>
                    ...
                    <th className="border border-neutral-300 dark:border-neutral-700 px-1 py-1 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
                      Units
                    </th>
                  </tr>
                  <tr>
                    <th className="border border-neutral-300 dark:border-neutral-700 px-1 py-1 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase"></th>
                    <th className="border border-neutral-300 dark:border-neutral-700 px-2 py-1 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
                      A-G
                    </th>
                    <th className="border border-neutral-300 dark:border-neutral-700 px-2 py-1 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
                      B-G
                    ...
                    <th className="border border-neutral-300 dark:border-neutral-700 px-1 py-1 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider"></th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="border border-neutral-300 dark:border-neutral-700 px-1 py-1 whitespace-nowrap text-xs text-neutral-900 dark:text-white">
                      Pre-Test
                    </td>
                    <td className="border border-neutral-300 dark:border-neutral-700 px-2 py-1 whitespace-nowrap">
                      <input
                        type="text"
                        value={formData.insulationTest.preTest.ag}
                        onChange={(e) =>
              ...
              <table className="min-w-full border-collapse border border-neutral-300 dark:border-neutral-700">
                <thead>
                  <tr>
                    ...
                    <th className="border border-neutral-300 dark:border-neutral-700 px-3 py-2 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider w-28">
                      kVAC
                    </th>
                    <th
                      className="border border-neutral-300 dark:border-neutral-700 px-3 py-2 text-center text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider"
                  ...
                  <tr>
                    ...
                    <th className="border border-neutral-300 dark:border-neutral-700 px-3 py-2 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
                      <select
                        ...
                        disabled={!isEditMode}
                        className={`w-16 rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand text-xs dark:bg-dark-100 dark:text-white ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
                      >
                        ...
                        {isMTSRoute ? (
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              readOnly={!isEditMode}
                              className={`w-full min-w-[3.5rem] rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white text-center ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
                            />
                            {isEditMode && (
                              <div className="relative">
                                <button
                                  ...
                                  className="px-2 py-1 border border-neutral-300 dark:border-neutral-700 rounded-md bg-white dark:bg-dark-100 text-neutral-700 dark:text-white"
                                  aria-label="Select kVAC"
                                >
                                  ▼
                                </button>
                                {openKvacIndex === index && (
                                  <div className="absolute right-0 z-10 mt-1 bg-white dark:bg-dark-150 border border-neutral-300 dark:border-neutral-700 rounded shadow-md max-h-48 overflow-auto">
                                    {mtsKVACOptions.map((v) => (
                                      <div
                                        key={v}
                                        className="px-2 py-1 cursor-pointer hover:bg-neutral-100 dark:hover:bg-dark-100"
                                        onClick={() => {
                                          handleWithstandTestChange(
                                            index,
                                            "kVAC",
                          ...
                          <input
                            ...
                            className={`w-full rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white text-center ${!isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""}`}
                          />
                        )}
                      </td>
                      <td className="border border-neutral-300 dark:border-neutral-700 px-3 py-2 whitespace-nowrap">
          ...
          <section className="mb-6 tan-delta-section">
            ...
            <div className="flex items-center mb-6">
              <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 w-64">
              ...
              <input
                ...
                readOnly={!isEditMode}
                className={`w-32 rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-100 dark:text-white ${
                  !isEditMode ? "bg-neutral-100 dark:bg-dark-200" : ""
                }`}
              />
              ...
              <table className="w-full border-collapse border border-neutral-300 dark:border-neutral-700">
                <thead>
                  <tr className="bg-neutral-100 dark:bg-dark-200">
                    <th
                      colSpan="8"
                      className="text-center border border-neutral-300 dark:border-neutral-700 px-3 py-2"
                    >
                      Tan Delta Test
                    </th>
                  </tr>
                  <tr className="bg-neutral-100 dark:bg-dark-200">
                    ...
                    <th
                      colSpan="2"
                      className="text-center border border-neutral-300 dark:border-neutral-700 px-3 py-2 text-blue-600"
                    >
                      A Phase
                    </th>
                    <th
                      colSpan="2"
                      className="text-center border border-neutral-300 dark:border-neutral-700 px-3 py-2 text-red-600"
                    >
                      B Phase
                    </th>
                    <th
                      colSpan="2"
                      className="text-center border border-neutral-300 dark:border-neutral-700 px-3 py-2 text-green-600"
                    >
                      C Phase
                    </th>
                  </tr>
                  <tr className="bg-neutral-100 dark:bg-dark-200">
                    <th className="border border-neutral-300 dark:border-neutral-700 px-3 py-2">
                      TD [E-3]
                    </th>
                    <th className="border border-neutral-300 dark:border-neutral-700 px-3 py-2">
                      Std. Dev
                ...
                <tbody>
                  {[0, 1, 2].map((rowIndex) => {
                    const defaultSteps = ["0.5 Uo", "1.0 Uo", "1.5 Uo"];
                    const defaultFactors = [0.5, 1.0, 1.5];
                    const baseVoltage =
                      parseFloat(formData.tanDeltaTest?.systemVoltageL2G) ||
                      14.4;
                    const defaultKV = (
                      baseVoltage * defaultFactors[rowIndex]
                    ).toFixed(3);

                    // Ensure the values array and its elements exist
                    const ensureValuesExist = () => {
                      const newValues = [
                        ...(formData.tanDeltaTest?.values || []),
                      ];
                      ...
                      >
                        <td className="border border-neutral-300 dark:border-neutral-700 px-3 py-2">
                          <input
                            type="text"
                            value={
                              formData.tanDeltaTest?.values?.[rowIndex]
                                ?.voltageStep || defaultSteps[rowIndex]
                            }
                            onChange={(e) => {
                        ...
                        <td className="border border-neutral-300 dark:border-neutral-700 px-3 py-2">
                          <input
                            ...
                            step="0.1"
                            value={
                              formData.tanDeltaTest?.values?.[rowIndex]?.phaseA
                                ?.td || ""
                            }
                            onChange={(e) => {
                              const newValues = ensureValuesExist();
                              if (!newValues[rowIndex])
                                newValues[rowIndex] = {};
                              if (!newValues[rowIndex].phaseA)
                                newValues[rowIndex].phaseA = {};
                        ...
                        <td className="border border-neutral-300 dark:border-neutral-700 px-3 py-2">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (!newValues[rowIndex].phaseA)
                                newValues[rowIndex].phaseA = {};
                              newValues[rowIndex].phaseA.stdDev =
                                e.target.value;
                              handleChange("tanDeltaTest", {
                                ...formData.tanDeltaTest,
                                values: newValues,
                        ...
                        <td className="border border-neutral-300 dark:border-neutral-700 px-3 py-2">
                          <input
                            ...
                            step="0.1"
                            value={
                              formData.tanDeltaTest?.values?.[rowIndex]?.phaseB
                                ?.td || ""
                            }
                            onChange={(e) => {
                              const newValues = ensureValuesExist();
                              if (!newValues[rowIndex])
                                newValues[rowIndex] = {};
                              if (!newValues[rowIndex].phaseB)
                                newValues[rowIndex].phaseB = {};
                        ...
                        <td className="border border-neutral-300 dark:border-neutral-700 px-3 py-2">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (!newValues[rowIndex].phaseB)
                                newValues[rowIndex].phaseB = {};
                              newValues[rowIndex].phaseB.stdDev =
                                e.target.value;
                              handleChange("tanDeltaTest", {
                                ...formData.tanDeltaTest,
                                values: newValues,
                        ...
                        <td className="border border-neutral-300 dark:border-neutral-700 px-3 py-2">
                          <input
                            ...
                            step="0.1"
                            value={
                              formData.tanDeltaTest?.values?.[rowIndex]?.phaseC
                                ?.td || ""
                            }
                            onChange={(e) => {
                              const newValues = ensureValuesExist();
                              if (!newValues[rowIndex])
                                newValues[rowIndex] = {};
                              if (!newValues[rowIndex].phaseC)
                                newValues[rowIndex].phaseC = {};
                        ...
                        <td className="border border-neutral-300 dark:border-neutral-700 px-3 py-2">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (!newValues[rowIndex].phaseC)
                                newValues[rowIndex].phaseC = {};
                              newValues[rowIndex].phaseC.stdDev =
                                e.target.value;
                              handleChange("tanDeltaTest", {
                                ...formData.tanDeltaTest,
                                values: newValues,
            ...
            <div className="mt-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium">
                  Tan Delta Test Results Chart
                </h3>
              </div>
              <div className="border rounded-md p-4 bg-white">
                <TanDeltaChart
                  values={formData.tanDeltaTest?.values || []}
                  chartData={formData.tanDeltaTest?.chartData}
                  systemVoltageL2G={
                    formData.tanDeltaTest?.systemVoltageL2G || ""
                  }
                />
              </div>
            </div>
                ...
                <div className="grid grid-cols-4 gap-4 print:hidden test-eqpt-onscreen">
                  <div>
                    ...
                    <EquipmentAutocomplete
                      ...
                      readOnly={!isEditMode}
                      className="w-full rounded-md border border-neutral-300 dark:border-neutral-700 shadow-sm"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                ...
                <div className="hidden print:block">
                  ...
                  >
                    <tbody>
                      <tr>
                        <td
                          className="p-2 border border-neutral-300 print:border-black min-h-[100px] align-top"
                          style={{
                            wordWrap: "break-word",
...
export default MediumVoltageCableVLFTanDeltaMTS23Report;
