...
// Main component
const LowVoltageCableMTS23Report: React.FC = () => {
  ...
  const [isEditMode, setIsEditMode] = useState<boolean>(!(reportId ?? reportIdFromUrl())); // Edit mode enabled by default for new reports
  ...
  const openReportId = reportId ?? reportIdFromUrl();
  ...
  useEffect(() => {
    // Recover from the address bar too: auto-save writes the new id there with
    // history.replaceState, which useParams never sees. Without this the ref
    // was wiped back to undefined and the next save inserted a second copy.
    const routeReportId = reportId ?? reportIdFromUrl();
  ...
  const reportSlug = "low-voltage-cable-mts23"; // This component handles the low-voltage-cable-mts23 route
  ...
  const [formData, setFormData] = useState<CableTestData>({
    ...
    numberOfCables: 12,
    inspectionResults: {
      "7.3.2.A.1": "Select One",
      "7.3.2.A.2": "Select One",
      "7.3.2.A.3": "Select One",
      "7.3.2.A.4": "Select One",
      "7.3.2.A.5": "Select One",
    },
    testVoltage: "1000V",
    testSets: [],
    testEquipment: {
  ...
  // Clear the green "Saved" state as soon as the form is edited again,
  // so the check mark never lies about unsaved changes.
  const lastSavedFormRef = React.useRef(formData);
  useEffect(() => {
    if (lastSavedFormRef.current !== formData) {
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .schema("neta_ops")
        .from("low_voltage_cable_mts23_reports")
        .select("*")
      ...
      if (reportData && reportData.data) {
        ...
        const migratedTestSets = (reportData.data.testSets || []).map(
          (set) => ({
            ...set,
            from: set.from,
            to: set.to,
        ...
        // Preserve From/To exactly as entered in existing reports (no auto line-break injection)
        const migratedTestSets = (reportData.report_data.testSets || []).map(
          (set) => ({
            ...set,
            from: set.from,
            to: set.to,
   ...
   * another copy of the same cable set on the job.
   */
  const persistReport = React.useCallback(async () => {
      ...
      .upsert(
        {
          ...
          job_id: jobId,
          user_id: user.id,
          // The MTS 23 table has one jsonb column: report_data.
          report_data: buildSavePayload(),
    ...
    if (!assetLinkedRef.current) {
      await ensureReportAssetLink(
        jobId,
        {
          name: getAssetName(reportSlug, formDataRef.current.identifier || ""),
    ...
    return reportId;
  }, [jobId, user?.id, buildSavePayload, reportSlug]);
  ...
  }, [jobId, user?.id, isEditMode, persistReport]);
  ...
  const handleSave = async () => {
    ...
    // Cancel any pending auto-save so it cannot fire mid-save.
    ...
    switch (e.key) {
      ...
      case "ArrowLeft":
        ...
        let prev = document.querySelector(
          `[data-position="${prevRow}-${prevCol}"]`,
        );
        while (!prev && (prevRow !== row || prevCol !== col)) {
          prevCol = (prevCol - 1 + TOTAL_COLS) % TOTAL_COLS;
          if (prevCol === TOTAL_COLS - 1)
            prevRow = (prevRow - 1 + TOTAL_ROWS) % TOTAL_ROWS;
    ...
    >
      <ReportWrapper isPrintMode={isPrintMode} disablePreview>
        ...
        <div className="p-6 max-w-7xl mx-auto space-y-6 dark:text-white">
          ...
          <div className="mb-6">
            <h2 className="section-job-info text-xl font-semibold mb-4 text-neutral-900 dark:text-white border-b dark:border-neutral-700 pb-2 print:text-black print:border-black print:font-bold">
              Job Information
            </h2>
              ...
              <div>
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    readOnly={!isEditMode}
                    className={`form-input ${!isEditMode ? "bg-neutral-100 dark:bg-dark-150" : ""} print:hidden`}
                  />
                  <span
              ...
              <div>
                ...
                <div className="mb-4 flex items-center">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    readOnly={!isEditMode}
                    className={`form-input w-20 temp-input-f ${!isEditMode ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                  />
                  <span className="mx-2 text-neutral-600 dark:text-white">
                    °F
                  </span>
                  <span className="mx-2 text-neutral-600 dark:text-white">
                  ...
                  <span className="mx-5 text-neutral-600 dark:text-white tcf-label">
                    TCF
                  </span>
                  <span className="font-medium text-neutral-900 dark:text-white tcf-value">
                    {tcf.toFixed(3)}
                  </span>
                </div>
                <div className="mb-4">
                  ...
                  />
                  <span className="ml-2 text-neutral-600 dark:text-white">
                    %
                  </span>
                </div>
                <div className="mb-4">
          ...
          {/* Cable Data Section */}
          <div className="mb-6">
            <h2 className="section-cable-data text-xl font-semibold mb-4 text-neutral-900 dark:text-white border-b dark:border-neutral-700 pb-2 print:text-black print:border-black print:font-bold">
              Cable Data
            </h2>
          ...
          <div className="mb-6">
            <h2 className="section-visual-mechanical text-xl font-semibold mb-4 text-neutral-900 dark:text-white border-b dark:border-neutral-700 pb-2 print:text-black print:border-black print:font-bold">
              ...
              <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700 visual-mechanical-table table-fixed">
                ...
                <tbody className="bg-white dark:bg-dark-150 divide-y divide-neutral-200 dark:divide-neutral-700">
                  {Object.entries({
                    ...
                    "7.3.2.A.2":
                      "Inspect bolted electrical connections for high resistance using a low-resistance ohmmeter in accordance with Section 7.3.2.B.1.",
                    "7.3.2.A.3": "Inspect cable tray and cable supports.",
                    "7.3.2.A.4":
                      "If cables are terminated through window-type current transformers, inspect to verify that neutral and ground conductors are correctly placed and shields are correctly terminated for operation of protective devices.",
                    "7.3.2.A.5":
          ...
          <section aria-labelledby="electrical-tests-heading" className="mb-6">
            <h2
              ...
              className="section-electrical-tests text-xl font-semibold mb-4 text-neutral-900 dark:text-white border-b dark:border-neutral-700 pb-2 print:text-black print:border-black print:font-bold"
            >
              Electrical Tests
            </h2>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-sm electrical-tests-table">
                ...
                <caption className="caption-bottom text-xs text-neutral-500 dark:text-white py-2">
                ...
                <thead>
                  <tr className="bg-neutral-50 dark:bg-dark-150">
                    <th
                      colSpan={2}
                      className="px-1.5 py-1.5 text-center text-xs font-medium text-neutral-900 dark:text-white border border-neutral-300 dark:border-neutral-600"
                    >
                      Circuit Designation
                    </th>
                    <th
                  ...
                  <tr className="bg-neutral-50 dark:bg-dark-150">
                    <th className="px-1.5 py-1.5 text-center text-xs font-medium text-neutral-900 dark:text-white border border-neutral-300 dark:border-neutral-600">
                      From
                    </th>
                    <th className="px-1.5 py-1.5 text-center text-xs font-medium text-neutral-900 dark:text-white border border-neutral-300 dark:border-neutral-600">
                      To
                ...
                <tbody>
                  {formData.testSets.map((set, index) => (
                    <React.Fragment key={set.id}>
                      ...
                      <tr className="hover:bg-neutral-50 dark:hover:bg-dark-200">
                        {/* From (under Circuit Designation) - rowSpan=2 */}
                        <td
                          className="px-1 py-0.5 text-center border border-neutral-300 dark:border-neutral-600"
                          rowSpan={2}
                          ...
                          <div className="relative">
                            ...
                            {isEditMode ? (
                              <textarea
                                ...
                                className="w-full text-xs text-center text-neutral-900 dark:text-white border-none bg-transparent resize-none"
                                rows={2}
                                style={{
                                  minHeight: "28px",
                                  height: "auto",
                              ...
                              <div
                                className="text-xs text-center w-full text-neutral-900 dark:text-white"
                                style={{
                                  minHeight: "28px",
                                  lineHeight: "1.1",
                                  fontSize: "9px",
                        ...
                        >
                          {isEditMode ? (
                            <select
                              ...
                              className="form-select text-xs py-1 px-1 text-center w-full text-neutral-900 dark:text-white border-none bg-transparent"
                        ...
                        <td className="px-1 py-0.5 text-center border border-neutral-300 dark:border-neutral-600">
                          RDG
                        </td>
                        ...
                        {[
                          ...
                          >
                            <input
                              ...
                              className="form-input text-xs py-1 px-1 text-center w-full text-neutral-900 dark:text-white border-none bg-transparent"
                              readOnly={!isEditMode}
                            />
                          </td>
                        ))}
                        ...
                        <td
                          rowSpan={2}
                          className="px-1 py-0.5 text-center border border-neutral-300 dark:border-neutral-600 align-middle"
                        >
                          {isEditMode ? (
                            <select
                      ...
                      <tr className="bg-neutral-50 dark:bg-dark-150 hover:bg-neutral-100 dark:hover:bg-dark-300">
                        ...
                        {[
                          ...
                          <td
                            className="px-1 py-0.5 text-center border border-neutral-300 dark:border-neutral-600 text-xs font-medium text-neutral-900 dark:text-white"
          ...
          <div className="mb-6 page-break-before">
            ...
            <div className="grid grid-cols-4 gap-4 print:hidden test-eqpt-onscreen">
              ...
              <div>
                <label
                  htmlFor="calDate"
                  className="form-label inline-block w-36"
                >
                  Calibration Date:
                </label>
                <input
...
export default LowVoltageCableMTS23Report;
