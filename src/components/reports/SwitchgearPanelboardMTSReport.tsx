...
const SwitchgearPanelboardMTSReport: React.FC = () => {
  ...
  const loadReport = useCallback(async () => {
    ...
    if (!currentReportId) {
      console.log('No currentReportId - setting up new report with PASS status');
      ...
      if (currentReportId) {
        ...
        if (result.data) {
          ...
          const assetData = {
            ...
            file_url: `report:/jobs/${jobId}/switchgear-panelboard-mts-report/${newReportId}`,
            user_id: user.id,
            template_type: 'MTS'
          };
          ...
          window.history.replaceState({}, '', `/jobs/${jobId}/switchgear-panelboard-mts-report/${newReportId}`);
        }
      }
    } catch (error) {
  ...
  const nextBusSectionLabel = () => `Bus ${formData.measuredInsulationResistance.length + 1}`;

  const addBusSection = () => {
    const busLabel = nextBusSectionLabel();
    setFormData(prev => ({
      ...prev,
      measuredInsulationResistance: [...prev.measuredInsulationResistance, { ...initialInsulationRow, busSection: busLabel }],
      contactResistanceTests: [...prev.contactResistanceTests, { ...initialContactResistanceRow, busSection: busLabel }],
      dielectricWithstandTests: [...prev.dielectricWithstandTests, { ...initialDielectricWithstandRow, busSection: busLabel }]
    }));
  };
  ...
  const removeBusSection = () => {
    ...
    setFormData(prev => ({
      ...prev,
      measuredInsulationResistance: prev.measuredInsulationResistance.slice(0, -1),
      ...
      <div className="max-w-7xl w-full space-y-6">
        ...
        {isEditing && (
          <div className="flex items-center gap-2 mb-4 print:hidden">
            ...
            <button type="button" onClick={removeBusSection} disabled={formData.measuredInsulationResistance.length <= 1} className="px-3 py-1 text-sm text-white bg-red-600 hover:bg-red-700 rounded-md disabled:opacity-50 disabled:cursor-not-allowed">- Remove Last Section</button>
            <span className="text-sm text-gray-600 dark:text-gray-400">({formData.measuredInsulationResistance.length} bus section{formData.measuredInsulationResistance.length !== 1 ? 's' : ''})</span>
          </div>
        )}
      ...
      {/* Job Information */}
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-x-4 gap-y-2 print:hidden job-info-onscreen">
            ...
            <div><label htmlFor="technicians" className="form-label">Technicians:</label><input id="technicians" type="text" value={formData.technicians} onChange={e => handleInputChange('technicians', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="date" className="form-label">Date:</label><input id="date" type="date" value={formData.date} onChange={e => handleInputChange('date', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="identifier" className="form-label">Identifier:</label><input id="identifier" type="text" value={formData.identifier} onChange={e => handleInputChange('identifier', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="temperature.fahrenheit" className="form-label">Temp:</label><input id="temperature.fahrenheit" type="number" value={formData.temperature.fahrenheit} onChange={e => handleFahrenheitChange(parseFloat(e.target.value))} readOnly={!isEditing} className={`form-input w-20 ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /><span className="ml-1">°F</span></div>
            <div className="md:col-span-2"><label htmlFor="temperature.celsius" className="form-label">Temp (°C):</label><input id="temperature.celsius" type="number" value={formData.temperature.celsius} readOnly className="form-input w-32 min-w-[7rem] bg-gray-100 dark:bg-dark-150" /></div>
            <div><label htmlFor="temperature.tcf" className="form-label">TCF:</label><input id="temperature.tcf" type="number" value={formData.temperature.tcf.toFixed(3)} readOnly className="form-input bg-gray-100 dark:bg-dark-150 w-16" /></div>
            <div><label htmlFor="substation" className="form-label">Substation:</label><input id="substation" type="text" value={formData.substation} onChange={e => handleInputChange('substation', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="eqptLocation" className="form-label">Eqpt. Location:</label><input id="eqptLocation" type="text" value={formData.eqptLocation} onChange={e => handleInputChange('eqptLocation', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div className="md:col-span-2"><label htmlFor="userName" className="form-label">User:</label><input id="userName" type="text" value={formData.userName} onChange={e => handleInputChange('userName', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="humidity" className="form-label">Humidity %:</label><input id="humidity" type="number" value={formData.temperature.humidity || ''} onChange={e => handleInputChange('temperature.humidity', e.target.value === '' ? null : parseFloat(e.target.value))} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} placeholder="Optional" /></div>
            <div className="md:col-span-6"><label htmlFor="customerLocation" className="form-label">Address:</label><input id="customerLocation" type="text" value={formData.customerLocation} readOnly className="form-input w-full bg-gray-100 dark:bg-dark-150" /></div>
          </div>
          <JobInfoPrintTable
            data={{
      ...
      {/* Nameplate Data */}
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-4 print:hidden nameplate-onscreen">
            ...
            <div><label className="form-label">Catalog Number:</label><input type="text" value={formData.nameplate.catalogNumber} onChange={e => handleInputChange('nameplate.catalogNumber', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label className="form-label">Serial Number:</label><input type="text" value={formData.nameplate.serialNumber} onChange={e => handleInputChange('nameplate.serialNumber', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            </div>
          <div className="grid grid-cols-3 gap-4 mt-4 print:hidden nameplate-onscreen">
            <div><label className="form-label">Series:</label><input type="text" value={formData.nameplate.series} onChange={e => handleInputChange('nameplate.series', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label className="form-label">Type:</label><input type="text" value={formData.nameplate.type} onChange={e => handleInputChange('nameplate.type', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label className="form-label">System Voltage:</label><input type="text" value={formData.nameplate.systemVoltage} onChange={e => handleInputChange('nameplate.systemVoltage', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
        </div>
          <div className="grid grid-cols-3 gap-4 mt-4 print:hidden nameplate-onscreen">
            <div><label className="form-label">Rated Voltage:</label><input type="text" value={formData.nameplate.ratedVoltage} onChange={e => handleInputChange('nameplate.ratedVoltage', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label className="form-label">Rated Current:</label><input type="text" value={formData.nameplate.ratedCurrent} onChange={e => handleInputChange('nameplate.ratedCurrent', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label className="form-label">SCCR:</label><input type="text" value={formData.nameplate.aicRating} onChange={e => handleInputChange('nameplate.aicRating', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
          </div>
          <div className="grid grid-cols-3 gap-4 mt-4 print:hidden nameplate-onscreen">
            <div><label className="form-label">Phase Configuration:</label><input type="text" value={formData.nameplate.phaseConfiguration} onChange={e => handleInputChange('nameplate.phaseConfiguration', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
          </div>
          <NameplatePrintTable
            data={{
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 visual-mechanical-table table-fixed">
            ...
            <thead className="bg-gray-50 dark:bg-dark-150">
              <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">NETA Section</th>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Result</th>
              </tr>
            </thead>
            <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-200 dark:divide-gray-700">
              {formData.visualInspectionItems.map((item, index) => (
                <tr key={item.id}>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-white whitespace-normal break-words">{item.description}</td>
                    <td className="px-6 py-4 text-right">
                      <div className="print:hidden">
                        <select value={item.result} onChange={e => handleListInputChange('visualInspectionItems', index, 'result', e.target.value)} disabled={!isEditing} className={`form-select w-full md:w-48 ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>
      ...
      {/* Electrical Tests - Measured Insulation Resistance */}
        ...
        <div className="overflow-x-auto section-insulation-resistance">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-fixed ir-table">
            ...
            <thead>
              <tr>
                <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider w-32">Bus Section</th>
              ...
              {formData.measuredInsulationResistance.map((test, index) => (
                <tr key={index}>
                  <td className="px-3 py-2">
                    ...
                    <div className="hidden print:block text-left">{test.busSection}</div>
                  </td>
                    ...
                    <div className="print:hidden">
                      ...
                      >
                        {INSULATION_RESISTANCE_UNITS.map(unit => (
                          <option key={unit} value={unit} className="dark:bg-dark-150 dark:text-white">{unit}</option>
                        ))}
                      </select>
                    </div>
            ...
            <div className="grid grid-cols-4 gap-4 border-b dark:border-gray-700 pb-4">
              <div>
                ...
                <EquipmentAutocomplete
                  ...
                  onChange={(value) => handleInputChange('testEquipment.megohmmeter.name', value)}
                  onSelect={(equipment) => {
                    ...
                    handleInputChange('testEquipment.megohmmeter.name', equipment.equipment_name);
                    handleInputChange('testEquipment.megohmmeter.serialNumber', equipment.serial_number || '');
                    handleInputChange('testEquipment.megohmmeter.ampId', equipment.amp_id || '');
                    handleInputChange('testEquipment.megohmmeter.calDate', formatLocalDateShort(equipment.calibration_date));
                  }}
                  readOnly={!isEditing}
                  className="w-full"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('testEquipment.megohmmeter.calDate', e.target.value)}
                  readOnly={!isEditing}
                  className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}
                />
              </div>
            ...
            <div className="grid grid-cols-4 gap-4 border-b dark:border-gray-700 pb-4">
              <div>
                ...
                <EquipmentAutocomplete
                  ...
                  onChange={(value) => handleInputChange('testEquipment.lowResistanceOhmmeter.name', value)}
                  onSelect={(equipment) => {
                    const formatDate = (dateString: string | null): string => {
                      if (!dateString) return '';
                      try {
                    ...
                    handleInputChange('testEquipment.lowResistanceOhmmeter.name', equipment.equipment_name);
                    handleInputChange('testEquipment.lowResistanceOhmmeter.serialNumber', equipment.serial_number || '');
                    handleInputChange('testEquipment.lowResistanceOhmmeter.ampId', equipment.amp_id || '');
                    handleInputChange('testEquipment.lowResistanceOhmmeter.calDate', formatLocalDateShort(equipment.calibration_date));
                  }}
                  readOnly={!isEditing}
                  className="w-full"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('testEquipment.lowResistanceOhmmeter.calDate', e.target.value)}
                  readOnly={!isEditing}
                  className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}
                />
              </div>
            ...
            <div className="grid grid-cols-4 gap-4">
              <div>
                ...
                <EquipmentAutocomplete
                  ...
                  onChange={(value) => handleInputChange('testEquipment.hipot.name', value)}
                  onSelect={(equipment) => {
                    const formatDate = (dateString: string | null): string => {
                      if (!dateString) return '';
                      try {
                    ...
                    handleInputChange('testEquipment.hipot.name', equipment.equipment_name);
                    handleInputChange('testEquipment.hipot.serialNumber', equipment.serial_number || '');
                    handleInputChange('testEquipment.hipot.ampId', equipment.amp_id || '');
                    handleInputChange('testEquipment.hipot.calDate', formatLocalDateShort(equipment.calibration_date));
                  }}
                  readOnly={!isEditing}
                  className="w-full"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('testEquipment.hipot.calDate', e.target.value)}
                  readOnly={!isEditing}
                  className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}
                />
              </div>
