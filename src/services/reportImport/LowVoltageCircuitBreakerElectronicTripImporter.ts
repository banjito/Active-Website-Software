...
export class LowVoltageCircuitBreakerElectronicTripImporter extends BaseImporter implements ReportImporter {
  ...
  protected prepareData(data: ReportData, jobId: string, userId: string, schema: DatabaseSchema): any {
    ...
    // Build final payload with proper column structure expected by this table
    const payload: any = {
      job_id: jobId,
      user_id: userId,
      report_info: {
      ...
      visual_mechanical: {
        items: Object.entries(normalized.visualInspection).map(([key, value]) => ({
          id: key,
          result: value
        }))
      },
      ...
      primary_injection: normalized.primaryInjection,
