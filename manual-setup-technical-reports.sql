-- Manual setup script for Technical Reports and Asset Status
...
-- Step 2: Create technical_reports table
...
-- Allow managers/admins to update any report
CREATE POLICY "Managers can update any technical report" ON neta_ops.technical_reports
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM auth.users 
            WHERE id = auth.uid() 
            AND (
                raw_user_meta_data->>'role' = 'manager' OR 
...
-- Grant sequence permissions (sequences are auto-generated for UUID columns, so we don't need to grant them)
-- UUID columns use uuid_generate_v4() function, not sequences
...
-- SELECT 'Assets table status column' as check_name, 
--        CASE WHEN EXISTS (
--            SELECT 1 FROM information_schema.columns 
--            WHERE table_schema = 'neta_ops' AND table_name = 'assets' AND column_name = 'status'
--        ) THEN 'EXISTS' ELSE 'MISSING' END as status;

-- SELECT 'Technical reports table' as check_name,
--        CASE WHEN EXISTS (
--            SELECT 1 FROM information_schema.tables 
--            WHERE table_schema = 'neta_ops' AND table_name = 'technical_reports'
--        ) THEN 'EXISTS' ELSE 'MISSING' END as status;

-- SELECT 'Asset reports table' as check_name,
--        CASE WHEN EXISTS (
--            SELECT 1 FROM information_schema.tables 
--            WHERE table_schema = 'neta_ops' AND table_name = 'asset_reports'
--        ) THEN 'EXISTS' ELSE 'MISSING' END as status; 