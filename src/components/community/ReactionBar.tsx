...
function cloneFeedPatch(fp: FeedPost, type: ReactionType, had: boolean): FeedPost {
...
export const ReactionBar: React.FC<Props> = ({ postId, currentUserId, feed, onUpdateFeedPost }) => {
  ...
  const toggle = async (type: ReactionType) => {
    ...
    try {
      if (had) {
        ...
        const { error } = await supabase.schema('common').from('post_reactions').insert({
          post_id: postId,
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 pt-1.5 border-t border-gray-100 dark:border-dark-200">
      <button
        type="button"
      ...
      >
        <ThumbsUp className={`h-3.5 w-3.5 ${likeActive ? 'fill-current' : ''}`} />
      ...
      <div className="relative">
        <button
          ...
          className={`inline-flex items-center gap-1 rounded-full px-1.5 py-1 text-xs text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-dark-200 ${
            !currentUserId ? 'opacity-50 cursor-not-allowed' : ''
          }`}
        ...
        {pickerOpen && (
          <>
            <button type="button" className="fixed inset-0 z-10 cursor-default" aria-label="Close" onClick={() => setPickerOpen(false)} />
            <div className="absolute left-0 bottom-full z-20 mb-1 flex gap-1 rounded-lg border border-gray-200 dark:border-dark-200 bg-white dark:bg-dark-150 px-2 py-1.5 shadow-lg">
              {ALL_REACTION_TYPES.filter((t) => t !== 'like').map((type) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-md px-1.5 py-0.5 text-lg leading-none hover:bg-gray-100 dark:hover:bg-dark-200 ${
                      active ? 'ring-2 ring-[#f26722]/50' : ''
                    }`}
                  >
                    <span className="sr-only">{type}</span>
                    {REACTION_EMOJI[type]}
                    {count > 0 ? <span className="ml-0.5 text-xs text-gray-500 dark:text-gray-400">{count}</span> : null}
                  </button>
                );
              })}
            </div>
