...
const emptyJobStep = (): JobStepRow => ({ jobStep: '', hazards: '', controls: '', rac: '' });
const emptyAttendee = (): Attendee => ({ name: '', signature: '' });
const emptyCompletedWork = (): CompletedWorkRow => ({ name: '', signature: '', tools_accounted_for: false });
const emptyFinalInspection = (): FinalInspectionRow => ({ name: '', signature: '' });
...
const RAC_PROBABILITIES = ['Frequent', 'Likely', 'Occasional', 'Seldom', 'Unlikely'] as const;
const RAC_SEVERITIES = ['Catastrophic', 'Critical', 'Marginal', 'Negligible'] as const;
const RAC_MATRIX: Record<typeof RAC_SEVERITIES[number], Record<typeof RAC_PROBABILITIES[number], RAC>> = {
  Catastrophic: { Frequent: 'E', Likely: 'E', Occasional: 'H', Seldom: 'H', Unlikely: 'M' },
  Critical:     { Frequent: 'E', Likely: 'H', Occasional: 'H', Seldom: 'M', Unlikely: 'L' },
  Marginal:     { Frequent: 'H', Likely: 'M', Occasional: 'M', Seldom: 'L', Unlikely: 'L' },
  Negligible:   { Frequent: 'M', Likely: 'L', Occasional: 'L', Seldom: 'L', Unlikely: 'L' },
};

const racBg = (r: RAC): string => {
  switch (r.trim().toUpperCase()) {
    case 'E': return 'bg-red-500 text-white';
    case 'H': return 'bg-orange-400 text-black';
...
const racCellBg = (r: RAC): string => {
  switch (r.trim().toUpperCase()) {
    case 'E': return 'bg-red-500 text-white font-bold';
    case 'H': return 'bg-orange-400 text-black font-bold';
    case 'M': return 'bg-yellow-300 text-black font-bold';
    case 'L': return 'bg-green-500 text-white font-bold';
    default:  return '';
  }
};
...
const JobHazardAnalysisForm: React.FC = () => {
    ...
    try {
      ...
      if (data) {
        ...
        setFormData({
          ...
          job_steps: Array.isArray(data.job_steps) && data.job_steps.length > 0
            ? data.job_steps.map((s: any): JobStepRow => ({
                jobStep: s.jobStep || '',
                hazards: s.hazards || '',
                controls: s.controls || '',
          ...
          inspection_requirements: normalizeStringList(data.inspection_requirements),
          safety_brief: {
            date: sb.date || '',
            location: sb.location || '',
            presented_by_name: sb.presented_by_name || '',
            presented_by_signature: sb.presented_by_signature || '',
            attendees: Array.isArray(sb.attendees) && sb.attendees.length > 0
              ? sb.attendees.map((a: any): Attendee => ({
                  name: a.name || '',
                  signature: a.signature || '',
                }))
              : Array.from({ length: 10 }, emptyAttendee),
            completed_work: Array.isArray(sb.completed_work) && sb.completed_work.length > 0
              ? sb.completed_work.map((c: any): CompletedWorkRow => ({
                  name: c.name || '',
                  signature: c.signature || '',
                  tools_accounted_for: !!c.tools_accounted_for,
                }))
              : Array.from({ length: 8 }, emptyCompletedWork),
            final_inspection: Array.isArray(sb.final_inspection) && sb.final_inspection.length > 0
              ? sb.final_inspection.map((f: any): FinalInspectionRow => ({
                  name: f.name || '',
        ...
        try {
          const { data: assetRow } = await supabase
            ...
            .from('assets')
            .select('status')
            .eq('file_url', `report:/jobs/${jobId}/${JOB_HAZARD_ANALYSIS_SLUG}/${currentReportId}`)
            .maybeSingle();
    ...
    try {
      ...
      if (currentReportId) {
        ...
        if (savedId) {
          ...
          const suffix = (formData.activity || formData.project_location || '').trim();
          ...
          navigate(`/jobs/${jobId}/${JOB_HAZARD_ANALYSIS_SLUG}/${savedId}${location.search || ''}`, { replace: true });
        }
      }
      ...
      alert(`Failed to save Job Hazard Analysis: ${err?.message || 'Unknown error'}`);
      return null;
    } finally {
      setSaving(false);
    }
  }, [jobId, user, formData, currentReportId, isLocked, navigate, location.search]);
  ...
  const updateJobStep = (index: number, patch: Partial<JobStepRow>) => {
    setFormData(prev => ({
      ...prev,
      job_steps: prev.job_steps.map((row, i) => (i === index ? { ...row, ...patch } : row)),
    }));
  };
  const addJobStep = () => setFormData(prev => ({ ...prev, job_steps: [...prev.job_steps, emptyJobStep()] }));
  const removeJobStep = (index: number) => setFormData(prev => ({
    ...prev,
    job_steps: prev.job_steps.length > 1 ? prev.job_steps.filter((_, i) => i !== index) : prev.job_steps,
  }));

  type EtiField = 'equipment_to_be_used' | 'training' | 'inspection_requirements';
  const ETI_FIELDS: EtiField[] = ['equipment_to_be_used', 'training', 'inspection_requirements'];
  ...
  const removeEtiRow = (index: number) => {
    setFormData(prev => {
      ...
      const pop = (arr: string[]) => (arr.length > index ? arr.filter((_, i) => i !== index) : arr);
      return {
        ...prev,
  ...
  const updateAttendee = (index: number, patch: Partial<Attendee>) => {
    setFormData(prev => ({
      ...
      safety_brief: {
        ...prev.safety_brief,
        attendees: prev.safety_brief.attendees.map((row, i) => (i === index ? { ...row, ...patch } : row)),
      },
    }));
  };
  const addAttendee = () => setFormData(prev => ({
    ...prev,
    safety_brief: { ...prev.safety_brief, attendees: [...prev.safety_brief.attendees, emptyAttendee()] },
  }));
  const removeAttendee = (index: number) => setFormData(prev => ({
    ...prev,
    safety_brief: {
      ...prev.safety_brief,
      attendees: prev.safety_brief.attendees.length > 1
        ? prev.safety_brief.attendees.filter((_, i) => i !== index)
        : prev.safety_brief.attendees,
    },
  }));

  const updateCompletedWork = (index: number, patch: Partial<CompletedWorkRow>) => {
    setFormData(prev => ({
      ...prev,
      safety_brief: {
        ...prev.safety_brief,
        completed_work: prev.safety_brief.completed_work.map((row, i) => (i === index ? { ...row, ...patch } : row)),
      },
    }));
  };
  const addCompletedWork = () => setFormData(prev => ({
    ...prev,
    safety_brief: { ...prev.safety_brief, completed_work: [...prev.safety_brief.completed_work, emptyCompletedWork()] },
  }));
  const removeCompletedWork = (index: number) => setFormData(prev => ({
    ...prev,
    safety_brief: {
      ...prev.safety_brief,
      completed_work: prev.safety_brief.completed_work.length > 1
        ? prev.safety_brief.completed_work.filter((_, i) => i !== index)
        : prev.safety_brief.completed_work,
    },
  }));

  const updateFinalInspection = (index: number, patch: Partial<FinalInspectionRow>) => {
    setFormData(prev => ({
      ...prev,
      safety_brief: {
        ...prev.safety_brief,
        final_inspection: prev.safety_brief.final_inspection.map((row, i) => (i === index ? { ...row, ...patch } : row)),
      },
    }));
  };
  ...
  return (
    <ReportWrapper isPrintMode={false} disablePreview>
      <div className="space-y-4">
        ...
        <div className="print:hidden flex justify-between items-center gap-2 flex-wrap">
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => navigate(`/jobs/${jobId}?tab=assets`)}
              className="inline-flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-dark-150 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-dark-200"
            >
              <ArrowLeft className="h-4 w-4" /> Back to Job
            </button>
            <h1 className="text-xl font-bold text-gray-900 dark:text-white">
              {JOB_HAZARD_ANALYSIS_DISPLAY_NAME}
            </h1>
            {assetStatus && (
              ...
              isEditing ? (
                <>
                  <button
                    ...
                    onClick={handleSave}
                    disabled={saving}
                    className="px-3 py-1.5 text-sm font-medium text-white bg-[#f26722] rounded-md hover:bg-[#e55611] disabled:opacity-60"
                  >
                    {saving ? 'Saving…' : 'Save'}
                  </button>
                  <button
                    ...
                    disabled={saving}
                    className="px-3 py-1.5 text-sm font-medium text-white bg-gray-600 rounded-md hover:bg-gray-700 disabled:opacity-60"
                  >
                    Save &amp; Close
                  </button>
                  <button
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-sm font-medium text-white bg-[#f26722] rounded-md hover:bg-[#e55611]"
                >
                  Edit Form
                </button>
              )
        ...
        <div className="border border-black bg-white text-black">
          ...
          <div className="grid grid-cols-[120px_1fr_120px] items-center border-b border-black">
            ...
            <div className="text-center py-2">
              <div className="text-base font-bold leading-tight">AMP Quality Energy Services</div>
              <div className="text-lg font-bold leading-tight">Job Hazard Analysis</div>
            </div>
            <div />
          </div>
          ...
          <table className="w-full border-collapse text-sm">
            <tbody>
              ...
              <tr>
                <td className="border border-black p-1 align-middle w-[20%]">Activity/Work Task:</td>
                ...
                <td className="border border-black p-1 align-middle text-center font-semibold w-[40%]" colSpan={5}>
                  Overall Risk Assessment Code (RAC) (Use Highest Code)
                </td>
                <td className={`border border-black p-1 align-middle text-center w-[10%] ${racCellBg(formData.overall_rac)}`}>
                  <input
                    type="text"
                    value={formData.overall_rac}
                    onChange={(e) => setFormData(prev => ({ ...prev, overall_rac: e.target.value }))}
                    readOnly={readOnly}
                    placeholder="E / H / M / L"
              ...
              <tr>
                <td className="border border-black p-1 align-middle">Project Location:</td>
                ...
                <td className="border border-black p-1 text-center font-bold" colSpan={6}>
                  Risk Assessment Code (RAC) Matrix
                </td>
              </tr>
              {/* Row 3: Contract Number / Severity + Probability column headers */}
              <tr>
                <td className="border border-black p-1 align-middle">Contract Number:</td>
                <td className="border border-black p-1 align-middle">
                  <input
                    type="text"
                ...
                <td className="border border-black p-1 text-center font-semibold align-middle" rowSpan={2}>
                  Severity
                </td>
                <td className="border border-black p-1 text-center font-semibold" colSpan={5}>
                  Probability
                </td>
              </tr>
              {/* Row 4: Date Prepared / Probability column labels */}
              <tr>
                <td className="border border-black p-1 align-middle">Date Prepared:</td>
                <td className="border border-black p-1 align-middle">
                  <input
                    type="date"
                    value={formData.date_prepared}
                    onChange={(e) => setFormData(prev => ({ ...prev, date_prepared: e.target.value }))}
                    readOnly={readOnly}
                    className={inputClass}
                  />
                </td>
                {RAC_PROBABILITIES.map((p) => (
                  <td key={p} className="border border-black p-1 text-center text-xs font-semibold">{p}</td>
                ))}
              </tr>
              ...
              {RAC_SEVERITIES.map((sev, sIdx) => {
                ...
                if (sIdx === 0) {
                  headerCells.push(
                    <td key="lbl" className="border border-black p-1 align-middle">Prepared By:</td>,
                    <td key="val" className="border border-black p-1 align-middle">
                      <input
                        type="text"
                  ...
                  headerCells.push(
                    <td key="lbl" className="border border-black p-1 align-middle">Reviewed By:</td>,
                    <td key="val" className="border border-black p-1 align-middle">
                      <input
                        type="text"
                        value={formData.reviewed_by}
                        onChange={(e) => setFormData(prev => ({ ...prev, reviewed_by: e.target.value }))}
                        readOnly={readOnly}
                        className={inputClass}
                      />
                    </td>
                  ...
                  headerCells.push(
                    <td key="lbl" className="border border-black p-1 align-top" rowSpan={2}>
                      Notes (Field Notes, Review Comments, etc.)
                    </td>,
                    <td key="val" className="border border-black p-1 align-top" rowSpan={2}>
                      <textarea
                        rows={3}
                ...
                return (
                  <tr key={sev}>
                    {headerCells}
                    <td className="border border-black p-1 text-center text-xs font-semibold">{sev}</td>
                    {RAC_PROBABILITIES.map((p) => {
                      const v = RAC_MATRIX[sev][p];
                      return (
                        <td key={p} className={`border border-black p-1 text-center text-sm font-bold ${racBg(v)}`}>
                          {v}
                        </td>
                      );
                    })}
              ...
              <tr>
                <td className="border border-black p-1 align-top" colSpan={2}>
                  <div className="font-semibold mb-1">Step 1: Review each "Hazard" with identified safety "Controls" and determine RAC (See Above)</div>
                  <div className="mb-1">
                    <span className="font-semibold">"Probability"</span> is the likelihood to cause an incident, near miss, or accident and identified as: Frequent, Likely, Occasional, Seldom, or Unlikely.
                  </div>
                  <div className="mb-1">
                    <span className="font-semibold">"Severity"</span> is the outcome/degree if an incident, near miss, or accident did occur and identified as: Catastrophic, Critical, Marginal, or Negligible.
                  </div>
                  <div>
                    <span className="font-semibold">"Step 2":</span> Identify the RAC (Probability/Severity) as E, H, M, or L for each "Hazard" on JHA. Annotate the overall highest RAC at the top of JHA.
                  </div>
                </td>
                <td className="border border-black p-0 align-top" colSpan={6}>
                  <div className="text-center font-bold py-1 border-b border-black">RAC Chart</div>
                  <div className="bg-red-500 text-white text-center font-bold py-1 border-b border-black">E = Extremely High Risk</div>
                  <div className="bg-orange-400 text-black text-center font-bold py-1 border-b border-black">H = High Risk</div>
                  <div className="bg-yellow-300 text-black text-center font-bold py-1 border-b border-black">M = Moderate Risk</div>
                  <div className="bg-green-500 text-white text-center font-bold py-1">L = Low Risk</div>
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-gray-100">
                <th className="border border-black p-1 w-[15%] text-left">Job Steps</th>
                <th className="border border-black p-1 w-[20%] text-left">Hazards</th>
                <th className="border border-black p-1 text-left">Controls</th>
                <th className="border border-black p-1 w-[8%] text-center">RAC</th>
                {!readOnly && <th className="border border-black p-1 w-[4%] print:hidden" />}
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={idx}>
                  <td className="border border-black p-1 align-top">
                    <textarea
                      rows={2}
                      value={row.jobStep}
                      onChange={(e) => updateJobStep(idx, { jobStep: e.target.value })}
                      readOnly={readOnly}
                      className={textareaClass}
                    />
                  </td>
                  <td className="border border-black p-1 align-top">
                    <textarea
                      rows={2}
                      value={row.hazards}
                      onChange={(e) => updateJobStep(idx, { hazards: e.target.value })}
                      readOnly={readOnly}
                      className={textareaClass}
                    />
                  </td>
                  <td className="border border-black p-1 align-top">
                    <textarea
                      rows={2}
                      value={row.controls}
                      onChange={(e) => updateJobStep(idx, { controls: e.target.value })}
                      readOnly={readOnly}
                      className={textareaClass}
                    />
                  </td>
                  <td className={`border border-black p-1 align-middle text-center ${racCellBg(row.rac)}`}>
                    <input
                      type="text"
                      value={row.rac}
                      onChange={(e) => updateJobStep(idx, { rac: e.target.value })}
                      readOnly={readOnly}
                      placeholder="E / H / M / L"
                      className="w-full bg-transparent text-center font-bold border-none focus:outline-none placeholder:font-normal placeholder:text-gray-500"
                    />
        ...
        <div className="border border-black bg-white text-black">
          <table className="w-full border-collapse text-sm table-fixed">
            <thead>
              <tr className="bg-gray-100">
                <th className="border border-black p-1 text-left">Equipment to be Used</th>
                <th className="border border-black p-1 text-left">Training</th>
                <th className="border border-black p-1 text-left">Inspection Requirements</th>
                {!readOnly && (
                  <th className="border border-black p-1 text-center w-10 print:hidden">&nbsp;</th>
                )}
              </tr>
            </thead>
            <tbody>
              {Array.from({ length: etiRowCount }).map((_, idx) => (
                <tr key={idx}>
                  {ETI_FIELDS.map((field) => (
                    <td key={field} className="border border-black p-1 align-top">
                      <textarea
                        rows={2}
                        value={formData[field][idx] ?? ''}
                        onChange={(e) => updateEtiCell(field, idx, e.target.value)}
                        readOnly={readOnly}
                        className={textareaClass}
                      />
                    </td>
        ...
        <div className="border border-black bg-white text-black p-4 space-y-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="text-center flex-1">
              <div className="text-base font-bold leading-tight">AMP Quality Energy Services</div>
              <div className="text-base font-bold leading-tight">Safety Brief Acknowledgement Form</div>
            </div>
            <div className="w-12" />
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, safety_brief: { ...prev.safety_brief, date: e.target.value } }))}
                  readOnly={readOnly}
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, safety_brief: { ...prev.safety_brief, location: e.target.value } }))}
                  readOnly={readOnly}
                  className={inputClass}
                />
              </div>
              <div className="flex items-center gap-2">
                <span className="font-medium whitespace-nowrap">By (Print Name)</span>
                <input
                  type="text"
                  value={formData.safety_brief.presented_by_name}
                  onChange={(e) => setFormData(prev => ({ ...prev, safety_brief: { ...prev.safety_brief, presented_by_name: e.target.value } }))}
                  readOnly={readOnly}
                  className={inputClass}
                />
              </div>
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, safety_brief: { ...prev.safety_brief, presented_by_signature: e.target.value } }))}
                  readOnly={readOnly}
                  className={inputClass}
                />
              </div>
          ...
          {/* Attendees */}
          <div>
            <div className="text-sm font-medium mb-1">The below listed employees attended this safety brief:</div>
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-gray-50">
                  <th className="border border-black p-1 w-[8%] text-center">#</th>
                  <th className="border border-black p-1 text-left">Print Name</th>
                  ...
                  <tr key={idx}>
                    ...
                    <td className="border border-black p-1">
                      <input
                        type="text"
                        value={row.signature}
                        onChange={(e) => updateAttendee(idx, { signature: e.target.value })}
                        readOnly={readOnly}
                        className={inputClass}
                      />
                    </td>
          ...
          <div>
            <div className="flex flex-wrap items-end justify-between gap-2 mb-1">
              <div className="text-sm font-medium">The below listed employees completed the work to be performed:</div>
              <div className="text-xs italic text-gray-600">Check box to acknowledge that all tools used are accounted for.</div>
            </div>
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="border border-black p-1 text-left">Print Name</th>
                  <th className="border border-black p-1 text-left">Signature</th>
                  <th className="border border-black p-1 w-[8%] text-center">Tools</th>
                  {!readOnly && <th className="border border-black p-1 w-[4%] print:hidden" />}
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={idx}>
                    ...
                    <td className="border border-black p-1">
                      <input
                        type="text"
                        value={row.name}
                        onChange={(e) => updateCompletedWork(idx, { name: e.target.value })}
                        readOnly={readOnly}
                        className={inputClass}
                      />
                    </td>
                    <td className="border border-black p-1">
                      <input
                        type="text"
                        value={row.signature}
                        onChange={(e) => updateCompletedWork(idx, { signature: e.target.value })}
                        readOnly={readOnly}
                        className={inputClass}
                      />
                    </td>
                    <td className="border border-black p-1 text-center">
                      <input
                        ...
                        onChange={(e) => updateCompletedWork(idx, { tools_accounted_for: e.target.checked })}
                        disabled={readOnly}
                    ...
                    {!readOnly && (
                      <td className="border border-black p-1 text-center print:hidden">
                        <button
                          ...
                          title="Remove row"
                          className="text-red-600 hover:text-red-800 disabled:opacity-30"
                          disabled={formData.safety_brief.completed_work.length <= 1}
                        >
                          <Trash2 className="h-4 w-4" />
                        </button>
                      </td>
          ...
          {/* Final inspection */}
          <div>
            <div className="text-sm font-medium mb-1">Final Inspection (two signatures required before re-energization)</div>
            <table className="w-full border-collapse text-sm">
              <tbody>
                {formData.safety_brief.final_inspection.map((row, idx) => (
                  <tr key={idx}>
                    <td className="border border-black p-1 w-[15%] font-medium">Print Name:</td>
                    <td className="border border-black p-1 w-[35%]">
                      <input
                        type="text"
                        value={row.name}
                        onChange={(e) => updateFinalInspection(idx, { name: e.target.value })}
                        readOnly={readOnly}
                        className={inputClass}
                      />
                    </td>
                    <td className="border border-black p-1 w-[15%] font-medium">Signature:</td>
                    <td className="border border-black p-1">
                      <input
                        type="text"
                        value={row.signature}
                        onChange={(e) => updateFinalInspection(idx, { signature: e.target.value })}
                        readOnly={readOnly}
                        className={inputClass}
                      />
                    </td>
        ...
        {/* Bottom toolbar (mirrors top toolbar for convenience) */}
        {!isLocked && isEditing && (
          <div className="print:hidden flex justify-end items-center gap-2 pb-4">
            <button
              type="button"
              onClick={handleSave}
              disabled={saving}
              className="px-4 py-2 text-sm font-medium text-white bg-[#f26722] rounded-md hover:bg-[#e55611] disabled:opacity-60"
            >
              {saving ? 'Saving…' : 'Save'}
            </button>
            <button
