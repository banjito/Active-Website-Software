...
const StandardReportTemplate: React.FC = () => {
  ...
  const [visualInspection, setVisualInspection] =
    useState<StandardVisualInspection>({
  ...
  const handleJobInfoChange = (
    field: keyof BaseReportData["jobInfo"],
    value: string,
  ) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleNameplateChange = (
    field: keyof StandardNameplateData,
    value: string,
  ) => {
  ...
  const handleVisualInspectionChange = (
    index: number,
    field: keyof VisualInspectionItem,
    value: string,
  ) => {
