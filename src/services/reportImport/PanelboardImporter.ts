...
export class PanelboardImporter extends BaseImporter implements ReportImporter {
  ...
  canImport(data: ReportData): boolean {
    ...
    // Only match if it's specifically a PanelboardReport, not SwitchgearPanelboardMTSReport
    const canImport = (reportType.includes('panelboardreport') || reportType.includes('panelboard-report')) &&
           !reportType.includes('switchgear') && !reportType.includes('mts');
