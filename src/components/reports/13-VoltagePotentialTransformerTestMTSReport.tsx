...
const VoltagePotentialTransformerTestMTSReport: React.FC = () => {
  ...
  const initialInsulationResistanceItems = [
    {
      id: "ir-primary-gnd",
      windingTested: "Primary to Ground",
      testVoltage: "",
      results: "",
      units: "MΩ",
      correctedResults: "",
    ...
    {
      id: "ir-secondary-gnd",
      windingTested: "Secondary to Ground",
      testVoltage: "",
      results: "",
      units: "MΩ",
      correctedResults: "",
    ...
    {
      id: "ir-primary-secondary",
      windingTested: "Primary to Secondary",
      testVoltage: "",
      results: "",
      units: "MΩ",
      correctedResults: "",
      ...
      if (data && data.report_data) {
        ...
        setFormData((prev) => ({
          ...
          visualMechanicalInspection:
            ...
            initialVisualInspectionItems,
          insulationResistance:
            data.report_data.insulationResistance ||
            initialInsulationResistanceItems,
          turnsRatioTest:
            data.report_data.turnsRatioTest || initialTurnsRatioTestItems,
          ...
          fuseResistanceTest: {
            ...prev.fuseResistanceTest,
            ...data.report_data.fuseResistanceTest,
          },
          testEquipmentUsed: {
            ...prev.testEquipmentUsed,
            ...data.report_data.testEquipmentUsed,
          },
        }));
        setIsEditing(false);
      } else {
  ...
  const handleChange = (path: string, value: any) => {
    ...
    setFormData((prev) => {
      ...
      if (
        path.startsWith("insulationResistance") ||
        path === "temperature.tcf"
      ) {
        const irArray = Array.isArray(updatedForm.insulationResistance)
          ? updatedForm.insulationResistance
          : [];
        const newInsulationResistance = irArray.map((item: any) => ({
          ...item,
          correctedResults: calculateTempCorrectedReading(
            item.results,
            updatedForm.temperature.tcf,
          ),
        }));
      ...
      if (
        path.startsWith("turnsRatioTest") ||
        path === "secondaryVoltageAsFoundTap" ||
        path === "revenueMetering"
      ) {
        if (
          updatedForm.turnsRatioTest &&
          updatedForm.turnsRatioTest.length > 0
        ) {
          ...
          const secondaryVoltage = parseFloat(
            updatedForm.secondaryVoltageAsFoundTap,
          );
          ...
          if (
            ...
            if (!isNaN(measuredH1H2)) {
              ...
              if (percentDeviation === "") {
                ...
                passFail = calculatePassFail(
                  devNum,
                  updatedForm.revenueMetering,
                );
              }
            } else {
          ...
          updatedForm.turnsRatioTest = [
            { ...item, calculatedRatio, percentDeviation, passFail },
          ];
        }
      }
      return updatedForm;
  ...
  const handleVisualInspectionChange = (
    index: number,
    field: keyof FormData["visualMechanicalInspection"][0],
    value: string,
  ) => {
    setJustSaved(false);
    setFormData((prev) => {
  ...
  const handleInsulationResistanceChange = (
    index: number,
    field: keyof FormData["insulationResistance"][0],
    value: string,
  ) => {
    handleChange(`insulationResistance[${index}].${field}`, value);
  };

  const handleTurnsRatioChange = (
    field: keyof FormData["turnsRatioTest"][0],
    value: string,
  ) => {
  ...
  const handleSave = async () => {
    ...
    const irArray = Array.isArray(finalFormData.insulationResistance)
      ? finalFormData.insulationResistance
      : [];
    finalFormData.insulationResistance = irArray.map((item) => ({
      ...item,
      correctedResults: calculateTempCorrectedReading(
        item.results,
        finalFormData.temperature.tcf,
      ),
    }));
    if (
      finalFormData.turnsRatioTest &&
      finalFormData.turnsRatioTest.length > 0
    ) {
      const item = finalFormData.turnsRatioTest[0];
      const primaryVoltage = parseFloat(item.primaryVoltage);
      const secondaryVoltage = parseFloat(
        finalFormData.secondaryVoltageAsFoundTap,
      );
      const measuredH1H2 = parseFloat(item.measuredH1H2);
      let calculatedRatio = "";
      let percentDeviation = "";
    ...
    try {
      ...
      if (reportId) {
        ...
        if (checkError) {
          ...
          throw new Error(
            `Report with ID ${reportId} not found: ${checkError.message}`,
          );
        }

        if (!existingReport) {
          throw new Error(
            `Report with ID ${reportId} does not exist in the database.`,
          );
        }
  ...
  return (
    <ReportWrapper isPrintMode={isPrintMode}>
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6 dark:text-white">
        ...
        <div className="mb-6">
          <div className="w-full h-1 bg-[#f26722] mb-4"></div>
          <h2 className="text-xl font-semibold mb-4 text-zinc-900 dark:text-white border-b dark:border-zinc-700 pb-2 print:text-black print:border-black print:font-bold section-device-data">
            Device Data
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 gap-x-4 gap-y-2 print:hidden device-onscreen">
            <div>
              <label htmlFor="deviceData.manufacturer" className="form-label">
                Manufacturer:
              </label>
              <input
                id="deviceData.manufacturer"
                type="text"
            ...
            <div>
              <label htmlFor="deviceData.catalogNumber" className="form-label">
                Catalog Number:
              </label>
              <input
                id="deviceData.catalogNumber"
                type="text"
            ...
            <div>
              <label htmlFor="deviceData.serialNumber" className="form-label">
                Serial Number:
              </label>
              <input
            ...
            <div>
              <label htmlFor="deviceData.accuracyClass" className="form-label">
                Accuracy Class:
              </label>
              <input
                id="deviceData.accuracyClass"
                type="text"
                value={formData.deviceData.accuracyClass}
                onChange={(e) =>
                  handleChange("deviceData.accuracyClass", e.target.value)
                }
                readOnly={!isEditing}
                className={`form-input ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
              />
            ...
            <div>
              <label
                htmlFor="deviceData.manufacturedYear"
                className="form-label"
              >
                Manufactured Year:
              </label>
              <input
                id="deviceData.manufacturedYear"
                type="text"
                value={formData.deviceData.manufacturedYear}
                onChange={(e) =>
                  handleChange("deviceData.manufacturedYear", e.target.value)
                }
                readOnly={!isEditing}
                className={`form-input ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
              />
            ...
            <div>
              <label htmlFor="deviceData.voltageRating" className="form-label">
                Voltage Rating:
              </label>
              <input
                id="deviceData.voltageRating"
                type="text"
            ...
            <div>
              <label
                htmlFor="deviceData.insulationClass"
                className="form-label"
              >
                Insulation Class:
              </label>
              <input
                id="deviceData.insulationClass"
                type="text"
                value={formData.deviceData.insulationClass}
                onChange={(e) =>
                  handleChange("deviceData.insulationClass", e.target.value)
                }
                readOnly={!isEditing}
                className={`form-input ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
              />
            ...
            <div>
              <label htmlFor="deviceData.frequency" className="form-label">
                Frequency:
              </label>
              <input
                id="deviceData.frequency"
                type="text"
          ...
          <div className="overflow-x-auto">
            <table className="w-full table-fixed border-collapse border border-zinc-200 dark:border-zinc-700">
              ...
              <tbody className="bg-white dark:bg-dark-150">
                {(Array.isArray(formData.visualMechanicalInspection)
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 print:hidden fuse-onscreen">
            <div>
              <label htmlFor="fuseData.manufacturer" className="form-label">
                Manufacturer:
              </label>
              <input
            ...
            <div>
              <label htmlFor="fuseData.catalogNumber" className="form-label">
                Catalog Number:
              </label>
              <input
            ...
            <div>
              <label htmlFor="fuseData.class" className="form-label">
                Class:
              </label>
              <input
                id="fuseData.class"
            ...
            <div>
              <label htmlFor="fuseData.voltageRatingKv" className="form-label">
                Voltage Rating (kV):
              </label>
              <input
                id="fuseData.voltageRatingKv"
                type="text"
                value={formData.fuseData.voltageRatingKv}
                onChange={(e) =>
                  handleChange("fuseData.voltageRatingKv", e.target.value)
                }
                readOnly={!isEditing}
                className={`form-input ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
              />
            ...
            <div>
              <label htmlFor="fuseData.ampacityA" className="form-label">
                Ampacity (A):
              </label>
              <input
                id="fuseData.ampacityA"
                type="text"
                value={formData.fuseData.ampacityA}
                onChange={(e) =>
                  handleChange("fuseData.ampacityA", e.target.value)
                }
                readOnly={!isEditing}
                className={`form-input ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
              />
            ...
            <div>
              <label htmlFor="fuseData.icRatingKa" className="form-label">
                I.C. Rating (kA):
              </label>
              <input
        ...
        <div className="mb-6">
          <div className="w-full h-1 bg-[#f26722] mb-4"></div>
          <h2 className="text-xl font-semibold mb-4 text-zinc-900 dark:text-white border-b dark:border-zinc-700 pb-2 print:text-black print:border-black print:font-bold section-fuse-resistance">
            Electrical Tests - Fuse Resistance
          </h2>
          <div className="overflow-x-auto">
            <table className="min-w-full w-max divide-y divide-zinc-200 dark:divide-zinc-700">
              ...
              <tbody className="bg-white dark:bg-dark-150 divide-y divide-zinc-200 dark:divide-zinc-700">
                <tr>
                  ...
                  <td className="px-6 py-4">
                    <input
                      type="text"
                      value={formData.fuseResistanceTest.asFound}
                      onChange={(e) =>
                        handleChange(
                          "fuseResistanceTest.asFound",
                          e.target.value,
                        )
                      }
                      readOnly={!isEditing}
                  ...
                  <td className="px-6 py-4">
                    <input
                      type="text"
                      value={formData.fuseResistanceTest.asLeft}
                      onChange={(e) =>
                        handleChange(
                          "fuseResistanceTest.asLeft",
                          e.target.value,
                        )
                      }
                      readOnly={!isEditing}
                  ...
                  <td className="px-6 py-4">
                    <select
                      value={formData.fuseResistanceTest.units}
                      onChange={(e) =>
                        handleChange("fuseResistanceTest.units", e.target.value)
                      }
                      disabled={!isEditing}
                      className={`form-select w-full ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
                    >
                      {fuseResistanceUnitOptions.map((opt) => (
        ...
        <div className="mb-6">
          <div className="w-full h-1 bg-[#f26722] mb-4"></div>
          <h2 className="text-xl font-semibold mb-4 text-zinc-900 dark:text-white border-b dark:border-zinc-700 pb-2 print:text-black print:border-black print:font-bold section-insulation-ratio">
            Electrical Tests - Insulation Resistance & Ratio
          </h2>
            ...
            <div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                  ...
                  <tbody className="bg-white dark:bg-dark-150 divide-y divide-zinc-200 dark:divide-zinc-700">
                    {(Array.isArray(formData.insulationResistance)
                      ...
                      <tr key={item.id}>
                        ...
                        <td className="px-3 py-2">
                          <select
                            value={item.testVoltage}
                            onChange={(e) =>
                              handleInsulationResistanceChange(
                                index,
                                "testVoltage",
                                e.target.value,
                              )
          ...
          <div>
            <div className="mb-4">
              <label
                htmlFor="secondaryVoltageAsFoundTap"
                className="form-label inline-block mr-2"
              >
                Secondary Voltage at as-found tap:
              </label>
              <input
                id="secondaryVoltageAsFoundTap"
                type="text"
                value={formData.secondaryVoltageAsFoundTap}
                onChange={(e) =>
                  handleChange("secondaryVoltageAsFoundTap", e.target.value)
                }
                readOnly={!isEditing}
                className={`form-input inline-block w-24 ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
              />
            ...
            <div className="mb-4 print:hidden">
              <label className="flex items-center space-x-2">
                <input
                  ...
                  disabled={!isEditing}
                  className={`form-checkbox h-4 w-4 text-[#f26722] focus:ring-[#f26722] border-zinc-300 rounded ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
                />
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                <thead className="bg-zinc-50 dark:bg-dark-150">
                  <tr>
                    {[
                      ...
                      <th
                        key={header}
                        className="px-2 py-3 text-left text-xs font-medium text-zinc-500 dark:text-white uppercase tracking-wider"
                      >
                        {header}
                      </th>
                    ))}
                ...
                <tbody className="bg-white dark:bg-dark-150 divide-y divide-zinc-200 dark:divide-zinc-700">
                  {formData.turnsRatioTest &&
                    formData.turnsRatioTest.length > 0 && (
                      <tr>
                        <td className="px-2 py-2">
                          <input
                            type="text"
                            value={formData.turnsRatioTest[0].tap}
                            onChange={(e) =>
                              handleTurnsRatioChange("tap", e.target.value)
                            }
                            readOnly={!isEditing}
                            className={`form-input w-full ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
                          />
                        ...
                        <td className="px-2 py-2">
                          <input
                            type="text"
                            value={formData.turnsRatioTest[0].primaryVoltage}
                            onChange={(e) =>
                        ...
                        <td className="px-2 py-2">
                          <input
                            type="text"
                            value={formData.turnsRatioTest[0].calculatedRatio}
                            readOnly
                            className="form-input w-full bg-zinc-100 dark:bg-dark-150"
                          />
                        </td>
                        <td className="px-2 py-2">
                          <input
                            type="text"
                            value={formData.turnsRatioTest[0].measuredH1H2}
                            onChange={(e) =>
                              handleTurnsRatioChange(
                                "measuredH1H2",
                                e.target.value,
                        ...
                        <td className="px-2 py-2">
                          <input
                            type="text"
                            value={formData.turnsRatioTest[0].percentDeviation}
                            readOnly
                            className="form-input w-full bg-zinc-100 dark:bg-dark-150"
                          />
                        </td>
                        <td className="px-2 py-2">
                          <input
                            type="text"
                            value={formData.turnsRatioTest[0].passFail}
                            readOnly
                            className="form-input w-full bg-zinc-100 dark:bg-dark-150"
                          />
                        </td>
        ...
        <div className="mb-6">
          ...
          <div className="space-y-4 print:hidden">
            {(
              Object.keys(formData.testEquipmentUsed) as Array<
                keyof FormData["testEquipmentUsed"]
              >
            ).map((equipmentKey) => {
              const equipment = formData.testEquipmentUsed[equipmentKey];
              const label = equipmentKey
                .replace(/([A-Z])/g, " $1")
              ...
              const handleEquipmentSelect = (selectedEquipment: any) => {
                ...
                handleChange(
                  `testEquipmentUsed.${equipmentKey}.name`,
                  selectedEquipment.equipment_name,
                );
                handleChange(
                  `testEquipmentUsed.${equipmentKey}.serialNumber`,
                  selectedEquipment.serial_number || "",
                );
                handleChange(
                  `testEquipmentUsed.${equipmentKey}.ampId`,
                  selectedEquipment.amp_id || "",
                );
                handleChange(
                  `testEquipmentUsed.${equipmentKey}.calDate`,
              ...
              return (
                ...
                >
                  ...
                  <div>
                    <label
                      htmlFor={`${equipmentKey}Serial`}
                      className="form-label block"
                    >
                      Serial Number:
                    </label>
                    <input
                      id={`${equipmentKey}Serial`}
                      type="text"
                  ...
                  <div>
                    <label
                      htmlFor={`${equipmentKey}AmpId`}
                      className="form-label block"
                    >
                      AMP ID:
                    </label>
                    <input
                      id={`${equipmentKey}AmpId`}
                      type="text"
                      value={equipment.ampId}
                      onChange={(e) =>
                        handleChange(
                          `testEquipmentUsed.${equipmentKey}.ampId`,
                          e.target.value,
                  ...
                  <div>
                    <label
                      htmlFor={`${equipmentKey}CalDate`}
                      className="form-label block"
                    >
                      Cal Date:
                    </label>
                    <input
                      id={`${equipmentKey}CalDate`}
                      type="text"
                      value={equipment.calDate || ""}
                      onChange={(e) =>
                        handleChange(
                          `testEquipmentUsed.${equipmentKey}.calDate`,
                          e.target.value,
          ...
          <div className="hidden print:block">
            <table className="w-full table-fixed border-collapse border border-zinc-200 dark:border-zinc-700">
              ...
              <tbody>
                ...
                <tr>
                  <td>
                    {formData.testEquipmentUsed.ttrTestSet.name ||
                      "TTR Test Set"}
                  </td>
        ...
        >
          ...
          <textarea
            ...
            readOnly={!isEditing}
            rows={1}
            className={`form-textarea w-full resize-none print:hidden ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
