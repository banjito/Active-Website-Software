...
export function QualityMetrics({ division }: QualityMetricsProps) {
  ...
  const [selectedEquipment, setSelectedEquipment] =
    useState<LabEquipment | null>(null);
  ...
  const filteredMetrics = metrics.filter((metric) => {
    ...
    const matchesSearch =
      ...
      (metric.equipment_name &&
        metric.equipment_name.toLowerCase().includes(searchTerm.toLowerCase()));

    // Filter by equipment
    const matchesEquipment =
      equipmentFilter === null || metric.equipment_id === equipmentFilter;
  ...
  const getMetricStatus = (
    ...
    if (
      metric.upper_threshold !== undefined &&
      metric.metric_value > metric.upper_threshold
    ) {
      return "above-threshold";
    }
    if (
      metric.lower_threshold !== undefined &&
      metric.metric_value < metric.lower_threshold
    ) {
      return "below-threshold";
    }
    return "within-threshold";
  ...
  const calculateMetricsSummary = () => {
    ...
    return {
      ...
      withinThreshold: filteredMetrics.filter(
        (m) => m.status === "within-threshold",
      ).length,
      belowThreshold: filteredMetrics.filter(
        (m) => m.status === "below-threshold",
      ).length,
      aboveThreshold: filteredMetrics.filter(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      !form.metric_name ||
      form.metric_value === undefined ||
      form.target_value === undefined ||
      !form.date_recorded
    ) {
      toast({
        title: "Error",
    ...
    const newMetric: Partial<QualityMetric> = {
      ...
      notes: form.notes || "",
      status: getMetricStatus(form as QualityMetric),
    ...
    try {
      ...
      if (editingMetricId) {
        if (response.data) {
          setMetrics(
            metrics.map((m) =>
              m.id === editingMetricId ? (response.data as QualityMetric) : m,
            ),
          );
        }
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (
      confirm(
        "Are you sure you want to delete this quality metric? This action cannot be undone.",
      )
    ) {
  ...
  const generatePDFReport = () => {
    ...
    doc.setFontSize(18);
    doc.text(
      `Quality Metrics Report - ${reportType.charAt(0).toUpperCase() + reportType.slice(1)}`,
      14,
      22,
    );
    ...
    doc.setFontSize(12);
    doc.text(
      `Period: ${new Date(reportDateRange.start).toLocaleDateString()} to ${new Date(reportDateRange.end).toLocaleDateString()}`,
      14,
      30,
    );
    ...
    doc.text(
      `Within Threshold: ${metricsSummary.withinThreshold} (${Math.round((metricsSummary.withinThreshold / metricsSummary.total) * 100)}%)`,
      14,
      58,
    );
    doc.text(
      `Below Threshold: ${metricsSummary.belowThreshold} (${Math.round((metricsSummary.belowThreshold / metricsSummary.total) * 100)}%)`,
      14,
      66,
    );
    doc.text(
      `Above Threshold: ${metricsSummary.aboveThreshold} (${Math.round((metricsSummary.aboveThreshold / metricsSummary.total) * 100)}%)`,
      14,
      74,
    );
    ...
    if (reportType !== "summary") {
      const tableData = filteredMetrics.map((metric) => [
        ...
        metric.status === "within-threshold"
          ? "Within Range"
          : metric.status === "below-threshold"
            ? "Below Range"
            : "Above Range",
        metric.equipment_name || "-",
      ...
      doc.autoTable({
        startY: 90,
        head: [
          ["Metric Name", "Value", "Status", "Equipment", "Date Recorded"],
        ],
        body: tableData,
      });
    }
    ...
    // Save the document
    doc.save(
      `quality-metrics-report-${new Date().toISOString().split("T")[0]}.pdf`,
    );
  ...
  return (
    <div className="space-y-6">
                    ...
                    <Card key={metric.id} className="overflow-hidden">
                      ...
                      <CardContent>
                        <div className="space-y-2">
                          ...
                          {(metric.lower_threshold !== null &&
                            metric.lower_threshold !== undefined) ||
                          (metric.upper_threshold !== null &&
                            metric.upper_threshold !== undefined) ? (
                            <div className="flex justify-between">
                              <span className="text-sm font-medium">
                                Acceptable Range:
      ...
      {showForm && (
        <Dialog open={showForm} onOpenChange={setShowForm}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>
                {editingMetricId ? "Edit Metric" : "Add New Quality Metric"}
              </DialogTitle>
            </DialogHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  name="target_value"
                  value={
                    form.target_value === undefined ? "" : form.target_value
                  }
                  onChange={handleInputChange}
                  placeholder="e.g., 100"
                  step="any"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    value={
                      form.lower_threshold === undefined
                        ? ""
                        : form.lower_threshold
                    }
                    onChange={handleInputChange}
                    placeholder="e.g., 95"
                    step="any"
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={
                      form.upper_threshold === undefined
                        ? ""
                        : form.upper_threshold
                    }
                    onChange={handleInputChange}
                    placeholder="e.g., 105"
                    step="any"
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <Select
                ...
                onChange={(e) =>
                  setReportType(
                    e.target.value as "summary" | "detailed" | "trends",
                  )
                }
                className="w-full"
                options={[
      ...
      {/* Alert Setup Dialog */}
      <Dialog
        open={showAlertSetupDialog}
        onOpenChange={setShowAlertSetupDialog}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="pt-4 flex justify-end space-x-2">
              <Button
                type="button"
                variant="outline"
                onClick={() => setShowAlertSetupDialog(false)}
              >
                Cancel
              </Button>
              <Button
