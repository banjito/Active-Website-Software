...
const LargeDryTypeTransformerReport: React.FC = () => {
  ...
  const reportSlug = currentPath.includes("large-dry-type-transformer-report")
    ? "large-dry-type-transformer-report"
    : currentPath.includes("large-dry-type-transformer")
  ...
  const getVisualInspectionDescription = (id: string): string => {
    ...
    const descriptions: { [key: string]: string } = {
      ...
      "7.2.1.2.A.8":
        "Inspect bolted electrical connections using low-resistance ohmmeter, torque wrench, or IR scan",
      "7.2.1.2.A.9":
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      const err = error as SupabaseError;
      console.error(
        `Error loading report from ${LARGE_TRANSFORMER_TABLE}:`,
        err,
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaving(false); // Finish loading indicator
    }
  };
