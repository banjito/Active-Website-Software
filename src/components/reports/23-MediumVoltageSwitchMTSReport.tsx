  ...
  const loadJobInfo = async () => {
    ...
    try {
      ...
      if (jobData?.customer_id) {
        ...
        setForm(prev => ({
          ...prev,
          customer: (customerData as any)?.company_name || (customerData as any)?.name || '',
          address: (jobData as any)?.site_address || (customerData as any)?.address || '',
        ...
        <div>
          <div className="mb-4 flex">
            ...
            <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
              <input type="text" value={form.customer} onChange={e => setJobInfo('customer', e.target.value)} readOnly={!isEditMode} className={`w-full bg-transparent border-none focus:ring-0 ${!isEditMode ? 'cursor-default' : ''}`} />
            </div>
          </div>
          ...
          <div className="mb-4 flex">
            ...
            <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
              <input type="text" value={form.address} onChange={e => setJobInfo('address', e.target.value)} readOnly={!isEditMode} className={`w-full bg-transparent border-none focus:ring-0 ${!isEditMode ? 'cursor-default' : ''}`} />
            </div>
          </div>
  ...
  const equipment = (
    <section className="mb-6">
      ...
      {([
        ...
        <div key={key} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
