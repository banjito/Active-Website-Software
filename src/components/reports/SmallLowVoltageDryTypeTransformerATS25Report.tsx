...
const SmallLowVoltageDryTypeTransformerATS25Report: React.FC = () => {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    visualInspectionItems: [
      ...
      { id: '7.2.1.1.A.6', description: 'Verify tightness of accessible bolted electrical connections by calibrated torque-wrench method or in accordance with 7.2.1.1.B.1 (Low Resistance Ohmmeter).', result: 'Select One' },
    ...
    insulationTestVoltage: '1000V', insulationDuration: '1 min', insulationUnit: 'MΩ',
    insulationRows: [
    ...
    insulationCriteriaValue: '≥ 500', insulationCriteriaUnits: 'MΩ',
    ...
    try {
      ...
      if (data) {
        ...
        setFormData(prev => ({
          ...
          insulationCriteriaValue: ir.criteriaValue || prev.insulationCriteriaValue,
          insulationCriteriaUnits: ir.criteriaUnits || prev.insulationCriteriaUnits,
          dielectricAbsorptionRatio: ir.dielectricAbsorptionRatio || prev.dielectricAbsorptionRatio,
          turnsRatio: tr.tapUnderTest ? { ...prev.turnsRatio, ...tr } : prev.turnsRatio,
  ...
  const getVoltageForRow = (rowIndex: number): string => {
    switch (rowIndex) {
      case 0: return formData.nameplate.primaryVoltage1; // Primary to Ground uses primary voltage
      case 1: return formData.nameplate.secondaryVoltage1; // Secondary to Ground uses secondary voltage
      case 2: return formData.nameplate.primaryVoltage1; // Primary to Secondary uses primary voltage
      default: return '';
    }
  };
    ...
    setFormData(prev => ({
      ...
      insulationRows: prev.insulationRows.map(row => {
        const correct = (v: string) => {
          ...
          if (!trimmed || trimmed.toLowerCase() === 'n/a' || trimmed.startsWith('>') || trimmed.startsWith('<')) return trimmed || 'N/A';
  ...
  }, [formData.temperature.tcf, JSON.stringify(formData.insulationRows.map(r => [r.measured05Min, r.measured1Min]))]);
  ...
  useEffect(() => {
    ...
    const allRatios = [priToGnd, secToGnd, priToSec].map(v => parseFloat(v)).filter(n => isFinite(n));
    ...
    setFormData(prev => ({
      ...prev,
      dielectricAbsorptionRatio: { ...prev.dielectricAbsorptionRatio, priToGnd, secToGnd, priToSec, result }
    }));
  ...
  useEffect(() => {
    ...
    const tapVoltageKey = `tapVoltage${tapUnderTest}` as keyof typeof formData.nameplate;
    const primaryWindingVoltage = (formData.nameplate[tapVoltageKey] as string) || '';
  ...
  useEffect(() => {
    ...
    const updatedRows = formData.turnsRatio.rows.map(row => {
      ...
      const result: 'Pass' | 'Fail' = (deviation <= 0.5 && deviation >= -0.5) ? 'Pass' : 'Fail';
      return { ...row, percentDeviation: deviation.toFixed(4), result };
    });

    // Calculate difference between MR (max - min)
    const measuredRatios = updatedRows.map(r => parseFloat(r.measuredRatio)).filter(n => isFinite(n));
    ...
    setFormData(prev => ({
      ...prev,
      turnsRatio: { ...prev.turnsRatio, calculatedRatio, rows: updatedRows, differenceBetweenMR, differenceResult }
    }));
  }, [formData.nameplate.primaryVoltage1, formData.nameplate.secondaryVoltage2, JSON.stringify(formData.turnsRatio.rows.map(r => r.measuredRatio))]);
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap justify-between items-center mb-4 gap-4">
              <div className="flex items-center gap-2">
                ...
                <input value={formData.insulationTemperature} onChange={e => setFormData(p => ({ ...p, insulationTemperature: e.target.value }))} readOnly={!isEditing} className={`form-input w-20 ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
              </div>
              <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <select value={formData.insulationTestVoltage} onChange={e => setFormData(p => ({ ...p, insulationTestVoltage: e.target.value }))} disabled={!isEditing} className={`form-select ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>
                  {INSULATION_RESISTANCE_TEST_VOLTAGES.map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-sm">Test Duration:</span>
                <input value={formData.insulationDuration} onChange={e => setFormData(p => ({ ...p, insulationDuration: e.target.value }))} readOnly={!isEditing} className={`form-input w-20 ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border border-gray-200 dark:border-gray-700 print:table-fixed print:w-full">
                <thead>
                  <tr>
                    <th className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-left text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px] print:w-[12%]" rowSpan={2}>
                      <span className="print:hidden">Winding Under Test</span>
                      <span className="hidden print:inline">Winding<br/>Under<br/>Test</span>
                    </th>
                    <th className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px]" colSpan={2}>Measured Values</th>
                    <th className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px]" colSpan={2}>Temp Corrected</th>
                    <th className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px] print:w-[8%]" rowSpan={2}>Units</th>
                    <th className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px]" colSpan={2}>Table 100.5</th>
                    <th className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px] print:w-[8%]" rowSpan={2}>Results</th>
                  </tr>
                  <tr>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white">0.5 Min.</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white">1 Min.</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white">0.5 Min.</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white">1 Min.</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white">Value</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white">Units</th>
                  </tr>
                </thead>
                <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-200 dark:divide-gray-700">
                  {formData.insulationRows.map((row, idx) => {
                    ...
                    const criteriaValue = getTable1005Criteria(voltageForRow, formData.insulationUnit);
                    const criteriaDisplay = criteriaValue ? `≥ ${criteriaValue}` : '-';
                    ...
                    if (isFinite(corrected1MinVal) && isFinite(criteriaNum)) {
                      result = corrected1MinVal >= criteriaNum ? 'Pass' : 'Fail';
                    }
                    return (
                      <tr key={idx}>
                        <td className="px-3 py-2 text-sm text-gray-900 dark:text-white">{row.windingUnderTest}</td>
                        <td className="px-3 py-2">
                          <input value={row.measured05Min} onChange={e => { const rows = [...formData.insulationRows]; rows[idx].measured05Min = e.target.value; setFormData(p => ({ ...p, insulationRows: rows })); }} readOnly={!isEditing} className={`form-input w-full text-center ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
                        </td>
                        <td className="px-3 py-2">
                          <input value={row.measured1Min} onChange={e => { const rows = [...formData.insulationRows]; rows[idx].measured1Min = e.target.value; setFormData(p => ({ ...p, insulationRows: rows })); }} readOnly={!isEditing} className={`form-input w-full text-center ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
                        </td>
                        <td className="px-3 py-2 text-center text-sm text-gray-900 dark:text-white">{row.corrected05Min || '-'}</td>
                        <td className="px-3 py-2 text-center text-sm text-gray-900 dark:text-white">{row.corrected1Min || '-'}</td>
                        <td className="px-3 py-2">
                          <select value={formData.insulationUnit} onChange={e => setFormData(p => ({ ...p, insulationUnit: e.target.value }))} disabled={!isEditing} className={`form-select w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>
                            {INSULATION_RESISTANCE_UNITS.map(u => <option key={u} value={u}>{u}</option>)}
                          </select>
                        </td>
                        <td className="px-3 py-2 text-center text-sm">{criteriaDisplay}</td>
                        <td className="px-3 py-2 text-center text-sm">{formData.insulationUnit}</td>
                        <td className={`px-3 py-2 text-center text-sm font-semibold ${result === 'Pass' ? 'text-green-600' : result === 'Fail' ? 'text-red-600' : ''}`}>{result}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="mt-4 overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border border-gray-200 dark:border-gray-700 print:table-fixed print:w-full">
                <thead>
                  <tr>
                    <th rowSpan={2} className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-left text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px] align-middle">
                      <span className="print:hidden">Dielectric Absorption Ratio</span>
                      <span className="hidden print:inline">Dielectric<br/>Absorption<br/>Ratio</span>
                    </th>
                    <th className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px]">Calculated as:</th>
                    <th className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px]">Pri to Gnd</th>
                    <th className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px]">Sec to Gnd</th>
                    <th className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px]">Pri to Sec</th>
                    <th className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px]">Criteria</th>
                    <th className="px-1 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 print:text-[7px]">Result</th>
                  </tr>
                  <tr>
                    <td className="px-2 py-2 text-center text-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-150 print:text-[8px]">1 Min. / 0.5 Min. Values</td>
                    <td className="px-2 py-2 text-center text-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-150 print:text-[8px]">{formData.dielectricAbsorptionRatio.priToGnd || '-'}</td>
                    <td className="px-2 py-2 text-center text-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-150 print:text-[8px]">{formData.dielectricAbsorptionRatio.secToGnd || '-'}</td>
                    <td className="px-2 py-2 text-center text-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-150 print:text-[8px]">{formData.dielectricAbsorptionRatio.priToSec || '-'}</td>
                    <td className="px-2 py-2 text-center text-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-150 print:text-[8px]">{formData.dielectricAbsorptionRatio.criteria}</td>
                    <td className={`px-2 py-2 text-center text-sm font-semibold border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-150 print:text-[8px] ${formData.dielectricAbsorptionRatio.result === 'Pass' ? 'text-green-600 print:bg-green-100' : formData.dielectricAbsorptionRatio.result === 'Fail' ? 'text-red-600 print:bg-red-100' : ''}`}>{formData.dielectricAbsorptionRatio.result || '-'}</td>
                  </tr>
                </thead>
              </table>
            </div>
            ...
            <div className="flex flex-wrap gap-4 items-start">
              ...
              <div className="flex-shrink-0">
                <table className="border-collapse border border-gray-200 dark:border-gray-700">
                  <tbody className="bg-white dark:bg-dark-150">
                    <tr>
                      <td className="px-2 py-1 text-sm font-medium text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 text-right whitespace-nowrap">Tap Under Test:</td>
                      <td className="px-2 py-1 border border-gray-200 dark:border-gray-700 w-20">
                        <div className="print:hidden">
                          <select value={formData.turnsRatio.tapUnderTest} onChange={e => setFormData(p => ({ ...p, turnsRatio: { ...p.turnsRatio, tapUnderTest: e.target.value } }))} disabled={!isEditing} className={`form-select w-full text-sm ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>
                            {TAP_POSITIONS.map(pos => <option key={pos} value={pos}>{pos}</option>)}
                          </select>
                        </div>
                        <div className="hidden print:block text-center">{formData.turnsRatio.tapUnderTest}</div>
                      </td>
                    </tr>
                    <tr>
                      <td className="px-2 py-1 text-sm font-medium text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 text-right whitespace-nowrap">Primary Winding Voltage:</td>
                      <td className="px-2 py-1 border border-gray-200 dark:border-gray-700 text-center text-sm">{formData.turnsRatio.primaryWindingVoltage || '-'} V</td>
                    </tr>
                    <tr>
                      <td className="px-2 py-1 text-sm font-medium text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 text-right whitespace-nowrap">Secondary Winding Voltage:</td>
                      <td className="px-2 py-1 border border-gray-200 dark:border-gray-700 text-center text-sm">{formData.turnsRatio.secondaryWindingVoltage || '-'} V</td>
                    </tr>
                    <tr>
                      <td className="px-2 py-1 text-sm font-medium text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 text-right whitespace-nowrap">Calculated Ratio (CR):</td>
                      <td className="px-2 py-1 border border-gray-200 dark:border-gray-700 text-center text-sm font-semibold">{formData.turnsRatio.calculatedRatio || '#DIV/0!'}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div className="flex-grow">
                <table className="border-collapse border border-gray-200 dark:border-gray-700 w-full">
                  <thead>
                    <tr>
                      <th className="px-2 py-1 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700">Primary Winding</th>
                      <th className="px-2 py-1 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700">Measured Ratio (MR)</th>
                      <th className="px-2 py-1 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700">% Deviation from CR</th>
                      <th className="px-2 py-1 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700">Results</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white dark:bg-dark-150">
                    {formData.turnsRatio.rows.map((row, idx) => (
                      <tr key={idx}>
                        <td className="px-2 py-1 text-center text-sm border border-gray-200 dark:border-gray-700">{row.primaryWinding}</td>
                        <td className="px-2 py-1 border border-gray-200 dark:border-gray-700">
                          <div className="print:hidden">
                            <input value={row.measuredRatio || ''} onChange={e => { const rows = [...formData.turnsRatio.rows]; rows[idx].measuredRatio = e.target.value; setFormData(p => ({ ...p, turnsRatio: { ...p.turnsRatio, rows } })); }} readOnly={!isEditing} className={`form-input w-full text-center text-sm ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
                          </div>
                          <div className="hidden print:block text-center text-sm">{row.measuredRatio || '-'}</div>
                        </td>
                        <td className="px-2 py-1 text-center text-sm border border-gray-200 dark:border-gray-700">{row.percentDeviation || '-'}</td>
                        <td className={`px-2 py-1 text-center text-sm font-semibold border border-gray-200 dark:border-gray-700 ${row.result === 'Pass' ? 'text-green-600 bg-green-100 dark:bg-green-900 print:bg-green-100' : row.result === 'Fail' ? 'text-red-600 bg-red-100 dark:bg-red-900 print:bg-red-100' : ''}`}>{row.result || '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="flex-shrink-0">
                <table className="border-collapse border border-gray-200 dark:border-gray-700">
                  <thead>
                    <tr>
                      <th className="px-2 py-1 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700" colSpan={2}>Difference (%) between MR</th>
                    </tr>
                    <tr>
                      <th className="px-2 py-1 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 w-16"></th>
                      <th className="px-2 py-1 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase border border-gray-200 dark:border-gray-700 w-16">Results</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white dark:bg-dark-150">
                    <tr>
                      <td className="px-2 py-1 text-center text-sm border border-gray-200 dark:border-gray-700">{formData.turnsRatio.differenceBetweenMR || 'N/A'}</td>
                      <td className={`px-2 py-1 text-center text-sm font-semibold border border-gray-200 dark:border-gray-700 ${formData.turnsRatio.differenceResult === 'Pass' ? 'text-green-600 bg-green-100 dark:bg-green-900 print:bg-green-100' : formData.turnsRatio.differenceResult === 'Fail' ? 'text-red-600 bg-red-100 dark:bg-red-900 print:bg-red-100' : ''}`}>{formData.turnsRatio.differenceResult || 'N/A'}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border border-gray-200 dark:border-gray-700">
                ...
                <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  <tr>
                    <td className="px-3 py-2 text-sm font-medium border border-gray-200 dark:border-gray-700">TTR Test Set:</td>
                    <td className="px-3 py-2 border border-gray-200 dark:border-gray-700">
                      <div className="print:hidden">
                        <EquipmentAutocomplete
                          value={formData.testEquipment.ttrTestSet.name}
                          onChange={value => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, ttrTestSet: { ...p.testEquipment.ttrTestSet, name: value } } }))}
                          onSelect={(equipment) => {
                            const formatDate = (dateString: string | null): string => {
                              if (!dateString) return '';
                              try {
                      ...
                      <div className="hidden print:block text-center text-sm">{formData.testEquipment.ttrTestSet.name || '-'}</div>
                    </td>
                    <td className="px-3 py-2 border border-gray-200 dark:border-gray-700">
                      <div className="print:hidden"><input className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} value={formData.testEquipment.ttrTestSet.serialNumber} onChange={e => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, ttrTestSet: { ...p.testEquipment.ttrTestSet, serialNumber: e.target.value } } }))} readOnly={!isEditing} /></div>
                      <div className="hidden print:block text-center text-sm">{formData.testEquipment.ttrTestSet.serialNumber || '-'}</div>
                    </td>
                    <td className="px-3 py-2 border border-gray-200 dark:border-gray-700">
                      <div className="print:hidden"><input className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} value={formData.testEquipment.ttrTestSet.ampId} onChange={e => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, ttrTestSet: { ...p.testEquipment.ttrTestSet, ampId: e.target.value } } }))} readOnly={!isEditing} /></div>
                      <div className="hidden print:block text-center text-sm">{formData.testEquipment.ttrTestSet.ampId || '-'}</div>
                    </td>
                    <td className="px-3 py-2 border border-gray-200 dark:border-gray-700">
                      <div className="print:hidden"><input className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} value={formData.testEquipment.ttrTestSet.calDate} onChange={e => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, ttrTestSet: { ...p.testEquipment.ttrTestSet, calDate: e.target.value } } }))} readOnly={!isEditing} /></div>
                      <div className="hidden print:block text-center text-sm">{formData.testEquipment.ttrTestSet.calDate || '-'}</div>
                    </td>
                  </tr>
                </tbody>
              </table>
