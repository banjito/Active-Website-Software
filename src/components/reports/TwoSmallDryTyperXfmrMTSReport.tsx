...
const TwoSmallDryTyperXfmrMTSReport: React.FC = () => {
  ...
  const loadJobInfo = useCallback(async () => {
    ...
    try {
      ...
      if (jobData?.customer_id) {
        ...
        customerAddress = (jobData as any).site_address || customerData?.address || customerAddress || '';
      }
      setFormData(prev => ({
        ...prev,
  ...
  useEffect(() => {
    ...
    const nextTests = formData.insulationResistance.tests.map(test => {
      const passThroughOrApply = (raw: string): string => {
        if (!raw) return '';
      ...
      const corrected0_5Min = passThroughOrApply(test.measured0_5Min);
      const corrected1Min = passThroughOrApply(test.measured1Min);
  ...
  useEffect(() => {
    ...
    setFormData(prev => {
      const newTurnsRatioTests = prev.turnsRatio.tests.map(test => {
        ...
        if (primaryVoltage && !isNaN(primaryVoltage) && secondaryVoltage && !isNaN(secondaryVoltage)) {
        ...
        const calculateDeviationAndResult = (measured: string, calculated: string) => {
          ...
          const deviation = ((calculatedValue - measuredValue) / calculatedValue * 100).toFixed(3);
    ...
    <ReportWrapper isPrintMode={isPrintMode}>
      ...
      <div className="print:flex hidden items-center justify-between border-b-2 border-gray-800 pb-4 mb-6">
        ...
        <div className="text-right font-extrabold text-xl" style={{ color: '#1a4e7c', width: '120px' }}>
          ...
          <div className="hidden print:block mt-2">
            <div
              className={`pass-fail-status-box ${String(formData.status).toLowerCase() === 'fail' ? 'fail' : 'pass'}`}
              style={{
      ...
      <div className="p-6 flex justify-center dark:bg-dark-150">
          ...
          <div className="hidden print:block space-y-4">
            ...
            <table className="min-w-full border-collapse border border-gray-300 dark:border-gray-600 nameplate-table">
              ...
              <tbody>
                ...
                <tr>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white">
                    <div className="text-xs font-bold">Manufacturer:</div>
                    <div className="text-sm mt-0.5">{formData.nameplate.manufacturer || ''}</div>
                  </td>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white">
                    <div className="text-xs font-bold">Catalog Number:</div>
                    <div className="text-sm mt-0.5">{formData.nameplate.catalogNumber || ''}</div>
                  </td>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white">
                    <div className="text-xs font-bold">Serial Number:</div>
                    <div className="text-sm mt-0.5">{formData.nameplate.serialNumber || ''}</div>
                  </td>
                </tr>
                ...
                <tr>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white">
                    ...
                    <div className="text-sm mt-0.5">{formData.nameplate.kvaBase || ''}{formData.nameplate.kvaCooling ? ` / ${formData.nameplate.kvaCooling}` : ''}</div>
                  </td>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white">
                    <div className="text-xs font-bold">Temp. Rise (°C):</div>
                    <div className="text-sm mt-0.5">{formData.nameplate.tempRise || ''}</div>
                  </td>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white">
                    <div className="text-xs font-bold">Impedance:</div>
                    <div className="text-sm mt-0.5">{formData.nameplate.impedance || ''}</div>
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <section className="mb-6">
          ...
          <div className="overflow-x-scroll turns-ratio-scroll" style={{ 
            scrollbarWidth: 'auto', 
            ...
            marginBottom: '8px'
              ...
              <table className="w-full border-collapse border border-gray-300 dark:border-gray-600 turns-ratio-table" style={{ minWidth: '1400px' }}>
                  <colgroup>
        ...
        <section className="mb-6">
          ...
          <div className="hidden print:block">
            <table className="w-full border-collapse border border-black test-equipment-table" style={{ tableLayout: 'fixed' }}>
              ...
              <thead>
                <tr>
                  <th className="px-4 py-2 bg-gray-50 text-left text-sm font-bold border border-black">Equipment:</th>
                  <th className="px-4 py-2 bg-gray-50 text-left text-sm font-bold border border-black">Serial Number:</th>
                  <th className="px-4 py-2 bg-gray-50 text-left text-sm font-bold border border-black">AMP ID:</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td className="px-4 py-3 border border-black text-sm font-medium">Megohmmeter</td>
                  <td className="px-4 py-3 border border-black text-sm">{formData.testEquipment.megohmmeter.serialNumber || ''}</td>
                  <td className="px-4 py-3 border border-black text-sm">{formData.testEquipment.megohmmeter.ampId || ''}</td>
                </tr>
                <tr>
                  <td className="px-4 py-3 border border-black text-sm font-medium">TTR Test Set</td>
                  <td className="px-4 py-3 border border-black text-sm">{formData.testEquipment.ttrTestSet.serialNumber || ''}</td>
                  <td className="px-4 py-3 border border-black text-sm">{formData.testEquipment.ttrTestSet.ampId || ''}</td>
                </tr>
              </tbody>
            </table>
          </div>
