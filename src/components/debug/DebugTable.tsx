...
export default function DebugTableCheck() {
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      ...
      {connectionInfo && (
        <div
          className={`mb-4 p-3 rounded ${connectionInfo.connected ? "bg-green-100" : "bg-red-100"}`}
        >
          <h2 className="font-medium">Connection Status:</h2>
          <p>Database URL: {connectionInfo.url}</p>
          <p>
            Status:{" "}
            {connectionInfo.connected ? "Connected" : "Connection Issues"}
          </p>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="bg-white shadow-md rounded p-6">
          ...
          {!tables.includes("transformer_reports") &&
            !tables.includes("transformer_reports exists") && (
              <div className="mb-6">
          ...
          {creationResult && (
            <div
              className={`mt-4 p-3 rounded ${creationResult.success ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
            >
