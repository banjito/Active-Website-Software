...
export function CertificateGenerator({ division }: CertificateGeneratorProps) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (certResponse.error) {
          ...
          const validCertificates = certResponse.data.filter(
            (cert) => cert !== null,
          ) as Certificate[];
  ...
  const getFilteredCertificates = () => {
    return certificates.filter((certificate) => {
      ...
      const matchesCustomer =
        customerFilter === null || certificate.issued_to === customerFilter;

      // Filter by type
      const matchesType =
        typeFilter === null || certificate.certificate_type === typeFilter;
    ...
    if (equipmentId) {
      try {
        ...
        if (!response.error && response.data) {
          ...
          setCalibrations(
            response.data.filter((calibration) => calibration !== null),
          );
        }
      } catch (err) {
  ...
  const generateCertificateNumber = () => {
    const prefix =
      form.certificate_type?.substring(0, 3).toUpperCase() || "CERT";
  ...
  const handleDelete = async (id: string) => {
    if (
      ...
      try {
        ...
        setCertificates((prev) =>
          prev.filter((cert) => cert !== null && cert.id !== id),
        );
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex items-center space-x-4 mb-4">
          <Select
            value={typeFilter || ""}
            onChange={(e) =>
              setTypeFilter(e.target.value === "" ? null : e.target.value)
            }
          ...
          <Select
            ...
            onChange={(e) =>
              setCustomerFilter(e.target.value === "" ? null : e.target.value)
            }
            className="min-w-[200px]"
            options={[
                    ...
                    <Card key={certificate.id} className="overflow-hidden">
                      <CardHeader className="pb-2">
                        <div className="flex justify-between items-start">
                          ...
                          <Badge
                            variant={getStatusBadgeVariant(certificate.status)}
                          >
                            {certificate.status}
                          </Badge>
                        </div>
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between">
                            ...
                            <span className="text-sm">
                              {customers.find(
                                (c) => c.id === certificate.issued_to,
                              )?.name || certificate.issued_to}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-sm font-medium">
                    ...
                    <Select
                      ...
                      options={[
                        ...
                        ...calibrations.map((cal) => {
                          ...
                          const [year, month, day] = cal.calibration_date
                            .split("-")
                            .map(Number);
                          const date = new Date(year, month - 1, day);
                          return {
                            value: cal.id,
                            label: `${date.toLocaleDateString()} - ${cal.performed_by || "Unknown"}`,
                          };
                        }),
                      ]}
                    />
              ...
              <div className="space-y-6">
                ...
                <div className="text-center">
                  <h2 className="text-2xl font-bold uppercase">
                    {
                      certificateTypes.find(
                        (t) => t.value === selectedCertificate.certificate_type,
                      )?.label
                    }
                  </h2>
                ...
                <div className="border-t border-b py-4">
                  ...
                  <p>
                    {customers.find(
                      (c) => c.id === selectedCertificate.issued_to,
                    )?.name || selectedCertificate.issued_to}
                  </p>
                </div>
                ...
                <div className="mt-8 flex justify-between items-end">
                  <div>
                    ...
                    <p>
                      {selectedCertificate.issuer_name ||
