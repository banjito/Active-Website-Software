...
export default function FieldTechJobsPage() {
  ...
  const [newCustomer, setNewCustomer] = useState({ company_name: '', name: '', email: '', phone: '', address: '' });
  ...
  async function handleCreateContact() {
    ...
    try {
      ...
      if (inserted?.id) setTMFormData(prev => ({ ...prev, contact_id: inserted.id }));
    } catch (err: unknown) {
              ...
              <Card className="p-4 hover:shadow-md transition-shadow duration-200 cursor-pointer">
                <div className="flex items-start justify-between">
                  <div className="min-w-0 flex-1">
                    <p className="font-medium text-[#f26722] truncate">{maskJobTitle(job.title)}</p>
                    <p className="text-xs text-gray-600 dark:text-white mt-1 truncate">{maskCustomerName(job.customers?.company_name || job.customers?.name) || 'No customer'}</p>
