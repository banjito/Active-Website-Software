...
export class SwitchgearImporter extends BaseImporter implements ReportImporter {
  ...
  protected requiredColumns = ['job_id', 'user_id', 'report_info', 'visual_mechanical', 'insulation_resistance', 'contact_resistance', 'comments', 'overall_status'];
  ...
  canImport(data: ReportData): boolean {
    console.log(`🔍 Switchgear Importer (ATS) checking: ${data.reportType}`);
    const reportType = data.reportType || (data as any)?.data?.reportType || '';
    
    // This importer handles basic switchgear reports (ATS version)
    // Exclude MTS versions to avoid conflicts
    const isMTS = reportType.toLowerCase().includes('mts') || 
                   reportType.toLowerCase().includes('panelboard');
    
    if (isMTS) {
      console.log(`🔍 Switchgear Importer (ATS) - Skipping MTS report: ${reportType}`);
      return false;
    }
    
    const canImport = reportType.toLowerCase().includes('switchgear') || 
           reportType.toLowerCase().includes('switchgearreport');
    console.log(`🔍 Switchgear Importer (ATS) canImport result: ${canImport}`);
    return canImport;
  }
  ...
  protected prepareData(data: ReportData, jobId: string, userId: string, schema: DatabaseSchema): any {
    console.log('🔍 SwitchgearImporter - Starting data preparation');
      ...
      data.sections.forEach(section => {
        ...
        if (section.title === 'Job Information') {
          section.fields.forEach(field => {
            if (field.label === 'Customer') reportData.customer = field.value;
            if (field.label === 'Address') reportData.address = field.value;
            ...
            if (field.label === 'Temp. °F') {
              const tempF = parseFloat(field.value) || 68;
            ...
            if (field.label === 'Humidity %') {
              if (reportData.temperature) {
                ...
                reportData.temperature = {
                  ...
                  celsius: tempC,
                  tcf: tcf,
                  humidity: parseFloat(field.value) || 50
                };
              }
            }
        ...
        if (section.title === 'Nameplate Data') {
          section.fields.forEach(field => {
            if (field.label === 'Manufacturer') reportData.manufacturer = field.value;
            if (field.label === 'Catalog Number') reportData.catalogNumber = field.value;
            if (field.label === 'Serial Number') reportData.serialNumber = field.value;
            if (field.label === 'Type') reportData.type = field.value;
            if (field.label === 'System Voltage') reportData.systemVoltage = field.value;
            if (field.label === 'Rated Voltage') reportData.ratedVoltage = field.value;
            if (field.label === 'Rated Current') reportData.ratedCurrent = field.value;
            if (field.label === 'Phase Configuration') reportData.phaseConfiguration = field.value;
          });
        }
        ...
        if (section.title === 'Electrical Tests - Measured Insulation Resistance Values') {
          if (section.fields[0]?.value?.rows) {
            reportData.insulationResistanceTests = section.fields[0].value.rows.map((row: any) => ({
              busSection: row.bus,
              values: {
                ag: row.ag || '',
                bg: row.bg || '',
              ...
              testVoltage: '500V', // Default value
              unit: row.unit || 'MΩ'
            }));
          }
        }
        ...
        if (section.title === 'Temperature Corrected Values') {
          if (section.fields[0]?.value?.rows) {
            reportData.temperatureCorrectedTests = section.fields[0].value.rows.map((row: any) => ({
              busSection: row.bus,
              values: {
                ag: row.ag || '',
                bg: row.bg || '',
    ...
    if (data.data && data.data.fields) {
      ...
      if (!reportData.customer) reportData.customer = fields.customer || '';
      if (!reportData.address) reportData.address = fields.address || '';
      ...
      // Nameplate data
      if (!reportData.manufacturer) reportData.manufacturer = fields.manufacturer || '';
      if (!reportData.catalogNumber) reportData.catalogNumber = fields.catalogNumber || '';
      if (!reportData.serialNumber) reportData.serialNumber = fields.serialNumber || '';
      if (!reportData.type) reportData.type = fields.type || '';
      if (!reportData.systemVoltage) reportData.systemVoltage = fields.systemVoltage || '';
      if (!reportData.ratedVoltage) reportData.ratedVoltage = fields.ratedVoltage || '';
      if (!reportData.ratedCurrent) reportData.ratedCurrent = fields.ratedCurrent || '';
      if (!reportData.phaseConfiguration) reportData.phaseConfiguration = fields.phaseConfiguration || '';
      ...
      if (!reportData.insulationResistanceTests && fields.insulationMeasured?.rows) {
        reportData.insulationResistanceTests = fields.insulationMeasured.rows.map((row: any) => ({
          busSection: row.bus,
          values: {
            ag: row.ag || '',
            bg: row.bg || '',
      ...
      if (!reportData.temperatureCorrectedTests && fields.insulationCorrected?.rows) {
        reportData.temperatureCorrectedTests = fields.insulationCorrected.rows.map((row: any) => ({
          busSection: row.bus,
          values: {
            ag: row.ag || '',
            bg: row.bg || '',
    ...
    if (!reportData.visualInspectionItems) {
      reportData.visualInspectionItems = [
        { id: '7.1.A.1', description: 'Compare equipment nameplate data with drawings and specifications.', result: 'Select One', comments: '' },
        { id: '7.1.A.2', description: 'Inspect physical, electrical, and mechanical condition of cords and connectors.', result: 'Select One', comments: '' },
        { id: '7.1.A.3', description: 'Inspect anchorage, alignment, grounding, and required area clearances.', result: 'Select One', comments: '' },
        { id: '7.1.A.4', description: 'Verify the unit is clean and all shipping bracing, loose parts, and documentation shipped inside cubicles have been removed.', result: 'Select One', comments: '' },
        { id: '7.1.A.5', description: 'Verify that fuse and circuit breaker sizes and types correspond to drawings and coordination study as well as to the circuit breaker address for microprocessor-communication packages.', result: 'Select One', comments: '' },
        { id: '7.1.A.6', description: 'Verify that current and voltage transformer ratios correspond to drawings.', result: 'Select One', comments: '' },
        { id: '7.1.A.7', description: 'Verify that wiring connections are tight and that wiring is secure to prevent damage during routine operation of moving parts.', result: 'Select One', comments: '' },
        { id: '7.1.A.8.1', description: 'Use of a low-resistance ohmmeter in accordance with Section 7.1.B.1.', result: 'Select One', comments: '' },
        { id: '7.1.A.9', description: 'Confirm correct operation and sequencing of electrical and mechanical interlock systems.', result: 'Select One', comments: '' },
        { id: '7.1.A.10', description: 'Verify appropriate lubrication on moving current-carrying parts and on moving and sliding surfaces.', result: 'Select One', comments: '' },
        { id: '7.1.A.11', description: 'Inspect insulators for evidence of physical damage or contaminated surfaces.', result: 'Select One', comments: '' },
        { id: '7.1.A.12', description: 'Verify correct barrier and shutter installation and operation.', result: 'Select One', comments: '' },
        { id: '7.1.A.13', description: 'Exercise all active components.', result: 'Select One', comments: '' },
        { id: '7.1.A.14', description: 'Inspect mechanical indicating devices for correct operation.', result: 'Select One', comments: '' },
        { id: '7.1.A.15', description: 'Verify that filters are in place and vents are clear.', result: 'Select One', comments: '' }
      ];
    }
    
    if (!reportData.insulationResistanceTests) {
      reportData.insulationResistanceTests = ['Bus 1', 'Bus 2', 'Bus 3', 'Bus 4', 'Bus 5', 'Bus 6'].map(bus => ({
        busSection: bus,
    ...
    if (!reportData.temperatureCorrectedTests) {
      reportData.temperatureCorrectedTests = ['Bus 1', 'Bus 2', 'Bus 3', 'Bus 4', 'Bus 5', 'Bus 6'].map(bus => ({
        busSection: bus,
        values: { ag: '', bg: '', cg: '', ab: '', bc: '', ca: '', an: '', bn: '', cn: '' },
        unit: 'MΩ'
      }));
    ...
    const dataToInsert: any = {
      ...
      report_info: {
        ...
        catalogNumber: reportData.catalogNumber,
        serialNumber: reportData.serialNumber,
        type: reportData.type,
        systemVoltage: reportData.systemVoltage,
        ratedVoltage: reportData.ratedVoltage,
        ratedCurrent: reportData.ratedCurrent,
        phaseConfiguration: reportData.phaseConfiguration,
      ...
      insulation_resistance: {
        ...
        correctedTests: reportData.temperatureCorrectedTests
      },
      contact_resistance: {
        tests: reportData.contactResistanceTests,
        dielectricTests: reportData.dielectricWithstandTests
      },
      comments: reportData.comments,
      overall_status: reportData.status
    };
