...
export class LargeDryTypeXfmrMTSImporter extends BaseImporter implements ReportImporter {
  ...
  canImport(data: ReportData): boolean {
    console.log(`LargeDryTypeXfmrMTS Importer checking: ${data.reportType}`);
    const canImport = data.reportType?.toLowerCase().includes('largedrytypexfmrmts') || 
           data.reportType?.toLowerCase().includes('large-dry-type-xfmr-mts');
    console.log(`LargeDryTypeXfmrMTS Importer canImport result: ${canImport}`);
    return canImport;
  }
  ...
  protected prepareData(data: ReportData, jobId: string, userId: string, schema: DatabaseSchema): any {
    console.log('🔄 Starting data preparation for LargeDryTypeXfmrMTS Report');
            ...
            if (field.type === 'table' && field.value && field.value.rows) {
              field.value.rows.forEach((row: any) => {
                if (row.id && row.description) {
                  visualInspection[`${row.id}_result`] = row.result || '';
                  visualInspection[`${row.id}_comment`] = '';
                }
              });
            }
          });
  ...
  protected getReportType(): string {
    return 'large-dry-type-xfmr-mts-report';
  }
}
