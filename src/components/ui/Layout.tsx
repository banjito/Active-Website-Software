...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const isOnDivisionJobsOrDashboard =
  ...
  useEffect(() => {
    ...
    if (location.pathname === "/all-jobs")
      sessionStorage.setItem("useGlobalJobs", "true");
    else if (isOnDivisionJobsOrDashboard)
      sessionStorage.setItem("useGlobalJobs", "false");
  }, [location.pathname, isOnDivisionJobsOrDashboard]);
  const useGlobalJobs =
    typeof sessionStorage !== "undefined" &&
    sessionStorage.getItem("useGlobalJobs") === "true";
    ...
    return (
      <>
        ...
        {!hideJobsAndScheduling && (
          <>
            <Link
              to={isGlobalPortal ? "/all-jobs" : `${basePath}/jobs`}
              onClick={() => setIsMobileSidebarOpen(false)}
            >
              <Button
                variant="ghost"
  ...
  const sidebar = !isMeetingsPage ? (
    ...
    >
      ...
      <div className="flex flex-col gap-1 p-3 lg:p-4 flex-grow overflow-y-auto mobile-space-y-1 min-w-0">
        <div className="flex flex-col gap-1 min-w-0">{renderMenuItems()}</div>
      </div>
    </div>
  ) : null;
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-yellow-700 dark:text-yellow-300">
              This report has been approved or sent and cannot be edited. To
              make changes, the report status must be changed first.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="w-full px-3 sm:px-4 lg:px-8">
        <div className="flex h-16 lg:h-20 items-center justify-between">
          <div className="flex items-center gap-2 lg:gap-4 min-w-0 flex-1 print:hidden">
            ...
            <h2 className="text-sm lg:text-lg font-semibold truncate mobile-nav-text">
              {location.pathname === "/all-jobs" || useGlobalJobs
                ? "Global Portal"
                : formatDivisionName(division) || "AMP Portal"}
            </h2>
          </div>
  ...
  return (
    <div
      className={`flex min-h-screen bg-background dark:bg-dark-background text-foreground ${useHeaderBarLayout ? "flex-col" : ""}`}
    >
      {isMobileSidebarOpen && (
        <div
