...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  const [myDocuments, setMyDocuments] = useState<Array<{ id: string; name: string; description?: string | null; category: string; file_url: string; created_at: string; expiration_date?: string | null }>>([]);
  const [myCertifications, setMyCertifications] = useState<Array<{ id: string; cert_name: string; cert_type: string; status: string; cert_date?: string; expiration_date?: string | null; renewal_date?: string | null; notes?: string | null }>>([]);
  const [myTitleHistory, setMyTitleHistory] = useState<Array<{ id: string; title: string; effective_from: string }>>([]);
  const [myCompensationHistory, setMyCompensationHistory] = useState<Array<{ id: string; amount: number; pay_type: string; pay_frequency: string | null; effective_from: string }>>([]);
  const [loadingMyData, setLoadingMyData] = useState(false);
  const [editingDocument, setEditingDocument] = useState<{ id: string; name: string; description: string; expiration_date: string } | null>(null);
  const [editingCertification, setEditingCertification] = useState<{ id: string; cert_name: string; cert_date: string; expiration_date: string; renewal_date: string; status: string; notes: string } | null>(null);
  const [savingEdit, setSavingEdit] = useState(false);
  const [frSizes, setFrSizes] = useState<{ shirt: string; pant: string; jacket: string; updated_at: string | null }>({ shirt: '', pant: '', jacket: '', updated_at: null });
  const [editingFrSizes, setEditingFrSizes] = useState(false);
  const [frSizesDraft, setFrSizesDraft] = useState({ shirt: '', pant: '', jacket: '' });
          ...
          if (!profileError && profileData) {
            ...
            const userData: UserData = {
              ...
              user_metadata: {
                ...
                current_compensation_amount: profileData.current_compensation_amount ?? userMetadata.current_compensation_amount,
                current_pay_type: profileData.current_pay_type || userMetadata.current_pay_type,
                ...
                if (directData && directData.length > 0) {
                  ...
                  const userData: UserData = {
                    ...
                    user_metadata: {
                      ...
                      current_compensation_amount: profileData.current_compensation_amount,
                  ...
                  try {
                    ...
                    if (userFromAdmin) {
                      const userData = {
                        ...
                        user_metadata: {
                          name: userFromAdmin.raw_user_meta_data?.name || userFromAdmin.user_metadata?.name || userFromAdmin.email?.split('@')[0] || `User ${userId.substring(0, 6)}`,
                          role: null,
                          profileImage: null,
                          bio: null,
                          division: null,
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.schema('common').from('job_title_history').select('id, title, effective_from').eq('profile_id', profileIdToFetch).order('effective_from', { ascending: false }).then(({ data }) => data || []),
      supabase.schema('common').from('compensation_history').select('id, amount, pay_type, pay_frequency, effective_from').eq('profile_id', profileIdToFetch).order('effective_from', { ascending: false }).then(({ data }) => data || []).catch(() => []),
      supabase.schema('common').from('profiles').select('job_title, department, current_compensation_amount, current_pay_type, current_pay_frequency, fr_shirt_size, fr_pant_size, fr_jacket_size, fr_sizes_updated_at').eq('id', profileIdToFetch).single().then(({ data }) => data)
    ]).then(([docs, certs, titleRows, compRows, profileRow]) => {
      setMyDocuments((docs as any[]).filter((d: any) => !d.archived).map((d: any) => ({
        id: d.id,
        name: d.name,
      ...
      setMyCertifications((certs as any[]).map((c: any) => ({
        id: c.id,
  ...
  const refetchMyData = useCallback(() => {
    ...
    Promise.all([
      ...
      setMyDocuments((docs as any[]).filter((d: any) => !d.archived).map((d: any) => ({
        id: d.id, name: d.name, description: d.description ?? undefined, category: d.category || 'general', file_url: d.file_url, created_at: d.created_at, expiration_date: d.expiration_date ?? undefined,
      })));
      setMyCertifications((certs as any[]).map((c: any) => ({
        id: c.id, cert_name: c.cert_name, cert_type: c.cert_type, status: c.status, cert_date: c.cert_date, expiration_date: c.expiration_date ?? undefined, renewal_date: c.renewal_date ?? undefined, notes: c.notes ?? undefined,
      })));
      setMyTitleHistory((titleRows as any[]) || []);
      setMyCompensationHistory((compRows as any[]) || []);
      if (profileRow) {
        ...
        setProfileUser((prev) => prev ? { ...prev, user_metadata: { ...prev.user_metadata, job_title: row.job_title, department: row.department, current_compensation_amount: row.current_compensation_amount, current_pay_type: row.current_pay_type, current_pay_frequency: row.current_pay_frequency } } : null);
      }
    });
  }, [profileIdToFetch]);
  ...
  const handleSaveDocumentEdit = async () => {
    ...
    try {
      ...
      toast({ title: 'Error', description: e?.message || 'Failed to update document', variant: 'destructive' });
    } finally {
      setSavingEdit(false);
    }
  };
  ...
  const handleSaveCertificationEdit = async () => {
    ...
    try {
      await updateCertification(editingCertification.id, {
        ...
        expiration_date: toDateOnlyISO(editingCertification.expiration_date) || null,
      ...
      toast({ title: 'Saved', description: 'Certification updated.', variant: 'success' });
      setEditingCertification(null);
      refetchMyData();
    } catch (e: any) {
      toast({ title: 'Error', description: e?.message || 'Failed to update certification', variant: 'destructive' });
    } finally {
      setSavingEdit(false);
    }
  };
  ...
  const handleSaveFrSizes = async () => {
    ...
    try {
      ...
      toast({ title: 'Saved', description: 'FR clothing sizes updated.', variant: 'success' });
      setFrSizes({
      ...
      toast({ title: 'Error', description: e?.message || 'Failed to update FR sizes', variant: 'destructive' });
    } finally {
  ...
  const handleStartEditFrSizes = () => {
    setFrSizesDraft({ shirt: frSizes.shirt, pant: frSizes.pant, jacket: frSizes.jacket });
  ...
  const formatCompensation = (amount: number | null | undefined, type: string | null | undefined, freq: string | null | undefined) => {
    if (amount == null) return '—';
    if (type === 'hourly') return `$${Number(amount).toLocaleString('en-US', { minimumFractionDigits: 2 })}/hr`;
    const freqLabel = freq === 'annual' ? 'Annual' : freq === 'monthly' ? 'Monthly' : freq === 'biweekly' ? 'Biweekly' : freq === 'weekly' ? 'Weekly' : freq || 'Annual';
  ...
  const isProfileSetUp = !!(bio || division || role || jobTitle || department || compensationAmount != null || profileImage || coverImage || birthday);
  
  // Get display name - prefer name from metadata, fallback to email username, then email
  const displayName = name || profileUser?.email?.split('@')[0] || profileUser?.email || 'Unknown User';
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Cover Image */}
          <div 
            className="h-40 flex-shrink-0 bg-gradient-to-r from-gray-300 to-gray-400 dark:from-dark-200 dark:to-dark-300 relative overflow-hidden cursor-pointer"
          ...
          <div className="px-6 pb-6">
            ...
            <div className="relative -mt-16 mb-4">
              <div 
                className={`w-32 h-32 rounded-full overflow-hidden border-4 border-white dark:border-dark-100 shadow-xl bg-gray-200 dark:bg-dark-150 ${profileImage ? 'cursor-pointer' : ''}`}
                onClick={(e) => profileImage && handlePhotoClick(e, profileImage)}
              >
                {profileImage ? (
                  <img src={profileImage} alt="Profile" className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center bg-gray-200 dark:bg-dark-150">
                    <span className="text-4xl text-gray-400 dark:text-white">{name ? name.charAt(0).toUpperCase() : '?'}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-6">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-amber-700 dark:text-amber-300">
                        This user hasn't completed their profile setup yet. Basic information is shown below.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {jobTitle && (
                    <div className="flex items-center text-gray-700 dark:text-white">
                      ...
                      <div>
                        <p className="text-sm text-gray-500 dark:text-white">Job title</p>
                        <p>{jobTitle}</p>
                      </div>
                    </div>
                  )}
                  {department && (
                    <div className="flex items-center text-gray-700 dark:text-white">
                      <Building2 className="mr-2 h-4 w-4 text-gray-500 dark:text-white flex-shrink-0" />
                      <div>
                  ...
                  {!limitedView && formattedBirthday && (
                    <div className="flex items-center text-gray-700 dark:text-white">
                      ...
                      <div>
                        <p className="text-sm text-gray-500 dark:text-white">Birthday</p>
                  ...
                  {!limitedView && profileUser?.email && (
                    <div className="flex items-center text-gray-700 dark:text-white">
                      <Mail className="mr-2 h-4 w-4 text-gray-500 dark:text-white" />
                      <div>
                ...
                {!limitedView && (emergencyContactName || emergencyContactPhone) && (
                  <div className="pt-4 border-t border-gray-200 dark:border-dark-200">
                    ...
                    <div className="text-sm text-gray-700 dark:text-gray-200 space-y-1">
                      ...
                      {emergencyContactRelationship && (
                        <p className="text-muted-foreground">{emergencyContactRelationship}</p>
                      )}
                      {emergencyContactPhone && (
                        <a href={`tel:${emergencyContactPhone}`} className="text-[#f26722] hover:underline">
                          {emergencyContactPhone}
                        </a>
                      )}
                    </div>
                ...
                {goalsText && (
                  <div className="pt-4 border-t border-gray-200 dark:border-dark-200">
                    ...
                    <p className="text-sm text-gray-700 dark:text-gray-200 whitespace-pre-wrap">{goalsText}</p>
                  </div>
                )}
                ...
                {!limitedView && canViewSensitiveSection && profileIdToFetch ? (
                      ...
                      <>
                        <div>
                          ...
                          {myDocuments.length === 0 ? (
                            ...
                            <ul className="space-y-1.5">
                              {myDocuments.map((doc) => (
                                <li key={doc.id} className="flex items-center justify-between gap-2 text-sm group">
                                  <span className="text-gray-700 dark:text-gray-200 truncate flex-1 min-w-0">{doc.name}</span>
                                  <div className="flex items-center gap-1 flex-shrink-0">
                                    {viewingOwnProfile && (
                                      <button
                                        type="button"
                                        onClick={() => setEditingDocument({ id: doc.id, name: doc.name, description: doc.description ?? '', expiration_date: doc.expiration_date ? doc.expiration_date.slice(0, 10) : '' })}
                                        className="p-1 rounded text-gray-500 hover:text-[#f26722] hover:bg-gray-100 dark:hover:bg-dark-200"
                                        title="Edit document"
                                      >
                        ...
                        <div>
                          <h3 className="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-2 mb-2">
                            ...
                            {viewingOwnProfile ? 'My certifications' : 'Certifications'}
                          </h3>
                          {myCertifications.length === 0 ? (
                            ...
                            <ul className="space-y-1.5">
                              {myCertifications.map((cert) => (
                                <li key={cert.id} className="text-sm text-gray-700 dark:text-gray-200 flex items-center justify-between gap-2 group">
                                  <span className="flex-1 min-w-0">{cert.cert_name}</span>
                                  <div className="flex items-center gap-2 flex-shrink-0">
                                    <span className={`text-xs px-1.5 py-0.5 rounded ${cert.status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-200' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'}`}>
                                      {cert.status}
                                    </span>
                                    {viewingOwnProfile && (
                                      <button
                                        ...
                                        onClick={() => setEditingCertification({
                                          ...
                                          cert_date: cert.cert_date ? cert.cert_date.slice(0, 10) : '',
                                          expiration_date: cert.expiration_date ? cert.expiration_date.slice(0, 10) : '',
                                          renewal_date: cert.renewal_date ? cert.renewal_date.slice(0, 10) : '',
                                          status: cert.status,
                                          notes: cert.notes ?? '',
                                        })}
                                        className="p-1 rounded text-gray-500 hover:text-[#f26722] hover:bg-gray-100 dark:hover:bg-dark-200"
                        ...
                        <div>
                          ...
                          {myTitleHistory.length === 0 ? (
                            <p className="text-sm text-muted-foreground">No title history yet.</p>
                          ) : (
                            <ul className="space-y-1.5">
                              {myTitleHistory.map((entry) => (
                                <li key={entry.id} className="text-sm text-gray-700 dark:text-gray-200 flex justify-between gap-2">
                        ...
                        <div>
                          ...
                          {compensationAmount != null ? (
                            <p className="text-sm text-gray-700 dark:text-gray-200">
                              {formatCompensation(compensationAmount, payType, payFrequency)}
                            </p>
                          ) : (
                            ...
                            <ul className="space-y-1.5 mt-3 text-sm text-muted-foreground">
                              {myCompensationHistory.slice(0, 5).map((entry) => (
                                <li key={entry.id} className="flex justify-between gap-2">
                                  <span>{formatCompensation(entry.amount, entry.pay_type, entry.pay_frequency)}</span>
                                  <span className="text-xs">{formatDateOnly(entry.effective_from)}</span>
                                </li>
                              ))}
                            </ul>
                          )}
                          ...
                          <div className="flex items-center justify-between gap-2 mb-2">
                            ...
                            {canEditEeData && !editingFrSizes && (
                              ...
                              >
                                ...
                                {frSizes.shirt || frSizes.pant || frSizes.jacket ? 'Edit sizes' : 'Add sizes'}
                              </button>
                            )}
                          </div>
                          ...
                          {editingFrSizes ? (
                            <div className="space-y-3">
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                <div>
                                  <Label htmlFor="fr-shirt" className="text-xs">Shirt</Label>
                                  <Input
                                    id="fr-shirt"
                                    value={frSizesDraft.shirt}
                                    onChange={(e) => setFrSizesDraft((d) => ({ ...d, shirt: e.target.value }))}
                                    placeholder="e.g. M, L, XL"
                                    className="mt-1 h-9"
                                  />
                                </div>
                                <div>
                                  <Label htmlFor="fr-pant" className="text-xs">Pants</Label>
                                  <Input
                                    id="fr-pant"
                                    value={frSizesDraft.pant}
                                    onChange={(e) => setFrSizesDraft((d) => ({ ...d, pant: e.target.value }))}
                                    placeholder="e.g. 32x30"
                                    className="mt-1 h-9"
                                  />
                                </div>
                                <div>
                                  <Label htmlFor="fr-jacket" className="text-xs">Jacket</Label>
                                  <Input
                                    id="fr-jacket"
                                    value={frSizesDraft.jacket}
                                    onChange={(e) => setFrSizesDraft((d) => ({ ...d, jacket: e.target.value }))}
                              ...
                              <div className="flex gap-2">
                                <Button size="sm" onClick={handleSaveFrSizes} disabled={savingFrSizes}>
                                  {savingFrSizes ? 'Saving…' : 'Save'}
                                </Button>
                                <Button size="sm" variant="outline" onClick={() => setEditingFrSizes(false)} disabled={savingFrSizes}>
                                  Cancel
                                </Button>
                              </div>
                            </div>
                            ...
                            <>
                              {(frSizes.shirt || frSizes.pant || frSizes.jacket) ? (
                                <div className="text-sm text-gray-700 dark:text-gray-200 space-y-1">
                                  {frSizes.shirt && <p><span className="text-muted-foreground">Shirt:</span> {frSizes.shirt}</p>}
                                  {frSizes.pant && <p><span className="text-muted-foreground">Pants:</span> {frSizes.pant}</p>}
                                  {frSizes.jacket && <p><span className="text-muted-foreground">Jacket:</span> {frSizes.jacket}</p>}
                                  {frSizes.updated_at && (
                                    <p className="text-xs text-muted-foreground mt-1">
                                      Last updated {formatDateOnly(frSizes.updated_at)}
                                    </p>
                                  )}
                                </div>
                              ) : (
                                <p className="text-sm text-muted-foreground">No FR sizes on file.</p>
                              )}
                            </>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Career development notes and plans will appear here when configured.
                          </p>
                        </div>
                      </>
                    )}
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-200 dark:border-dark-200">
                    <p className="text-sm text-muted-foreground">
                      Documents, certifications, title history, and compensation are only visible to the profile owner, their manager, or admins.
                    </p>
                  </div>
                ) : null}
              </div>
      ...
      <Dialog open={!!editingDocument} onOpenChange={(open) => !open && setEditingDocument(null)}>
        <DialogContent className="sm:max-w-md">
            ...
            <div className="grid gap-4 py-2">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setEditingDocument((d) => d ? { ...d, name: e.target.value } : null)}
                  className="mt-1"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setEditingDocument((d) => d ? { ...d, description: e.target.value } : null)}
                  className="mt-1"
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setEditingDocument((d) => d ? { ...d, expiration_date: e.target.value } : null)}
                  className="mt-1"
                />
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveDocumentEdit} disabled={savingEdit}>{savingEdit ? 'Saving…' : 'Save'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!editingCertification} onOpenChange={(open) => !open && setEditingCertification(null)}>
        <DialogContent className="sm:max-w-md">
            ...
            <div className="grid gap-4 py-2">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setEditingCertification((c) => c ? { ...c, cert_name: e.target.value } : null)}
                  className="mt-1"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingCertification((c) => c ? { ...c, cert_date: e.target.value } : null)}
                    className="mt-1"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingCertification((c) => c ? { ...c, expiration_date: e.target.value } : null)}
                    className="mt-1"
                  />
                </div>
              </div>
              <div>
                <Label htmlFor="edit-cert-renewal">Renewal date (optional)</Label>
                <Input
                  id="edit-cert-renewal"
                  type="date"
                  value={editingCertification.renewal_date}
                  onChange={(e) => setEditingCertification((c) => c ? { ...c, renewal_date: e.target.value } : null)}
                  className="mt-1"
                />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditingCertification((c) => c ? { ...c, status: e.target.value } : null)}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setEditingCertification((c) => c ? { ...c, notes: e.target.value } : null)}
                  className="mt-1"
                  placeholder="Notes"
                />
              </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditingCertification(null)}>Cancel</Button>
            <Button onClick={handleSaveCertificationEdit} disabled={savingEdit}>{savingEdit ? 'Saving…' : 'Save'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
