...
export const ShortcutManagerDndKit: React.FC<ShortcutManagerProps> = ({
  isOpen,
  onClose,
}) => {
  const { user } = useAuth();
  ...
  const handleSaveShortcut = async (e: React.FormEvent) => {
    ...
    if (!isEditing && shortcuts.length >= MAX_SHORTCUTS) {
      setError(
        `You can have at most ${MAX_SHORTCUTS} shortcuts. Remove one to add another.`,
      );
      return;
    }
  ...
  const quickAddItems = React.useMemo(() => {
    ...
    BUILTIN_PORTALS.forEach((p) => {
      p.options.forEach((opt) => {
        ...
        if (
          opt.label.toLowerCase().includes("drag to reorder") ||
          opt.label.toLowerCase().includes("reorder your shortcuts")
        )
          return;
  ...
  const handleQuickAdd = async () => {
    ...
    if (slotsLeft <= 0) {
      setError(
        `You can have at most ${MAX_SHORTCUTS} shortcuts. Remove one to add more.`,
      );
      setQuickAddOpen(false);
      return;
    }
    ...
    try {
      ...
      await ShortcutService.bulkCreateShortcuts(
        user.id,
        toAdd.map((c) => ({ title: c.label, url: c.path })),
      );
    } finally {
      setLoading(false);
      setQuickAddOpen(false);
    ...
    if (active.id !== over.id) {
      setShortcuts((items) => {
        ...
        try {
          ...
          ShortcutService.reorderShortcuts(user.id, shortcutIds).catch(
            (err) => {
