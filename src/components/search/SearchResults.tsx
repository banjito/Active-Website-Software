    ...
    switch (result.type) {
      case "customer": {
        ...
        return (
          <div
            onClick={handleClick}
            className="flex items-start p-4 hover:bg-zinc-50 dark:hover:bg-dark-200 rounded-md cursor-pointer"
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-zinc-900 dark:text-dark-900 truncate">
                {customerResult.company_name}
              </p>
              <p className="text-xs text-zinc-500 dark:text-dark-400">
                Customer • {customerResult.division || "Global"}
              </p>
              <p className="mt-1 text-xs text-zinc-500 dark:text-dark-500">
                Created:{" "}
                {new Date(
                  customerResult.metadata.created_at,
                ).toLocaleDateString()}
              </p>
            </div>
          </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-zinc-500 dark:text-dark-400">
                ...
                {jobResult.division || "Unknown Division"}
              </p>
              <p className="mt-1 text-xs text-zinc-500 dark:text-dark-500">
                Customer: {jobResult.customer.name} • Due:{" "}
                {new Date(jobResult.due_date).toLocaleDateString()}
              </p>
            </div>
          </div>
        );
        ...
        return (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-zinc-500 dark:text-dark-400">
                Report •{" "}
                {new Date(reportResult.created_at).toLocaleDateString()}
              </p>
              <p className="mt-1 text-xs text-zinc-500 dark:text-dark-500">
...
export const SearchResults: React.FC = () => {
  ...
  if (!loading && hasSearched && searchResults.length === 0) {
    return (
      <div className="mt-4 p-6 text-center bg-white dark:bg-dark-150 rounded-md shadow-sm border border-zinc-200 dark:border-dark-300">
        <div className="text-zinc-400 dark:text-dark-500 mb-2">
          <svg
        ...
        <h3 className="text-lg font-medium text-zinc-900 dark:text-dark-900">
          No results found
        </h3>
        <p className="mt-1 text-sm text-zinc-500 dark:text-dark-400">
          No matches were found for "{query}". Try adjusting your search terms
          or filters.
        </p>
      </div>
    );
  ...
  if (loading) {
    return (
      <div className="mt-4 p-8 text-center">
        ...
        <p className="mt-2 text-sm text-zinc-500 dark:text-dark-400">
          Searching...
        </p>
      </div>
    );
  ...
  return (
    <div className="mt-4">
      <div className="bg-white dark:bg-dark-150 rounded-md shadow-sm border border-zinc-200 dark:border-dark-300 overflow-hidden">
        <div className="p-4 border-b border-zinc-200 dark:border-dark-300">
          <h2 className="text-lg font-medium text-zinc-900 dark:text-dark-900">
            Search Results
          </h2>
          <p className="mt-1 text-sm text-zinc-500 dark:text-dark-400">
        ...
        <div className="divide-y divide-zinc-200 dark:divide-dark-300">
          ...
          {Object.entries(groupedResults).map(([type, results]) => (
            <div key={type} className="py-2">
              <h3 className="px-4 py-2 text-sm font-medium text-zinc-500 dark:text-dark-400 uppercase tracking-wider">
