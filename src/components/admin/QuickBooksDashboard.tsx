...
export const QuickBooksDashboard: React.FC = () => {
  ...
  const [cashFlowViewMode, setCashFlowViewMode] = useState<
    "balance" | "moneyInOut"
  >("balance");
  ...
  const [profitLossDateRange, setProfitLossDateRange] =
    useState<DateRangeType>("last30days");
  const [expensesDateRange, setExpensesDateRange] =
    useState<DateRangeType>("last30days");
  const [transactionsDateRange, setTransactionsDateRange] =
    useState<DateRangeType>("last30days");

  // Cash flow separate timeline
  type CashFlowPeriod = "3months" | "6months" | "9months" | "12months";
  const [cashFlowPeriod, setCashFlowPeriod] =
    useState<CashFlowPeriod>("12months");
  ...
  const getDateRangeFor = (range: DateRangeType) => {
    ...
    const currentFiscalYearStart =
      today.getMonth() >= fiscalYearStartMonth
        ? new Date(today.getFullYear(), fiscalYearStartMonth, 1)
        : new Date(today.getFullYear() - 1, fiscalYearStartMonth, 1);

    const monthInFiscalYear =
      (today.getMonth() - fiscalYearStartMonth + 12) % 12;
    ...
    const quarterStartYear =
      quarterStartMonth < fiscalYearStartMonth
        ? today.getFullYear()
        : today.getMonth() >= fiscalYearStartMonth
          ? today.getFullYear()
          : today.getFullYear() - 1;
    ...
    switch (range) {
      ...
      case "thisFiscalYear":
        ...
        endDate = new Date(
          currentFiscalYearStart.getFullYear() + 1,
      ...
      case "lastFiscalQuarter":
        ...
        const lastQStartYear =
          lastQStartMonth > quarterStartMonth
            ? quarterStartYear - 1
            : quarterStartYear;
      ...
      case "lastFiscalYear":
        ...
        endDate = new Date(
          currentFiscalYearStart.getFullYear(),
          fiscalYearStartMonth,
          0,
        );
        break;
  ...
  const DateRangeSelector = ({
    ...
    >
      <optgroup label="Current Period">
        ...
        <option value="thisFiscalQuarterToDate">
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!isConnected) {
        setError(
          "QuickBooks is not connected. Go to Admin Dashboard → Integrations to connect.",
        );
        setLoading(false);
      ...
      batch1.forEach((result, index) => {
        ...
        if (result.status === "rejected") {
          console.error(
            `[QB Dashboard] Failed to load ${name}:`,
            result.reason,
          );
        } else {
          console.log(
            `[QB Dashboard] Loaded ${name}:`,
      ...
      console.log(
        "[QB Dashboard] Setting loading to false, connected:",
        isConnected,
      );

      // Check what data we actually have
      const hasCompanyInfo =
        batch1[0].status === "fulfilled" && batch1[0].value;
      const hasPnL = batch1[1].status === "fulfilled" && batch1[1].value;
      const hasAccounts =
        batch1[2].status === "fulfilled" && Array.isArray(batch1[2].value);
      console.log(
        "[QB Dashboard] Data status - CompanyInfo:",
        hasCompanyInfo,
        "P&L:",
        hasPnL,
        "Accounts:",
      ...
      const batch2a = await Promise.allSettled([
        ...
        getQuickBooksTimeActivities(
          transactionsDateRangeObj.start,
          transactionsDateRangeObj.end,
        ).catch((err) => {
          console.warn(
            "[QB Dashboard] TimeActivities failed, continuing without it:",
            err,
          );
          return [];
        }),
      ...
      if (batch2[2].status === "fulfilled")
        setTimeActivities(batch2[2].value || []);
      ...
      if (batch3[1].status === "fulfilled")
        setSalesReceipts(batch3[1].value || []);
      if (batch3[2].status === "fulfilled")
        setCreditMemos(batch3[2].value || []);
      if (batch3[3].status === "fulfilled")
        setRefundReceipts(batch3[3].value || []);
      if (batch3[4].status === "fulfilled")
        setJournalEntries(batch3[4].value || []);
      if (batch3[5].status === "fulfilled")
        setBillPayments(batch3[5].value || []);
      ...
      if (batch4[1].status === "fulfilled")
        setDepartments(batch4[1].value || []);
      if (batch4[2].status === "fulfilled")
        setVendorCredits(batch4[2].value || []);
      if (batch4[3].status === "fulfilled") setTransfers(batch4[3].value || []);
      if (batch4[4].status === "fulfilled")
        setPaymentMethods(batch4[4].value || []);
      if (batch4[5].status === "fulfilled")
        setPurchaseOrders(batch4[5].value || []);
  ...
  useEffect(() => {
    ...
    const reloadPnlForExpenses = async () => {
      try {
        console.log(
          "[QB Dashboard] Reloading P&L for expenses date range:",
          expensesDateRange,
        );
        const { start, end } = getDateRangeFor(expensesDateRange);
        const pnl = await getQuickBooksProfitAndLoss(undefined, start, end);
        console.log(
          "[QB Dashboard] P&L reloaded for expenses with date range:",
          start,
          "to",
          end,
        );
  ...
  useEffect(() => {
    ...
    const reloadTransactions = async () => {
      try {
        console.log(
          "[QB Dashboard] Reloading transactions for date range:",
        ...
        const [invoices, payments, deposits, ta] = await Promise.allSettled([
          ...
          getQuickBooksTimeActivities(start, end).catch((err) => {
            console.warn(
              "[QB Dashboard] TimeActivities failed during reload:",
              err,
            );
            return [];
          }),
        ...
        console.log(
          "[QB Dashboard] Reloaded transactions - Invoices:",
          (invoicesResult || []).length,
          "Payments:",
          (paymentsResult || []).length,
          "Deposits:",
          (depositsResult || []).length,
        );
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const reloadCashFlow = async () => {
      try {
        console.log(
          "[QB Dashboard] Reloading Cash Flow for period:",
          cashFlowPeriod,
        );
  ...
  const filterByDateRange = <
    T extends { TxnDate?: string; MetaData?: { CreateTime?: string } },
  >(
    items: T[],
  ...
  const getInvoiceStats = () => {
    ...
    const paidTotal = filteredPayments.reduce(
      (sum, p) => sum + (p.TotalAmt || 0),
      0,
    );

    // Deposited: deposits in this period
    const depositedTotal = filteredDeposits.reduce(
      (sum, d) => sum + (d.TotalAmt || 0),
      0,
    );
    ...
    profitAndLoss.Rows.Row.forEach((row: any) => {
      ...
      if (row.Summary?.ColData) {
        ...
        if (
          ...
          group === "CostOfSales" ||
          header.includes("cost of goods sold") ||
          header.includes("cost of sales")
        ) {
          cogs = Math.abs(value);
        }
        // Match Gross Profit
        ...
        if (
          ...
          group === "TotalExpenses" ||
          group === "Expense" ||
          header.includes("total expenses") ||
          header.includes("total expense")
        ) {
        ...
        if (
          ...
          group === "NetOrdinaryIncome" ||
          header.includes("net income") ||
          header.includes("net operating income")
        ) {
          netIncome = value;
        }
      }
  ...
  const getExpenseBreakdown = () => {
    ...
    const extractExpenseItems = (section: any) => {
      ...
      section.Rows.Row.forEach((row: any) => {
        ...
        if (row.ColData && row.type === "Data") {
          ...
          const valueStr =
            row.ColData[1]?.value || row.ColData[row.ColData.length - 1]?.value;
          if (name && valueStr) {
            const amount = Math.abs(
              parseFloat(String(valueStr).replace(/,/g, "")) || 0,
            );
            if (amount > 0) {
        ...
        if (row.Summary?.ColData && row.Header?.ColData?.[0]?.value) {
          ...
          if (name && valueStr && !name.toLowerCase().includes("total")) {
            ...
            if (amount > 0) {
              ...
              const hasSubItems = expenseCategories.some(
                (e) => e.name !== name,
              );
    ...
    const uniqueExpenses = expenseCategories.reduce(
      (acc: { name: string; amount: number }[], curr) => {
    ...
    return uniqueExpenses.sort((a, b) => b.amount - a.amount).slice(0, 15); // Show top 15 instead of 10
  };
  ...
  const parseCashFlowReport = () => {
    ...
    console.log(
      "[QB Cash Flow] Full report structure:",
    ...
    const processRow = (row: any, depth = 0) => {
      ...
      if (row.Summary?.ColData) {
        ...
        if (
          ...
          headerLower.includes("operating activities") ||
        ...
        // Investing Activities
          ...
          headerLower.includes("investing activities") ||
        ...
        // Financing Activities
          ...
          headerLower.includes("financing activities") ||
        ...
        // Net Increase/Decrease in Cash
          ...
          group === "NetCashFlow" ||
          headerLower.includes("net increase") ||
          headerLower.includes("net decrease") ||
          headerLower.includes("net cash") ||
    ...
    cashFlow.Rows.Row.forEach((row: any) => {
      const header = row.Header?.ColData?.[0]?.value?.toLowerCase() || "";
      if (
        header.includes("operating") ||
        header.includes("investing") ||
        header.includes("financing")
      ) {
    ...
    if (moneyIn === 0 && moneyOut === 0) {
      ...
      else if (investingActivities < 0)
        moneyOut += Math.abs(investingActivities);

      if (financingActivities > 0) moneyIn += financingActivities;
      else if (financingActivities < 0)
        moneyOut += Math.abs(financingActivities);
    }

    console.log(
      "[QB Cash Flow] Parsed - Operating:",
      operatingActivities,
      "Investing:",
      investingActivities,
      "Financing:",
    ...
    console.log(
      "[QB Cash Flow] Calculated - Money In:",
      moneyIn,
      "Money Out:",
      moneyOut,
    );
  ...
  const getMoneyFlow = () => {
    ...
    const moneyIn =
      paymentsInRange.reduce((sum, p) => sum + (p.TotalAmt || 0), 0) +
    ...
    const moneyOut =
      purchasesInRange.reduce((sum, p) => sum + (p.TotalAmt || 0), 0) +
    ...
    console.log(
      "[QB Cash Flow] Calculated from transactions - Money In:",
      moneyIn,
      "Money Out:",
      moneyOut,
      "Period:",
    ...
    console.log(
      "[QB Cash Flow] Transaction counts - Payments:",
      paymentsInRange.length,
      "Deposits:",
      depositsInRange.length,
      "Sales Receipts:",
      salesReceiptsInRange.length,
      "Purchases:",
      purchasesInRange.length,
      "Bills:",
      billsInRange.length,
      "Bill Payments:",
      billPaymentsInRange.length,
      "Vendor Credits:",
      vendorCreditsInRange.length,
    );
  ...
  const invoiceStats = useMemo(
    () => getInvoiceStats(),
    [filteredInvoices, filteredPayments, filteredDeposits],
  );
  ...
  const parsePayrollData = () => {
    const employeePayData: Record<
      string,
      { grossPay: number; netPay: number; taxes: number; deductions: number }
    > = {};
    let totalGrossPay = 0;
    let totalNetPay = 0;
    let totalTaxes = 0;
      ...
      payrollSummary.Rows.Row.forEach((row: any) => {
        ...
        if (row.ColData) {
          ...
          const amount = parseFloat(
            row.ColData[1]?.value?.replace(/,/g, "") || "0",
          );
        ...
        if (row.Rows?.Row) {
          row.Rows.Row.forEach((subRow: any) => {
            if (subRow.ColData) {
              ...
              const amount = parseFloat(
                subRow.ColData[1]?.value?.replace(/,/g, "") || "0",
              );
              if (employeeName && amount) {
                if (!employeePayData[employeeName]) {
                  employeePayData[employeeName] = {
  ...
  const parseTimeOffBalances = () => {
    const balances: Record<
      string,
      { vacation: number; sick: number; pto: number }
    > = {};
  ...
  const getEmployeeStats = () => {
    ...
    const employeeTimeData = activeEmployees.map((employee) => {
      ...
      const compensation = employeeCompensation.find(
        (ec) =>
          ec.id === employee.Id || ec.displayName === employee.DisplayName,
      );
      ...
      employeeTimeActivities.forEach((ta) => {
        ...
        switch (category) {
          case "pto":
            ptoHours += activityHours;
            break;
          case "vacation":
            vacationHours += activityHours;
            break;
          case "sick":
            sickHours += activityHours;
            break;
          case "holiday":
            holidayHours += activityHours;
            break;
          case "unpaid":
            unpaidHours += activityHours;
            break;
          case "personal":
            personalHours += activityHours;
            break;
          case "overtime":
            overtimeHours += activityHours;
            break;
          default:
            regularHours += activityHours;
            break;
        }
      ...
      const hourlyRate =
        compensation?.hourlyRate || employee.BillRate || employee.CostRate || 0;
      ...
      const empName =
        ...
        "";
      const payData = employeePayData[empName] ||
        employeePayData[employee.DisplayName] || {
          grossPay: 0,
          netPay: 0,
          taxes: 0,
          deductions: 0,
      ...
      const timeOffBalance = timeOffBalanceData[empName] ||
        timeOffBalanceData[employee.DisplayName] || {
          vacation: 0,
          sick: 0,
          pto: 0,
        };
      ...
      const totalPaidTimeOff =
        ptoHours + vacationHours + sickHours + holidayHours + personalHours;

      // Determine if employee is salaried based on available data
      // If no hourly rates are set, assume salaried
      const isSalaried =
        (hourlyRate === 0 && costRate === 0 && billRate === 0) ||
      ...
      return {
        ...
        phone:
          ...
          employee.Mobile?.FreeFormNumber ||
          compensation?.phone,
    ...
    const totalTrackedHours = employeeTimeData.reduce(
      (sum, e) => sum + e.totalHours,
      0,
    );
    const totalBillableHours = employeeTimeData.reduce(
      (sum, e) => sum + e.billableHours,
      0,
    );
    const totalLabourCost = employeeTimeData.reduce(
      (sum, e) => sum + e.totalHours * e.costRate,
      0,
    );
    const totalPTO = employeeTimeData.reduce((sum, e) => sum + e.ptoHours, 0);
    const totalVacation = employeeTimeData.reduce(
      (sum, e) => sum + e.vacationHours,
      0,
    );
    const totalSick = employeeTimeData.reduce((sum, e) => sum + e.sickHours, 0);
    const totalHoliday = employeeTimeData.reduce(
      (sum, e) => sum + e.holidayHours,
      0,
    );
    const totalUnpaid = employeeTimeData.reduce(
      (sum, e) => sum + e.unpaidHours,
      0,
    );
    const totalOvertime = employeeTimeData.reduce(
      (sum, e) => sum + e.overtimeHours,
      0,
    );
    const totalPaidTimeOff = employeeTimeData.reduce(
      (sum, e) => sum + e.totalPaidTimeOff,
      0,
    );
    const totalGrossPay = employeeTimeData.reduce(
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Card className="bg-gradient-to-br from-orange-500 to-orange-600 text-white">
            <CardContent className="p-5">
              ...
              <p className="text-3xl font-bold">
                {formatCurrency(invoiceStats.notPaid.total)}
              </p>
              <p className="text-sm opacity-75 mt-1">
                {invoiceStats.notPaid.count} invoices outstanding
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-green-500 to-green-600 text-white">
            <CardContent className="p-5">
              ...
              <p className="text-3xl font-bold">
                {formatCurrency(invoiceStats.paid.total)}
              </p>
              <p className="text-sm opacity-75 mt-1">
                {invoiceStats.paid.count} invoices paid
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white">
            <CardContent className="p-5">
              ...
              <p className="text-3xl font-bold">
                {formatCurrency(invoiceStats.deposited.total)}
              </p>
              <p className="text-sm opacity-75 mt-1">
                {invoiceStats.deposited.count} deposits
              </p>
            </CardContent>
          </Card>
        </div>
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {pnlSummary.income > 0 && (
                  ...
                  >
                    {Math.round(
                      (pnlSummary.netIncome / pnlSummary.income) * 100,
                    )}
                    % margin
                  </span>
                )}
            ...
            <div className="mb-3">
              ...
              <div className="w-full bg-neutral-200 rounded h-4">
                <div
                  ...
                  style={{
                    width: `${pnlSummary.income > 0 && pnlSummary.expenses > 0 ? Math.min((pnlSummary.income / Math.max(pnlSummary.income, pnlSummary.expenses)) * 100, 100) : 50}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="w-full bg-neutral-200 rounded h-4">
                <div
                  ...
                  style={{
                    width: `${pnlSummary.expenses > 0 && pnlSummary.income > 0 ? Math.min((pnlSummary.expenses / Math.max(pnlSummary.income, pnlSummary.expenses)) * 100, 100) : 50}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {expenseBreakdown.length === 0 ? (
                <div className="text-neutral-500 text-sm">
                  ...
                  {pnlSummary.expenses > 0 && (
                    <p className="text-xs mt-1">
                      Total expenses: {formatCurrency(pnlSummary.expenses)}{" "}
                ...
                expenseBreakdown.map((item, index) => {
                  const percentage =
                    pnlSummary.expenses > 0
                      ? Math.round((item.amount / pnlSummary.expenses) * 100)
                      : 0;
                  const colors = [
                    "bg-blue-500",
                    "bg-green-500",
        ...
        <CardContent className="pt-0 pb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              ...
              <p
                className={`text-2xl font-bold ${moneyFlow.moneyIn - moneyFlow.moneyOut >= 0 ? "text-green-600" : "text-red-600"}`}
              >
            ...
            {moneyFlow.transactions.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b text-left">
                      ...
                      <th className="p-2 text-sm font-semibold text-right">
                        Amount
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {(() => {
                      const filtered = moneyFlow.transactions.filter((t) => {
                        ...
                        if (!showMoneyOut && t.direction === "out")
                          return false;
                        return true;
                      });
                      return filtered
                        .slice(
                          (pages.transactions - 1) * ITEMS_PER_PAGE,
                          pages.transactions * ITEMS_PER_PAGE,
                        )
                        .map((txn) => (
                          <tr
                            key={`${txn.type}-${txn.id}`}
                ...
                <Pagination
                  ...
                  totalItems={
                    moneyFlow.transactions.filter((t) => {
                      if (!showMoneyIn && t.direction === "in") return false;
            ...
            {bankAccounts.map((account) => (
              ...
              >
                <CardContent className="p-4">
                  ...
                  <p
                    className={`text-2xl font-bold ${account.CurrentBalance < 0 ? "text-red-300" : ""}`}
                  >
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
            ...
            <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
              ...
              <p className="text-2xl font-bold text-emerald-600">
                {formatCurrency(
                  employeeStats.totals.totalGrossPay ||
                    employeeStats.totals.payrollTotalGross ||
                    0,
                )}
              </p>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-left bg-neutral-50 dark:bg-neutral-800/50">
                      ...
                      <th className="p-2 font-semibold text-right text-cyan-600">
                        PTO
                      </th>
                      <th className="p-2 font-semibold text-right text-purple-600">
                        Vacation
                      </th>
                      <th className="p-2 font-semibold text-right text-rose-600">
                        Sick
                      </th>
                      <th className="p-2 font-semibold text-right text-red-600">
                        Unpaid
                      </th>
                      ...
                      <th className="p-2 font-semibold text-right text-emerald-600">
                        Gross Pay
                      </th>
                    </tr>
                  </thead>
                  ...
                  <tfoot className="bg-neutral-100 dark:bg-neutral-800 font-medium">
                    <tr>
                      ...
                      <td className="p-2 text-right font-mono">
                        {(
                          employeeStats.totals.totalTrackedHours -
                          employeeStats.totals.totalOvertime -
                          employeeStats.totals.totalPaidTimeOff -
                          employeeStats.totals.totalUnpaid
                        ).toFixed(1)}
                      </td>
                      <td className="p-2 text-right font-mono text-yellow-600">
                        {employeeStats.totals.totalOvertime.toFixed(1)}
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b text-left">
                        ...
                        <th className="p-2 text-sm font-semibold text-center">
                          Billable
                        </th>
                        <th className="p-2 text-sm font-semibold text-right">
                          Hours
                    ...
                    <tbody>
                      {timeActivities
                        ...
                        .slice(
                          (pages.timeActivities - 1) * ITEMS_PER_PAGE,
                          pages.timeActivities * ITEMS_PER_PAGE,
                        )
                        .map((activity) => {
                          const hours =
                            (activity.Hours || 0) +
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="p-2 text-sm">
                                {activity.EmployeeRef?.name || "—"}
                              </td>
                              <td className="p-2 text-sm truncate max-w-xs">
                                {activity.CustomerRef?.name ||
                                  activity.ItemRef?.name ||
                                  "—"}
                              </td>
                              <td className="p-2 text-sm truncate max-w-xs">
                              ...
                              <td className="p-2 text-sm text-right font-mono">
                                {activity.HourlyRate
                                  ? `${formatCurrency(activity.HourlyRate)}/hr`
                                  : "—"}
                              </td>
                            </tr>
                          );
            ...
            <div className="overflow-x-auto mt-4">
              <table className="w-full">
                ...
                <tbody>
                  {customers
                    .slice(
                      (pages.customers - 1) * ITEMS_PER_PAGE,
                      pages.customers * ITEMS_PER_PAGE,
                    )
                    .map((customer) => (
                      <tr
                        key={customer.Id}
          ...
          <CardContent className="pt-0 border-t">
            {filteredBills.length === 0 ? (
              <div className="text-center py-8 text-neutral-500">
                ...
                <p className="text-xs mt-1">
                  Bills data may not be available for this account
                </p>
              </div>
            ) : (
              <div className="overflow-x-auto mt-4">
                <table className="w-full">
                  ...
                  <tbody>
                    {filteredBills
                      .slice(
                        (pages.bills - 1) * ITEMS_PER_PAGE,
                        pages.bills * ITEMS_PER_PAGE,
                      )
                      .map((bill) => (
                        <tr
                          key={bill.Id}
                        ...
                        >
                          ...
                          <td
                            className={`p-2 text-sm text-right font-mono ${bill.Balance > 0 ? "text-red-600" : "text-green-600"}`}
                          >
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {salesReceipts
                        .slice(
                          (pages.salesReceipts - 1) * ITEMS_PER_PAGE,
                          pages.salesReceipts * ITEMS_PER_PAGE,
                        )
                        .map((receipt) => (
                          <tr
                            key={receipt.Id}
                          ...
                          >
                            ...
                            <td className="p-2 text-sm text-right font-mono text-green-600">
                              {formatCurrency(receipt.TotalAmt)}
                            </td>
                          </tr>
                        ))}
                    </tbody>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {creditMemos
                        .slice(
                          (pages.creditMemos - 1) * ITEMS_PER_PAGE,
                          pages.creditMemos * ITEMS_PER_PAGE,
                        )
                        .map((credit) => (
                          <tr
                            key={credit.Id}
                          ...
                          >
                            ...
                            <td className="p-2 text-sm">
                              {formatDate(credit.TxnDate)}
                            </td>
                            <td className="p-2 text-sm">
                              {credit.CustomerRef?.name || "—"}
                            </td>
                            <td className="p-2 text-sm text-right font-mono">
                              {formatCurrency(credit.TotalAmt)}
                            </td>
                            <td className="p-2 text-sm text-right font-mono text-yellow-600">
                              {formatCurrency(credit.RemainingCredit || 0)}
                            </td>
                          </tr>
                        ))}
                    </tbody>
              ...
              <div className="overflow-x-auto mt-4">
                <table className="w-full">
                  ...
                  <tbody>
                    {journalEntries
                      .slice(
                        (pages.journalEntries - 1) * ITEMS_PER_PAGE,
                        pages.journalEntries * ITEMS_PER_PAGE,
                      )
                      .map((entry) => (
                        <tr
                          key={entry.Id}
              ...
              <div className="overflow-x-auto mt-4">
                <table className="w-full">
                  ...
                  <tbody>
                    {purchaseOrders
                      .slice(
                        (pages.purchaseOrders - 1) * ITEMS_PER_PAGE,
                        pages.purchaseOrders * ITEMS_PER_PAGE,
                      )
                      .map((po) => (
                        <tr
                          key={po.Id}
              ...
              <div className="overflow-x-auto mt-4">
                <table className="w-full">
                  ...
                  <tbody>
                    {transfers
                      .slice(
                        (pages.transfers - 1) * ITEMS_PER_PAGE,
                        pages.transfers * ITEMS_PER_PAGE,
                      )
                      .map((transfer) => (
                        <tr
                          key={transfer.Id}
                        ...
                        >
                          ...
                          <td className="p-2 text-sm">
                            {transfer.FromAccountRef?.name || "—"}
                          </td>
                          <td className="p-2 text-sm">
                            {transfer.ToAccountRef?.name || "—"}
                          </td>
                          <td className="p-2 text-sm text-right font-mono">
          ...
          <CardContent className="pt-0 border-t">
            ...
            terms.length === 0 ? (
              <div className="text-center py-8 text-neutral-500">
                ...
                <p className="text-xs mt-1">
                  Classes, departments, and tax codes may not be configured for
                  this account
                </p>
              </div>
            ) : (
