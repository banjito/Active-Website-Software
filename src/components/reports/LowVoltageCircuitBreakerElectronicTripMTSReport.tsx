...
const tableStyles = {
  ...
  headerCell:
    "px-2 py-1 text-left text-xs font-medium text-zinc-500 dark:text-white uppercase tracking-wider whitespace-normal",
  cell: "px-2 py-1 text-sm text-zinc-900 dark:text-white whitespace-normal",
  input:
    "w-full text-sm py-1 rounded-md border-zinc-300 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white",
  select:
    "w-full text-sm py-1 rounded-md border-zinc-300 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white",
};
  ...
  if (readOnly) {
    return (
      <input
        ...
        value={searchTerm}
        readOnly
        className={`form-input bg-zinc-100 dark:bg-dark-150 ${className}`}
      />
    );
  }
  ...
  return (
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      {isOpen && filteredOptions.length > 0 && (
        <div className="absolute z-50 w-full mt-1 bg-white dark:bg-dark-150 border border-zinc-300 dark:border-zinc-600 rounded-md shadow-lg max-h-60 overflow-auto">
          {filteredOptions.map((option, index) => (
            <div
              key={index}
              onClick={() => handleSelect(option)}
              className="px-3 py-2 cursor-pointer hover:bg-zinc-100 dark:hover:bg-dark-100 text-sm"
            >
              <div className="font-medium text-zinc-900 dark:text-white">
                {option.name}
              </div>
              <div className="text-zinc-500 dark:text-zinc-400">
                SER# {option.serialNumber}
                {option.ampId && `, AMP ID: ${option.ampId}`}
              </div>
            </div>
          ))}
        </div>
...
const LowVoltageCircuitBreakerElectronicTripMTSReport: React.FC = () => {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    visualInspectionItems: [
      ...
      },
      {
        id: "7.6.1.2.A.8",
        description:
          "Verify that primary and secondary contact wipe and other dimensions vital to satisfactory operation of the breaker are in accordance with manufacturer's published data.",
        result: "",
      },
      {
        id: "7.6.1.2.A.9",
    ...
    try {
      ...
      if (generic && generic.data) {
        ...
        setFormData((prev) => ({
          ...
          temperature: {
            ...
            humidity:
              d.reportInfo?.temperature?.humidity ?? prev.temperature.humidity,
          },
          ...
          tripUnitType:
            normalizeTripUnitType(d.nameplateData?.tripUnitType) ||
            prev.tripUnitType,
          ...
          // Visual / Mechanical
          visualInspectionItems:
            d.visualMechanical?.items || prev.visualInspectionItems,
          ...
          insulationResistance:
            d.insulationResistance ?? prev.insulationResistance,
      ...
      if (data) {
        ...
        setFormData((prev) => ({
          ...
          tripUnitType: normalizeTripUnitType(
            data.nameplate_data?.tripUnitType || "",
          ),
  ...
  const handleSave = async () => {
    ...
    const insertPayload = { job_id: jobId, user_id: user.id, data: normalized };
    const updatePayload = { data: normalized };
    console.log("Saving MTS report to low_voltage_cable_test_3sets:", {
      currentReportId,
      insertPayload,
      updatePayload,
    });
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "Update returned no rows. Possible RLS on row owner. Attempting upsert as fallback...",
        );
  ...
  useEffect(() => {
    console.log("Temperature correction useEffect triggered:", {
      isEditing,
    ...
    const calculateCorrectedValue = (value: string): string => {
      ...
      if (isNaN(numericValue) || trimmedValue !== numericValue.toString()) {
        console.log(
          "Contains symbols/letters or not pure number, returning original:",
          trimmedValue,
        );
      ...
      console.log("Pure number, applying TCF:", {
        numericValue,
        tcf,
        result: (numericValue * tcf).toFixed(2),
      });
    ...
    setFormData((prev) => {
      ...
      if (shouldRecalculate && path.includes("primaryInjection.results")) {
        ...
        if (sectionResults && sectionResults.ratedAmperes1) {
          const calculated = calculateTestValuesWithFormData(
            sectionResults.ratedAmperes1,
            section,
            sectionResults,
          );
          if (calculated) {
  ...
  const calculateTestValuesWithFormData = (
    ratedAmperes: string,
    section: "longTime" | "shortTime" | "instantaneous" | "groundFault",
    sectionData: any,
  ) => {
    ...
    const testMultiplier = !isNaN(multiplierNum)
      ? multiplierNum
      : section === "longTime"
        ? 3.0
        : section === "shortTime"
    ...
    const toleranceMin = !isNaN(toleranceMinNum)
      ? toleranceMinNum
      : section === "instantaneous"
        ? -0.2
        : section === "groundFault"
    ...
    const toleranceMax = !isNaN(toleranceMaxNum)
      ? toleranceMaxNum
      : section === "instantaneous"
        ? 0.2
        : section === "groundFault"
    ...
    return {
      testAmperes1:
        section === "instantaneous" ? "" : (value * testMultiplier).toFixed(1),
      testAmperes2: value.toString(),
    ...
    <ReportWrapper isPrintMode={isPrintMode}>
      ...
      <div className="p-6 flex justify-center">
        <div className="max-w-7xl w-full space-y-6">
          ...
          {isEditing && (
            <div
              className={`${isPrintMode ? "hidden" : ""} print:hidden mb-6 flex justify-end`}
            >
              <div className="flex flex-col items-center">
                <span
                  className={`text-[10px] font-medium mb-0.5 ${formData.irDlroOnly ? "text-purple-600 dark:text-purple-400" : "text-zinc-500 dark:text-zinc-400"}`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                    formData.irDlroOnly
                      ...
                      : "bg-zinc-200 text-zinc-700 hover:bg-zinc-300 focus:ring-zinc-400 dark:bg-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-600"
                  }`}
                >
                  IR & DLRO Only
                </button>
          ...
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full table-fixed border-collapse border border-zinc-300 dark:border-zinc-600 visual-mechanical-table">
                <colgroup>
                  <col style={{ width: "15%" }} />
                  <col style={{ width: "65%" }} />
                  <col style={{ width: "20%" }} />
                ...
                <thead className="bg-zinc-50 dark:bg-dark-150">
                  <tr>
                    <th className="border border-zinc-300 dark:border-zinc-600 px-4 py-2 text-left text-sm font-medium text-zinc-900 dark:text-white">
                      NETA Section
                    </th>
                    <th className="border border-zinc-300 dark:border-zinc-600 px-4 py-2 text-left text-sm font-medium text-zinc-900 dark:text-white">
                      Description
          ...
          {!formData.irDlroOnly && (
            <section className="mb-6 primary-injection-section">
                  ...
                  <table className={tableStyles.table}>
                    <thead className="bg-zinc-50 dark:bg-dark-150">
                      <tr>
                        ...
                        <th className={`${tableStyles.headerCell} text-center`}>
                          Setting
                        </th>
                        <th className={`${tableStyles.headerCell} text-center`}>
                          Delay
              ...
              <div className="mb-4 p-4 bg-zinc-50 dark:bg-dark-200 rounded-lg border border-zinc-300 dark:border-zinc-600 no-print-tolerance-section print:hidden">
                <h3 className="text-md font-semibold mb-3 text-zinc-900 dark:text-white">
                  Tolerance Settings
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    ...
                    const label = settingType
                      .replace("Time", " Time")
                      .replace("Fault", " Fault");
                    const sectionData =
                      formData.primaryInjection.results[settingType];
                    return (
                      <div
                        key={`tolerance-${settingType}`}
                        className="space-y-2"
                      >
                        ...
                        <div className="flex gap-3">
                          <div className="flex-1 min-w-[100px]">
                            ...
                            <input
                              type="text"
                              value={(sectionData?.toleranceMin || "").replace(
                                /%/g,
                                "",
                              )}
                              onChange={(e) => {
                                ...
                                handleChange(
                                  `primaryInjection.results.${settingType}.toleranceMin`,
                                  v ? `${v}%` : "",
                                );
                              }}
                              readOnly={!isEditing}
                              className="w-full px-4 py-2 text-base border border-zinc-300 dark:border-zinc-600 rounded-md shadow-sm focus:outline-none focus:ring-[#f26722] focus:border-[#f26722] dark:bg-dark-150 dark:text-white text-left"
                              placeholder="Min"
                          ...
                          <div className="flex-1 min-w-[100px]">
                            ...
                            <input
                              type="text"
                              value={(sectionData?.toleranceMax || "").replace(
                                /%/g,
                                "",
                              )}
                              onChange={(e) => {
                                ...
                                handleChange(
                                  `primaryInjection.results.${settingType}.toleranceMax`,
                                  v ? `${v}%` : "",
                                );
                              }}
                              readOnly={!isEditing}
              ...
              <div className="relative overflow-x-auto shadow-md sm:rounded-lg primary-injection-container print:hidden">
                <table className="w-full table-fixed text-sm text-left primary-injection-table">
                  ...
                  <thead className="bg-zinc-50 dark:bg-dark-150">
                    <tr>
                      <th
                        className={`${tableStyles.headerCell} text-left py-0 text-xs`}
                        rowSpan={2}
                      >
                        Function
                      </th>
                      ...
                      <th
                        className={`${tableStyles.headerCell} text-center`}
                        colSpan={2}
                      >
                        Multiplier %
                      </th>
                  ...
                  <tbody className="bg-white dark:bg-dark-150 divide-y divide-zinc-200 dark:divide-zinc-700">
                    ...
                    <tr>
                      <td
                        className={`${tableStyles.cell} py-0 text-xs`}
                        rowSpan={2}
                      >
                        Long Time
                      </td>
              ...
              <div className="hidden print:block primary-injection-pdf-layout">
                <table className="w-full table-fixed border-collapse border border-zinc-300 print:border-black primary-injection-main-pdf-table">
                  <colgroup>
                    <col style={{ width: "16%" }} />
                    <col style={{ width: "12%" }} />
                    <col style={{ width: "14%" }} />
                  ...
                  <tbody>
                    {primaryInjectionPrintSections.map(
                      ({ key, label, pickupLabel }) => {
                ...
                <table className="w-full table-fixed border-collapse border border-zinc-300 print:border-black primary-injection-poles-pdf-table">
                  <colgroup>
                    <col style={{ width: "18%" }} />
                    <col style={{ width: "16%" }} />
                    <col style={{ width: "22%" }} />
                        ...
                        return (
                          <React.Fragment key={`${key}-poles`}>
                            <tr>
                              ...
                              <td>Timing</td>
                              <td>
                                {formatPoleValue(result.pole1.sec, "sec.")}
                              </td>
                              <td>
                                {formatPoleValue(result.pole2.sec, "sec.")}
                              </td>
                              <td>
                                {formatPoleValue(result.pole3.sec, "sec.")}
                              </td>
                            </tr>
                            <tr>
                              <td>{pickupLabel}</td>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <input
                    ...
                    readOnly={!isEditing}
                    className={`form-input w-full text-zinc-900 dark:text-white ${!isEditing ? "bg-zinc-100 dark:bg-dark-150" : ""}`}
                  />
                </div>
                <div className="min-w-0">
                  <label className="block text-sm font-medium text-zinc-900 dark:text-white mb-1">
          ...
          {formData.comments?.trim() && (
            <div className="hidden print:block mb-6 comments-print-section">
              ...
              <div
                className="comments-cell border-2 border-black px-4 py-4 text-sm"
                style={{
                  minHeight: "120px",
                  whiteSpace: "pre-wrap",
