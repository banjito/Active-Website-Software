...
export function DivisionAnalyticsDialog({
  division,
  isOpen,
  onClose,
  ...
  async function fetchDivisionStats() {
    ...
    try {
      ...
      const awardedOpps = opportunities.filter(
        (opp) => opp.status.toLowerCase() === "awarded",
      );
      const totalValue = awardedOpps.reduce(
        (sum, opp) => sum + (opp.quoted_amount || 0),
        0,
      );
      ...
      const lost = opportunities.filter(
        (opp) => opp.status.toLowerCase() === "lost",
      ).length;
      const winRate =
        awardedCount + lost > 0
          ? (awardedCount / (awardedCount + lost)) * 100
          : 0;

      // Calculate changes from last month for other stats
      const lastMonthActive = lastMonthOpps.filter(
        (opp) =>
          opp.sales_stage &&
          (opp.sales_stage.toLowerCase() === "quote" ||
            opp.sales_stage.toLowerCase() === "decision"),
      ).length;
      const lastMonthAwardedOpps = lastMonthOpps.filter(
        (opp) => opp.status.toLowerCase() === "awarded",
      );
      const lastMonthValue = lastMonthAwardedOpps.reduce(
        (sum, opp) => sum + (opp.quoted_amount || 0),
        0,
      );
      const lastMonthAwardedCount = lastMonthAwardedOpps.length;
      const lastMonthLost = lastMonthOpps.filter(
        (opp) => opp.status.toLowerCase() === "lost",
      ).length;
      const lastMonthWinRate =
        lastMonthAwardedCount + lastMonthLost > 0
          ? (lastMonthAwardedCount / (lastMonthAwardedCount + lastMonthLost)) *
            100
          : 0;
      ...
      const chartData = months.map((date) => {
        ...
        const monthOpportunities = awardedOpps.filter((opp) => {
          const awardedDate = opp.awarded_date
            ? new Date(opp.awarded_date)
            : new Date(opp.created_at);
          return awardedDate >= start && awardedDate <= end;
        });
        const monthTotalValue = monthOpportunities.reduce(
          (sum, opp) => sum + (opp.quoted_amount || 0),
          0,
        );
        return {
