...
CREATE POLICY "Users can view chat rooms they have access to" ON common.chat_rooms
  FOR SELECT USING (
    (role_access = 'All') OR 
...
CREATE POLICY "Users can send messages to rooms they have access to" ON common.chat_messages
...
CREATE POLICY "Users can view messages in rooms they have access to" ON common.chat_messages
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM common.chat_rooms cr
      WHERE cr.id = room_id
