...
const initialVisualMechanicalItems = [
  {
    netaSection: "7.2.2.A.1",
    description: "Inspect physical and mechanical condition.",
    result: "",
  },
  {
    netaSection: "7.2.2.A.2",
    description: "Inspect anchorage, alignment, and grounding.",
    result: "",
  },
  {
    netaSection: "7.2.2.A.3",
    description: "Verify the presence of PCB labeling.",
    result: "",
  },
  {
    netaSection: "7.2.2.A.4*",
    description:
      "Prior to cleaning the unit, perform as-found tests. *Optional",
    result: "",
  },
  {
    netaSection: "7.2.2.A.5",
    description: "Clean bushings and control cabinets.",
    result: "",
  },
  {
    netaSection: "7.2.2.A.6",
    description:
  ...
  {
    netaSection: "7.2.2.A.7",
    description: "Verify that cooling fans and pumps operate correctly.",
    result: "",
  },
  {
    netaSection: "7.2.2.A.8.1",
    description:
      "Inspect Bolted connections for high resistance: Use of a low-resistance ohmmeter in accordance with Section 7.2.2.B.1.",
    result: "",
  },
  {
    netaSection: "7.2.2.A.9",
    description: "Verify correct liquid level in tanks and bushings.",
    result: "",
  },
  {
    netaSection: "7.2.2.A.10",
    description:
      "Verify that positive pressure is maintained on gas-blanketed transformers.",
    result: "",
  },
  {
    netaSection: "7.2.2.A.11",
    description:
      "Perform inspections and mechanical tests as recommended by the manufacturer.",
    result: "",
  },
  {
    netaSection: "7.2.2.A.12",
    description: "Test load tap-changer in accordance with Section 7.12.",
    result: "",
  },
  {
    netaSection: "7.2.2.A.13",
    description: "Verify the presence of transformer surge arresters.",
    result: "",
  },
  {
    netaSection: "7.2.2.A.15",
    description:
...
const LiquidXfmrVisualMTSReport: React.FC = () => {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    nameplate: {
      ...
      secondaryVolts1: "",
      secondaryVolts2: "",
      primaryConnectionDelta: false,
      primaryConnectionWye: false,
      primaryConnectionSinglePhase: false,
      secondaryConnectionDelta: false,
      secondaryConnectionWye: false,
      secondaryConnectionSinglePhase: false,
      primaryWindingMaterialAluminum: false,
      primaryWindingMaterialCopper: false,
      secondaryWindingMaterialAluminum: false,
      secondaryWindingMaterialCopper: false,
      tapVoltages: Array(7).fill(""),
      tapPositions: Array(7).fill(""),
      tapPositionLeft1: "",
      tapPositionLeft2: "",
      tapVoltsSpecific: "",
      tapPercentSpecific: "",
    ...
    visualMechanicalInspection: JSON.parse(
      JSON.stringify(initialVisualMechanicalItems),
    ),
  ...
  const calculateRatio = useCallback(
    (numeratorStr: string, denominatorStr: string): string => {
  ...
  const calculateCorrectedValue = useCallback(
    (valueStr: string, tcf: number): string => {
  ...
  // Stable refs for values read inside updateCalculations — avoids useCallback
  // dependency on formData objects that change identity every render.
  const calcRef = useRef(formData.electricalTestsInsulationResistance);
  const tcfRef = useRef(formData.temperature.tcf);
  useEffect(() => {
    calcRef.current = formData.electricalTestsInsulationResistance;
    tcfRef.current = formData.temperature.tcf;
  });

  const updateCalculations = useCallback(
    (testKey: keyof FormData["electricalTestsInsulationResistance"]) => {
      if (
        testKey === "primaryToGround" ||
        testKey === "secondaryToGround" ||
        testKey === "primaryToSecondary"
      ) {
        const testEntry = calcRef.current[testKey];
        const tcf = tcfRef.current;

        const correctedHalfMin = calculateCorrectedValue(
          testEntry.values.halfMin,
          tcf,
        );
        const correctedOneMin = calculateCorrectedValue(
          testEntry.values.oneMin,
          tcf,
        );
        const correctedTenMin = calculateCorrectedValue(
          testEntry.values.tenMin,
          tcf,
        );
        ...
        setFormData((prev) => {
          ...
          const currentCorrected =
            prev.electricalTestsInsulationResistance[testKey].correctedValues;
          if (
            currentCorrected.halfMin === correctedHalfMin &&
            currentCorrected.oneMin === correctedOneMin &&
            currentCorrected.tenMin === correctedTenMin
          ) {
            return prev;
          }
          ...
          const allValues = [
            nextDA.primary,
            nextDA.secondary,
            nextDA.primaryToSecondary,
            nextPI.primary,
            nextPI.secondary,
            nextPI.primaryToSecondary,
          ]
            .map((v) => parseFloat(String(v)))
            .filter((v) => !Number.isNaN(v));
          const acceptableDAPI =
            allValues.length > 0 && allValues.every((v) => v > 1.0)
              ? "Yes"
              : "No";
          ...
          return {
            ...
            electricalTestsInsulationResistance: {
              ...
              [testKey]: {
                ...
                correctedValues: {
                  halfMin: correctedHalfMin,
                  oneMin: correctedOneMin,
                  tenMin: correctedTenMin,
                },
              },
    ...
    [calculateCorrectedValue, calculateRatio],
  );
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      console.log(
        `Loading report from low_voltage_cable_test_3sets with ID: ${reportId}`,
      );
      ...
      if (data) {
        ...
        if (importedData) {
          ...
          console.log(
            "🏭 nameplateData:",
            importedData.reportInfo?.nameplateData,
          );
          console.log("🌡️ temperature:", importedData.reportInfo?.temperature);
          console.log(
            "🔌 insulationResistance:",
            importedData.reportInfo?.insulationResistance,
          );
          ...
          setFormData((prev) => {
            const updatedData = {
              ...
              technicians:
                importedData.reportInfo?.technicians ?? prev.technicians,
              // Top-level PASS/FAIL status from stored report payload
              status: (() => {
                const raw = (importedData as any)?.status;
                if (typeof raw === "string") {
                  const up = raw.toUpperCase();
                  return up === "FAIL" ? "FAIL" : "PASS";
                }
                return prev.status;
              })(),
              ...
              temperature: {
                ...prev.temperature,
                fahrenheit:
                  importedData.reportInfo?.temperature?.fahrenheit ??
                  prev.temperature.fahrenheit,
                celsius:
                  importedData.reportInfo?.temperature?.celsius ??
                  prev.temperature.celsius,
                tcf: (() => {
                  const c = importedData.reportInfo?.temperature?.celsius;
                  return typeof c === "number"
                    ? getTCF(c)
                    : (importedData.reportInfo?.temperature?.correctionFactor ??
                        prev.temperature.tcf);
                })(),
                humidity: (() => {
                  const h = importedData.reportInfo?.temperature?.humidity;
                  if (h === null || h === undefined)
                    return prev.temperature.humidity;
                  return typeof h === "number" ? String(h) : String(h);
                })(),
              },
              substation:
                importedData.reportInfo?.substation ?? prev.substation,
              eqptLocation:
                importedData.reportInfo?.eqptLocation ?? prev.eqptLocation,
              identifier:
                importedData.reportInfo?.identifier ?? prev.identifier,

              // Nameplate Data
              nameplate: {
                ...prev.nameplate,
                manufacturer:
                  importedData.reportInfo?.nameplateData?.manufacturer ??
                  prev.nameplate.manufacturer,
                catalogNumber:
                  importedData.reportInfo?.nameplateData?.catalogNumber ??
                  prev.nameplate.catalogNumber,
                serialNumber:
                  importedData.reportInfo?.nameplateData?.serialNumber ??
                  prev.nameplate.serialNumber,
                kVA:
                  importedData.reportInfo?.nameplateData?.kva ??
                  prev.nameplate.kVA,
                tempRise:
                  importedData.reportInfo?.nameplateData?.tempRise ??
                  prev.nameplate.tempRise,
                impedance:
                  importedData.reportInfo?.nameplateData?.impedance ??
                  prev.nameplate.impedance,
                fluidType:
                  importedData.reportInfo?.nameplateData?.fluidType ??
                  prev.nameplate.fluidType,
                fluidVolume:
                  importedData.reportInfo?.nameplateData?.fluidVolume ??
                  prev.nameplate.fluidVolume,
                // Voltages
                primaryVolts1:
                  importedData.reportInfo?.nameplateData?.primary?.volts ??
                  prev.nameplate.primaryVolts1,
                primaryVolts2:
                  importedData.reportInfo?.nameplateData?.primary
                    ?.voltsSecondary ?? prev.nameplate.primaryVolts2,
                secondaryVolts1:
                  importedData.reportInfo?.nameplateData?.secondary?.volts ??
                  prev.nameplate.secondaryVolts1,
                secondaryVolts2:
                  importedData.reportInfo?.nameplateData?.secondary
                    ?.voltsSecondary ?? prev.nameplate.secondaryVolts2,
                // Connections (as radio booleans)
                primaryConnectionDelta:
                  importedData.reportInfo?.nameplateData?.primary
                    ?.connection === "Delta",
                primaryConnectionWye:
                  importedData.reportInfo?.nameplateData?.primary
                    ?.connection === "Wye",
                primaryConnectionSinglePhase:
                  importedData.reportInfo?.nameplateData?.primary
                    ?.connection === "Single Phase" ||
                  importedData.reportInfo?.nameplateData?.primary
                    ?.connection === "SinglePhase",
                secondaryConnectionDelta:
                  importedData.reportInfo?.nameplateData?.secondary
                    ?.connection === "Delta",
                secondaryConnectionWye:
                  importedData.reportInfo?.nameplateData?.secondary
                    ?.connection === "Wye",
                secondaryConnectionSinglePhase:
                  importedData.reportInfo?.nameplateData?.secondary
                    ?.connection === "Single Phase" ||
                  importedData.reportInfo?.nameplateData?.secondary
                    ?.connection === "SinglePhase",
                // Winding materials (as radio booleans)
                primaryWindingMaterialAluminum:
                  importedData.reportInfo?.nameplateData?.primary?.material ===
                  "Aluminum",
                primaryWindingMaterialCopper:
                  importedData.reportInfo?.nameplateData?.primary?.material ===
                  "Copper",
                secondaryWindingMaterialAluminum:
                  importedData.reportInfo?.nameplateData?.secondary
                    ?.material === "Aluminum",
                secondaryWindingMaterialCopper:
                  importedData.reportInfo?.nameplateData?.secondary
                    ?.material === "Copper",
                // Tap configuration
                tapVoltages: Array.isArray(
                  importedData.reportInfo?.nameplateData?.tapConfiguration
                    ?.voltages,
                )
                  ? importedData.reportInfo.nameplateData.tapConfiguration
                      .voltages
                  : prev.nameplate.tapVoltages,
                tapPositions: Array.isArray(
                  importedData.reportInfo?.nameplateData?.tapConfiguration
                    ?.positions,
                )
                  ? importedData.reportInfo.nameplateData.tapConfiguration.positions.map(
                      (p: any) => String(p),
                    )
                  : prev.nameplate.tapPositions,
                tapPositionLeft1:
                  importedData.reportInfo?.nameplateData?.tapConfiguration
                    ?.currentPosition ?? prev.nameplate.tapPositionLeft1,
                tapPositionLeft2:
                  importedData.reportInfo?.nameplateData?.tapConfiguration
                    ?.currentPositionSecondary ??
                  prev.nameplate.tapPositionLeft2,
                tapVoltsSpecific:
                  importedData.reportInfo?.nameplateData?.tapConfiguration
                    ?.tapVoltsSpecific ?? prev.nameplate.tapVoltsSpecific,
                tapPercentSpecific:
                  importedData.reportInfo?.nameplateData?.tapConfiguration
                    ?.tapPercentSpecific ?? prev.nameplate.tapPercentSpecific,
              },
              ...
              indicatorGaugeValues: {
                ...
                oilLevel:
                  importedData.reportInfo?.oilLevel ??
                  prev.indicatorGaugeValues.oilLevel,
                tankPressure:
                  importedData.reportInfo?.tankPressure ??
                  prev.indicatorGaugeValues.tankPressure,
                oilTemperature:
                  importedData.reportInfo?.oilTemperature ??
                  prev.indicatorGaugeValues.oilTemperature,
                windingTemperature:
                  importedData.reportInfo?.windingTemperature ??
                  prev.indicatorGaugeValues.windingTemperature,
                oilTempRange:
                  importedData.reportInfo?.oilTempRange ??
                  prev.indicatorGaugeValues.oilTempRange,
                windingTempRange:
                  importedData.reportInfo?.windingTempRange ??
                  prev.indicatorGaugeValues.windingTempRange,
              },

              // Visual Inspection - map the visualInspection data to the visualMechanicalInspection array
              visualMechanicalInspection: prev.visualMechanicalInspection.map(
                (item) => {
                  const raw = importedData.visualInspection?.[item.netaSection];
                  const normalized = typeof raw === "string" ? raw.trim() : "";
                  const lower = normalized.toLowerCase();
                  const mapped =
                    lower === "s" || lower === "satisfactory"
                      ? "Satisfactory"
                      : lower === "u" || lower === "unsatisfactory"
                        ? "Unsatisfactory"
                        : lower === "na" || lower === "not applicable"
                          ? "Not Applicable"
                          : lower === "r" || lower === "repaired"
                            ? "Repaired"
                            : lower === "c" || lower === "cleaned"
                              ? "Cleaned"
                              : lower === "sc" || lower === "see comments"
                                ? "See Comments"
                                : item.result;
                  console.log(
                    `🔍 Mapping ${item.netaSection}: ${normalized} -> ${mapped}`,
                  );
                  return {
                    ...item,
                    result: mapped,
              ...
              testEquipmentUsed: {
                ...
                megohmmeter:
                  importedData.testEquipment?.megohmmeter?.name ??
                  prev.testEquipmentUsed.megohmmeter,
                serialNumber:
                  importedData.testEquipment?.megohmmeter?.serialNumber ??
                  prev.testEquipmentUsed.serialNumber,
                ampId:
                  importedData.testEquipment?.megohmmeter?.ampId ??
                  prev.testEquipmentUsed.ampId,
                calDate:
                  importedData.testEquipment?.megohmmeter?.calDate ??
                  prev.testEquipmentUsed.calDate,
              },

              // Comments
              visualMechanicalInspectionComments:
                importedData.reportInfo?.visualComments ??
                importedData.reportInfo?.comments ??
                prev.visualMechanicalInspectionComments,
              electricalTestComments:
                importedData.reportInfo?.electricalComments ??
                prev.electricalTestComments,
              ...
              electricalTestsInsulationResistance: {
                ...
                primaryToGround: {
                  ...
                  testVoltage:
                    importedData.reportInfo?.insulationResistance
                      ?.primaryToGround?.testVoltage ??
                    prev.electricalTestsInsulationResistance.primaryToGround
                      .testVoltage,
                  units:
                    importedData.reportInfo?.insulationResistance
                      ?.primaryToGround?.unit ??
                    prev.electricalTestsInsulationResistance.primaryToGround
                      .units,
                  values: {
                    halfMin:
                      importedData.reportInfo?.insulationResistance
                        ?.primaryToGround?.readings?.halfMinute ??
                      prev.electricalTestsInsulationResistance.primaryToGround
                        .values.halfMin,
                    oneMin:
                      importedData.reportInfo?.insulationResistance
                        ?.primaryToGround?.readings?.oneMinute ??
                      prev.electricalTestsInsulationResistance.primaryToGround
                        .values.oneMin,
                    tenMin:
                      importedData.reportInfo?.insulationResistance
                        ?.primaryToGround?.readings?.tenMinute ??
                      prev.electricalTestsInsulationResistance.primaryToGround
                        .values.tenMin,
                  },
                },
                secondaryToGround: {
                  ...
                  testVoltage:
                    importedData.reportInfo?.insulationResistance
                      ?.secondaryToGround?.testVoltage ??
                    prev.electricalTestsInsulationResistance.secondaryToGround
                      .testVoltage,
                  units:
                    importedData.reportInfo?.insulationResistance
                      ?.secondaryToGround?.unit ??
                    prev.electricalTestsInsulationResistance.secondaryToGround
                      .units,
                  values: {
                    halfMin:
                      importedData.reportInfo?.insulationResistance
                        ?.secondaryToGround?.readings?.halfMinute ??
                      prev.electricalTestsInsulationResistance.secondaryToGround
                        .values.halfMin,
                    oneMin:
                      importedData.reportInfo?.insulationResistance
                        ?.secondaryToGround?.readings?.oneMinute ??
                      prev.electricalTestsInsulationResistance.secondaryToGround
                        .values.oneMin,
                    tenMin:
                      importedData.reportInfo?.insulationResistance
                        ?.secondaryToGround?.readings?.tenMinute ??
                      prev.electricalTestsInsulationResistance.secondaryToGround
                        .values.tenMin,
                  },
                },
                primaryToSecondary: (() => {
                  ...
                  return {
                    ...
                    testVoltage:
                      pts?.testVoltage ??
                      prev.electricalTestsInsulationResistance
                        .primaryToSecondary.testVoltage,
                    units:
                      pts?.unit ??
                      prev.electricalTestsInsulationResistance
                        .primaryToSecondary.units,
                    values: {
                      halfMin:
                        pts?.readings?.halfMinute ??
                        pts?.r05 ??
                        prev.electricalTestsInsulationResistance
                          .primaryToSecondary.values.halfMin,
                      oneMin:
                        pts?.readings?.oneMinute ??
                        pts?.r1 ??
                        prev.electricalTestsInsulationResistance
                          .primaryToSecondary.values.oneMin,
                      tenMin:
                        pts?.readings?.tenMinute ??
                        pts?.r10 ??
                        prev.electricalTestsInsulationResistance
                          .primaryToSecondary.values.tenMin,
                    },
                  };
                })(),
                dielectricAbsorption: {
                  primary:
                    importedData.reportInfo?.insulationResistance
                      ?.dielectricAbsorption?.primary ??
                    prev.electricalTestsInsulationResistance
                      .dielectricAbsorption.primary,
                  secondary:
                    importedData.reportInfo?.insulationResistance
                      ?.dielectricAbsorption?.secondary ??
                    prev.electricalTestsInsulationResistance
                      .dielectricAbsorption.secondary,
                  primaryToSecondary:
                    importedData.reportInfo?.insulationResistance
                      ?.dielectricAbsorption?.primaryToSecondary ??
                    importedData.reportInfo?.insulationResistance
                      ?.dielectricAbsorption?.priToSec ??
                    prev.electricalTestsInsulationResistance
                      .dielectricAbsorption.primaryToSecondary,
                },
                polarizationIndex: {
                  primary:
                    importedData.reportInfo?.insulationResistance
                      ?.polarizationIndex?.primary ??
                    prev.electricalTestsInsulationResistance.polarizationIndex
                      .primary,
                  secondary:
                    importedData.reportInfo?.insulationResistance
                      ?.polarizationIndex?.secondary ??
                    prev.electricalTestsInsulationResistance.polarizationIndex
                      .secondary,
                  primaryToSecondary:
                    importedData.reportInfo?.insulationResistance
                      ?.polarizationIndex?.primaryToSecondary ??
                    importedData.reportInfo?.insulationResistance
                      ?.polarizationIndex?.priToSec ??
                    prev.electricalTestsInsulationResistance.polarizationIndex
                      .primaryToSecondary,
                },
                acceptableDAPI:
                  importedData.reportInfo?.insulationResistance?.acceptable ??
                  prev.electricalTestsInsulationResistance.acceptableDAPI,
              },
            };
            ...
            console.log(
              "🌡️ Indicator gauge values mapped:",
              updatedData.indicatorGaugeValues,
            );
            console.log(
              "🔍 Test equipment mapped:",
              updatedData.testEquipmentUsed,
            );
            console.log(
              "💬 Comments mapped:",
              updatedData.visualMechanicalInspectionComments,
            );
            console.log(
              "⚡ Electrical tests mapped:",
              updatedData.electricalTestsInsulationResistance,
            );
            console.log(
              "🔍 Visual inspection mapped:",
      ...
      try {
        ...
        if (reportId) {
          ...
          setFormData((prev) => ({
            ...
            temperature: {
              ...prev.temperature,
              celsius: Math.round(initialCelsius),
              tcf: initialTcf,
            },
          }));
        }
  ...
  // Debug effect — logs specific fields only when they change
  useEffect(() => {
    console.log(
      "🔄 FormData changed - Visual inspection:",
      ...
      report_data: {
        reportInfo: {
          ...
          nameplateData: {
            ...
            primary: {
              ...
              connection: formData.nameplate.primaryConnectionDelta
                ? "Delta"
                : formData.nameplate.primaryConnectionWye
                  ? "Wye"
                  : "",
              material: formData.nameplate.primaryWindingMaterialAluminum
                ? "Aluminum"
                : formData.nameplate.primaryWindingMaterialCopper
                  ? "Copper"
                  : "",
            },
            secondary: {
              ...
              connection: formData.nameplate.secondaryConnectionDelta
                ? "Delta"
                : formData.nameplate.secondaryConnectionWye
                  ? "Wye"
                  : "",
              material: formData.nameplate.secondaryWindingMaterialAluminum
                ? "Aluminum"
                : formData.nameplate.secondaryWindingMaterialCopper
                  ? "Copper"
                  : "",
            },
            tapConfiguration: {
          ...
          insulationResistance: {
            primaryToGround: {
              testVoltage:
                formData.electricalTestsInsulationResistance.primaryToGround
                  .testVoltage,
              unit: formData.electricalTestsInsulationResistance.primaryToGround
                .units,
              readings: {
                halfMinute:
                  formData.electricalTestsInsulationResistance.primaryToGround
            ...
            secondaryToGround: {
              testVoltage:
                formData.electricalTestsInsulationResistance.secondaryToGround
                  .testVoltage,
              unit: formData.electricalTestsInsulationResistance
                .secondaryToGround.units,
              readings: {
                halfMinute:
                  formData.electricalTestsInsulationResistance.secondaryToGround
                    .values.halfMin,
            ...
            primaryToSecondary: {
              testVoltage:
                formData.electricalTestsInsulationResistance.primaryToSecondary
                  .testVoltage,
              unit: formData.electricalTestsInsulationResistance
                .primaryToSecondary.units,
              readings: {
                halfMinute:
                  formData.electricalTestsInsulationResistance
                    .primaryToSecondary.values.halfMin,
                oneMinute:
                  formData.electricalTestsInsulationResistance
                    .primaryToSecondary.values.oneMin,
            ...
            acceptable:
              formData.electricalTestsInsulationResistance.acceptableDAPI,
          },
          // Store separate comment fields to prevent cross-population
          visualComments: formData.visualMechanicalInspectionComments,
          electricalComments: formData.electricalTestComments,
          // Legacy field retained for backward compatibility
          comments: formData.visualMechanicalInspectionComments,
        },
        visualInspection: formData.visualMechanicalInspection.reduce(
          (acc, item) => {
        ...
        testEquipment: {
          megohmmeter: {
            ...
            calDate: formData.testEquipmentUsed.calDate,
          },
        },
  ...
  const setConnectionSelection = (
    which: "primary" | "secondary",
    value: "Delta" | "Wye" | "SinglePhase",
  ) => {
    setFormData((prev) => {
      const updated = { ...prev };
      if (which === "primary") {
  ...
  const setMaterialSelection = (
    ...
    setFormData((prev) => {
      ...
      if (which === "primary") {
        ...
        updated.nameplate.secondaryWindingMaterialAluminum =
          value === "Aluminum";
  ...
  const handlePrint = () => {
    ...
    const identifier = formData.identifier || formData.eqptLocation || "Report";
    const date = formData.date
      ? new Date(formData.date + "T00:00:00")
          ...
          .replace(/\//g, "-")
      : "";
    document.title = `Liquid Filled Xfmr Visual MTS - ${identifier}${date ? " - " + date : ""}`;
    window.print();
    setTimeout(() => {
  ...
  return (
    <ReportWrapper isPrintMode={isPrintMode}>
      ...
      {/* Print-only: job info table directly under header */}
        ...
        <JobInfoPrintTable
          data={{
            ...
            temperature: {
              ...
              humidity: formData.temperature.humidity
                ? Number(formData.temperature.humidity)
                : undefined,
            },
          }}
        />
        <div className="w-full h-1 bg-[#f26722] my-4" />
      </div>

      {/* Screen-only header with title and buttons */}
      <div className={`${isPrintMode ? "hidden" : ""} print:hidden`}>
        {renderHeader()}
      </div>

      <div className="space-y-8">
        {/* Job Information - screen layout; print uses block above */}
        <section className="mb-6 job-info-section print:hidden">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-x-4 gap-y-2 job-info-onscreen">
              <div>
                ...
                <input
                  ...
                  readOnly
                  className="form-input bg-gray-200 dark:bg-dark-150 w-full text-gray-900 dark:text-white"
                />
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  readOnly={!isEditing}
                  className={`form-input w-full ${!isEditing ? "bg-gray-200 dark:bg-dark-150 text-gray-900 dark:text-white" : ""}`}
                />
              </div>
              <div>
              ...
              <div className="flex items-center space-x-1">
                ...
                <div>
                  ...
                  <input
                    ...
                    readOnly
                    className="form-input w-16 bg-gray-100 dark:bg-dark-150"
                  />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleChange("temperature.humidity", e.target.value)
                  }
                  readOnly={!isEditing}
                  className={`form-input w-16 ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : ""}`}
                />
        ...
        <section className="mb-6 nameplate-section">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-x-6 gap-y-4 print:hidden nameplate-onscreen">
            ...
            <div>
              ...
              <input
                ...
                readOnly={!isEditing}
                className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleChange("nameplate.kVA", e.target.value)}
                readOnly={!isEditing}
                className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleChange("nameplate.fluidType", e.target.value)
                }
                readOnly={!isEditing}
                className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleChange("nameplate.fluidVolume", e.target.value)
                }
                readOnly={!isEditing}
                className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
              />
          ...
          <div className="mt-6 print:hidden nameplate-onscreen">
            <div className="grid grid-cols-[100px_1fr_1fr_1fr] gap-4 items-center">
              ...
              <div className="text-center text-sm font-medium text-gray-700 dark:text-white border-b dark:border-gray-700 border-r dark:border-gray-700">
                Volts
              </div>
              <div className="text-center text-sm font-medium text-gray-700 dark:text-white border-b dark:border-gray-700 border-r dark:border-gray-700">
                Connections
              ...
              {/* Primary Row */}
              <div className="text-sm font-medium text-gray-700 dark:text-white border-b dark:border-gray-700">
                Primary
              </div>
              <div className="flex justify-center items-center space-x-2">
                <input
                  type="text"
                  value={formData.nameplate.primaryVolts1}
                  onChange={(e) =>
                    handleChange("nameplate.primaryVolts1", e.target.value)
                  }
                  readOnly={!isEditing}
                  className={`w-24 text-center rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                />
                <span className="text-gray-500 dark:text-white">/</span>
                <input
                  type="text"
                  value={formData.nameplate.primaryVolts2}
                  onChange={(e) =>
                    handleChange("nameplate.primaryVolts2", e.target.value)
                  }
                  readOnly={!isEditing}
                  className={`w-24 text-center rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                />
              ...
              <div className="flex justify-center space-x-4">
                <label className="inline-flex items-center">
                  <input
                    ...
                    checked={formData.nameplate.primaryConnectionDelta}
                    onChange={() => setConnectionSelection("primary", "Delta")}
                    disabled={!isEditing}
                    className="h-4 w-4 text-[#f26722] border-gray-300 dark:border-gray-600 focus:ring-[#f26722]"
                  />{" "}
                ...
                <label className="inline-flex items-center">
                  <input
                    type="radio"
                    name="primary-connection"
                    checked={formData.nameplate.primaryConnectionWye}
                    onChange={() => setConnectionSelection("primary", "Wye")}
                    disabled={!isEditing}
                    className="h-4 w-4 text-[#f26722] border-gray-300 dark:border-gray-600 focus:ring-[#f26722]"
                  />{" "}
                  <span className="ml-2 text-sm text-gray-700 dark:text-white">
                ...
                <label className="inline-flex items-center">
                  <input
                    type="radio"
                    name="primary-connection"
                    checked={formData.nameplate.primaryConnectionSinglePhase}
                    onChange={() =>
                      setConnectionSelection("primary", "SinglePhase")
                    }
                    disabled={!isEditing}
                    className="h-4 w-4 text-[#f26722] border-gray-300 dark:border-gray-600 focus:ring-[#f26722]"
                  />{" "}
              ...
              <div className="flex justify-center space-x-4">
                <label className="inline-flex items-center">
                  <input
                    ...
                    checked={formData.nameplate.primaryWindingMaterialAluminum}
                    onChange={() => setMaterialSelection("primary", "Aluminum")}
                    disabled={!isEditing}
                    className="h-4 w-4 text-[#f26722] border-gray-300 dark:border-gray-600 focus:ring-[#f26722]"
                  />{" "}
                  <span className="ml-2 text-sm text-gray-700 dark:text-white">
                ...
                <label className="inline-flex items-center">
                  <input
                    type="radio"
                    name="primary-material"
                    checked={formData.nameplate.primaryWindingMaterialCopper}
                    onChange={() => setMaterialSelection("primary", "Copper")}
                    disabled={!isEditing}
                    className="h-4 w-4 text-[#f26722] border-gray-300 dark:border-gray-600 focus:ring-[#f26722]"
                  />{" "}
                  <span className="ml-2 text-sm text-gray-700 dark:text-white">
              ...
              <div className="flex justify-center items-center space-x-2">
                <input
                  type="text"
                  value={formData.nameplate.secondaryVolts1}
                  onChange={(e) =>
                    handleChange("nameplate.secondaryVolts1", e.target.value)
                  }
                  readOnly={!isEditing}
                  className={`w-24 text-center rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                />
                ...
                <input
                  type="text"
                  value={formData.nameplate.secondaryVolts2}
                  onChange={(e) =>
                    handleChange("nameplate.secondaryVolts2", e.target.value)
                  }
                  readOnly={!isEditing}
                  className={`w-24 text-center rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                />
              ...
              <div className="flex justify-center space-x-4">
                <label className="inline-flex items-center">
                  <input
                    ...
                    checked={formData.nameplate.secondaryConnectionDelta}
                    onChange={() =>
                      setConnectionSelection("secondary", "Delta")
                    }
                    disabled={!isEditing}
                    className="h-4 w-4 text-[#f26722] border-gray-300 dark:border-gray-600 focus:ring-[#f26722]"
                  />{" "}
                ...
                <label className="inline-flex items-center">
                  <input
                    type="radio"
                    name="secondary-connection"
                    checked={formData.nameplate.secondaryConnectionWye}
                    onChange={() => setConnectionSelection("secondary", "Wye")}
                    disabled={!isEditing}
                    className="h-4 w-4 text-[#f26722] border-gray-300 dark:border-gray-600 focus:ring-[#f26722]"
                  />{" "}
                  <span className="ml-2 text-sm text-gray-700 dark:text-white">
                ...
                <label className="inline-flex items-center">
                  <input
                    type="radio"
                    name="secondary-connection"
                    checked={formData.nameplate.secondaryConnectionSinglePhase}
                    onChange={() =>
                      setConnectionSelection("secondary", "SinglePhase")
                    }
                    disabled={!isEditing}
                    className="h-4 w-4 text-[#f26722] border-gray-300 dark:border-gray-600 focus:ring-[#f26722]"
                  />{" "}
              ...
              <div className="flex justify-center space-x-4">
                <label className="inline-flex items-center">
                  <input
                    ...
                    checked={
                      formData.nameplate.secondaryWindingMaterialAluminum
                    }
                    onChange={() =>
                      setMaterialSelection("secondary", "Aluminum")
                    }
                    disabled={!isEditing}
                    className="h-4 w-4 text-[#f26722] border-gray-300 dark:border-gray-600 focus:ring-[#f26722]"
                  />{" "}
                ...
                <label className="inline-flex items-center">
                  <input
                    type="radio"
                    name="secondary-material"
                    checked={formData.nameplate.secondaryWindingMaterialCopper}
                    onChange={() => setMaterialSelection("secondary", "Copper")}
                    disabled={!isEditing}
                    className="h-4 w-4 text-[#f26722] border-gray-300 dark:border-gray-600 focus:ring-[#f26722]"
                  />{" "}
                  <span className="ml-2 text-sm text-gray-700 dark:text-white">
            ...
            <div className="space-y-3">
                ...
                <div className="grid grid-cols-7 gap-2 flex-1">
                  {formData.nameplate.tapVoltages.map((voltage, index) => (
                    <input
                      ...
                      readOnly={!isEditing}
                      className={`w-full text-center rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                      placeholder={index > 4 ? "-" : ""}
                    />
                  ))}
                </div>
              ...
              <div className="flex items-center">
                ...
                <div className="grid grid-cols-7 gap-2 flex-1">
                  {formData.nameplate.tapPositions.map((_, index) => (
                    <div
                      key={`tap-pos-label-${index}`}
                      className="text-center text-sm text-gray-700 dark:text-white font-medium bg-gray-100 dark:bg-dark-150 py-1 rounded-md"
                    >
                      {index + 1}
                    </div>
                  ))}
              ...
              <div className="flex items-center pt-2">
                ...
                <div className="flex items-center space-x-2">
                  <input
                    type="text"
                    value={formData.nameplate.tapPositionLeft1}
                    onChange={(e) =>
                      handleChange("nameplate.tapPositionLeft1", e.target.value)
                    }
                    readOnly={!isEditing}
                    className={`w-16 text-center rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                  />
                  <span className="text-gray-500 dark:text-white">/</span>
                  <input
                    type="text"
                    value={formData.nameplate.tapPositionLeft2}
                    onChange={(e) =>
                      handleChange("nameplate.tapPositionLeft2", e.target.value)
                    }
                    readOnly={!isEditing}
                    className={`w-16 text-center rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                  />
                ...
                <div className="flex items-center space-x-2 ml-8">
                  ...
                  <input
                    type="text"
                    value={formData.nameplate.tapVoltsSpecific}
                    onChange={(e) =>
                      handleChange("nameplate.tapVoltsSpecific", e.target.value)
                    }
                    readOnly={!isEditing}
                    className={`w-24 text-center rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                  />
                ...
                <div className="flex items-center space-x-2 ml-8">
                  ...
                  <input
                    type="text"
                    value={formData.nameplate.tapPercentSpecific}
                    onChange={(e) =>
                      handleChange(
                        "nameplate.tapPercentSpecific",
                        e.target.value,
                      )
                    }
                    readOnly={!isEditing}
          ...
          <div className="hidden print:block space-y-4">
            ...
            <table className="min-w-full border-collapse border border-gray-300 dark:border-gray-600">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white">
                    <span className="font-medium">Fluid Volume (gal):</span>{" "}
                    {formData.nameplate.fluidVolume || ""}
                  </td>
                </tr>
                <tr>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white">
            ...
            <table className="min-w-full border-collapse border border-gray-300 dark:border-gray-600">
              ...
              <thead>
                <tr>
                  ...
                  <th
                    className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-sm font-medium text-gray-700 dark:text-white border border-gray-300 dark:border-gray-600"
                    colSpan={1}
                  >
                    Volts
                  </th>
              ...
              <tbody>
                <tr>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium text-gray-900 dark:text-white">
                    Primary
                  </td>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white text-center">
                    {formData.nameplate.primaryVolts1 || ""} /{" "}
                    {formData.nameplate.primaryVolts2 || ""}
                  </td>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white text-center">
                    {formData.nameplate.primaryConnectionDelta ? "☒" : "☐"}{" "}
                    Delta
                  </td>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white text-center">
                    {formData.nameplate.primaryConnectionWye ? "☒" : "☐"} Wye
                  </td>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white text-center">
                    {formData.nameplate.primaryConnectionSinglePhase
                      ? "☒"
                      : "☐"}{" "}
                    Single Phase
                  </td>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white text-center">
                    {formData.nameplate.primaryWindingMaterialAluminum
                      ? "☒"
                      : "☐"}{" "}
                    Aluminum
                  </td>
                ...
                <tr>
                  ...
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white text-center">
                    ...
                    {formData.nameplate.secondaryVolts2 || ""}
                  </td>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white text-center">
                    {formData.nameplate.secondaryConnectionDelta ? "☒" : "☐"}{" "}
                    Delta
                  </td>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white text-center">
                    {formData.nameplate.secondaryConnectionWye ? "☒" : "☐"} Wye
                  </td>
                  <td className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white text-center">
                    {formData.nameplate.secondaryConnectionSinglePhase
                      ? "☒"
                      : "☐"}{" "}
                    Single Phase
                  </td>
            ...
            <table className="min-w-full border-collapse border border-gray-300 dark:border-gray-600">
              <thead>
                <tr>
                  <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-left text-sm font-medium text-gray-700 dark:text-white border border-gray-300 dark:border-gray-600">
                    Tap Position
                  </th>
                  <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-sm font-medium text-gray-700 dark:text-white border border-gray-300 dark:border-gray-600">
                    1
                  </th>
                  <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-sm font-medium text-gray-700 dark:text-white border border-gray-300 dark:border-gray-600">
                    2
                  ...
                  <th className="px-3 py-2 bg-gray-300 dark:bg-dark-150 text-center text-sm font-medium text-gray-700 dark:text-white border border-gray-300 dark:border-gray-600">
                    7
                  </th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  ...
                  {formData.nameplate.tapVoltages.map((voltage, index) => (
                    <td
                      key={index}
                      className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white text-center"
                    >
                      {voltage || ""}
                    </td>
                  ))}
                ...
                <tr>
                  ...
                  <td
                    colSpan={7}
                    className="px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white"
                  >
                    Position: {formData.nameplate.tapPositionLeft1 || ""} /{" "}
                    {formData.nameplate.tapPositionLeft2 || ""} | Volts:{" "}
                    {formData.nameplate.tapVoltsSpecific || ""} | Percent:{" "}
                    {formData.nameplate.tapPercentSpecific || ""}
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 print:hidden indicator-gauges-onscreen">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleChange(
                      "indicatorGaugeValues.oilTemperature",
                      e.target.value,
                    )
                  }
                  readOnly={!isEditing}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleChange(
                      "indicatorGaugeValues.oilTempRange",
                      e.target.value,
                    )
                  }
                  readOnly={!isEditing}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleChange(
                      "indicatorGaugeValues.tankPressure",
                      e.target.value,
                    )
                  }
                  readOnly={!isEditing}
              ...
              <div>
                ...
                <input
                  type="text"
                  value={formData.indicatorGaugeValues.windingTemperature}
                  onChange={(e) =>
                    handleChange(
                      "indicatorGaugeValues.windingTemperature",
                      e.target.value,
                    )
                  }
                  readOnly={!isEditing}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleChange(
                      "indicatorGaugeValues.windingTempRange",
                      e.target.value,
                    )
                  }
                  readOnly={!isEditing}
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td className="p-2 align-top border border-gray-300 print:border-black print:border">
                      ...
                      <div className="mt-0">
                        {formData.indicatorGaugeValues.windingTempRange || ""}
                      </div>
                    </td>
                  </tr>
                </tbody>
                    ...
                    <td className="px-3 py-2">
                      <div className="print:hidden">
                        <select
                          value={item.result}
                          onChange={(e) =>
                            handleVisualInspectionChange(index, e.target.value)
                          }
                          disabled={!isEditing}
                          className={`w-full text-sm rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${
                            !isEditing
                              ? "bg-gray-200 dark:bg-dark-150 text-gray-900 dark:text-white cursor-not-allowed"
        ...
        <section className="mb-6 visual-mechanical-comments">
          ...
          <div className="space-y-4">
            <textarea
              ...
              readOnly={!isEditing}
              rows={4}
              className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"} print:hidden`}
            />
                  ...
                  <tr>
                    <td className="p-2 align-top border border-gray-300 print:border-black">
                      ...
                      <div className="mt-0 whitespace-pre-wrap break-words">
                        {formData.visualMechanicalInspectionComments || ""}
                      </div>
                    </td>
                  </tr>
                </tbody>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <style>{`
                            @media print {
                              .ir-wrap td:first-child, .ir-wrap th:first-child { white-space: normal !important; word-break: break-word !important; }
                            }
                          `}</style>
                <table className="w-full border border-gray-300 dark:border-gray-700 ir-wrap">
                  <thead className="bg-gray-50 dark:bg-dark-150">
                    <tr>
                      <th
                        colSpan={6}
                        className="px-2 py-2 text-center text-sm font-medium text-gray-700 dark:text-white border-b dark:border-gray-700"
                      >
                  ...
                  <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-300 dark:divide-gray-700">
                    {[
                      ...
                      const key = testKey as keyof Pick<
                        FormData["electricalTestsInsulationResistance"],
                        | "primaryToGround"
                        | "secondaryToGround"
                        | "primaryToSecondary"
                      >;
                      const testData =
                        formData.electricalTestsInsulationResistance[key];
                      const title = testKey
                        .replace(/([A-Z])/g, " $1")
                        .replace(/^./, (str) => str.toUpperCase());
                      return (
                        <tr key={testKey}>
                          <td className="td-cell-small font-medium whitespace-normal break-words">
                            {title}
                          </td>
                          <td className="td-cell-small">
                            <select
                              ...
                              onChange={(e) =>
                                handleChange(
                                  `electricalTestsInsulationResistance.${key}.testVoltage`,
                                  e.target.value,
                                )
                              }
                              disabled={!isEditing}
                              className={`w-full text-sm text-center rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                            >
                          ...
                          <td className="td-cell-small">
                            <input
                              type="text"
                              value={testData.values.halfMin}
                              onChange={(e) =>
                                handleChange(
                                  `electricalTestsInsulationResistance.${key}.values.halfMin`,
                                  e.target.value,
                                )
                              }
                              readOnly={!isEditing}
                              className={`w-full text-sm text-center rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-200 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                            />
                          </td>
                          <td className="td-cell-small">
                            <input
                              type="text"
                              value={testData.values.oneMin}
                              onChange={(e) =>
                                handleChange(
                                  `electricalTestsInsulationResistance.${key}.values.oneMin`,
                                  e.target.value,
                                )
                              }
                              readOnly={!isEditing}
                          ...
                          <td className="td-cell-small">
                            <input
                              type="text"
                              value={testData.values.tenMin}
                              onChange={(e) =>
                                handleChange(
                                  `electricalTestsInsulationResistance.${key}.values.tenMin`,
                                  e.target.value,
                                )
                              }
                              readOnly={!isEditing}
                          ...
                          <td className="td-cell-small border-r-0">
                            <select
                              ...
                              onChange={(e) =>
                                handleChange(
                                  `electricalTestsInsulationResistance.${key}.units`,
                                  e.target.value,
                                )
                              }
                              disabled={!isEditing}
                              className={`w-full text-sm text-center rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                            >
                              {insulationResistanceUnits.map((opt) => (
                                <option key={opt} value={opt}>
                                  {opt}
                                </option>
                              ))}
                      ...
                      return (
                        <tr key={`${testKey}-corr`}>
                          <td className="td-cell-small">
                            <input
                              type="text"
                              value={testData.correctedValues.halfMin}
                              readOnly
                              className="w-full text-sm text-center rounded-md border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-dark-150 shadow-sm text-gray-900 dark:text-white cursor-not-allowed"
                            />
                          </td>
                          <td className="td-cell-small">
                            <input
                              type="text"
                              value={testData.correctedValues.oneMin}
                              readOnly
                              className="w-full text-sm text-center rounded-md border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-dark-150 shadow-sm text-gray-900 dark:text-white cursor-not-allowed"
                            />
                          </td>
                          <td className="td-cell-small">
                            <input
                              type="text"
                              value={testData.correctedValues.tenMin}
                              readOnly
                              className="w-full text-sm text-center rounded-md border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-dark-150 shadow-sm text-gray-900 dark:text-white cursor-not-allowed"
                            />
                          </td>
            ...
            <div className="mt-6">
              <table className="w-full border border-gray-300 dark:border-gray-700">
                ...
                <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-300 dark:divide-gray-700">
                  <tr>
                    <td className="td-cell-small font-medium">
                    ...
                    <td className="td-cell-small border-r-0">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleChange(
                            "electricalTestsInsulationResistance.acceptableDAPI",
                            e.target.value,
                          )
                        }
                        readOnly={!isEditing}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 print:hidden test-eqpt-onscreen">
              <div>
                ...
                <EquipmentAutocomplete
                  value={formData.testEquipmentUsed.megohmmeter}
                  onChange={(value) =>
                    handleChange("testEquipmentUsed.megohmmeter", value)
                  }
                  onSelect={(equipment) => {
                    ...
                    handleChange(
                      "testEquipmentUsed.megohmmeter",
                      equipment.equipment_name,
                    );
                    handleChange(
                      "testEquipmentUsed.serialNumber",
                      equipment.serial_number || "",
                    );
                    handleChange(
                      "testEquipmentUsed.ampId",
                      equipment.amp_id || "",
                    );
                    handleChange(
                      "testEquipmentUsed.calDate",
              ...
              <div>
                ...
                <input
                  type="text"
                  value={formData.testEquipmentUsed.serialNumber}
                  onChange={(e) =>
                    handleChange(
                      "testEquipmentUsed.serialNumber",
                      e.target.value,
              ...
              <div>
                ...
                <input
                  type="text"
                  value={formData.testEquipmentUsed.ampId}
                  onChange={(e) =>
                    handleChange("testEquipmentUsed.ampId", e.target.value)
                  }
                  readOnly={!isEditing}
                  className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                />
              ...
              <div>
                ...
                <input
                  type="text"
                  value={formData.testEquipmentUsed.calDate}
                  onChange={(e) =>
                    handleChange("testEquipmentUsed.calDate", e.target.value)
                  }
                  readOnly={!isEditing}
                  className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                />
                ...
                <tbody>
                  <tr>
                    <td className="p-2 align-top border border-gray-300 print:border-black print:border">
                      ...
                      <div className="mt-0">
                        {formData.testEquipmentUsed.megohmmeter || ""}
                      </div>
                    </td>
                    <td className="p-2 align-top border border-gray-300 print:border-black print:border">
                      <div className="font-semibold">Serial Number:</div>
                      <div className="mt-0">
                        {formData.testEquipmentUsed.serialNumber || ""}
                      </div>
                    </td>
                    <td className="p-2 align-top border border-gray-300 print:border-black print:border">
                      <div className="font-semibold">AMP ID:</div>
                      <div className="mt-0">
                        {formData.testEquipmentUsed.ampId || ""}
                      </div>
                    </td>
                    <td className="p-2 align-top border border-gray-300 print:border-black print:border">
                      <div className="font-semibold">Cal Date:</div>
                      <div className="mt-0">
                        {formData.testEquipmentUsed.calDate || ""}
                      </div>
                    </td>
                  </tr>
                </tbody>
        ...
        <section className="mb-6 electrical-comments-section">
          ...
          <div className="space-y-4">
            <textarea
              ...
              onChange={(e) =>
                handleChange("electricalTestComments", e.target.value)
              }
              readOnly={!isEditing}
              rows={4}
              className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] text-gray-900 dark:text-white ${!isEditing ? "bg-gray-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"} print:hidden`}
                ...
                <tbody>
                  <tr>
                    <td className="p-2 align-top border border-gray-300 print:border-black">
                      ...
                      <div className="mt-0 whitespace-pre-wrap break-words">
                        {formData.electricalTestComments || ""}
                      </div>
                    </td>
                  </tr>
                </tbody>
  ...
  style.textContent = `
    ...
    @media print {
      ...
      *[class*="Back to Job"],
      ...
      /* Preserve grid layouts inside report sections (but NOT the report-body wrapper) */
      #report-container section .grid,
      #report-container table .grid {
        display: grid !important;
      }

      /* Preserve flexbox layouts inside report sections (but NOT the report-body wrapper) */
      #report-container section .flex,
      #report-container table .flex,
      #report-container .job-info-print .flex {
      ...
      .indicator-gauges-onscreen, .indicator-gauges-onscreen * { display: none !important; }
...
const tableStyles = `
  .th-cell-small {
    @apply px-2 py-2 bg-gray-50 dark:bg-dark-150 text-left text-xs font-medium text-gray-700 dark:text-white border-b dark:border-gray-700 border-r dark:border-gray-700;
  }
  .td-cell-small {
