        ...
        <div className="max-w-7xl w-full space-y-6">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-x-4 gap-y-2 print:hidden job-info-onscreen">
              ...
              <div>
                ...
                <input id="address" type="text" name="address" value={formData.customerLocation} onChange={(e) => setFormData(prev => ({ ...prev, customerLocation: e.target.value }))} readOnly className={`form-input text-sm bg-gray-100 dark:bg-dark-150 cursor-not-allowed`} />
              </div>
              <div>
          ...
          <div className="mb-6 section-dielectric">
            ...
            <div className="flex justify-end mb-2">
              <div className="flex items-center space-x-2">
                <span className="text-sm font-medium text-gray-700 dark:text-white">Test Voltage:</span>
                <select value={formData.dielectricWithstandTests[0]?.testVoltage || ''} onChange={(e) => { const newTests = formData.dielectricWithstandTests.map(test => ({ ...test, testVoltage: e.target.value })); setFormData({ ...formData, dielectricWithstandTests: newTests }); }} disabled={!isEditing} className={`rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>
                  <option value="">Select...</option>
                  <option value="250V">250V</option>
                  <option value="500V">500V</option>
                  <option value="1000V">1000V</option>
                ...
                <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-200 dark:divide-gray-700">
                  {formData.dielectricWithstandTests.map((test, index) => (
                    <tr key={index}>
                      ...
                      {['ag', 'bg', 'cg'].map((key) => (
                        <td key={key} className="px-3 py-2"><div className="print:hidden"><input type="text" value={test.values[key]} onChange={(e) => { const newTests = [...formData.dielectricWithstandTests]; if (!newTests[index]) { newTests[index] = { busSection: '', values: { ag: '', bg: '', cg: '' }, testVoltage: newTests[0]?.testVoltage || '', unit: 'µA' }; } newTests[index].values[key] = e.target.value; setFormData({ ...formData, dielectricWithstandTests: newTests }); }} readOnly={!isEditing} className={`block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div><div className="hidden print:block text-center">{test.values[key]}</div></td>
                      ))}
                      <td className="px-3 py-2"><div className="print:hidden"><select value={test.unit} onChange={(e) => { const newTests = [...formData.dielectricWithstandTests]; if (!newTests[index]) { newTests[index] = { busSection: '', values: { ag: '', bg: '', cg: '' }, testVoltage: newTests[0]?.testVoltage || '', unit: e.target.value }; } newTests[index].unit = e.target.value; setFormData({ ...formData, dielectricWithstandTests: newTests }); }} disabled={!isEditing} className={`block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}><option value="µA">µA</option><option value="mA">mA</option></select></div><div className="hidden print:block text-center">{test.unit}</div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 gap-6 test-eqpt-onscreen print:hidden">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 dark:text-white">Low Resistance</label><input type="text" value={formData.testEquipment.lowResistance.name} onChange={(e) => setFormData(prev => ({ ...prev, testEquipment: { ...prev.testEquipment, lowResistance: { ...prev.testEquipment.lowResistance, name: e.target.value } } }))} readOnly={!isEditing} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 dark:text-white">Hipot</label><input type="text" value={formData.testEquipment.hipot.name} onChange={(e) => setFormData(prev => ({ ...prev, testEquipment: { ...prev.testEquipment, hipot: { ...prev.testEquipment.hipot, name: e.target.value } } }))} readOnly={!isEditing} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
                <div><label className="block text sm font-medium text-gray-700 dark:text-white">Serial Number</label><input type="text" value={formData.testEquipment.hipot.serialNumber} onChange={(e) => setFormData(prev => ({ ...prev, testEquipment: { ...prev.testEquipment, hipot: { ...prev.testEquipment.hipot, serialNumber: e.target.value } } }))} readOnly={!isEditing} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
                <div><label className="block text-sm font-medium text-gray-700 dark:text-white">AMP ID</label><input type="text" value={formData.testEquipment.hipot.ampId} onChange={(e) => setFormData(prev => ({ ...prev, testEquipment: { ...prev.testEquipment, hipot: { ...prev.testEquipment.hipot, ampId: e.target.value } } }))} readOnly={!isEditing} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
              </div>
            </div>
          </div>
