...
export const FormBuilder: React.FC = () => {
  ...
  const { user } = useAuth();
  const isAdmin = (user?.user_metadata?.role as string) === 'Admin' || (user?.user_metadata?.role as string) === 'Super Admin';

  // Admin-saved component default overrides (merged when adding a section)
  const [componentDefaultOverrides, setComponentDefaultOverrides] = useState<Record<string, Partial<SectionConfig>>>({});
  ...
  const [viewingSavedComponent, setViewingSavedComponent] = useState<SavedComponent | null>(null);
  ...
  const [formulaEditingSectionId, setFormulaEditingSectionId] = useState<string | null>(null);
  ...
  const deleteNetaSection = async (name: string) => {
    try {
      ...
      if ((netaSectionInput || '').trim().toLowerCase() === name.trim().toLowerCase()) {
        setNetaSectionInput('');
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      ...
      setTemplate(prev => ({ ...prev, id: result.data.id, isPublished: result.data.is_published ?? false }));
      setIsDirty(false);
  ...
  const handlePublishToggle = async () => {
    ...
    try {
      ...
      toast.success(newPublished ? 'Template published! It will now appear in jobs.' : 'Template unpublished. It will no longer appear in jobs.');
    } catch (error: any) {
      console.error('Error toggling publish:', error);
      toast.error(`Failed to ${newPublished ? 'publish' : 'unpublish'} template`);
    }
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.data.current?.type === 'library-component') {
      ...
      const dropIndex = template.structure.sections.findIndex(s => s.id === over.id);
      const insertAtIndex = dropIndex >= 0 ? dropIndex : template.structure.sections.length;
    ...
    // If dropping a saved component (custom table etc.) — add section from saved config
      ...
      const { sectionConfig, name, savedComponentId } = active.data.current as { sectionConfig: SectionConfig; name: string; savedComponentId: string };
      const dropIndex = template.structure.sections.findIndex(s => s.id === over.id);
      const insertAtIndex = dropIndex >= 0 ? dropIndex : template.structure.sections.length;
  ...
  const addSection = (componentType: ComponentType, insertAtIndex?: number) => {
    ...
    const mergedDefault = getMergedDefaultConfig(componentType, componentDefaultOverrides);
  ...
  const updateSection = (sectionId: string, updates: Partial<SectionConfig>) => {
    setTemplate(prev => {
      ...
      const copyCellFormulasForNewRows = (s: SectionConfig, prevRows: number, nextRows: number): SectionConfig => {
        if (nextRows <= prevRows || !s.cellFormulas || !s.columns?.length) return s;
          ...
          structure: {
            ...
            sections: prev.structure.sections.map(s => {
              ...
              if (s.rowCountLinkGroupId === groupId) {
                ...
                return copyCellFormulasForNewRows({ ...s, rows: newRows }, prevR, newRows);
              }
              return s;
            }),
          },
  ...
  const linkRowCountWith = (currentSectionId: string, otherSectionId: string | null) => {
    if (!otherSectionId) {
    ...
    setTemplate(prev => {
      ...
      const currentSection = prev.structure.sections.find(s => s.id === currentSectionId);
      const otherSection = prev.structure.sections.find(s => s.id === otherSectionId);
      const otherGroupId = otherSection?.rowCountLinkGroupId;
      const syncedRows = Math.max(currentSection?.rows ?? 1, otherSection?.rows ?? 1);
      return {
        ...prev,
        structure: {
          ...prev.structure,
          sections: prev.structure.sections.map(s => {
            if (s.id === currentSectionId) return { ...s, rowCountLinkGroupId: groupId, rows: syncedRows };
            if (s.id === otherSectionId) return { ...s, rowCountLinkGroupId: groupId, rows: syncedRows };
            if (otherGroupId != null && s.rowCountLinkGroupId === otherGroupId) return { ...s, rowCountLinkGroupId: groupId, rows: syncedRows };
            return s;
          }),
        },
      };
  ...
  const appendFieldOption = useCallback((sectionId: string, columnIndex: number) => {
    setTemplate(prev => {
      const sections = prev.structure.sections.map(s => {
        ...
        columns[columnIndex] = {
          ...
          field: {
            ...col.field,
            options: [...currentOptions, { label: 'New option', value: 'new-option' }],
          },
        };
        return { ...s, columns };
      });
  ...
  const appendOptionToSectionField = useCallback((sectionId: string) => {
    setTemplate(prev => {
      const sections = prev.structure.sections.map(s => {
        ...
        return { ...s, field: { ...s.field, options: [...currentOptions, { label: 'New option', value: 'new-option' }] } };
      });
      return { ...prev, structure: { ...prev.structure, sections } };
    });
    setIsDirty(true);
  ...
  const appendOptionToSectionFieldsField = useCallback((sectionId: string, fieldIndex: number) => {
    setTemplate(prev => {
      const sections = prev.structure.sections.map(s => {
        if (s.id !== sectionId || !s.fields?.[fieldIndex]) return s;
        const newFields = [...s.fields];
        const f = newFields[fieldIndex];
        newFields[fieldIndex] = { ...f, options: [...(f.options || []), { label: 'New option', value: 'new-option' }] };
  ...
  const handleCellFormulaChange = (sectionId: string, rowIndex: number, colId: string, formula: string) => {
    setTemplate(prev => {
      const sections = prev.structure.sections.map(s => {
        if (s.id !== sectionId) return s;
        ...
        return { ...s, cellFormulas: Object.keys(cellFormulas).length > 0 ? cellFormulas : undefined };
      });
      return { ...prev, structure: { ...prev.structure, sections } };
    });
    setIsDirty(true);
    ...
    <div className="h-screen flex flex-col bg-gray-50 dark:bg-dark-200">
        ...
        <div className="flex items-center justify-between gap-2 md:gap-4 mb-2 md:mb-0">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              title={showSidebar ? 'Hide component library' : 'Show component library'}
              size="sm"
              className="shrink-0"
            >
              {showSidebar ? <PanelLeftClose className="w-4 h-4 md:mr-2" /> : <PanelLeftOpen className="w-4 h-4 md:mr-2" />}
              <span className="hidden sm:inline">{showSidebar ? 'Hide' : 'Components'}</span>
            </Button>
            ...
            <Button
              ...
              className={`shrink-0 ${template.isPublished ? 'border-green-500 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20' : 'bg-green-600 hover:bg-green-700 text-white'}`}
              size="sm"
              title={template.isPublished ? 'Unpublish — hide from jobs' : 'Publish — make available in jobs'}
            >
              {template.isPublished ? <Globe className="w-4 h-4 md:mr-2" /> : <Lock className="w-4 h-4 md:mr-2" />}
              <span className="hidden md:inline">{template.isPublished ? 'Published' : 'Publish'}</span>
            </Button>
          </div>
        </div>

        <div className="flex items-center gap-2 mt-2">
          <div className="flex-1 relative" ref={netaSectionInputRef}>
            <Input
              type="text"
              ...
              onBlur={() => {
                setTimeout(() => {
                  if (!netaSectionInputRef.current?.contains(document.activeElement)) {
                    setShowNetaSectionDropdown(false);
                  }
                }, 200);
              }}
              className="w-full"
              placeholder="Type NETA section (e.g. 7.3.3, ATS) or pick saved"
            />
            {showNetaSectionDropdown && (
              <div className="absolute z-50 w-full mt-1 bg-white dark:bg-dark-150 border border-gray-300 dark:border-gray-700 rounded-md shadow-lg max-h-60 overflow-y-auto">
                {netaSections
                  .filter(s => !netaSectionInput || s.toLowerCase().includes(netaSectionInput.toLowerCase()))
                  .map((section) => (
                    <div
                      key={section}
                      ...
                      onClick={() => {
                        ...
                        setTemplate(prev => ({ ...prev, netaSection: section }));
                        setIsDirty(true);
                        setShowNetaSectionDropdown(false);
                      }}
                    >
                ...
                {netaSectionInput.trim() &&
                  !netaSections.some(s => s.toLowerCase() === netaSectionInput.trim().toLowerCase()) && (
                  <div
                    ...
                    onClick={async () => {
                      const success = await createNetaSection(netaSectionInput.trim());
                      if (success) {
                        setTemplate(prev => ({ ...prev, netaSection: netaSectionInput.trim() }));
                        setShowNetaSectionDropdown(false);
                        toast.success(`"${netaSectionInput.trim()}" saved for future use`);
                      } else {
                ...
                {netaSections.filter(s => !netaSectionInput || s.toLowerCase().includes(netaSectionInput.toLowerCase())).length === 0 && !netaSectionInput.trim() && (
                  <div className="px-3 py-4 text-center text-gray-500 dark:text-gray-400 text-sm">
                    No saved sections. Type to add one, then save it for next time.
                  </div>
                )}
              </div>
            )}
        ...
        >
          ...
          {selectedSectionId && (
            <SectionEditor
              ...
              onLinkRowCountWith={(otherSectionId) => linkRowCountWith(selectedSectionId, otherSectionId)}
              onAppendFieldOption={appendFieldOption}
              onAppendOptionToSectionField={appendOptionToSectionField}
              onAppendOptionToSectionFieldsField={appendOptionToSectionFieldsField}
              onClose={() => { setSelectedSectionId(null); setFormulaEditingSectionId(null); }}
              isAdmin={isAdmin}
              ...
              onSaveAsDefault={
                ...
                template.structure.sections.find(s => s.id === selectedSectionId)?.componentType === ComponentType.CUSTOM_TABLE
                  ? undefined
                  : async (sectionFromEditor) => {
                    if (!sectionFromEditor || !user?.id) return;
                    const { error } = await saveComponentDefaultOverride(sectionFromEditor.componentType, sectionFromEditor, user.id);
                    if (error) {
                      toast.error(error.message || 'Could not save as default. Only admins can save component defaults.');
                    } else {
                      const { id, order, ...rest } = sectionFromEditor;
                      setComponentDefaultOverrides(prev => ({ ...prev, [sectionFromEditor.componentType]: rest as Partial<SectionConfig> }));
                      toast.success(`Component default saved. "${sectionFromEditor.title}" will be used for new instances.`, { duration: 5000 });
                    }
                    if (sectionFromEditor.savedComponentId) {
                      const updateErr = await updateSavedComponent(sectionFromEditor.savedComponentId, sectionFromEditor);
                      if (updateErr.error) {
                        toast.error(updateErr.error.message || 'Could not update saved component in library.');
                      } else {
                        refetchSavedComponents();
                      }
                    }
              ...
              onSaveAsNewComponent={async (sectionFromEditor) => {
                if (!sectionFromEditor || !user?.id) {
                  if (!user?.id) toast.error('You must be logged in to save a component to the library.');
                  return;
                }
                const defaultName = (sectionFromEditor.title as string) || 'Custom table';
                const name = window.prompt('Name this component for the library (e.g. "Insulation Resistance Table")', defaultName);
                if (!name?.trim()) return;
                const result = await saveSavedComponent(name.trim(), sectionFromEditor, user.id);
                if ('error' in result) {
                  toast.error(result.error.message);
                  return;
                }
                updateSection(sectionFromEditor.id, { savedComponentId: result.id } as Partial<SectionConfig>);
                refetchSavedComponents();
                toast.success(`"${name}" saved to the library. Use "Update saved component" to push future edits.`, { duration: 5000 });
              }}
              onUpdateSavedComponent={async (sectionFromEditor) => {
                if (!sectionFromEditor?.savedComponentId || !user?.id) return;
                const updateErr = await updateSavedComponent(sectionFromEditor.savedComponentId, sectionFromEditor);
                if (updateErr.error) {
                  toast.error(updateErr.error.message || 'Could not update saved component.');
                  return;
                }
                refetchSavedComponents();
                toast.success(`Saved component "${sectionFromEditor.title}" updated in the library.`, { duration: 5000 });
              }}
            />
          )}
      ...
      <SavedComponentsDialog
        ...
        onAddToForm={(sectionConfig, savedComponentId, selectForEdit) => {
          addSectionFromSaved(sectionConfig, undefined, savedComponentId, selectForEdit ?? false);
        }}
        onDelete={async (id) => {
