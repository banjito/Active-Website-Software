...
const INSPECTION_ITEMS = [
  ...
  { id: "7.10.2.A.5", description: "Verify the unit is clean." },
  {
    id: "7.10.2.A.6.1",
    description:
      "Use of low-resistance ohmmeter in accordance with Section 7.10.2.B.1.",
  },
  {
    id: "7.10.2.A.7",
    description:
      "Verify that all required grounding and connections provide good electrical contact.",
  },
  {
    id: "7.10.2.A.8",
    description:
...
const convertFahrenheitToCelsius = (fahrenheit: number): number => {
  const exactMatch = TEMP_CONVERSION_DATA.find(
    (data) => Math.abs(data.fahrenheit - fahrenheit) < 0.01,
  );
  if (exactMatch) return exactMatch.celsius;

  // Linear interpolation if no exact match
  const sortedData = TEMP_CONVERSION_DATA.sort(
    (a, b) => a.fahrenheit - b.fahrenheit,
  );

  if (fahrenheit <= sortedData[0].fahrenheit) return sortedData[0].celsius;
  if (fahrenheit >= sortedData[sortedData.length - 1].fahrenheit)
    return sortedData[sortedData.length - 1].celsius;
  ...
  for (let i = 0; i < sortedData.length - 1; i++) {
    ...
    if (fahrenheit >= curr.fahrenheit && fahrenheit <= next.fahrenheit) {
      const ratio =
        (fahrenheit - curr.fahrenheit) / (next.fahrenheit - curr.fahrenheit);
...
const getTCF = (celsius: number): number => {
  ...
  if (celsius >= sortedData[sortedData.length - 1].celsius)
    return sortedData[sortedData.length - 1].multiplier;
...
const PotentialTransformerATSReport: React.FC = () => {
  ...
  const reportSlug = "potential-transformer-ats-report";
  ...
  const temperatureValue =
    typeof formData.temperature === "object" && formData.temperature !== null
  ...
  const handleFuseResistanceChange = (
    field: keyof FuseResistanceRow,
    value: string,
  ) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleInsulationChange = (
    index: number,
    field: keyof InsulationRow,
    value: string,
  ) => {
    setFormData((prev) => {
      const rows = Array.isArray(prev.insulationResistance?.rows)
        ? prev.insulationResistance.rows
        : [];
      return {
        ...prev,
        insulationResistance: {
  ...
  const handleInsulationCorrectedChange = (
    index: number,
    field: keyof InsulationCorrectedRow,
    value: string,
  ) => {
    setFormData((prev) => {
      const rows = Array.isArray(prev.insulationCorrected?.rows)
        ? prev.insulationCorrected.rows
        : [];
      return {
        ...prev,
        insulationCorrected: {
  ...
  const handleTurnsRatioChange = (
    index: number,
    field: keyof TurnsRatioRow,
    value: string,
  ) => {
    setFormData((prev) => {
      const rows = Array.isArray(prev.turnsRatio?.rows)
        ? prev.turnsRatio.rows
        : [];
        ...
        if (i === index) {
          ...
          if (field === "calculatedRatio" || field === "measuredH1H2") {
            const deviation = calculateDeviation(
              updatedRow.calculatedRatio,
              updatedRow.measuredH1H2,
            );
  ...
  const handleEquipmentChange = (
    equipment: keyof typeof formData.equipment,
    field: keyof EquipmentInfo,
    value: string,
  ) => {
    setJustSaved(false);
    setFormData((prev) => ({
  ...
  return (
    <div className="p-6 flex justify-center" id="report-container">
      <div className="max-w-7xl w-full space-y-6">
        ...
        <ReportHeader
          title="Potential Transformer ATS"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 print:hidden job-info-onscreen">
            <div className="space-y-4">
              <div className="flex items-center">
                ...
                <input
                  className={`form-input flex-1 ${!isEditMode ? "bg-gray-100 dark:bg-dark-150" : ""}`}
              ...
              <div className="flex items-center">
                ...
                <input
                  className={`form-input w-40 ${!isEditMode ? "bg-gray-100 dark:bg-dark-150" : ""}`}
                  value={formData.date}
        ...
        {/* Device Data */}
        <div className="mb-6">
          <h2 className="section-device-data text-xl font-semibold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2">
            Device Data
          </h2>
              ...
              <tbody>
                <tr>
                  ...
                  <td className="border border-black px-2 py-1 align-top">
                    ...
                    <div className="mt-1">
                      {formData.deviceData.accuracyClass}
                    </div>
                  </td>
                </tr>
                <tr>
                  <td className="border border-black px-2 py-1 align-top">
                    ...
                    <div className="mt-1">
                      {formData.deviceData.manufacturedYear}
                    </div>
                  </td>
                  <td className="border border-black px-2 py-1 align-top">
                    <div className="font-semibold">Voltage Rating:</div>
                    <div className="mt-1">
                      {formData.deviceData.voltageRating}
                    </div>
                  </td>
                  <td className="border border-black px-2 py-1 align-top">
                    <div className="font-semibold">Insulation Class:</div>
                    <div className="mt-1">
                      {formData.deviceData.insulationClass}
                    </div>
                  </td>
                  <td className="border border-black px-2 py-1 align-top">
        ...
        <div className="mb-6">
          <h2 className="section-visual-inspection text-xl font-semibold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2">
            Visual and Mechanical Inspection
          </h2>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse border border-gray-300 dark:border-gray-700 print:border-black visual-mechanical-table table-fixed">
              ...
              <thead>
                <tr>
                  <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-150 print:bg-gray-50 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">
                    NETA Section
                  </th>
                  <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-150 print:bg-gray-50 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">
                    Description
              ...
              <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-200 dark:divide-gray-700">
                {INSPECTION_ITEMS.map((item) => (
                  ...
                  >
                    <td className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                      {item.id}
                    </td>
                    <td className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 text-sm text-gray-900 dark:text-white whitespace-normal break-words">
                      {item.description}
                    </td>
                    ...
                    <td className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2">
                      <div className="print:hidden">
                        <input
                          type="text"
                          value={
                            formData.visualInspection[`${item.id}_comments`] ||
                            ""
                          }
                          onChange={(e) =>
                            handleInspectionChange(
        ...
        {/* Fuse Data */}
        <div className="mb-6">
          <h2 className="section-fuse-data text-xl font-semibold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2">
            Fuse Data
          </h2>
        ...
        {/* Electrical Tests */}
        <div className="mb-6">
          <h2 className="section-electrical-tests text-xl font-semibold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2">
            Electrical Tests
          </h2>
              ...
              <table className="w-full border-collapse border border-gray-300 dark:border-gray-700 print:border-black">
                <thead>
                  <tr>
                    ...
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-150 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">
                      As Found
                    </th>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-150 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">
                      As Left
          ...
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-gray-300 dark:border-gray-700 print:border-black">
                ...
                <tbody>
                  {(Array.isArray(formData.insulationResistance?.rows)
                    ? formData.insulationResistance.rows
                    : []
                  ).map((row, index) => (
                    <tr key={index}>
          ...
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-gray-300 dark:border-gray-700 print:border-black">
                ...
                <tbody>
                  {(Array.isArray(formData.insulationCorrected?.rows)
                    ? formData.insulationCorrected.rows
                    : []
                  ).map((row, index) => (
                    <tr key={index}>
                      <td className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2">
          ...
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-gray-300 dark:border-gray-700 print:border-black">
                ...
                <tbody>
                  {(Array.isArray(formData.turnsRatio?.rows)
                    ? formData.turnsRatio.rows
                    : []
                  ).map((row, index) => (
                    <tr key={index}>
                      <td className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...
                    turnsRatio: {
                      ...prev.turnsRatio,
                      secondaryVoltage: e.target.value,
                    },
                  }))
                }
                readOnly={!isEditMode}
                ...
                <tr>
                  ...
                  <td className="border border-gray-300 dark:border-gray-700 print:border-black px-2 py-1">
                    <EquipmentAutocomplete
                      ...
                      onChange={(value) =>
                        handleEquipmentChange("ttrTestSet", "model", value)
                      }
                      onSelect={(equipment) => {
                        const formatDate = (
                          dateString: string | null,
