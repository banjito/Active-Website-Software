...
const testMethodOptions = [
  "Single test from all phases to ground",
  "Each phase to ground with other phases grounded",
  "Other",
];
...
const defaultFormData: FormData = {
  ...
  frequency: "",
  connections: "",
  visualInspectionItems: VISUAL_INSPECTION_ITEMS.map(({ id, description }) => ({
    id,
    description,
    result: "",
  })),
  insulationResistance: {
    ...
    windings: Object.fromEntries(
      WINDING_ROWS.map((r) => [r.key, defaultWindingRow()]),
    ),
    dielectricAbsorption: {
      aPhase: "",
      bPhase: "",
...
const EmergencySystemsEngineGeneratorATS25Report: React.FC = () => {
  ...
  React.useEffect(() => {
    try {
      const substations = JSON.parse(
        localStorage.getItem("emergency-engine-substations") || "[]",
      );
      const identifiers = JSON.parse(
        localStorage.getItem("emergency-engine-identifiers") || "[]",
      );
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      if (data?.report_data) {
        ...
        const windings =
          rd.insulationResistance?.windings ||
          defaultFormData.insulationResistance.windings;
        // Backward compat: old reports had single "voltages"; now we have voltages1 and voltages2
        const voltages1 =
          rd.voltages1 ?? (rd.voltages != null ? rd.voltages : "");
        ...
        setFormData((prev) => ({
          ...
          visualInspectionItems:
            rd.visualInspectionItems || prev.visualInspectionItems,
          insulationResistance: {
            ...prev.insulationResistance,
            ...rd.insulationResistance,
            windings: { ...prev.insulationResistance.windings, ...windings },
          },
          testEquipment: rd.testEquipment
            ? { ...prev.testEquipment, ...rd.testEquipment }
            : prev.testEquipment,
        }));
        setIsEditing(false);
      }
  ...
  const handleFahrenheitChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      temperature: { ...prev.temperature, fahrenheit: val, celsius, tcf },
    }));
  };
  ...
  const handleCelsiusChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      temperature: { ...prev.temperature, celsius: val, fahrenheit, tcf },
    }));
  };
  ...
  useEffect(() => {
    ...
    WINDING_ROWS.forEach(({ key }) => {
      ...
      if (!w) return;
      const corrected30Sec = calculateInsulationCorrectedValue(
        w.measured30Sec,
        tcfNum,
      );
      const corrected1Min = calculateInsulationCorrectedValue(
        w.measured1Min,
        tcfNum,
      );
      const corrected10Min = calculateInsulationCorrectedValue(
        w.measured10Min,
        tcfNum,
      );
      if (
        corrected30Sec !== w.corrected30Sec ||
        corrected1Min !== w.corrected1Min ||
        corrected10Min !== w.corrected10Min
      ) {
        hasChanges = true;
      }
      nextWindings[key] = {
  ...
  useEffect(() => {
    const windings = formData.insulationResistance.windings;
    const da = {
      aPhase: calculateRatio(
        windings.aPhaseToGround?.corrected1Min ?? "",
        windings.aPhaseToGround?.corrected30Sec ?? "",
      ),
      bPhase: calculateRatio(
        windings.bPhaseToGround?.corrected1Min ?? "",
        windings.bPhaseToGround?.corrected30Sec ?? "",
      ),
      cPhase: calculateRatio(
        windings.cPhaseToGround?.corrected1Min ?? "",
        windings.cPhaseToGround?.corrected30Sec ?? "",
      ),
      phasesTo: calculateRatio(
        windings.phasesToGround?.corrected1Min ?? "",
        windings.phasesToGround?.corrected30Sec ?? "",
      ),
    };
    const pi = {
      aPhase: calculateRatio(
        windings.aPhaseToGround?.corrected10Min ?? "",
        windings.aPhaseToGround?.corrected1Min ?? "",
      ),
      bPhase: calculateRatio(
        windings.bPhaseToGround?.corrected10Min ?? "",
        windings.bPhaseToGround?.corrected1Min ?? "",
      ),
      cPhase: calculateRatio(
        windings.cPhaseToGround?.corrected10Min ?? "",
        windings.cPhaseToGround?.corrected1Min ?? "",
      ),
      phasesTo: calculateRatio(
        windings.phasesToGround?.corrected10Min ?? "",
        windings.phasesToGround?.corrected1Min ?? "",
      ),
    };
    ...
    const daChanged =
      da.aPhase !== prevDA.aPhase ||
      da.bPhase !== prevDA.bPhase ||
      da.cPhase !== prevDA.cPhase ||
      da.phasesTo !== prevDA.phasesTo;
    const piChanged =
      pi.aPhase !== prevPI.aPhase ||
      pi.bPhase !== prevPI.bPhase ||
      pi.cPhase !== prevPI.cPhase ||
      pi.phasesTo !== prevPI.phasesTo;
    if (daChanged || piChanged) {
      setFormData((prev) => ({
        ...prev,
        insulationResistance: {
          ...prev.insulationResistance,
          dielectricAbsorption: daChanged
            ? { ...prev.insulationResistance.dielectricAbsorption, ...da }
            : prev.insulationResistance.dielectricAbsorption,
          polarizationIndex: piChanged
            ? { ...prev.insulationResistance.polarizationIndex, ...pi }
            : prev.insulationResistance.polarizationIndex,
        },
      }));
    }
  }, [
  ...
  return (
    <ReportWrapper isPrintMode={isPrintMode}>
      ...
      <div className="print:flex hidden items-center justify-between border-b-2 border-neutral-800 pb-4 mb-6">
        ...
        <div className="flex-1 text-center">
          <h1 className="text-2xl font-bold text-black mb-1">
            Emergency Systems Engine Generator Test
          </h1>
        </div>
        <div
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6 dark:text-white">
        ...
        <div className="space-y-6">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-x-4 gap-y-2 print:hidden">
              ...
              <div className="flex items-center space-x-1">
                ...
                <div>
                  <label className="form-label sr-only">°C</label>
                  <input
                    type="number"
          ...
          <div className="mb-6">
            <div className="print:hidden">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <div>
                  <label className="form-label">Cat./Model No.:</label>
                  <input
                    type="text"
                    value={formData.catalogModelNo}
                    onChange={(e) =>
                ...
                <div>
                  <label className="form-label">Year Mfd.:</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="form-label">Rated KVA:</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="form-label">Rated kW:</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="form-label">Voltages (V):</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="form-label">Voltages (V) 2:</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="form-label">Current Rating (A):</label>
                  <input
                    type="text"
            ...
            <div className="hidden print:block print:mt-2">
              ...
              <table className="w-full table-fixed border-collapse border border-neutral-300 print:border-black text-[0.85rem]">
                <tbody>
                  ...
                  <tr>
                    <td
                      className="p-2 align-top border border-neutral-300 print:border-black"
                      colSpan={4}
                    >
                      <div className="font-semibold">Connections:</div>
                      <div>{formData.connections || "-"}</div>
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <div className="mb-6">
            ...
            <table className="w-full border-collapse border border-neutral-300 dark:border-neutral-600 vm-standard visual-mechanical-table">
              <thead className="bg-neutral-50 dark:bg-dark-150">
                <tr>
                  <th className="border border-neutral-300 dark:border-neutral-600 px-2 py-1 text-left text-sm font-medium text-neutral-900 dark:text-white">
                    NETA Section
                  </th>
                  <th className="border border-neutral-300 dark:border-neutral-600 px-2 py-1 text-left text-sm font-medium text-neutral-900 dark:text-white">
                    Description
                  </th>
                  <th className="border border-neutral-300 dark:border-neutral-600 px-2 py-1 text-center text-sm font-medium text-neutral-900 dark:text-white">
                    Results
                  </th>
                </tr>
              </thead>
              <tbody className="bg-white dark:bg-dark-150">
                {formData.visualInspectionItems.map((item, idx) => (
                  <tr key={item.id}>
                    <td className="border border-neutral-300 dark:border-neutral-600 px-2 py-1 text-sm text-neutral-900 dark:text-white">
                      {item.id}
                    </td>
                    <td className="border border-neutral-300 dark:border-neutral-600 px-2 py-1 text-sm text-neutral-900 dark:text-white">
                      {item.description}
                    ...
                    <td className="border border-neutral-300 dark:border-neutral-600 px-2 py-1">
                      <select
                        ...
                        onChange={(e) =>
                          handleChange(
                            `visualInspectionItems.${idx}.result`,
                            e.target.value,
                          )
                        }
                        disabled={!isEditing}
                        className={`w-full text-sm ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                      >
                        <option value="">-</option>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 print:hidden">
              <div>
                ...
                <select
                  value={formData.insulationResistance.insulationType}
                  onChange={(e) =>
                    handleChange(
                      "insulationResistance.insulationType",
                      e.target.value,
                    )
                  }
                  disabled={!isEditing}
              ...
              <div>
                ...
                <select
                  value={formData.insulationResistance.testMethod}
                  onChange={(e) =>
                    handleChange(
                      "insulationResistance.testMethod",
                      e.target.value,
                    )
                  }
                  disabled={!isEditing}
              ...
              <div>
                ...
                <input
                  type="text"
                  value={formData.insulationResistance.insulationTempC}
                  onChange={(e) =>
                    handleChange(
                      "insulationResistance.insulationTempC",
                      e.target.value,
                    )
                  }
                  readOnly={!isEditing}
              ...
              <div>
                ...
                <input
                  type="text"
                  value={formData.insulationResistance.tcfTo40C}
                  onChange={(e) =>
                    handleChange(
                      "insulationResistance.tcfTo40C",
                      e.target.value,
                    )
                  }
                  readOnly={!isEditing}
            ...
            <div className="hidden print:block print:mb-3">
              <table className="w-full table-fixed border-collapse border border-neutral-300 print:border-black text-[0.85rem]">
                <tbody>
                  <tr>
                    <td className="p-2 align-top border border-neutral-300 print:border-black w-1/4">
                    ...
                    <td className="p-2 align-top border border-neutral-300 print:border-black w-1/4">
                      ...
                      <div>{formData.insulationResistance.tcfTo40C || "-"}</div>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-neutral-300 dark:border-neutral-600 electrical-tests-table emergency-engine-ir-table">
                <colgroup>
                  <col style={{ width: "20%" }} />
                  <col style={{ width: "6%" }} />
                  <col style={{ width: "12%" }} />
                ...
                <thead className="bg-neutral-50 dark:bg-dark-150">
                  <tr>
                    <th className="border border-neutral-300 dark:border-neutral-600 px-1 py-1 text-left text-xs">
                      Winding Under Test
                    </th>
                    <th className="border border-neutral-300 dark:border-neutral-600 px-1 py-1 text-left text-xs">
                      Test Voltage
                    ...
                    <th
                      colSpan={3}
                      className="border border-neutral-300 dark:border-neutral-600 px-1 py-1 text-center text-xs"
                    >
                      Measured Values
                    </th>
                    <th
                  ...
                  <tr>
                    <th className="border border-neutral-300 dark:border-neutral-600 px-1 py-1" />
                    <th className="border border-neutral-300 dark:border-neutral-600 px-1 py-1" />
                    <th className="border border-neutral-300 dark:border-neutral-600 px-1 py-1 text-xs">
                      30 Sec.
                    </th>
                    <th className="border border-neutral-300 dark:border-neutral-600 px-1 py-1 text-xs">
                      1 Min.
                ...
                <tbody className="bg-white dark:bg-dark-150">
                  {WINDING_ROWS.map(({ key, label }) => {
                    const w =
                      formData.insulationResistance.windings[key] ||
                      defaultWindingRow();
                    return (
                      <tr key={key}>
                        <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1 text-xs">
                          {label}
                        </td>
                        <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1">
                          <select
                            ...
                            onChange={(e) =>
                              handleChange(
                                `insulationResistance.windings.${key}.testVoltage`,
                                e.target.value,
                              )
                            }
                            disabled={!isEditing}
                            className={`w-full text-xs ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                          >
                        ...
                        <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1">
                          <input
                            ...
                            onChange={(e) =>
                              handleChange(
                                `insulationResistance.windings.${key}.measured30Sec`,
                                e.target.value,
                              )
                            }
                            readOnly={!isEditing}
                        ...
                        <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1">
                          <input
                            ...
                            onChange={(e) =>
                              handleChange(
                                `insulationResistance.windings.${key}.measured1Min`,
                                e.target.value,
                              )
                            }
                            readOnly={!isEditing}
                        ...
                        <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1">
                          <input
                            ...
                            onChange={(e) =>
                              handleChange(
                                `insulationResistance.windings.${key}.measured10Min`,
                                e.target.value,
                              )
                            }
                            readOnly={!isEditing}
                        ...
                        <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1">
                          <input
                            ...
                            readOnly
                            className="w-full text-xs bg-neutral-100 dark:bg-dark-150"
                        ...
                        <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1">
                          <select
                            ...
                            onChange={(e) =>
                              handleChange(
                                `insulationResistance.windings.${key}.units`,
                                e.target.value,
                              )
                            }
                            disabled={!isEditing}
            ...
            <div className="mt-4 overflow-x-auto">
              <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700 border border-neutral-300 dark:border-neutral-600 table-fixed w-full">
                <colgroup>
                  <col style={{ width: "18%" }} />
                  <col style={{ width: "28%" }} />
                  <col style={{ width: "12%" }} />
                ...
                <thead>
                  <tr>
                    <th className="px-2 py-2 bg-neutral-50 dark:bg-dark-150 text-left text-xs font-medium text-neutral-500 dark:text-white uppercase border border-neutral-300 dark:border-neutral-600 print:text-[7px]"></th>
                    <th className="px-2 py-2 bg-neutral-50 dark:bg-dark-150 text-center text-xs font-medium text-neutral-500 dark:text-white uppercase border border-neutral-300 dark:border-neutral-600 print:text-[7px]"></th>
                    <th className="px-2 py-2 bg-neutral-50 dark:bg-dark-150 text-center text-xs font-medium text-neutral-500 dark:text-white uppercase border border-neutral-300 dark:border-neutral-600 print:text-[7px]">
                      A Phase
                    </th>
                    <th className="px-2 py-2 bg-neutral-50 dark:bg-dark-150 text-center text-xs font-medium text-neutral-500 dark:text-white uppercase border border-neutral-300 dark:border-neutral-600 print:text-[7px]">
                      B Phase
                ...
                <tbody className="bg-white dark:bg-dark-150">
                  <tr>
                    <td className="px-2 py-2 text-xs font-medium text-neutral-700 dark:text-neutral-300 border border-neutral-300 dark:border-neutral-600">
                      Dielectric Absorption
                    </td>
                    <td className="px-2 py-2 text-xs text-neutral-700 dark:text-neutral-300 border border-neutral-300 dark:border-neutral-600">
                      (Ratio of 1 Min. to 30 Sec. Result)
                    </td>
                    <td className="px-2 py-2 text-center text-sm border border-neutral-300 dark:border-neutral-600">
                      <input
                        type="text"
                        value={
                          formData.insulationResistance.dielectricAbsorption
                            .aPhase
                        }
                        readOnly
                        className="w-full text-center text-sm border-0 bg-neutral-100 dark:bg-dark-150"
                    ...
                    <td
                      className={`px-2 py-2 text-center text-sm font-semibold border border-neutral-300 dark:border-neutral-600 ${formData.insulationResistance.dielectricAbsorption.result === "PASS" ? "text-green-600 bg-green-100 dark:bg-green-900 print:bg-green-100" : formData.insulationResistance.dielectricAbsorption.result === "FAIL" ? "text-red-600 bg-red-100 dark:bg-red-900 print:bg-red-100" : ""}`}
                    >
                      <select
                        value={
                          formData.insulationResistance.dielectricAbsorption
                        ...
                        onChange={(e) =>
                          handleChange(
                            "insulationResistance.dielectricAbsorption.result",
                            e.target.value,
                          )
                        }
                        disabled={!isEditing}
                        className={`w-full text-center text-sm border-0 bg-transparent ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                      >
                        <option value="">-</option>
                  ...
                  <tr>
                    ...
                    <td className="px-2 py-2 text-center text-sm border border-neutral-300 dark:border-neutral-600">
                      <input
                        type="text"
                        value={
                          formData.insulationResistance.polarizationIndex
                            .phasesTo
                        }
                        readOnly
                        className="w-full text-center text-sm border-0 bg-neutral-100 dark:bg-dark-150"
                    ...
                    <td
                      className={`px-2 py-2 text-center text-sm font-semibold border border-neutral-300 dark:border-neutral-600 ${formData.insulationResistance.polarizationIndex.result === "PASS" ? "text-green-600 bg-green-100 dark:bg-green-900 print:bg-green-100" : formData.insulationResistance.polarizationIndex.result === "FAIL" ? "text-red-600 bg-red-100 dark:bg-red-900 print:bg-red-100" : ""}`}
                    >
                      <select
                        value={
                          formData.insulationResistance.polarizationIndex.result
                        }
                        onChange={(e) =>
                          handleChange(
                            "insulationResistance.polarizationIndex.result",
                            e.target.value,
                          )
                        }
                        disabled={!isEditing}
            ...
            <div className="mt-4 print:hidden">
              <label className="form-label">
                Compare temperature corrected values to IEEE-43 & NETA minimum
                insulation resistances:
              </label>
              <select
                value={formData.insulationResistance.complianceResult}
                onChange={(e) =>
                  handleChange(
                    "insulationResistance.complianceResult",
                    e.target.value,
                  )
                }
                disabled={!isEditing}
                className={`form-input w-48 mt-1 ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
              >
                <option value="">-</option>
                {equipmentEvaluationResultOptions.map((o) => (
                  <option key={o} value={o}>
            ...
            <div className="hidden print:block print:mt-3">
              <table className="w-full table-fixed border-collapse border border-neutral-300 print:border-black text-[0.85rem]">
                <thead>
                  <tr>
                    <th className="p-2 text-left border border-neutral-300 print:border-black font-semibold text-black">
                      Description
                    </th>
                    <th className="p-2 text-left border border-neutral-300 print:border-black font-semibold text-black w-24">
                      Result
                    </th>
                  </tr>
                </thead>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 print:hidden test-eqpt-onscreen">
              ...
              <div>
                ...
                <input
                  ...
                  readOnly={!isEditing}
                  className={`mt-1 block w-full rounded-none border-neutral-300 dark:border-neutral-700 shadow-sm text-neutral-900 dark:text-white ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-neutral-700 dark:text-white">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={formData.testEquipment.megohmmeter.calibrationDate}
                  onChange={(e) =>
                    handleChange(
                      "testEquipment.megohmmeter.calibrationDate",
                      e.target.value,
                    )
                  }
                  readOnly={!isEditing}
            ...
            <div className="hidden print:block">
              <table className="w-full border-collapse border border-neutral-300 print:border-black">
                <thead>
                  <tr>
                    <th className="border border-neutral-300 print:border-black p-1 text-left">
                      Equipment
                    </th>
                    <th className="border border-neutral-300 print:border-black p-1 text-left">
                      Serial Number
                ...
                <tbody>
                  <tr>
                    <td className="border border-neutral-300 print:border-black p-1">
          ...
          >
            ...
            {formData.comments?.trim() && (
              <div className="hidden print:block">
                <table className="w-full border-collapse border border-neutral-300 print:border-black text-[0.85rem]">
                  <tbody>
                    <tr>
                      <td className="p-3 align-top border border-neutral-300 print:border-black min-h-[80px] whitespace-pre-wrap text-black">
                        {formData.comments}
                      </td>
                    </tr>
                  </tbody>
                  ...
                  try {
                    ...
                    if (!updatedRows || updatedRows.length === 0) {
                      ...
                      if (candidates && candidates.length > 0) {
                        ...
                        const { data: fixedRows, error: fixError } =
                          await supabase
                            .schema("neta_ops")
                            .from("assets")
                            .update({
                        ...
                        updatedRows =
                          (fixedRows as Array<{ id: string }> | null) || [];
                      }
                    }
                    ...
                    if (!updatedRows || updatedRows.length === 0) {
                      ...
                      const { data: newAsset, error: insertError } =
                        await supabase
                          .schema("neta_ops")
                          .from("assets")
                          .insert({
                      ...
                      if (newAsset) {
                        ...
                        if (linkError) {
                          console.warn(
                            "Asset created but job link failed:",
                            linkError,
                          );
                        }
                      }
                    ...
                    alert(
                      `Failed to mark as ready: ${err?.message || "Unknown error"}`,
                    );
                  }
                }}
      ...
      <datalist id="substation-options">
        {pastSubstations.map((v, i) => (
          <option key={i} value={v} />
        ))}
      </datalist>
      <datalist id="identifier-options">
        {pastIdentifiers.map((v, i) => (
          <option key={i} value={v} />
        ))}
      </datalist>
    </ReportWrapper>
