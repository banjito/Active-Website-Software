...
export class MediumVoltageCableVLFWithTanDeltaMTSImporter extends BaseImporter implements ReportImporter {
  // Combined VLF + Tan Delta MTS form (MediumVoltageCableVLFTest.jsx)
  protected tableName = 'medium_voltage_cable_vlf_test';
  ...
  canImport(data: ReportData): boolean {
    ...
    const titleMatch = t.includes('medium voltage cable vlf test with tan delta mts') || t.includes('tandeltatestmtsform');
    // Also accept if tan delta data present in payload
    const fields: any = (data as any)?.data?.fields || {};
    const hasTanDelta = !!(fields.mvTanDelta || fields.tanDelta || fields.tandelta);
    return titleMatch || hasTanDelta;
  }
  ...
  protected getReportType(_data?: ReportData): string {
    return 'medium-voltage-cable-vlf-test-mts';
  }
}
