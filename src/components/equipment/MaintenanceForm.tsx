...
export default function MaintenanceForm({
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<Omit<MaintenanceRecord, "id" | "created_at" | "updated_at">>({
  ...
  useEffect(() => {
    const loadMaintenanceTypes = async () => {
      try {
        const { data } =
          await equipmentMaintenanceService.getMaintenanceTypes();
        if (data) {
  ...
  const onSubmit = async (
    ...
    try {
      if (maintenanceRecord?.id) {
        ...
        const { error } =
          await equipmentMaintenanceService.updateMaintenanceRecord(
        ...
        // Create new record
        const { error } =
          await equipmentMaintenanceService.createMaintenanceRecord(data);
