...
interface EquipmentInfo {
  ...
  hipot: { model: string; serial: string; ampId: string; calDate: string };
}
...
const INSPECTION_ITEMS: { id: string; description: string }[] = [
  ...
  },
  {
    id: "7.5.4.A.6",
    description:
      "Verify correct operation of SF6 gas pressure alarms and limit switches, as recommended by the manufacturer.",
  },
  {
    id: "7.5.4.A.7",
    description:
  ...
  },
  {
    id: "7.5.4.A.10",
    description:
      "Verify that fuse sizes and types are in accordance with drawings, short-circuit study, and manufacturer's data.",
  },
  {
    id: "7.5.4.A.12",
    description:
...
const MediumVoltageSwitchSF6Report: React.FC = () => {
  ...
  const [formData, setFormData] = useState<ReportData>({
    ...
    visualInspection: Object.fromEntries(
      INSPECTION_ITEMS.map((i) => [i.id, "Select One"] as const),
    ),
    counterReadings: [
    ...
    equipment: {
      ...
      hipot: { model: "", serial: "", ampId: "", calDate: "" },
    },
    comments: "",
  });
  ...
  // Nested handlers
  const handleNameplateChange = (key: keyof NameplateData, value: string) =>
    setFormData((prev) => ({
      ...prev,
      nameplate: { ...prev.nameplate, [key]: value },
    }));
  const handleVFIChange = (key: keyof VFIData, value: string) =>
    setFormData((prev) => ({ ...prev, vfi: { ...prev.vfi, [key]: value } }));
  const handleInspectionChange = (id: string, value: ResultOption) =>
    setFormData((prev) => ({
      ...prev,
      visualInspection: { ...prev.visualInspection, [id]: value },
    }));
  const handleCounterChange = (
    idx: number,
    key: keyof CounterReadingRow,
    value: string,
  ) =>
    setFormData((prev) => ({
      ...prev,
      counterReadings: prev.counterReadings.map((r, i) =>
        i === idx ? { ...r, [key]: value } : r,
      ),
    }));
  const handleInsulationRowChange = (
    section: "insulationMeasured" | "insulationCorrected",
    idx: number,
    key: keyof InsulationRow,
    value: string,
  ) =>
    setFormData(
      (prev) =>
        ({
          ...
          [section]: {
            ...prev[section],
            rows: prev[section].rows.map((r, i) =>
              i === idx ? { ...r, [key]: value } : r,
            ),
          },
        }) as any,
    );
  const handleInsulationVoltageChange = (
    section: "insulationMeasured" | "insulationCorrected",
    value: string,
  ) =>
    setFormData(
      (prev) =>
        ({
          ...prev,
          [section]: { ...prev[section], testVoltage: value },
        }) as any,
    );
  const handleContactRowChange = (
    idx: number,
    ...
    setFormData((prev) => ({
      ...prev,
      contactResistance: {
        rows: prev.contactResistance.rows.map((r, i) =>
          i === idx ? { ...r, [key]: value } : r,
        ),
      },
    }));
  const handleDielectricRowChange = (
    idx: number,
    key: keyof DielectricRow,
    value: string,
  ) =>
    setFormData((prev) => ({
      ...prev,
      dielectricWithstand: {
        ...
        rows: prev.dielectricWithstand.rows.map((r, i) =>
          i === idx ? { ...r, [key]: value } : r,
        ),
      },
    }));
  const handleDielectricVoltageChange = (value: string) =>
    setFormData((prev) => ({
      ...prev,
      dielectricWithstand: { ...prev.dielectricWithstand, testVoltage: value },
    }));
  const handleDielectricVFIChange = (
    idx: number,
    key: keyof DielectricVFIRow,
    value: string,
  ) =>
    setFormData((prev) => ({
      ...prev,
      dielectricVFI: {
        ...prev.dielectricVFI,
        rows: prev.dielectricVFI.rows.map((r, i) =>
          i === idx ? { ...r, [key]: value } : r,
        ),
      },
    }));
  const handleEquipmentChange = (
    ...
    setFormData((prev) => ({
      ...
      equipment: {
        ...prev.equipment,
        [group]: { ...(prev.equipment as any)[group], [key]: value },
      },
    }));
      ...
      try {
        ...
        if (data) {
          ...
          setFormData((prev) => ({
            ...
            temperature:
              typeof ri.temperature === "number"
                ? ri.temperature
                : prev.temperature,
            humidity:
              typeof ri.humidity === "number" ? ri.humidity : prev.humidity,
            ...
            insulationMeasured:
              data.insulation_resistance_measured || prev.insulationMeasured,
            insulationCorrected:
              data.insulation_resistance_corrected || prev.insulationCorrected,
            contactResistance:
              data.contact_resistance || prev.contactResistance,
            dielectricWithstand:
              data.dielectric_withstand || prev.dielectricWithstand,
  ...
  const getTCF = (celsius: number): number => {
    ...
    const exact = TCF_DATA.find(
      (d) => d.celsius === Number(celsius.toFixed(0)),
    );
  ...
  useEffect(() => {
    ...
    setFormData((prev) => {
      ...
      const same =
        JSON.stringify(prev.insulationCorrected.rows) ===
  ...
  return (
    <div id="report-container" className="w-full overflow-visible">
      <div className="p-6 max-w-7xl mx-auto space-y-6 dark:text-white">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-x-4 gap-y-2 print:hidden">
            <div>
              ...
              <input
                className={`form-input text-sm ${!isEditMode ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
            ...
            <div className="flex items-end">
              <div className="flex items-center mt-auto">
                ...
                <input
                  ...
                  readOnly
                  className="form-input text-sm w-24 bg-neutral-100 dark:bg-dark-150"
                />
        ...
        {/* Nameplate Data */}
        <div className="mb-6">
          <h2 className="section-nameplate-data text-xl font-semibold mb-4 text-neutral-900 dark:text-white border-b dark:border-neutral-700 pb-2">
            Nameplate Data
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:hidden">
            <div className="space-y-3">
              <div>
                <label className="form-label inline-block w-40">
                  Manufacturer
                </label>
                <input
                  ...
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleNameplateChange("manufacturer", e.target.value)
                  }
                />
              </div>
              <div>
                ...
                <input
                  className={`form-input`}
                  value={formData.nameplate.catalogNo}
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleNameplateChange("catalogNo", e.target.value)
                  }
                />
              </div>
              <div>
                ...
                <input
                  ...
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleNameplateChange("serialNumber", e.target.value)
                  }
                />
              </div>
              <div>
                ...
                <input
                  className={`form-input`}
                  value={formData.nameplate.dateOfMfg}
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleNameplateChange("dateOfMfg", e.target.value)
                  }
                />
              </div>
              <div>
                <label className="form-label inline-block w-40">Type</label>
                <input
                  className={`form-input`}
                  value={formData.nameplate.type}
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleNameplateChange("type", e.target.value)
                  }
                />
              </div>
              <div>
                ...
                <input
                  className={`form-input`}
                  value={formData.nameplate.sf6GasMass}
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleNameplateChange("sf6GasMass", e.target.value)
                  }
                />
              </div>
            </div>
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleNameplateChange("systemVoltage", e.target.value)
                  }
                />
              </div>
              <div>
                ...
                <input
                  ...
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleNameplateChange("ratedVoltage", e.target.value)
                  }
                />
              </div>
              <div>
                ...
                <input
                  ...
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleNameplateChange("ratedCurrent", e.target.value)
                  }
                />
              </div>
              <div>
                ...
                <input
                  ...
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleNameplateChange("aicRating", e.target.value)
                  }
                />
              </div>
              <div>
                ...
                <input
                  className={`form-input`}
                  value={formData.nameplate.impulseLevelBIL}
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleNameplateChange("impulseLevelBIL", e.target.value)
                  }
                />
              </div>
            </div>
                    ...
                    {[0, 1].map((row) => (
                      <tr key={row}>
                        {cells.slice(row * 6, row * 6 + 6).map((c, idx) => (
                          <td
                            key={idx}
                            className="px-2 py-1 align-top border border-neutral-300 dark:border-neutral-600"
                          >
        ...
        {/* VFI Data */}
        <div className="mb-6">
          <h2 className="section-vfi-data text-xl font-semibold mb-4 text-neutral-900 dark:text-white border-b dark:border-neutral-700 pb-2">
            VFI Data
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:hidden">
            <div className="space-y-3">
              <div>
                ...
                <input
                  className={`form-input`}
                  value={formData.vfi.manufacturer}
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleVFIChange("manufacturer", e.target.value)
                  }
                />
              </div>
              <div>
                ...
                <input
                  className={`form-input`}
                  value={formData.vfi.catalogNo}
                  readOnly={!isEditMode}
                  onChange={(e) => handleVFIChange("catalogNo", e.target.value)}
                />
              </div>
              <div>
                <label className="form-label inline-block w-40">Type</label>
                <input
                  className={`form-input`}
                  value={formData.vfi.type}
                  readOnly={!isEditMode}
                  onChange={(e) => handleVFIChange("type", e.target.value)}
                />
              </div>
            </div>
            <div className="space-y-3">
              <div>
                ...
                <input
                  className={`form-input`}
                  value={formData.vfi.ratedVoltage}
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleVFIChange("ratedVoltage", e.target.value)
                  }
                />
              </div>
              <div>
                ...
                <input
                  className={`form-input`}
                  value={formData.vfi.ratedCurrent}
                  readOnly={!isEditMode}
                  onChange={(e) =>
                    handleVFIChange("ratedCurrent", e.target.value)
                  }
                />
              </div>
              <div>
                ...
                <input
                  className={`form-input`}
                  value={formData.vfi.aicRating}
                  readOnly={!isEditMode}
                  onChange={(e) => handleVFIChange("aicRating", e.target.value)}
                />
              </div>
            </div>
          </div>
        ...
        <div className="mb-6">
          <h2 className="section-visual-mechanical text-xl font-semibold mb-4 text-neutral-900 dark:text-white border-b dark:border-neutral-700 pb-2">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleInspectionChange(
                            item.id,
                            e.target.value as ResultOption,
                          )
                        }
                        disabled={!isEditMode}
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-neutral-50 dark:bg-dark-150">
                  <th
                    colSpan={3}
                    className="px-2 py-1 text-center border border-neutral-300 dark:border-neutral-600"
                  >
                    Counter Readings
                  </th>
                </tr>
                <tr className="bg-neutral-50 dark:bg-dark-150">
                  {["Identifier", "As Found", "As Left"].map((h) => (
                    <th
                      key={h}
                      className="px-2 py-1 text-left border border-neutral-300 dark:border-neutral-600"
                    >
                      {h}
                    </th>
                  ))}
                ...
                {formData.counterReadings.map((row, i) => (
                  ...
                  >
                    <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1">
                      <input
                        className={`form-input text-xs ${!isEditMode ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                        value={row.identifier}
                        readOnly={!isEditMode}
                        onChange={(e) =>
                          handleCounterChange(i, "identifier", e.target.value)
                        }
                      />
                    </td>
                    <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1">
                      <input
                        ...
                        readOnly={!isEditMode}
                        onChange={(e) =>
                          handleCounterChange(i, "asFound", e.target.value)
                        }
                      />
                    </td>
                    <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1">
                      <input
                        ...
                        readOnly={!isEditMode}
                        onChange={(e) =>
                          handleCounterChange(i, "asLeft", e.target.value)
                        }
                      />
                    </td>
                  </tr>
              ...
              <tbody>
                {formData.insulationMeasured.rows.map((row, i) => (
                  <tr
                    ...
                    className="hover:bg-neutral-50 dark:hover:bg-dark-200"
                  >
                    <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1 text-center">
                      <input
                        className={`form-input text-xs text-center ${!isEditMode ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                        value={row.waySection}
                        readOnly={!isEditMode}
                        onChange={(e) =>
                          handleInsulationRowChange(
                            "insulationMeasured",
                            i,
                            "waySection",
                            e.target.value,
        ...
        <section className="mb-6">
          ...
          <div className="flex justify-end mb-2">
            <div className="w-40">
              ...
              <select
                className="form-select text-sm"
                value={formData.insulationCorrected.testVoltage}
                onChange={(e) =>
                  handleInsulationVoltageChange(
                    "insulationCorrected",
                    e.target.value,
        ...
        <section className="mb-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-neutral-50 dark:bg-dark-150">
                  <th className="px-2 py-1 text-center border border-neutral-300 dark:border-neutral-600">
                    Way Section
                  </th>
                  {[
                    "A-Phase",
                  ...
                  >
                    ...
                    {(
                      ...
                      >
                        <input
                          ...
                          readOnly={!isEditMode}
                          onChange={(e) =>
                            handleContactRowChange(i, key, e.target.value)
                          }
                        />
                      </td>
                    ))}
                    <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1">
                      <select
                        ...
                        value={row.units}
                        onChange={(e) =>
                          handleContactRowChange(i, "units", e.target.value)
                        }
                        disabled={!isEditMode}
                      >
                        <option value=""></option>
          ...
          <div className="flex justify-end mb-2">
            <div className="w-40">
              ...
              <select
                ...
                onChange={(e) => handleDielectricVoltageChange(e.target.value)}
                disabled={!isEditMode}
              >
                {TEST_VOLTAGE_DW.map((v) => (
                  <option key={v} value={v}>
                    {v}
                  </option>
                ))}
              ...
              <tbody>
                {formData.dielectricWithstand.rows.map((row, i) => (
                  ...
                  >
                    ...
                    {(["ag", "bg", "cg"] as (keyof DielectricRow)[]).map(
                      (key) => (
                        <td
                          key={String(key)}
                          className="border border-neutral-300 dark:border-neutral-600 px-1 py-1"
                        >
                          <input
                            ...
                            readOnly={!isEditMode}
                            onChange={(e) =>
                              handleDielectricRowChange(i, key, e.target.value)
                            }
                          />
                        </td>
                      ),
                    ...
                    <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1">
                      <select
                        ...
                        value={row.units}
                        onChange={(e) =>
                          handleDielectricRowChange(i, "units", e.target.value)
                        }
                        disabled={!isEditMode}
                      >
                        <option value=""></option>
                        {DIELECTRIC_UNITS.map((u) => (
                          <option key={u} value={u}>
                            {u}
                          </option>
                        ))}
              ...
              <tbody>
                {formData.dielectricVFI.rows.map((row, i) => (
                  ...
                  >
                    ...
                    {(["a", "b", "c"] as (keyof DielectricVFIRow)[]).map(
                      (key) => (
                        <td
                          key={String(key)}
                          className="border border-neutral-300 dark:border-neutral-600 px-1 py-1"
                        >
                          <input
                            ...
                            readOnly={!isEditMode}
                            onChange={(e) =>
                              handleDielectricVFIChange(i, key, e.target.value)
                            }
                          />
                        </td>
                      ),
                    ...
                    <td className="border border-neutral-300 dark:border-neutral-600 px-1 py-1">
                      <select
                        ...
                        value={row.units}
                        onChange={(e) =>
                          handleDielectricVFIChange(i, "units", e.target.value)
                        }
                        disabled={!isEditMode}
                      >
                        <option value=""></option>
                ...
                <tr>
                  ...
                  <td className="border border-neutral-300 dark:border-neutral-700 print:border-black px-2 py-1">
                    <EquipmentAutocomplete
                      ...
                      onChange={(value) =>
                        handleEquipmentChange("megohmmeter", "model", value)
                      }
                      onSelect={(equipment) => {
                        const formatDate = (
                ...
                <tr>
                  ...
                  <td className="border border-neutral-300 dark:border-neutral-700 print:border-black px-2 py-1">
                    <EquipmentAutocomplete
                      ...
                      onChange={(value) =>
                        handleEquipmentChange("lowResOhmmeter", "model", value)
                      }
                      onSelect={(equipment) => {
                        const formatDate = (
                          dateString: string | null,
                ...
                <tr>
                  ...
                  <td className="border border-neutral-300 dark:border-neutral-700 print:border-black px-2 py-1">
                    <input
                      ...
                      readOnly={!isEditMode}
                      onChange={(e) =>
                        handleEquipmentChange("hipot", "model", e.target.value)
                      }
                    />
                  </td>
                  <td className="border border-neutral-300 dark:border-neutral-700 print:border-black px-2 py-1">
                    <input
                      ...
                      readOnly={!isEditMode}
                      onChange={(e) =>
                        handleEquipmentChange("hipot", "serial", e.target.value)
                      }
                    />
                  </td>
                  <td className="border border-neutral-300 dark:border-neutral-700 print:border-black px-2 py-1">
                    <input
                      ...
                      readOnly={!isEditMode}
                      onChange={(e) =>
                        handleEquipmentChange("hipot", "ampId", e.target.value)
                      }
                    />
                  </td>
                  <td className="border border-neutral-300 dark:border-neutral-700 print:border-black px-2 py-1">
