...
export default function JobDiagnostics() {
  ...
  async function runDiagnostics() {
    ...
    try {
      ...
      setJob(jobData);
      log(
        `✓ Found job: ${jobData.title} (${jobData.job_number || "No job number"})`,
      );
      ...
      log(
        `✓ Found customer: ${customerData.name} ${customerData.company_name ? `(${customerData.company_name})` : ""}`,
      );
      ...
      if (!customerData.company_name)
        log(`ℹ️ No company name specified (optional)`);
      ...
      if (missingFields.length > 0) {
        log(
          `⚠️ Customer is missing some recommended fields: ${missingFields.join(", ")}`,
        );
      } else {
      ...
      try {
        ...
        if (jobFetchError) {
          log(
            `❌ Error testing useJobDetails data flow: ${jobFetchError.message}`,
          );
        } else if (jobData) {
          const hookCustomerData = jobData.customers?.[0];
          if (hookCustomerData) {
            log(
              `✓ useJobDetails hook should receive customer data: ${hookCustomerData.name}${hookCustomerData.company_name ? ` (${hookCustomerData.company_name})` : ""}`,
            );
          } else {
            log(
              `❌ useJobDetails hook is not receiving customer data. This may cause display issues.`,
            );
          }
        }
      } catch (err) {
        log(
          `❌ Error testing useJobDetails data flow: ${err instanceof Error ? err.message : "Unknown error"}`,
        );
      }
