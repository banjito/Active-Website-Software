...
export const GuideViewer: React.FC = () => {
  ...
  const loadRelatedGuides = async (category: string, tags: string[], currentId: string) => {
    try {
      const { data, error } = await supabase
        .schema('common')
        .from('help_guides')
      ...
      if (!error && data) {
        setRelatedGuides(data.map(g => ({
          id: g.id,
          title: g.title,
          description: g.description,
          category: g.category,
          tags: g.tags || [],
          content: { blocks: [], settings: { showTableOfContents: false, allowComments: false, showLastUpdated: false } },
        })));
      }
    } catch (error) {
  ...
  if (!guide) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-dark-200">
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-dark-200">
      {/* Header */}
      <div className="bg-white dark:bg-dark-150 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 py-8">
        <div className="flex gap-8">
          ...
          <div className="flex-1 min-w-0">
            <article className="bg-white dark:bg-dark-150 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 lg:p-10">
              {/* Guide Header */}
                ...
                {guide.tags && guide.tags.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-4">
                    {guide.tags.map((tag) => (
                      <span
                        key={tag}
                        className="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 dark:bg-dark-100 text-gray-600 dark:text-gray-400 rounded-full text-sm"
                      >
          ...
          <aside className="hidden lg:block w-72 flex-shrink-0">
            <div className="sticky top-24 space-y-6">
              {/* Table of Contents */}
              {headings.length > 0 && guide.content.settings?.showTableOfContents && (
                <Card>
                      ...
                      {headings.map((heading) => (
                        <a
                          ...
                          className={`block text-sm hover:text-[#f26722] transition-colors ${
                            heading.level === 1 ? 'font-medium text-gray-900 dark:text-white' :
                            heading.level === 2 ? 'text-gray-700 dark:text-gray-300 pl-3' :
                  ...
                  <CardContent className="pt-0">
                    <div className="space-y-2">
                      {relatedGuides.map((related) => (
                        <button
                          key={related.id}
                          onClick={() => navigate(`/help-center/guide/${related.id}`)}
                          className="w-full text-left p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-dark-100 transition-colors group"
                        >
                          <span className="text-sm font-medium text-gray-900 dark:text-white group-hover:text-[#f26722] transition-colors line-clamp-2">
                            {related.title}
                          </span>
                        </button>
                      ))}
  ...
  switch (block.type) {
    case ContentBlockType.HEADING:
      ...
      return (
        <HeadingTag 
          id={block.id}
          className={`${headingClasses[hConfig.level]} text-gray-900 dark:text-white scroll-mt-24`}
        >
          {hConfig.content}
        </HeadingTag>
      );
      ...
      return (
        <ul className="list-disc list-outside ml-6 mb-4 space-y-1 text-gray-700 dark:text-gray-300">
          {((config as ListBlockConfig).items || []).map((item, i) => (
            <li key={i} className="leading-relaxed">{item.content}</li>
          ))}
        </ul>
      );
    ...
    case ContentBlockType.CALLOUT:
      ...
      return (
        <div className={`mb-6 border-l-4 ${style.border} ${style.bg} p-4 rounded-r-lg`}>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                {(tConfig.headers || []).map((header, i) => (
                  <th 
                    key={i} 
                    className="border border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-dark-200 px-4 py-3 text-left font-semibold text-gray-900 dark:text-white"
                  >
                    {header}
                  </th>
                ))}
            ...
            <tbody>
              {(tConfig.rows || []).map((row, ri) => (
                <tr key={ri} className={ri % 2 === 0 ? 'bg-white dark:bg-dark-150' : 'bg-gray-50 dark:bg-dark-100'}>
                  {row.map((cell, ci) => (
                    <td 
                      key={ci} 
