...
serve(async (req) => {
  ...
  try {
    ...
    if (!issueId) return new Response(JSON.stringify({ error: 'issueId required' }), { headers, status: 400 })

    // 1. Get issue via PostgREST
    const restHeaders = { 'Authorization': `Bearer ${key}`, 'apikey': key, 'Accept': 'application/json', 'Accept-Profile': 'common' }
    ...
    if (!issue) return new Response(JSON.stringify({ error: 'not found' }), { headers, status: 404 })
    console.log("Issue:", issue.title, "status:", issue.status, "action:", action)

    // For resolved action, verify the issue is actually resolved
    if (action === 'resolved' && issue.status !== 'resolved' && issue.status !== 'closed') {
      return new Response(JSON.stringify({ emailSent: false, message: 'not resolved' }), { headers })
    }
    ...
    if (emailsToSend.length === 0) {
      return new Response(JSON.stringify({ emailSent: false, message: 'no emails found' }), { headers })
    }

    // 4. Build email content based on action
    const pmKey = Deno.env.get('POSTMARK_API_KEY')
    if (!pmKey) return new Response(JSON.stringify({ emailSent: false, message: 'no POSTMARK_API_KEY' }), { headers })
    ...
    if (action === 'created' && issue.reporter_id) {
      try {
        const rptRes = await fetch(`${url}/auth/v1/admin/users/${issue.reporter_id}`, {
          headers: { 'Authorization': `Bearer ${key}`, 'apikey': key }
        })
        if (rptRes.ok) {
          const rptData = await rptRes.json()
          reporterName = rptData?.user_metadata?.full_name || rptData?.email || 'Unknown'
        }
      } catch { /* skip */ }
    }

    const safeDescription = (issue.description || '').replace(/</g, '&lt;').replace(/\n/g, '<br>')
    ...
    if (action === 'created') {
      ...
      const resolvedAt = issue.resolved_at ? new Date(issue.resolved_at).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' }) : 'Recently'
      ...
      detailRows = `<tr><td style="padding:12px;border-bottom:1px solid #eee;font-weight:bold;background:#f8f9fa">Title</td><td style="padding:12px;border-bottom:1px solid #eee">${safeTitle}</td></tr><tr><td style="padding:12px;font-weight:bold;background:#f8f9fa">Resolved</td><td style="padding:12px">${resolvedAt}</td></tr>`
    }
    ...
    for (const email of emailsToSend) {
      try {
        ...
        const pmRes = await fetch('https://api.postmarkapp.com/email', {
          ...
          body: JSON.stringify({ From: fromHeader, To: email, Subject: emailSubject, HtmlBody: htmlBody, TextBody: textBodyStr, MessageStream: 'outbound' })
        })
    ...
    return new Response(JSON.stringify({ emailSent: sentTo.length > 0, sentTo }), { headers })
  } catch (e) {
