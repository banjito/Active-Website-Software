      ...
      if (data && data.data) {
        ...
        setFormData(prev => ({
          ...
          tanDeltaTest: (() => {
            ...
            const filterOut2Uo = (arr) => (arr || []).filter((v) => (v?.voltageStep || v?.voltageLabel || '').trim() !== '2.0 Uo');
            const filterPointsOut2Uo = (arr) => (arr || []).filter((p) => p.kV < 1.9);
            const defaultTanDelta = {
              systemVoltageL2G: '14.4',
              testVoltage: '',
              frequency: '0.1',
              values: [
                { voltageStep: "0.5 Uo", kV: "7.200", phaseA: { td: '', stdDev: '10' }, phaseB: { td: '', stdDev: '5' }, phaseC: { td: '', stdDev: '5' } },
                { voltageStep: "1.0 Uo", kV: "14.400", phaseA: { td: '', stdDev: '10' }, phaseB: { td: '', stdDev: '5' }, phaseC: { td: '', stdDev: '5' } },
                { voltageStep: "1.5 Uo", kV: "21.600", phaseA: { td: '', stdDev: '10' }, phaseB: { td: '', stdDev: '5' }, phaseC: { td: '', stdDev: '5' } }
              ],
              points: [
                { kV: 0.5, phaseA: 1.1, phaseB: 1.2, phaseC: 1.3 },
                { kV: 1.0, phaseA: 1.3, phaseB: 1.4, phaseC: 1.5 },
            ...
            return {
              ...raw,
              values: filteredValues.length ? filteredValues : defaultTanDelta.values,
              points: (raw.points && filteredPoints.length) ? filteredPoints : defaultTanDelta.points
            };
          })(),
        }));
          ...
          <div className="mt-2">
            <div
              ...
              style={{
                ...
                '--mv-status-bg': formData.status === TestStatus.PASS ? '#22c55e' : '#ef4444',
                '--mv-status-border': formData.status === TestStatus.PASS ? '#16a34a' : '#dc2626',
              }}
            >
              {formData.status === TestStatus.PASS ? 'PASS' : 'FAIL'}
            </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-4 print:hidden test-eqpt-onscreen">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Cal Date</label>
                <input
                  type="text"
                  value={formData.equipment?.ohmCalDate || ''}
                  onChange={(e) => handleChange('equipment', {...formData.equipment, ohmCalDate: e.target.value})}
                  readOnly={!isEditMode}
                  className={`w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-100 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleChange('equipment', {...formData.equipment, megohmCalDate: e.target.value})}
                  readOnly={!isEditMode}
                  className={`w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-100 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`}
                />
              </div>
