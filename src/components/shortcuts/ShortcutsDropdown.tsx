...
export const ShortcutsDropdown: React.FC<ShortcutsDropdownProps> = ({
  ...
  return (
    <div className="flex max-h-[32rem] w-80 flex-col overflow-hidden rounded-none bg-white shadow-xl ring-1 ring-black/5 dark:bg-dark-150 dark:ring-white/10">
      {/* Header */}
      <div className="flex shrink-0 items-center gap-2 border-b border-neutral-100 px-3 py-2.5 dark:border-dark-200">
        {view !== "list" && (
          <button
            ...
            className="-ml-1 rounded-none p-1 text-neutral-500 hover:bg-neutral-100 hover:text-neutral-700 dark:text-neutral-400 dark:hover:bg-dark-200"
          >
        ...
        {view === "list" && (
          <span className="rounded-none bg-neutral-100 px-2 py-0.5 text-[11px] font-medium text-neutral-500 dark:bg-dark-200 dark:text-neutral-400">
      ...
      {error && (
        <div className="mx-3 mt-2 rounded-none border border-red-200 bg-red-50 px-2.5 py-1.5 text-xs text-red-600 dark:border-red-800 dark:bg-red-900/30 dark:text-red-300">
          {error}
        </div>
      )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        {view === "form" && (
          <form onSubmit={handleSave} className="space-y-3 p-3">
            {!editingId && (
              <div className="flex rounded-none bg-neutral-100 p-0.5 text-xs font-medium dark:bg-dark-200">
                <button
                  type="button"
                  onClick={() => setUseBuiltin(true)}
                  className={`flex-1 rounded-none px-2 py-1.5 transition-colors ${
                    useBuiltin
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button
                ...
                className="rounded-none px-3 py-1.5 text-sm font-medium text-neutral-600 hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-dark-200"
              >
                Cancel
              </button>
              <button
            ...
            {quickAddItems.length === 0 ? (
              ...
              <ul className="px-1.5 pb-1.5">
                {quickAddItems.map((it) => {
                  ...
                  return (
                    <li key={it.key}>
                      <label className="flex cursor-pointer items-center gap-2.5 rounded-none px-2 py-2 hover:bg-neutral-50 dark:hover:bg-dark-200">
                        <span
      ...
      {view === "list" && (
        <div className="flex shrink-0 items-center gap-2 border-t border-neutral-100 p-2 dark:border-dark-200">
          <button
            ...
            disabled={atMax}
            className="flex flex-1 items-center justify-center gap-1.5 rounded-none border border-neutral-300 px-2 py-1.5 text-xs font-medium text-neutral-700 hover:bg-neutral-50 disabled:opacity-50 dark:border-dark-200 dark:text-neutral-200 dark:hover:bg-dark-200"
          >
