...
// Dielectric withstand levels offered on the MTS 23 busway sheet.
const DIELECTRIC_TEST_VOLTAGES = [
  "1.6 kVAC",
  "2.2 kVAC",
  "14 kVAC",
  "27 kVAC",
...
interface FormData {
  ...
  /** Busway run length in feet; the MTS 23 limit is 100 / length, in MΩ. */
  buswayLength: string;
  insulationOverallResult: string;
  dielectricTestVoltage: string;
  dielectricTestDuration: string;
  dielectricResult: string;
...
const MetalEnclosedBuswayMTS23Report: React.FC = () => {
  ...
  const reportSlug = "metal-enclosed-busway-mts23"; // This component handles the metal-enclosed-busway-mts23 route
  ...
  // MTS 23 insulation resistance limit for a busway run: 100 / length, in MΩ.
  const buswayCriteria = React.useMemo(() => {
    const feet = parseFloat(formData.buswayLength);
    if (!Number.isFinite(feet) || feet <= 0) return "";
    return `${(100 / feet).toFixed(2)} MΩ`;
  }, [formData.buswayLength]);
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      if (data) {
        setRawRow(data);
        console.log("🔍 MetalEnclosedBuswayMTS23Report - Loaded data:", data);
        console.log(
        ...
        const nonEmptyOnly = (src: Record<string, string>) =>
          Object.fromEntries(
        ...
        // Results are keyed by NETA id. Descriptions are only a fallback for a
        // row that somehow arrived without one.
        const guessIdFromDescription = (desc: string): string | null => {
          const d = (desc || "").toLowerCase();
          if (!d) return null;
          if (d.includes("physical") && d.includes("mechanical"))
            return "7.4.A.1";
          if (d.includes("anchorage")) return "7.4.A.2";
          if (d.includes("bolted") || d.includes("high resistance"))
            return "7.4.A.3";
          if (d.includes("orientation") || d.includes("cooling"))
            return "7.4.A.4";
          if (d.includes("weep") || d.includes("drain")) return "7.4.A.5";
          if (d.includes("joint") && d.includes("shield")) return "7.4.A.6";
          if (d.includes("ventilating")) return "7.4.A.7";
          return null;
        };
        ...
        const newFormData = {
          ...
          // A report can carry readings in either place, and either one can be
          // present but blank, so start from report_info and let non-empty
          // values from the readings object win. Preferring readings outright
          // showed an empty grid for a report whose values sat in report_info.
          insulationResistance: {
            ...normalizeInsulationObj(R("insulationResistance", {})),
            ...nonEmptyOnly(
              normalizeInsulationObj(J("insulation_resistance")?.readings),
            ),
          },
          correctedInsulationResistance: {
            ...normalizeInsulationObj(R("correctedInsulationResistance", {})),
            ...nonEmptyOnly(
              normalizeInsulationObj(
                J("insulation_resistance")?.correctedReadings,
              ),
            ),
          },
          ...
          status: data.status ?? payload.status ?? "PASS",

          buswayLength: R("buswayLength") || "",
          insulationOverallResult: R("insulationOverallResult") || "",
          dielectricTestVoltage: R("dielectricTestVoltage") || "2.3 kVDC",
          dielectricTestDuration: R("dielectricTestDuration") || "1 min.",
          dielectricResult: R("dielectricResult") || "",
        ...
        console.log(
          "🔍 MetalEnclosedBuswayMTS23Report - Mapped formData:",
          newFormData,
        );
          ...
          <div>
            ...
            <ul className="list-disc pl-5 text-sm text-blue-600 dark:text-blue-300">
              {suggestedReports.map((r) => (
                <li key={r.id}>
                  <a
                    href={`/jobs/${jobId}/metal-enclosed-busway-mts23/${r.id}`}
                    className="underline"
                  >
                    {r.id}
                  </a>
        ...
        <div className="max-w-7xl w-full space-y-6">
          ...
          {searchParams.get("debug") === "true" && (
            <section className="mb-4">
              <h2 className="text-sm font-semibold mb-2 text-neutral-900 dark:text-white">
                Debug: Raw Loaded Row
              </h2>
              <pre className="p-3 bg-neutral-100 dark:bg-dark-150 rounded text-xs overflow-auto max-h-64 text-neutral-900 dark:text-white">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    readOnly={!isEditing}
                    className={`block w-full px-3 py-2 border border-neutral-300 dark:border-neutral-700 rounded-none text-neutral-900 dark:text-white text-sm focus:border-brand focus:ring-1 focus:ring-brand ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                  />
                </div>
              </div>
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-1">
                    <label className="text-xs font-medium text-neutral-700 dark:text-white">
                      Temp.
                    </label>
                    <input
                      type="number"
                      ...
                      onChange={handleInputChange}
                      readOnly={!isEditing}
                      className={`w-12 px-1 py-1 text-right border border-neutral-300 dark:border-neutral-700 rounded text-neutral-900 dark:text-white text-xs focus:border-brand focus:ring-1 focus:ring-brand ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : "bg-white dark:bg-dark-150"}`}
                    />
                  ...
                  <div className="flex items-center space-x-1">
                    <input
                      ...
                      readOnly
                      className="w-10 px-1 py-1 text-right border border-neutral-300 dark:border-neutral-700 rounded bg-neutral-50 dark:bg-dark-150 text-neutral-900 dark:text-white text-xs"
                    />
                    <span className="text-xs text-neutral-500 dark:text-white">
                      °C
                    </span>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    readOnly={!isEditing}
                    className={`mt-1 block w-full p-2 text-center rounded-none border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-150 dark:text-white ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-neutral-700 dark:text-white">
                      ...
                      <td className="px-3 py-2 text-sm text-neutral-900 dark:text-white whitespace-normal break-words">
                        {(() => {
                          switch (id) {
                            ...
                            case "7.4.A.3":
                              return "Inspect bolted electrical connections for high resistance.";
                            case "7.4.A.4":
                              return "Confirm physical orientation in accordance with manufacturer's labels to ensure adequate cooling.";
                            case "7.4.A.5":
                              return "Verify weep or drain plugs are in accordance with manufacturer's published data.";
                            case "7.4.A.6":
                              return "Verify correct installation of joint shield.";
                            case "7.4.A.7":
                              return "Inspect and clean ventilating openings.";
                            default:
                              return "";
                          }
                        })()}
                      ...
                      <td className="px-3 py-2 whitespace-nowrap">
                        <div className="print:hidden">
                          <select
                            ...
                            disabled={!isEditing}
                            className={`block w-full p-2 text-center rounded-none border border-neutral-300 dark:border-neutral-700 shadow-sm focus:border-brand focus:ring-brand dark:bg-dark-150 dark:text-white ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                          >
          ...
          <section className="mb-6">
            ...
            <div className="flex justify-end mb-2 space-x-4">
              <div className="w-48 border border-neutral-300 dark:border-neutral-700 p-2">
                <label className="block text-sm font-medium text-neutral-700 dark:text-white">
                  Test Voltage:
                </label>
                <select
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700 ir-table">
                ...
                <thead>
                  <tr>
                    <th
                      colSpan={10}
                      className="px-3 py-2 bg-neutral-50 dark:bg-dark-150 text-center text-xs font-medium text-neutral-500 dark:text-white uppercase tracking-wider border border-neutral-300 dark:border-neutral-700"
                    >
                      Insulation Resistance
                    </th>
                    <th className="px-3 py-2 bg-neutral-50 dark:bg-dark-150 text-center text-xs font-medium text-neutral-500 dark:text-white uppercase tracking-wider ir-units border border-neutral-300 dark:border-neutral-700">
                      Units
                    </th>
                  </tr>
                  <tr>
                    <th className="px-3 py-2 bg-neutral-50 dark:bg-dark-150 text-center text-xs font-medium text-neutral-500 dark:text-white uppercase border border-neutral-300 dark:border-neutral-700">
                      A-B
                    </th>
                    <th className="px-3 py-2 bg-neutral-50 dark:bg-dark-150 text-center text-xs font-medium text-neutral-500 dark:text-white uppercase border border-neutral-300 dark:border-neutral-700">
                      B-C
                    ...
                    <th className="px-3 py-2 bg-neutral-50 dark:bg-dark-150 text-center text-xs font-medium text-neutral-500 dark:text-white uppercase border border-neutral-300 dark:border-neutral-700"></th>
                  </tr>
                </thead>
                <tbody className="bg-white dark:bg-dark-150 divide-y divide-neutral-200 dark:divide-neutral-700">
                  <tr>
                    {[
                      ...
                      <td key={key} className="px-2 py-1 border border-neutral-300 dark:border-neutral-700">
                        <input
                          type="text"
                          name={key}
                    ...
                    <td className="px-3 py-2 text-center text-sm text-neutral-900 dark:text-white ir-units border border-neutral-300 dark:border-neutral-700">
            ...
            <div className="flex justify-end mb-2">
              <div className="w-48 border border-neutral-300 dark:border-neutral-700 p-2">
                ...
                <input
                  ...
                  value={formData.testVoltage1}
                  readOnly
                  className="mt-1 block w-full rounded-none border border-neutral-300 dark:border-neutral-700 bg-neutral-50 dark:bg-dark-150 shadow-sm dark:text-white"
                />
              </div>
            </div>
          ...
          {/* Length-based limit and overall result (MTS 23: 100 / length, in MΩ) */}
          <section className="mb-6">
            <div className="flex flex-wrap items-end gap-6 border border-neutral-300 dark:border-neutral-700 p-3">
              <div>
                <label className="block text-sm font-medium text-neutral-700 dark:text-white">
                  Length (ft)
                </label>
                <input
                  type="text"
                  name="buswayLength"
                  value={formData.buswayLength}
                  onChange={handleInputChange}
                  readOnly={!isEditing}
                  className={`mt-1 block w-32 rounded-none border border-neutral-300 dark:border-neutral-700 shadow-sm dark:bg-dark-150 dark:text-white ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-neutral-700 dark:text-white">
                ...
                <input
                  type="text"
                  readOnly
                  value={buswayCriteria}
                  className="mt-1 block w-40 rounded-none border border-neutral-300 dark:border-neutral-700 bg-neutral-50 dark:bg-dark-150 shadow-sm dark:text-white"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-neutral-700 dark:text-white">
                ...
                <select
                  name="insulationOverallResult"
                  value={formData.insulationOverallResult}
                  onChange={handleInputChange}
                  disabled={!isEditing}
                  className={`mt-1 block w-40 rounded-none border border-neutral-300 dark:border-neutral-700 shadow-sm dark:bg-dark-150 dark:text-white ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                >
                  {["", "N/A", "PASS", "FAIL"].map((o) => (
          ...
          <section className="mb-6">
            ...
            <div className="flex flex-wrap gap-4 mb-3">
              <div>
                ...
                <select
                  name="dielectricTestVoltage"
                  value={formData.dielectricTestVoltage}
                  onChange={handleInputChange}
                  disabled={!isEditing}
                  className={`mt-1 block w-40 rounded-none border border-neutral-300 dark:border-neutral-700 shadow-sm dark:bg-dark-150 dark:text-white ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                >
                  {DIELECTRIC_TEST_VOLTAGES.map((v) => (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full border-collapse border border-neutral-300 dark:border-neutral-700">
                <thead className="bg-neutral-50 dark:bg-dark-150">
                  <tr>
                    {["A-G", "B-G", "C-G", "Units", "Results"].map((h) => (
                      <th
                        key={h}
                        className="px-3 py-2 text-center text-xs font-medium text-neutral-500 dark:text-white uppercase tracking-wider border border-neutral-300 dark:border-neutral-700"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody>
                  <tr>
                    {(["dielectricPhaseA", "dielectricPhaseB", "dielectricPhaseC"] as const).map(
                      (name) => (
                        <td
                          key={name}
                        ...
                        >
                          <input
                            ...
                            onChange={handleInputChange}
                            readOnly={!isEditing}
                            className={`block w-full rounded-none border border-neutral-300 dark:border-neutral-700 shadow-sm text-center dark:bg-dark-150 dark:text-white ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                          />
                        </td>
                      ),
                    )}
                    <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-700">
                      <select
                        name="dielectricWithstandUnit"
                        value={formData.dielectricWithstandUnit}
                        onChange={handleInputChange}
                        disabled={!isEditing}
                        className={`block w-full rounded-none border border-neutral-300 dark:border-neutral-700 shadow-sm dark:bg-dark-150 dark:text-white ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                      >
                        {["μA", "mA"].map((u) => (
                    ...
                    <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-700">
                      <select
                        name="dielectricResult"
                        value={formData.dielectricResult}
                        onChange={handleInputChange}
                        disabled={!isEditing}
                        className={`block w-full rounded-none border border-neutral-300 dark:border-neutral-700 shadow-sm dark:bg-dark-150 dark:text-white ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                      >
          ...
          <section className="mb-6">
            ...
            <div className="hidden print:block">
              <table className="w-full table-fixed border-collapse border border-neutral-300 print:border-black te-print">
                <colgroup>
                  <col style={{ width: "30%" }} />
                  <col style={{ width: "25%" }} />
                  <col style={{ width: "20%" }} />
...
export default MetalEnclosedBuswayMTS23Report;
