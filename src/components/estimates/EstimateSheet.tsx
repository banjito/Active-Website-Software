...
export default function EstimateSheet({ opportunityId, mode, openSignal }: EstimateSheetProps) {
  ...
  const calculateDefaultLaborHours = (data: any) => {
    ...
    // Calculate days onsite from total labor hours (SOV + non-SOV)
    const daysOnsite = men > 0 && hoursPerDay > 0 
      ? totalLaborHours / (men * hoursPerDay) 
      : 0;
  ...
  const loadQuoteData = (quote: QuoteData) => {
    try {
      ...
      const completeData = {
        ...
        // Restore the manual labor hours flag if it was saved
  ...
  async function saveQuote() {
    ...
    const generateQuoteVersion = () => {
     ...
     const dataWithEmbeddedTravel = { 
       ...
       isManualLaborHours: isManualLaborHours, // Persist manual labor hours flag
  ...
  useEffect(() => {
    if (isLetterProposalOpen && letterHtml && letterEditorRef.current) {
      ...
      setTimeout(() => {
        ...
        // Bind scope quantity controls to update header label only (do not change pricing or table values)
        try {
          const qtyInputs = Array.from(editor.querySelectorAll('input.scope-qty')) as HTMLInputElement[];
          qtyInputs.forEach((inp) => {
            if ((inp as any)._boundQty) return;
            (inp as any)._boundQty = true;
            // Prevent initial autofocus/scroll jump when clicking into the field
            inp.addEventListener('mousedown', (e) => {
              // Keep the editor from moving caret to the end on first click
              e.stopPropagation();
            });
            inp.addEventListener('focus', (e) => {
              try {
                e.stopPropagation();
                // Do not force editor focus or selection; keep viewport stable
              } catch {}
            });
            const updateQty = () => {
              const qty = Math.max(1, Math.floor(Number(inp.value) || 1));
              const block = inp.closest('.amp-scope-block') || editor;
              const label = block.querySelector('.scope-qty-label') as HTMLElement | null;
              if (label) label.textContent = String(qty);
              // Persist current input value into the DOM attribute so innerHTML reflects the change on save
              try { inp.setAttribute('value', String(qty)); } catch {}

              // Recompute grand total pricing (sum of per-scope base prices * quantities)
              try {
                const nf = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2, maximumFractionDigits: 2 });
                const kinds = ['mobilization','net30','net60','net90'] as const;
                kinds.forEach((kind) => {
                  // Sum over all scope blocks
                  let sum = 0;
                  const blocks = Array.from(editor.querySelectorAll('.amp-scope-block')) as HTMLElement[];
                  blocks.forEach((b) => {
                    const qInp = b.querySelector('input.scope-qty') as HTMLInputElement | null;
                    const q = Math.max(1, Math.floor(Number(qInp?.value || '1') || 1));
                    const prices = Array.from(b.querySelectorAll(`.scope-price[data-kind="${kind}"]`)) as HTMLElement[];
                    prices.forEach((p) => {
                      const base = Number((p.getAttribute('data-base') || '0')) || 0;
                      sum += q * base;
                    });
                  });
                  // Update grand price display if present
                  const targets = Array.from(editor.querySelectorAll(`.grand-price[data-kind="${kind}"]`)) as HTMLElement[];
                  targets.forEach((t) => {
                    t.textContent = nf.format(sum);
                    t.setAttribute('data-base', String(sum));
                  });
                });
              } catch {}
            };
            inp.addEventListener('input', updateQty);
    ...
    setLetterHtml(`
      <div id="letter-proposal" class="print-content" style="max-width: 800px; margin: 0 auto; font-family: Arial, sans-serif; position:relative; font-size: 11pt; line-height: 1.2;">
        ...
        <div class="amp-section amp-keep-with-next" style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
          <b>Scope</b>
          <div class="print-hidden" style="display:flex;align-items:center;gap:6px;">
            <label style="font-size:12px;color:#374151;">Scope Qty</label>
            <input type="number" min="1" value="1" class="scope-qty" style="width:64px;padding:4px;border:1px solid #d1d5db;border-radius:6px;" />
            <span class="scope-qty-tag" style="font-size:12px;color:#374151;">(<span class="scope-qty-label">1</span>)</span>
          </div>
        </div>
  ...
  function handleSelectQuotesForCombinedLetter() {
    ...
    const combinedOption1Raw = Math.ceil(combinedFinalValue * paymentTermFactors.net30);
    const combinedOption2Raw = Math.ceil(combinedFinalValue * paymentTermFactors.net60);
    const combinedOption3Raw = Math.ceil(combinedFinalValue * paymentTermFactors.net90);
    const combinedOption1 = formatCurrency(combinedOption1Raw);
    const combinedOption2 = formatCurrency(combinedOption2Raw);
    const combinedOption3 = formatCurrency(combinedOption3Raw);
    ...
    const sovTablesHtml = processedQuotes.map((processedQuote, index) => {
      ...
      // Calculate individual pricing for this scope (raw numbers + formatted)
      ...
      const scopeOption1Raw = Math.ceil(processedQuote.finalValue * paymentTermFactors.net30);
      const scopeOption2Raw = Math.ceil(processedQuote.finalValue * paymentTermFactors.net60);
      const scopeOption3Raw = Math.ceil(processedQuote.finalValue * paymentTermFactors.net90);
      const scopeMobilization = formatCurrency(scopeMobilizationRaw);
      const scopeOption1 = formatCurrency(scopeOption1Raw);
      const scopeOption2 = formatCurrency(scopeOption2Raw);
      const scopeOption3 = formatCurrency(scopeOption3Raw);
      ...
      return `
        <div class="amp-scope-block" style="margin-bottom:12px;border:1px solid #f0c8b3;border-left:4px solid #f26722;border-radius:8px;padding:10px;background:#fff7f2;">
          ...
          <div class="amp-section amp-keep-with-next" style="display:flex;align-items:center;justify-content:space-between;gap:12px;background:#fff0e6;padding:6px 8px;border-radius:6px;margin-bottom:6px;">
            <b>${headingText}</b>
            <div class="print-hidden" style="display:flex;align-items:center;gap:6px;">
              <label style="font-size:14px;color:#111827;">Scope Qty</label>
              <input type="number" min="1" value="1" class="scope-qty" style="width:64px;padding:4px;border:1px solid #d1d5db;border-radius:6px;" />
              <span class="scope-qty-tag" style="font-size:14px;color:#111827;">(<span class="scope-qty-label">1</span>)</span>
            </div>
          </div>
        <table class="amp-section" style='width:100%;border-collapse:collapse;margin-bottom:16px;'>
            <thead>
        ...
        <div class="amp-section">Mobilization costs of <b class="scope-price" data-base="${scopeMobilizationRaw}" data-kind="mobilization">${scopeMobilization}</b> shall be paid before the first day of work in addition to:</div>
        <ul class="amp-section" style="margin: 4px 0;">
          <li>Option 1: Where NET 30 Terms are applicable and agreed upon: <b class="scope-price" data-base="${scopeOption1Raw}" data-kind="net30">${scopeOption1}</b></li>
          <li>Option 2: Where NET 60 Terms are applicable and agreed upon: <b class="scope-price" data-base="${scopeOption2Raw}" data-kind="net60">${scopeOption2}</b></li>
          <li>Option 3: Where NET 90 Terms are applicable and agreed upon: <b class="scope-price" data-base="${scopeOption3Raw}" data-kind="net90">${scopeOption3}</b></li>
        </ul>
        </div>
      `;
    }).join('');
    ...
    setLetterHtml(`
      <div id="letter-proposal" class="print-content" style="max-width: 800px; margin: 0 auto; font-family: Arial, sans-serif; position:relative; font-size: 11pt; line-height: 1.2;">
        ...
        <div class="amp-scope-block" style="margin-bottom:12px;border:1px solid #f0c8b3;border-left:4px solid #f26722;border-radius:8px;padding:10px;background:#fff7f2;">
          ...
          <div class="amp-section" style="margin:4px 0;">Mobilization costs of <b class="grand-price" data-kind="mobilization" data-base="${combinedMobilization.replace(/[^0-9.]/g,'')}">${combinedMobilization}</b> shall be paid before the first day of work in addition to:</div>
          <ul class="amp-section" style="margin: 4px 0 4px 16px;">
            <li>Option 1: Where NET 30 Terms are applicable and agreed upon: <b class="grand-price" data-kind="net30" data-base="${combinedOption1Raw}">${combinedOption1}</b></li>
            <li>Option 2: Where NET 60 Terms are applicable and agreed upon: <b class="grand-price" data-kind="net60" data-base="${combinedOption2Raw}">${combinedOption2}</b></li>
            <li>Option 3: Where NET 90 Terms are applicable and agreed upon: <b class="grand-price" data-kind="net90" data-base="${combinedOption3Raw}">${combinedOption3}</b></li>
          </ul>
        </div>
  ...
  function handlePrintLetter() {
    ...
    if (printWindow) {
      ...
      const bodyHtml = currentContent || letterHtml || '';
      if (!bodyHtml) {
                  ...
                  <div style={{...styles.summarySection, width: '700px', marginTop: '20px'}}>
                    ...
                    <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '15px', padding: '10px', backgroundColor: '#f8f9fa', borderRadius: '4px', border: '1px solid #dee2e6'}}>
                      <div style={{fontWeight: 'bold', color: '#495057'}}>
                        Hours Quoted: {data.hoursSummary.workHours + data.hoursSummary.nonSovHours}
                      </div>
                      <div style={{display: 'flex', alignItems: 'center', gap: '10px'}}>
                        <div style={{fontWeight: 'bold', color: (() => {
                          const quotedHours = data.hoursSummary.workHours + data.hoursSummary.nonSovHours;
      ...
      <Dialog open={isLetterProposalOpen} onClose={() => {
        ...
        <div className="relative z-50 bg-white w-full h-full max-w-5xl mx-auto my-8 rounded-lg shadow-lg flex flex-col">
              ...
              <Button onClick={async () => {
                try {
                  ...
                  try {
                    ...
                    let grand = container.querySelector('.grand-price[data-kind="net30"]') as HTMLElement | null;
                    let grandBaseAttr = grand?.getAttribute('data-base');
                    if (grandBaseAttr) {
                  ...
                  const updateOpportunityFromLetter = async (proposalId: string | null, net30: number) => {
                    try {
                      // Read current quoted_amount to avoid overwriting manual edits
                      ...
                      try {
                        const { data: oppRow } = await supabase
                          ...
                          .eq('id', opportunityId)
                          .maybeSingle();
                        currentQuoted = (oppRow?.quoted_amount as number) ?? null;
                      } catch {}

                      const payload: any = { selected_letter_proposal: proposalId };
                      // Only set quoted_amount if empty/zero to preserve manual edits
                      if (net30 && net30 > 0 && (!currentQuoted || Number(currentQuoted) === 0)) {
          ...
          <div className="flex items-center gap-3">
            <label htmlFor="letter-name" className="text-sm font-medium text-gray-700">
              Save by name (optional):
            </label>
            <input
              id="letter-name"
              type="text"
