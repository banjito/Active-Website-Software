...
const GFITripTestReport: React.FC = () => {
  const { id: jobId, reportId, substation: urlSubstation } = useParams<{ id: string; reportId?: string; substation?: string }>();
  ...
  console.log('[GFIReport] Component mounted with params:', { jobId, reportId, urlSubstation, fromApproval: searchParams.get('fromApproval') });
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      setLoading(true);
      console.log('[GFIReport] Fetching from', TABLE_NAME, 'with id:', reportId);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      navigateAfterSave(navigate, jobId, returnToAssets);
    } catch (error: any) {
  ...
  return (
    <ReportWrapper isPrintMode={isPrintMode}>
      ...
      {isPrintMode && (
        <div className="bg-yellow-100 border border-yellow-500 p-2 text-xs mb-4" style={{ display: 'none' }}>
      ...
      <div className="p-6 flex justify-center">
        <div className="max-w-7xl w-full space-y-6">
          ...
          <div className={`${isPrintMode ? 'hidden' : ''} print:hidden flex justify-between items-center mb-6`}>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Ground Fault Trip Test Report</h1>
          ...
          <section className="mb-6">
            <div className="w-full h-1 bg-[#f26722] mb-4" style={{ width: '100%', height: '4px', backgroundColor: '#f26722', marginBottom: '1rem', WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}></div>
            <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2 print:text-black print:border-black print:font-bold">Job Information</h2>
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 ${isPrintMode ? 'hidden' : ''} print:hidden`}>
              <div>
            ...
            <div className={`${isPrintMode ? 'block' : 'hidden print:block'} job-info-print`}>
                ...
                <tbody>
                  <tr className="allow-row-break">
                    <td className="p-3 align-middle text-center border border-gray-300 print:border-black print:border">
                      ...
                      <div className="mt-1 whitespace-pre-wrap break-words text-center">{formData.customer || ''}</div>
                    </td>
                    <td className="p-3 align-middle text-center border border-gray-300 print:border-black print:border">
                      <div className="font-semibold text-center">Job #:</div>
                      <div className="mt-1 whitespace-pre-wrap break-words text-center">{formData.jobNumber || ''}</div>
                    </td>
                    <td className="p-3 align-middle text-center border border-gray-300 print:border-black print:border">
                      <div className="font-semibold text-center">Technicians:</div>
                      <div className="mt-1 whitespace-pre-wrap break-words text-center">{formData.technicians || ''}</div>
                    </td>
                    <td className="p-3 align-middle text-center border border-gray-300 print:border-black print:border">
                      <div className="font-semibold text-center">Date:</div>
                      <div className="mt-1 whitespace-pre-wrap break-words text-center">{formData.date ? new Date(formData.date + 'T00:00:00').toLocaleDateString() : ''}</div>
                    </td>
                  </tr>
                  <tr>
                    <td className="p-3 align-middle text-center border border-gray-300 print:border-black print:border">
                      <div className="font-semibold text-center">Address:</div>
                      <div className="mt-1 whitespace-pre-wrap break-words text-center">{maskCustomerAddress(formData.address || '')}</div>
                    </td>
                    <td className="p-3 align-middle text-center border border-gray-300 print:border-black print:border">
                      <div className="font-semibold text-center">Identifier:</div>
                      <div className="mt-1 whitespace-pre-wrap break-words text-center">{formData.identifier || ''}</div>
                    </td>
                    <td className="p-3 align-middle text-center border border-gray-300 print:border-black print:border">
                      <div className="font-semibold text-center">Substation:</div>
                      <div className="mt-1 whitespace-pre-wrap break-words text-center">{formData.substation || ''}</div>
                    </td>
                    <td className="p-3 align-middle text-center border border-gray-300 print:border-black print:border">
                      <div className="font-semibold text-center">Eqpt. Location:</div>
                      <div className="mt-1 whitespace-pre-wrap break-words text-center">{formData.eqptLocation || ''}</div>
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <section className="mb-6">
            ...
            <div className={`grid grid-cols-1 md:grid-cols-4 gap-x-6 gap-y-4 ${isPrintMode ? 'hidden' : ''} print:hidden`}>
              <div>
                <label className="form-label inline-block w-24">Model:</label>
                <EquipmentAutocomplete
                  value={formData.testEquipment.model}
                  onChange={(value) => handleTestEquipmentChange('model', value)}
                  onSelect={(equipment) => {
                    ...
                    handleTestEquipmentChange('model', equipment.equipment_name);
              ...
              <div>
                <label className="form-label inline-block w-24">Serial #:</label>
                <input
                  type="text"
                  value={formData.testEquipment.serialNumber}
                  onChange={(e) => handleTestEquipmentChange('serialNumber', e.target.value)}
                  readOnly={!isEditing}
                  className="form-input"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleTestEquipmentChange('ampId', e.target.value)}
                  readOnly={!isEditing}
                  className="form-input"
                />
              </div>
              <div>
                <label className="form-label inline-block w-24">Cal Date:</label>
                <input
                  type="date"
                  value={formData.testEquipment.calDate}
                  onChange={(e) => handleTestEquipmentChange('calDate', e.target.value)}
                  readOnly={!isEditing}
                  className="form-input"
                />
              </div>
            ...
            <div className={`${isPrintMode ? 'block' : 'hidden print:block'}`}>
              <table className="w-full border-collapse border border-gray-300 print:border-black">
                ...
                <tbody>
                  <tr>
                    <td className="p-2 border border-gray-300 print:border-black">{formData.testEquipment.model || ''}</td>
                    <td className="p-2 border border-gray-300 print:border-black">{formData.testEquipment.serialNumber || ''}</td>
                    <td className="p-2 border border-gray-300 print:border-black">{formData.testEquipment.ampId || ''}</td>
                    <td className="p-2 border border-gray-300 print:border-black">{formData.testEquipment.calDate ? new Date(formData.testEquipment.calDate + 'T00:00:00').toLocaleDateString() : ''}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <section className="mb-6">
            <div className="w-full h-1 bg-[#f26722] mb-4" style={{ width: '100%', height: '4px', backgroundColor: '#f26722', marginBottom: '1rem', WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}></div>
            <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2 print:text-black print:border-black print:font-bold">Overview</h2>
            
            {/* Screen view - with input fields */}
            <div className={`overflow-x-auto ${isPrintMode ? 'hidden' : ''} print:hidden`}>
              <table className="min-w-full border-collapse border border-gray-200 dark:border-gray-700">
                <thead>
                  <tr>
                    <th className="border border-gray-200 dark:border-gray-700 p-2 bg-gray-50 dark:bg-dark-150 font-semibold text-left text-gray-900 dark:text-white" colSpan={2}>
                      Data
                    </th>
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="border border-gray-200 dark:border-gray-700 px-4 py-2 align-top" rowSpan={4}>
                      <textarea
                  ...
                  <tr>
                    ...
                    <td className="border border-gray-200 dark:border-gray-700 px-4 py-2">
                      <input
                        ...
                        onChange={(e) => handleChange('groundFaultSetting', e.target.value)}
                        readOnly={!isEditing}
                        className="form-input w-full"
                  ...
                  <tr>
                    ...
                    <td className="border border-gray-200 dark:border-gray-700 px-4 py-2">
                      <input
                        ...
                        onChange={(e) => handleChange('groundFaultTrip', e.target.value)}
                        readOnly={!isEditing}
                        className="form-input w-full"
                        placeholder="e.g., 750 A"
                      />
            ...
            <div className={`${isPrintMode ? 'block' : 'hidden print:block'} overview-print`}>
              <table className="w-full border-collapse border border-gray-300 print:border-black">
                <colgroup>
                  <col style={{ width: '22%' }} />
                  <col style={{ width: '28%' }} />
                ...
                <thead>
                  <tr>
                    <th className="p-2 border border-gray-300 print:border-black bg-gray-50 print:bg-gray-100 font-semibold text-left" colSpan={2}>
                      Data
                    </th>
                ...
                <tbody>
                  <tr className="allow-row-break">
                    ...
                    <td className="p-2 border border-gray-300 print:border-black align-top" rowSpan={4}>
                      <div
