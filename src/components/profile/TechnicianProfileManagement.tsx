...
import { TechnicianSkill, PortalType } from "@/lib/types/scheduling";
...
export function TechnicianProfileManagement({
  portalType,
  division,
}: TechnicianProfileManagementProps) {
  const { user } = useAuth();
  ...
  const [hasAdminPermission, setHasAdminPermission] = useState<boolean | null>(
    null,
  );
    ...
    const fetchTechnicians = async () => {
      ...
      try {
        ...
        if (userError) {
          ...
          if (userError2) {
            console.error(
              "Error fetching technicians with admin API:",
              userError2,
            );
            setError(
              "Failed to load technicians list. This may be due to missing permissions.",
            );
            return;
          }

          console.log(
            "Admin API returned users:",
            userData2?.users?.length || 0,
          );
          ...
          const techUsers = userData2.users.filter((u) => {
            ...
            const inDivision =
              !division || u.user_metadata?.division === division;
          ...
          const currentUserAsTech = techUsers.find(
          ...
          console.log(
            "Successfully fetched users from auth.users:",
            userData?.length || 0,
          );
          ...
          const filteredUsers = division
            ? formattedUsers.filter(
                (u) => u.user_metadata?.division === division,
              )
            : formattedUsers;
          ...
          // Set current user as selected if they're a technician
          const currentUserAsTech = filteredUsers.find(
            (tech) => tech.id === user?.id,
          );
          if (currentUserAsTech) {
            setSelectedTechnician(currentUserAsTech.id);
        ...
        setError(
          "An unexpected error occurred while loading technicians. Check browser console for details.",
        );
  ...
  useEffect(() => {
    const fetchSkills = async () => {
      ...
      setIsLoading(true);
      try {
        const { data, error } = await schedulingService.getTechnicianSkills(
          selectedTechnician,
          portalType,
        );
        if (error) {
  ...
  const getCertificationStatus = (
    ...
    status: "valid" | "approaching" | "expired";
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-amber-700 mt-1">
                    This component requires admin permissions to access the user
                    list. Your current access level doesn't allow managing
                    technicians. Contact your administrator for assistance.
                  </p>
                </div>
              </div>
            </div>
              ...
              <Tabs defaultValue="skills">
                ...
                <TabsContent value="skills">
                  ...
                  <div className="space-y-4">
                    {isLoading ? (
                      ...
                      <p>
                        No skills or certifications recorded for this
                        technician.
                      </p>
                    ) : (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="flex justify-between">
                                  <div>
                                    <div className="flex items-center">
                                      ...
                                      <Badge
                                        className={`ml-2 ${getProficiencyColor(skill.proficiency_level)}`}
                                      >
                                        {getProficiencyLabel(
                ...
                <TabsContent value="schedule">
                  <div className="p-4 bg-neutral-50 rounded-none text-center">
                    ...
                    <Button onClick={() => navigate(`/${division}/schedule`)}>
