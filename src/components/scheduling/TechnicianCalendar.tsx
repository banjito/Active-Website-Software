...
export function TechnicianCalendar({
  ...
  const [currentMonthLabel, setCurrentMonthLabel] = useState<string>(dayjs().format('MMMM YYYY'));
  const [currentDate, setCurrentDate] = useState<Date>(dayjs().toDate());
  const calendarRef = useRef<any>(null);
  const [monthPicker, setMonthPicker] = useState<string>(dayjs().format('YYYY-MM'));
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (o.selectedTechs && Array.isArray(o.selectedTechs)) setSelectedTechIds(o.selectedTechs);
      }
    } catch {}
  }, []);
  const saveMainFilters = () => localStorage.setItem('sched_main_filters_v1', JSON.stringify({ 
  ...
  const [users, setUsers] = useState<Array<{ id: string; label: string; email?: string }>>([]);
  ...
  const [removeScopeEvent, setRemoveScopeEvent] = useState<CalendarEvent | null>(null);

  // Determine if a job has additional future days beyond the selected event date
  const hasFutureJobDays = useCallback((jobId?: string, fromISO?: string) => {
    if (!jobId || !fromISO) return false;
    const fromDay = dayjs(fromISO).startOf('day');
    return events.some(ev => ev.job?.id === jobId && dayjs(ev.start).startOf('day').isAfter(fromDay));
  }, [events]);
    ...
    (async () => {
      try {
        ...
        if (!res.error && res.data && mounted) {
          const mapped = res.data.map((u: any) => ({ id: u.id, label: (u.raw_user_meta_data?.name ?? u.email) as string, email: u.email as string }));
          setUsers(mapped);
          return;
        }
      } catch {}
      ...
      if (mounted && technicians && technicians.length > 0) {
        setUsers(technicians.map(t => ({ id: t.user_id, label: t.full_name || t.email || t.user_id, email: t.email })));
      }
    })();
    ...
    (async () => {
      try {
        const { data } = await supabase
          ...
          .limit(500);
        if (mounted && data) {
          setJobs(data.map((j: any) => ({ id: j.id, label: `${j.job_number ?? ''} ${j.title ?? ''}`.trim() })));
        }
      } catch {}
    })();
    return () => { mounted = false; };
  ...
  useEffect(() => {
      ...
      try {
        try {
          ...
          if (assignmentsError) {
            ...
            const techMap = new Map<string, { name?: string; email?: string }>();
            technicians.forEach(t => techMap.set(t.user_id, { name: t.full_name || undefined, email: t.email || undefined }));
            const userMap = new Map<string, { name?: string; email?: string }>();
            users.forEach(u => userMap.set(u.id, { name: u.label, email: u.email || undefined }));
    ...
    fetchCalendarData();
  }, [startDate, endDate, filteredTechnician, portalType, division, users, technicians, reloadKey, jobColors]);
  ...
  const handleSelectRange = (info: any) => {
    ...
    const end = (info.allDay ? endExclusive.subtract(1, 'day') : endExclusive).format('YYYY-MM-DD');
    if (onDateClick) {
  ...
  const handleRemoveEvent = async (eventId: string) => {
    try {
      if (eventId.startsWith('assignment-')) {
        ...
        const { error } = await schedulingService.deleteTechnicianAssignment(id);
  ...
  const handleRemoveGroupAll = async (ev: CalendarEvent) => {
    try {
      const ids = (ev.technicians || []).map(t => t.assignment_id).filter(Boolean) as string[];
      await Promise.all(ids.map(id => schedulingService.deleteTechnicianAssignment(id)));
  ...
  const handleRemoveGroupOne = async (assignmentId?: string) => {
    ...
    try {
      ...
      setGroupEvent(prev => {
        if (!prev) return prev;
        const remaining = (prev.technicians || []).filter(t => t.assignment_id !== assignmentId);
  ...
  const handleAddTechsToGroup = async () => {
    ...
    const assigned = new Set((groupEvent.technicians || []).map(t => String(t.id)));
    const toAdd = gmSelectedTechnicians.filter(id => !assigned.has(String(id)));
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to add technicians');
    }
  };
  ...
  const isAmpEmail = (email?: string | null) => !!email && /@ampqes\.com$/i.test(String(email));
  ...
  const baseTechOpts: SelectOption[] = technicians
    ...
    .map(tech => ({ value: tech.user_id, label: formatDisplayName(tech.full_name || undefined, tech.email) }));
  ...
  const allowedIds = new Set<string>([
    ...technicians.filter(t => isAmpEmail(t.email) && isNotLabTech(t)).map(t => String(t.user_id)),
  ...
  const technicianOptionsAll: SelectOption[] = [{ value: '', label: 'All Technicians' }, ...mergedTechOpts];
  const technicianOptions: SelectOption[] = technicianOptionsAll.filter(o =>
    !techSearch || String(o.label).toLowerCase().includes(techSearch.toLowerCase())
  );
  const jobOptions: SelectOption[] = [
    { value: '', label: 'All Jobs' },
    ...jobs.map(j => ({ value: j.id, label: j.label }))
  ];
  const jobOptionsFiltered: SelectOption[] = jobOptions.filter(o =>
    !jobSearchMain || String(o.label).toLowerCase().includes(jobSearchMain.toLowerCase())
  );

  // After users/jobs are loaded, backfill search boxes from saved IDs if names missing
  useEffect(() => {
    if (mainFilterTechId && !techSearch) {
      const found = technicianOptionsAll.find(o => String(o.value) === String(mainFilterTechId));
      if (found) setTechSearch(String(found.label));
    }
  }, [technicianOptionsAll, mainFilterTechId]);
  useEffect(() => {
    if (mainFilterJobId && !jobSearchMain) {
      const found = jobOptions.find(o => String(o.value) === String(mainFilterJobId));
  ...
  useEffect(() => {
    try {
      if (qaForm.jobId || qaJobSearch) {
        localStorage.setItem('sched_qa_last_job', JSON.stringify({ id: qaForm.jobId, label: qaJobSearch }));
      }
    } catch {}
  ...
  const handleQuickAssignSave = async () => {
    try {
      const techIds = qaSelectedTechnicians.length ? qaSelectedTechnicians : (qaForm.technicianId ? [qaForm.technicianId] : []);
      ...
      const endTimeToUse = qaForm.isAllDay ? '23:59' : (qaForm.unknownHours ? qaForm.startTime : qaForm.endTime);
      ...
      await Promise.all(payloads.map(p => schedulingService.saveTechnicianAssignment(p)));
  ...
  return (
    ...
    <Card className="overflow-hidden">
      <CardHeader className="p-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-2 pr-3 mr-2 border-r dark:border-gray-700">
            <Button variant="outline" size="sm" onClick={() => { calendarRef.current?.getApi().prev(); }}>
              Prev
            </Button>
            <Button variant="outline" size="sm" onClick={() => { calendarRef.current?.getApi().today(); setCurrentDate(dayjs().toDate()); }}>
              Today
            </Button>
            <Button variant="outline" size="sm" onClick={() => { calendarRef.current?.getApi().next(); }}>
              Next
            </Button>
            <span className="ml-1 text-sm font-medium text-gray-800 dark:text-white">{currentMonthLabel}</span>
            <input
              type="month"
              value={monthPicker}
              onChange={(e) => {
              ...
              className="ml-2 border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-sm bg-white dark:bg-dark-150 text-gray-900 dark:text-white"
            />
          </div>
          <div className="relative">
            <button
              ...
              className="w-[280px] px-3 py-2 text-left text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-dark-150 text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-dark-200 flex items-center justify-between"
            >
              <span>
            ...
            {isTechFocused && (
              <>
                ...
                />
                <div className="absolute z-50 mt-1 w-full max-h-80 overflow-auto border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-dark-150 shadow-lg">
                <div className="sticky top-0 bg-white dark:bg-dark-150 p-2 border-b border-gray-200 dark:border-gray-700">
                  <Input
                  ...
                  <div className="flex gap-2 mt-2">
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        const allTechIds = mergedTechOpts.map(opt => String(opt.value)).filter(id => id);
                ...
                {technicianOptions.map(opt => {
                  ...
                  return (
                    <div
                      key={techId}
                      className="flex items-center gap-2 px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-dark-200 cursor-pointer"
                      onClick={(e) => {
                        e.stopPropagation();
                        handleToggle();
                      }}
                    >
                      <input
                        type="checkbox"
                        checked={techId === '' ? selectedTechIds.length === 0 : isSelected}
                        onChange={(e) => {
                          e.stopPropagation();
                          handleToggle();
                        }}
                ...
                {technicianOptions.length === 0 && (
                  <div className="px-3 py-2 text-sm text-gray-500">No technicians</div>
                )}
                </div>
              </>
            )}
          ...
          <div className="relative">
            ...
            />
            {isJobFocused && (
              <div className="absolute z-50 mt-1 w-full max-h-56 overflow-auto border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-dark-150 shadow">
                {jobOptionsFiltered.map(opt => (
                  <button
                    key={String(opt.value)}
                    type="button"
                    onMouseDown={() => { setMainFilterJobId(String(opt.value)); setJobSearchMain(String(opt.label)); setIsJobFocused(false); }}
                    className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-dark-200"
                  >
                    {String(opt.label)}
                  </button>
                ))}
                {jobOptionsFiltered.length === 0 && (
                  <div className="px-3 py-2 text-sm text-gray-500">No jobs</div>
                )}
              </div>
            )}
          </div>
          <Button variant="outline" size="sm" onClick={clearMainFilters}>Clear</Button>
          <Button variant="outline" size="sm" onClick={saveMainFilters}>Save</Button>
          {(selectedTechIds.length > 0 || mainFilterJobId) && (
            <div className="ml-2 flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
              <span>Active filters:</span>
              {selectedTechIds.length > 0 && (
                <span className="px-2 py-1 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                  {selectedTechIds.length} Technician{selectedTechIds.length > 1 ? 's' : ''}
                </span>
              )}
              {mainFilterJobId && (
                <span className="px-2 py-1 rounded-full bg-gray-100 dark:bg-dark-200">
              ...
              <FullCalendar
                ...
                initialDate={currentDate}
                  events={events.filter(ev => {
                    ...
                    const matchTech = selectedTechIds.length === 0 || 
                      selectedTechIds.includes(String(ev?.technician?.id)) || 
                      (ev as any)?.technicians?.some((t: any) => selectedTechIds.includes(String(t.id)));
                    const matchJob = !mainFilterJobId || ev?.job?.id === mainFilterJobId;
                ...
                eventContent={(arg) => {
                  const isAllDay = arg.event.allDay || arg.event.extendedProps?.allDay;
                  const timeDisplay = isAllDay ? 'All Day' : arg.timeText;
                  const techs: Array<{ name?: string }> | undefined = (arg.event.extendedProps as any)?.technicians;
                  
                  if (techs && techs.length) {
                    const allNames = techs.map(t => t.name || '').filter(Boolean);
                    ...
                    return (
                      <div className="fc-event-custom leading-tight w-full overflow-hidden">
                        <div className="font-medium text-[12px] truncate max-w-full overflow-hidden">{arg.event.title}</div>
                        <div className="text-[11px] opacity-80 truncate max-w-full whitespace-nowrap overflow-hidden">{timeDisplay}</div>
                        <div className="text-[11px] opacity-80 truncate max-w-full whitespace-nowrap overflow-hidden">{namesLine}</div>
                      </div>
                    ) as any;
                  }
    ...
    <Dialog open={showDaySchedule} onOpenChange={setShowDaySchedule}>
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          <div className="max-h-60 overflow-auto border rounded-md dark:border-gray-700">
            {events.filter(ev => {
              const d = dayjs(ev.start).format('YYYY-MM-DD');
              return d >= dayScheduleStart && d <= dayScheduleEnd;
            }).length === 0 ? (
              <div className="p-3 text-sm text-gray-500">No events.</div>
            ) : (
              <ul className="divide-y dark:divide-gray-700">
                {events.filter(ev => {
                  ...
                  <li key={ev.id} className="p-2 flex items-center justify-between">
                    <div className="text-sm">
                      <div className="font-medium text-gray-900 dark:text-white">{ev.title}</div>
                      <div className="text-gray-600 dark:text-gray-300">
                        {dayjs(ev.start).format('HH:mm')} - {ev.end ? dayjs(ev.end).format('HH:mm') : '—'}
                        {(ev as any)?.technicians?.length
                          ? ` • ${(ev as any).technicians.map((t: any) => t.name).filter(Boolean).join(', ')}`
                          : ev.technician?.name ? ` • ${ev.technician.name}` : ''}
                      </div>
                    </div>
                    {(((ev.id.startsWith('assignment-') && !ev.id.startsWith('assignment-group-')))
                      || ev.id.startsWith('exception-')) && (
                      <Button variant="outline" size="sm" onClick={() => handleRemoveEvent(ev.id)}>Remove</Button>
                    )}
                    {ev.id.startsWith('assignment-group-') && (
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" onClick={() => { setGroupEvent(ev); setShowGroupManage(true); }}>Manage</Button>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => {
                            const shouldPrompt = hasFutureJobDays(ev.job?.id, ev.start);
                            if (shouldPrompt) {
              ...
              <Button
                onClick={() => {
                  ...
                  setQaForm(prev => ({
                    ...
                    technicianId: filteredTechnician || mainFilterTechId || prev.technicianId,
                  }));
                  const defaultTech = (filteredTechnician || mainFilterTechId);
                  setQaSelectedTechnicians((prev) => (defaultTech && !prev.length ? [defaultTech] : prev));
                }}
              >
                Assign Technician
              </Button>
    ...
    <Dialog open={showQuickAssign} onOpenChange={setShowQuickAssign}>
      <DialogContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white mb-1">Technicians</label>
            <input
              type="text"
              placeholder="Search technicians..."
              value={qaTechSearch}
              onChange={(e) => setQaTechSearch(e.target.value)}
              className="mb-2 w-full p-2 border rounded-md dark:bg-dark-150 dark:border-gray-700"
            />
            <div className="max-h-48 overflow-auto border rounded-md dark:border-gray-700">
              {mergedTechOpts
                .filter(o => !qaTechSearch || String(o.label).toLowerCase().includes(qaTechSearch.toLowerCase()))
                .map(opt => (
                  <label key={String(opt.value)} className="flex items-center gap-2 px-3 py-2 text-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-dark-200">
                    <input
                      type="checkbox"
                      checked={qaSelectedTechnicians.includes(String(opt.value))}
                      onChange={(e) => {
                        const id = String(opt.value);
                        setQaSelectedTechnicians(prev => e.target.checked ? [...prev, id] : prev.filter(t => t !== id));
                      }}
                    />
                    <span>{String(opt.label)}</span>
                  </label>
                ))}
              {mergedTechOpts.filter(o => !qaTechSearch || String(o.label).toLowerCase().includes(qaTechSearch.toLowerCase())).length === 0 && (
                <div className="px-3 py-2 text-sm text-gray-500">No matching technicians</div>
              )}
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white mb-1">Job</label>
            <input
              type="text"
            ...
            {qaJobOpen && (
              <div className="max-h-48 overflow-auto border rounded-md dark:border-gray-700">
                {[...jobs]
                  .filter(j => !qaJobSearch || String(j.label).toLowerCase().includes(qaJobSearch.toLowerCase()))
                  .map(j => (
                    <button
                      key={j.id}
                      type="button"
                      className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-dark-200 ${qaForm.jobId === j.id ? 'bg-gray-100 dark:bg-dark-200' : ''}`}
                      onMouseDown={() => { setQaForm(prev => ({ ...prev, jobId: j.id })); setQaJobSearch(j.label); setQaJobOpen(false); }}
                    >
                      {j.label}
                    </button>
                  ))}
                {[...jobs].filter(j => !qaJobSearch || String(j.label).toLowerCase().includes(qaJobSearch.toLowerCase())).length === 0 && (
                  <div className="px-3 py-2 text-sm text-gray-500">No matching jobs</div>
                )}
              </div>
            )}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white mb-1">Start Date</label>
            <input
              type="date"
              value={qaForm.startDate}
              onChange={(e) => setQaForm(prev => ({ ...prev, startDate: e.target.value }))}
              className="w-full p-2 border rounded-md dark:bg-dark-150 dark:border-gray-700"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white mb-1">End Date</label>
            <input
              type="date"
              value={qaForm.endDate}
              onChange={(e) => setQaForm(prev => ({ ...prev, endDate: e.target.value }))}
              className="w-full p-2 border rounded-md dark:bg-dark-150 dark:border-gray-700"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white mb-1">Job Color</label>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {['#ef4444', '#f97316', '#eab308', '#22c55e', '#3b82f6', '#8b5cf6', '#ec4899', '#64748b'].map(color => (
                  <button
                    ...
                    className={`w-8 h-8 rounded border-2 ${qaForm.color === color ? 'border-gray-900 dark:border-white' : 'border-gray-300 dark:border-gray-600'}`}
                ...
                <input
                  type="color"
                  value={qaForm.color}
                  onChange={(e) => setQaForm(prev => ({ ...prev, color: e.target.value }))}
                  className="h-8 w-16 border rounded cursor-pointer dark:bg-dark-150 dark:border-gray-700"
                  title="Custom color"
                />
              </div>
              <span className="text-xs text-gray-600 dark:text-gray-400">
                {qaForm.jobId && jobColors[qaForm.jobId] ? 'Using saved color for this job' : 'New color will be saved for this job'}
              </span>
            </div>
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => setQaForm(prev => ({ ...prev, startTime: e.target.value }))}
              className="w-full p-2 border rounded-md dark:bg-dark-150 dark:border-gray-700"
              disabled={qaForm.isAllDay}
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white mb-1">End Time</label>
            <input
              type="time"
              value={qaForm.endTime}
              onChange={(e) => setQaForm(prev => ({ ...prev, endTime: e.target.value }))}
              disabled={qaForm.unknownHours || qaForm.isAllDay}
              className="w-full p-2 border rounded-md dark:bg-dark-150 dark:border-gray-700 disabled:opacity-60"
            />
            <label className="mt-2 inline-flex items-center gap-2 text-sm text-gray-700 dark:text-white">
              <input
                type="checkbox"
                checked={qaForm.unknownHours}
                onChange={(e) => setQaForm(prev => ({ ...prev, unknownHours: e.target.checked }))}
                disabled={qaForm.isAllDay}
              />
              Unknown hours
            </label>
            <label className="mt-2 ml-4 inline-flex items-center gap-2 text-sm text-gray-700 dark:text-white">
              <input
                type="checkbox"
                checked={qaForm.isAllDay}
                onChange={(e) => setQaForm(prev => ({ ...prev, isAllDay: e.target.checked, unknownHours: e.target.checked ? false : prev.unknownHours }))}
              />
              All Day
            </label>
          </div>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button onClick={handleQuickAssignSave} disabled={(qaSelectedTechnicians.length === 0 && !qaForm.technicianId) || !qaForm.jobId || !qaForm.startDate}>
            Save Assignment
          </Button>
        </div>
      </DialogContent>
    ...
    <Dialog open={showRemoveScope} onOpenChange={setShowRemoveScope}>
      <DialogContent>
        ...
        <div className="space-y-3">
          <p className="text-sm text-gray-700 dark:text-white">Choose how much to remove for: <span className="font-medium">{removeScopeEvent?.title}</span></p>
          <div className="flex flex-col gap-2">
            ...
            <Button
              onClick={async () => {
                ...
                const fromDate = dayjs(removeScopeEvent.start).format('YYYY-MM-DD');
    ...
    <Dialog open={showGroupManage} onOpenChange={setShowGroupManage}>
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          <ul className="divide-y dark:divide-gray-700 max-h-60 overflow-auto">
            {(groupEvent?.technicians || []).map(t => (
              <li key={t.assignment_id || t.id} className="p-2 flex items-center justify-between">
                <div className="text-sm text-gray-800 dark:text-white">
                  {t.name || t.id}
                </div>
                <Button variant="outline" size="sm" onClick={() => handleRemoveGroupOne(t.assignment_id)}>
                  Remove
                </Button>
              </li>
            ))}
            {(groupEvent?.technicians || []).length === 0 && (
              <div className="p-3 text-sm text-gray-500">No assignments remaining.</div>
            )}
          </ul>
          {/* Add technicians to this job/day */}
          <div className="pt-2 border-t dark:border-gray-700">
            <div className="mb-2 text-sm font-medium text-gray-800 dark:text-white">Add Technicians</div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-white mb-1">Technicians</label>
                <input
                  ...
                  className="mb-2 w-full p-2 border rounded-md dark:bg-dark-150 dark:border-gray-700"
                />
                <div className="max-h-40 overflow-auto border rounded-md dark:border-gray-700">
                  {mergedTechOpts
                    .filter(o => !gmTechSearch || String(o.label).toLowerCase().includes(gmTechSearch.toLowerCase()))
                    .filter(o => !(groupEvent?.technicians || []).some(t => String(t.id) === String(o.value)))
                    .map(opt => (
                      <label key={String(opt.value)} className="flex items-center gap-2 px-3 py-2 text-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-dark-200">
                        <input
                          type="checkbox"
                          checked={gmSelectedTechnicians.includes(String(opt.value))}
                          onChange={(e) => {
                            const id = String(opt.value);
                            setGmSelectedTechnicians(prev => e.target.checked ? [...prev, id] : prev.filter(t => t !== id));
                          }}
                        />
                        <span>{String(opt.label)}</span>
                      </label>
                  ...
                  {mergedTechOpts
                    .filter(o => !gmTechSearch || String(o.label).toLowerCase().includes(gmTechSearch.toLowerCase()))
                    .filter(o => !(groupEvent?.technicians || []).some(t => String(t.id) === String(o.value))).length === 0 && (
                    <div className="px-3 py-2 text-sm text-gray-500">No matching technicians</div>
                  )}
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-white mb-1">Times</label>
            ...
            <div className="mt-3 flex justify-end">
              <Button onClick={handleAddTechsToGroup} disabled={gmSelectedTechnicians.length === 0}>Add</Button>
            </div>
          </div>
          {(groupEvent?.technicians || []).length > 0 && (
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setShowGroupManage(false)}>Close</Button>
              <Button onClick={() => groupEvent && handleRemoveGroupAll(groupEvent)}>Remove All</Button>
            </div>
          )}
        </div>
      </DialogContent>
