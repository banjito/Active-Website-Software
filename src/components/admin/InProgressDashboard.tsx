...
const InProgressDashboard: React.FC = () => {
  ...
  const [showOppModal, setShowOppModal] = useState<{
    open: boolean;
    id: string | null;
  }>({ open: false, id: null });
      ...
      try {
        ...
        try {
          ...
          if (jobs && jobs.length > 0) {
            ...
            const opportunityFields = Object.keys(sampleJob).filter(
              (key) =>
            ...
            console.log(
              "Fields that might contain opportunity data:",
              opportunityFields,
            );
            ...
            if (opportunityIds.length > 0) {
              ...
              if (opportunities && opportunities.length > 0) {
                ...
                console.log("Letter proposals query result:", {
                  proposals,
                  proposalsError,
                });
              }
              ...
              if (opportunities && !opportunitiesError) {
                ...
                if (letterProposals) {
                  ...
                  Object.values(byOpp).forEach((proposalsForOpp: any[]) => {
                    ...
                    console.log("Raw proposal data:", {
                      ...
                      dataKeys: proposal.data
                        ? Object.keys(proposal.data)
                        : "no data",
                    });
                    ...
                    if (proposal.data) {
                      ...
                      if (!quotedAmount && proposal.html) {
                        try {
                          ...
                          if (m && m[1]) {
                            quotedAmount =
                              Number(m[1].replace(/,/g, "")) || null;
                            console.log(
                              "Parsed NET30 from HTML:",
                              quotedAmount,
                            );
                          }
                        } catch (e) {
                      ...
                      if (!quotedAmount && proposal.data.calculatedValues) {
                        quotedAmount =
                          proposal.data.calculatedValues.net30Price ||
                          proposal.data.calculatedValues.totalPrice ||
                          proposal.data.calculatedValues.finalPrice;
                        console.log(
                          "Found calculatedValues:",
                          proposal.data.calculatedValues,
                        );
                      }
                      ...
                      if (!quotedAmount && proposal.data.hoursSummary) {
                        quotedAmount =
                          proposal.data.hoursSummary.totalCost ||
                          proposal.data.hoursSummary.finalCost;
                        console.log(
                          "Found hoursSummary:",
                          proposal.data.hoursSummary,
                        );
                      }

                      console.log(
                        "Final quoted amount from proposal:",
                        quotedAmount,
                      );
                      if (quotedAmount) {
                ...
                opportunities.forEach((opp) => {
                  ...
                  opportunityMap.set(opp.id, {
                    id: opp.id,
                    quoted_amount:
                      proposalQuotedAmount || opp.quoted_amount || null,
                ...
                console.log(
                  ...
                  Array.from(opportunityMap.entries()),
                );

                // Attach opportunity data to jobs
                jobs = jobs.map((job) => {
                  const opportunityData = job.opportunity_id
                    ? opportunityMap.get(job.opportunity_id) || null
                    : null;
                  console.log(
                    `Job ${job.id} (${job.title}) -> Opportunity ${job.opportunity_id} -> Data:`,
                    opportunityData,
                  );
                ...
                console.log(
                  "No opportunities found or error:",
                  opportunitiesError,
                );
              }
            }
        ...
        if (!jobs || jobs.length === 0) {
          console.log(
            "No jobs found with current filters, trying broader query...",
          );
      ...
      jobs: region.jobs.filter((job) => {
        ...
        if (job.created_at) {
          ...
          if (timeframe === "custom") {
            ...
            switch (timeframe) {
              case "quarterly":
                // Show 4 months per quarter (includes last month of previous quarter for context)
                // Q1: Dec (prev year) – Mar, Q2: Mar – Jun, Q3: Jun – Sep, Q4: Sep – Dec
                const currentYear = now.getFullYear();
                if (selectedQuarter === 1) {
                  startDate = new Date(currentYear - 1, 11, 1); // Dec 1 of previous year
                  ...
                  startDate = new Date(currentYear, 8, 1); // Sep 1
                  endDate = new Date(currentYear, 11, 31); // Dec 31
                }
                break;
              case "6months":
                startDate = new Date(now);
                startDate.setMonth(now.getMonth() - 5); // Go back 5 months so with current month = 6 total
        ...
        if (statusFilter !== "all") {
          ...
          switch (statusFilter) {
            case "active":
              if (!["in_progress", "planning", "pending"].includes(jobStatus))
                return false;
              break;
            case "billed":
              if (jobStatus !== "billed" && jobStatus !== "invoiced")
                return false;
              break;
            case "to_be_billed":
              if (jobStatus !== "completed" && jobStatus !== "ready_to_bill")
                return false;
              break;
            case "pending":
            ...
            case "completed":
              ...
              break;
            case "on_hold":
              if (jobStatus !== "on_hold" && jobStatus !== "on-hold")
                return false;
              break;
          }
        }
        ...
        if (searchQuery.trim()) {
          ...
          const matchesCustomer =
            job.customers?.company_name?.toLowerCase().includes(query) ||
  ...
  const getTotalStats = () => {
    return filteredData.reduce(
      (acc, region) => ({
        totalQuotedValue:
          acc.totalQuotedValue +
          region.jobs.reduce(
            (sum, job) => sum + (job.opportunities?.quoted_amount ?? 0),
            0,
          ),
        totalBudget:
          acc.totalBudget +
          region.jobs.reduce((sum, job) => sum + (job.budget ?? 0), 0),
      ...
      <div className="max-w-7xl mx-auto space-y-6">
              ...
              <div className="flex items-center gap-3 mt-2">
                <p className="text-neutral-600 dark:text-neutral-400">
                  {timeframe === "quarterly" &&
                    (() => {
                      ...
                      if (selectedQuarter === 2) return "Q2: Apr 1 – Jun 30";
                      if (selectedQuarter === 3) return "Q3: Jul 1 – Sep 30";
                      return "Q4: Oct 1 – Dec 31";
                    })()}
                ...
                {timeframe === "quarterly" && (
                  <div className="flex gap-1">
                    <Button
                      variant={selectedQuarter === 1 ? "primary" : "outline"}
                      size="sm"
                    ...
                    <Button
                      variant={selectedQuarter === 2 ? "primary" : "outline"}
                      size="sm"
                    ...
                    <Button
                      variant={selectedQuarter === 3 ? "primary" : "outline"}
                      size="sm"
                    ...
                    <Button
                      variant={selectedQuarter === 4 ? "primary" : "outline"}
                      size="sm"
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-4 items-center">
                  ...
                  <div className="flex gap-2">
                    <Button
                      variant={
                        timeframe === "quarterly" ? "primary" : "outline"
                      }
                      size="sm"
                      onClick={() => {
                  ...
                  {showCustomDatePicker && (
                    <div className="flex items-center gap-2 ml-4">
                      <input
                        ...
                        className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 rounded-none bg-white dark:bg-neutral-800 text-neutral-900 dark:text-white text-sm"
                      />
                ...
                <div className="flex flex-wrap gap-4 items-center border-t dark:border-neutral-700 pt-4">
                  ...
                  <select
                    ...
                    className="px-4 py-2 border border-neutral-300 dark:border-neutral-600 rounded-none bg-white dark:bg-neutral-800 text-neutral-900 dark:text-white text-sm font-medium"
                  >
                    <option value="all">All Statuses</option>
                    <option value="active">
                      Active (In Progress/Planning/Pending)
                    </option>
                    <option value="to_be_billed">
                      To Be Billed (Completed/Ready)
                    </option>
                  ...
                  <div className="flex items-center gap-2 ml-auto">
                    ...
                    <input
                      ...
                      className="px-4 py-2 border border-neutral-300 dark:border-neutral-600 rounded-none bg-white dark:bg-neutral-800 text-neutral-900 dark:text-white text-sm w-64"
                    />
                  </div>
                </div>
              </div>
          ...
          <Card className="border-0 shadow-sm hover:shadow-md transition-shadow">
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                ...
                <div className="bg-green-500/10 p-3 rounded-none">
          ...
          <Card className="border-0 shadow-sm hover:shadow-md transition-shadow">
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                ...
                <div className="bg-purple-500/10 p-3 rounded-none">
                  ...
                  <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-2">
                    {statusFilter === "all"
                      ...
                      : statusFilter === "active"
                        ...
                        : statusFilter === "billed"
                          ? "Billed jobs"
                          : statusFilter === "to_be_billed"
                            ? "Ready to bill"
                            : statusFilter.replace("_", " ")}
                  </p>
                </div>
                <div className="bg-blue-500/10 p-3 rounded-none">
              ...
              >
                <LineChart
                  data={(() => {
                    ...
                    // Determine the date range based on the timeframe selection
                    const now = new Date();
                    let rangeStart: Date;
                    ...
                    if (
                      ...
                      const currentYear = now.getFullYear();
                      if (selectedQuarter === 1) {
                        rangeStart = new Date(currentYear - 1, 11, 1); // Dec 1 of previous year
                        rangeEnd = new Date(currentYear, 2, 31); // Mar 31
                      } else if (selectedQuarter === 2) {
                        rangeStart = new Date(currentYear, 2, 1); // Mar 1
                        rangeEnd = new Date(currentYear, 5, 30); // Jun 30
                      } else if (selectedQuarter === 3) {
                        rangeStart = new Date(currentYear, 5, 1); // Jun 1
                        rangeEnd = new Date(currentYear, 8, 30); // Sep 30
                      } else {
                        rangeStart = new Date(currentYear, 8, 1); // Sep 1
                        rangeEnd = new Date(currentYear, 11, 31); // Dec 31
                      }
                    } else if (timeframe === "6months") {
                      rangeStart = new Date(now);
                      rangeStart.setMonth(now.getMonth() - 5); // Go back 5 months so with current month = 6 total
                      rangeStart.setDate(1); // Start from 1st of that month
                    } else {
                      // yearly
                      rangeStart = new Date(now.getFullYear(), 0, 1);
                    }

                    // Initialize all months in the selected range with 0 revenue
                    ...
                    while (tempDate <= rangeEnd) {
                      ...
                      const monthLabel = tempDate.toLocaleDateString("en-US", {
                        month: "short",
                        year: "numeric",
                      });
                    ...
                    // Add revenue from filtered jobs
                    filteredData.forEach((region) => {
                      region.jobs.forEach((job) => {
                        if (job.start_date) {
                          ...
                          existing.revenue +=
                            job.opportunities?.quoted_amount ?? 0;
                        }
                      });
                    });

                    // Sort by date and return all months in the selected timeframe
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <div
                            className={`inline-block px-3 py-1 rounded-none text-xs font-medium ${getJobStatusColor(job.status)}`}
                          >
            ...
            <div className="flex flex-wrap gap-4 items-center">
              <div className="flex items-center gap-2">
                ...
                <select
                  value={selectedRegion}
                  onChange={(e) => setSelectedRegion(e.target.value)}
                  className="px-3 py-2 border border-neutral-300 dark:border-neutral-700 rounded-none bg-white dark:bg-neutral-800 text-neutral-900 dark:text-white text-sm"
                >
                  <option value="all">All Divisions</option>
                  {regionData.map((region) => (
                    <option key={region.region} value={region.region}>
              ...
              <div className="flex items-center gap-2 ml-auto">
                ...
                <input
                  ...
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="px-3 py-2 border border-neutral-300 dark:border-neutral-700 rounded-none bg-white dark:bg-neutral-800 text-neutral-900 dark:text-white text-sm w-64"
                />
              </div>
            </div>
          </CardContent>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="bg-blue-500/10 p-2 rounded-none">
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <div
                              className={`inline-block px-3 py-1 rounded-none text-xs font-medium ${getPriorityColor(job.priority)}`}
                            >
                              {formatStatusLabel(job.priority)}
                            </div>
                            {job.job_number && (
                              <div className="inline-block px-3 py-1 rounded-none text-xs font-medium bg-neutral-100 text-neutral-800 dark:bg-neutral-700 dark:text-neutral-300">
                                #{job.job_number}
                              </div>
                            )}
                          </div>
                        ...
                        <div className="text-right">
                          {(() => {
                            const quotedAmount =
                              job.opportunities?.quoted_amount ?? 0;
                            const usedSource = job.opportunities?.quoted_amount
                              ? "quoted_amount"
                              : "fallback_0";
                            return (
                              <>
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  setModalError(
                                    e?.message || "Failed to load opportunity",
                                  );
                                } finally {
        ...
        {showOppModal.open && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-neutral-900 rounded-none shadow-xl w-full max-w-5xl p-0 overflow-hidden">
              <div className="flex items-center justify-between mb-2">
                <h3 className="sr-only">Opportunity</h3>
                <button
                  className="m-2 px-3 py-1.5 text-sm text-neutral-600 dark:text-neutral-300 border border-neutral-300 dark:border-neutral-700 rounded-none hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors"
        ...
        {showJobModal.open && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-neutral-900 rounded-none shadow-xl w-full max-w-6xl p-0 overflow-hidden">
              <div className="flex items-center justify-between mb-2">
