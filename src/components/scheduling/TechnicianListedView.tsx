...
export const TechnicianListedView: React.FC<Props> = ({ portalType, division, days = 21, onAssignmentClick }) => {
  ...
  useEffect(() => {
    const updateAtMidnight = () => {
      const msUntilMidnight = dayjs().endOf('day').add(1, 'millisecond').diff(dayjs());
      const t = setTimeout(() => setStartDate(dayjs().format('YYYY-MM-DD')), msUntilMidnight);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const { data } = await schedulingService.getAvailableTechnicians(portalType, division);
        if (mounted) setTechnicians((data || []).reduce((acc: AvailableTechnician[], t) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const { data } = await schedulingService.getTechnicianAssignments(undefined, portalType, start, end, division);
  ...
  const techIdToName = useMemo(() => {
    ...
    technicians.forEach((t: any) => {
      if (t.email) {
        const candidate = typeof t.full_name === 'string' ? t.full_name : (t.full_name?.raw_user_meta_data?.name || t.full_name?.name);
    ...
    assignments.forEach(a => {
      if (a.user?.email) {
        const metaName = a.user?.user_metadata?.name || (a.user as any)?.raw_user_meta_data?.name;
        m.set(a.user_id, formatDisplayName(metaName, a.user.email));
        console.log('[TechnicianListedView] Set name for', a.user_id, ':', formatDisplayName(metaName, a.user.email), 'from email:', a.user.email);
      } else {
        console.warn('[TechnicianListedView] Assignment missing email for user:', a.user_id, a.user);
      }
    });
    return m;
  ...
  const displayTechs = useMemo(() => {
    ...
    technicians.filter(t => isAmp(t.email) && isNotLabTech(t)).forEach(t => map.set(t.user_id, t));
    assignments.forEach(a => {
      const assignmentPortalType = a.portal_type || a.division;
      if (!map.has(a.user_id) && isAmp(a.user?.email || '') && assignmentPortalType !== 'lab') {
        map.set(a.user_id, {
          user_id: a.user_id,
          full_name: a.user?.user_metadata?.name || (a.user as any)?.raw_user_meta_data?.name || a.user?.email,
          email: a.user?.email || '',
          division,
  ...
  const filteredTechs = useMemo(() => {
    ...
    if (q) {
      result = result.filter(t => (t.full_name || t.email || '').toLowerCase().includes(q));
    }
    
    return result;
  }, [displayTechs, techFilter, selectedTechIds]);

  const goPrev = () => setStartDate(dayjs(startDate).subtract(days, 'day').format('YYYY-MM-DD'));
  const goNext = () => setStartDate(dayjs(startDate).add(days, 'day').format('YYYY-MM-DD'));
      ...
      <CardContent>
          ...
          <div className="flex gap-2 items-center">
            <div className="relative w-64">
              <button
                ...
                className="w-full px-3 py-2 text-left text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-dark-150 text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-dark-200 flex items-center justify-between"
              >
                <span>
              ...
              {isTechFilterOpen && (
                <>
                  ...
                  {displayTechs
                    .filter(t => !techFilter || (t.full_name || t.email || '').toLowerCase().includes(techFilter.toLowerCase()))
                    .map(tech => {
                      const isSelected = selectedTechIds.includes(tech.user_id);
                      const displayName = formatDisplayName(tech.full_name, tech.email);
        ...
        <div className="overflow-auto">
          <div className="min-w-[900px]">
            {/* Header row */}
            <div className="grid" style={{ gridTemplateColumns: `240px repeat(${dateList.length}, minmax(120px, 1fr))` }}>
              <div className="sticky left-0 z-10 bg-white dark:bg-dark-150 border border-gray-200 dark:border-gray-700 px-3 py-2 font-semibold">Technician</div>
              {dateList.map((d) => (
                <div key={d} className="border border-gray-200 dark:border-gray-700 text-center px-2 py-2 text-sm bg-gray-50 dark:bg-dark-200">
            ...
            {loading ? (
              ...
              filteredTechs.map((t) => {
                ...
                const name = techIdToName.get(techId) || t.full_name || t.email || techId;
                const rowMap = assignmentsByTechByDate.get(techId);
                return (
                  <div key={techId} className="grid" style={{ gridTemplateColumns: `240px repeat(${dateList.length}, minmax(120px, 1fr))` }}>
                    <div className="sticky left-0 z-10 bg-white dark:bg-dark-150 border border-gray-200 dark:border-gray-700 px-3 py-2 text-sm truncate">{name}</div>
                    {dateList.map((d) => {
                      const list = rowMap?.get(d) || [];
                      return (
                        <div 
                          key={d} 
                          className="border border-gray-200 dark:border-gray-700 px-2 py-1 min-h-[44px] cursor-pointer hover:bg-gray-50 dark:hover:bg-dark-100"
                          onClick={() => {
                            setQaSelectedTechnicians([techId]);
                            setQaForm((prev) => ({ ...prev, startDate: d, endDate: d }));
                            setShowQuickAssign(true);
                          }}
                          title={`Click to add assignment for ${dayjs(d).format('MMM D')}`}
                        >
                          {list.length === 0 ? (
                            <div className="w-full h-full text-xs text-gray-400">
                              —
                            </div>
                          ) : (
                            <div className="flex flex-col gap-1">
                              {list.sort((a, b) => a.start_time.localeCompare(b.start_time)).map((a) => {
                                const isAllDay = (a.start_time?.startsWith('00:00') && (a.end_time?.startsWith('23:59') || a.end_time?.startsWith('24:00')));
                                const isUnknown = a.start_time?.slice(0,5) === a.end_time?.slice(0,5);
                                const timeLabel = isAllDay ? 'All Day' : (isUnknown ? 'Unknown' : `${a.start_time?.slice(0,5)}-${a.end_time?.slice(0,5)}`);
                                const jobColor = a.job_id ? jobColors[a.job_id] : null;
                                ...
                                return (
                                  <div 
                                    ...
                                    className={`text-[11px] px-2 py-1 rounded border overflow-hidden whitespace-nowrap text-ellipsis cursor-pointer hover:opacity-80 transition-opacity ${!jobColor ? 'border-gray-200 dark:border-gray-600 bg-white dark:bg-dark-150' : ''}`}
                                    style={colorStyles}
                                    onClick={(e) => {
                                      e.stopPropagation();
                                  ...
                                  >
                                    <span className="font-medium">{a.job?.job_number || ''}</span> {maskJobTitle(a.job?.title) || ''}
    ...
    <Dialog open={showQuickAssign} onOpenChange={setShowQuickAssign}>
      <DialogContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div className="max-h-48 overflow-auto border rounded-md dark:border-gray-700">
              {displayTechs
                .filter(t => !qaTechSearch || (formatDisplayName(t.full_name, t.email)).toLowerCase().includes(qaTechSearch.toLowerCase()))
                .map(tech => (
                  <label key={tech.user_id} className="flex items-center gap-2 px-3 py-2 text-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-dark-200">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span>{formatDisplayName(tech.full_name, tech.email)}</span>
                  </label>
                ))}
              {displayTechs.filter(t => !qaTechSearch || (formatDisplayName(t.full_name, t.email)).toLowerCase().includes(qaTechSearch.toLowerCase())).length === 0 && (
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <Button
            onClick={async () => {
              if (!qaForm.jobId || !qaForm.startDate || qaSelectedTechnicians.length === 0) return;
              ...
              try {
                ...
                await Promise.all(payloads.map(p => schedulingService.saveTechnicianAssignment(p as any)));
            ...
            disabled={!qaForm.jobId || !qaForm.startDate || qaSelectedTechnicians.length === 0}
          >
            Save Assignment
          </Button>
        </div>
