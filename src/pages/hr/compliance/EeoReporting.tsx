...
import {
  eeoComplianceService,
  EeoAggregation,
  EeoPipelineSummary,
} from "../../../services/hr/eeoComplianceService";
...
export const EeoReporting: React.FC = () => {
  ...
  const [overallSummary, setOverallSummary] = useState<Awaited<
    ReturnType<typeof eeoComplianceService.getOverallSummary>
  > | null>(null);
  ...
  useEffect(() => {
    eeoComplianceService
      .getPipelineBreakdown(filterPosition)
      .then(setPipeline)
      .catch(() => {});
  }, [filterPosition]);
  ...
  const positionOptions = [
    ...
    ...aggregations.map((a) => ({
      value: a.position_title,
      label: a.position_title,
    })),
  ];
  ...
  const displaySummary =
    filterPosition === "all"
      ...
      : (() => {
          ...
          if (!agg) return null;
          const pct = (n: number) =>
            agg.total > 0 ? Math.round((n / agg.total) * 100) : 0;
          return {
            total: agg.total,
            genderBreakdown: agg.genderBreakdown,
            genderPercent: Object.fromEntries(
              Object.entries(agg.genderBreakdown).map(([k, v]) => [k, pct(v)]),
            ),
            raceBreakdown: agg.raceBreakdown,
            racePercent: Object.fromEntries(
              Object.entries(agg.raceBreakdown).map(([k, v]) => [k, pct(v)]),
            ),
  ...
  const handleExport = async () => {
    try {
      ...
      toast({
        title: "Exported",
        description: "Anonymized EEO compliance CSV downloaded.",
      });
    } catch (e: any) {
      toast({
        title: "Error",
        description: e?.message || "Export failed",
        variant: "destructive",
      });
    }
  };
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  unique positions with EEO data
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {displaySummary.veteranCount}
                </div>
                <p className="text-xs text-muted-foreground">
                  {displaySummary.veteranPercent}% of applicants
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {displaySummary.disabilityCount}
                </div>
                <p className="text-xs text-muted-foreground">
                  {displaySummary.disabilityPercent}% of applicants
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          {pipeline.length > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Demographics by hiring stage — see how representation changes
                  from application through hire
                </CardDescription>
              </CardHeader>
              <CardContent>
                        ...
                        {stage.total > 0 ? (
                          <div className="space-y-3">
                            <div>
                              ...
                              <div className="space-y-1">
                                {Object.entries(stage.genderBreakdown)
                                  .sort((a, b) => b[1] - a[1])
                                  .map(([label, count]) => (
                                    <div
                                      key={label}
                            ...
                            <div>
                              ...
                              <div className="space-y-1">
                                {Object.entries(stage.raceBreakdown)
                                  .sort((a, b) => b[1] - a[1])
                                  .map(([label, count]) => (
                                    <div
                                      key={label}
                                    ...
                                    >
                                      ...
                                      <span className="font-medium">
                                        {count} ({stage.racePercent[label] || 0}
                                        %)
                                      </span>
                                    </div>
                                  ))}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Applicant self-identification for EEO-1 reporting
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3 max-w-xl">
          ...
          {filteredAggregations.length > 1 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Aggregated demographics per open requisition
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {filteredAggregations
                        ...
                        .map((agg) => {
                          const topGender = Object.entries(agg.genderBreakdown)
                            .sort((a, b) => b[1] - a[1])
                            .slice(0, 2);
                          const topRace = Object.entries(agg.raceBreakdown)
                            .sort((a, b) => b[1] - a[1])
                            .slice(0, 2);
                          const pct = (n: number) =>
                            agg.total > 0
                              ? Math.round((n / agg.total) * 100)
                              : 0;
                          return (
                            <tr
                              key={agg.position_title}
                            ...
                            >
                              ...
                              <td className="p-3 text-right">
                                {agg.veteranCount} ({pct(agg.veteranCount)}%)
                              </td>
                              <td className="p-3 text-right">
                                {agg.disabilityCount} (
                                {pct(agg.disabilityCount)}%)
                              </td>
                            </tr>
                          );
                        })}
