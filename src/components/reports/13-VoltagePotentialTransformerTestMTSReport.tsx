            ...
            {(Object.keys(formData.testEquipmentUsed) as Array<keyof FormData['testEquipmentUsed']>).map(equipmentKey => {
                ...
                return (
                    <div key={equipmentKey} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                        <div><label htmlFor={`${equipmentKey}Name`} className="form-label block">{label}:</label><input id={`${equipmentKey}Name`} type="text" value={equipment.name} onChange={(e) => handleChange(`testEquipmentUsed.${equipmentKey}.name`, e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
