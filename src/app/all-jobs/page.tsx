...
const DIVISION_BADGE_CLASS: Record<string, string> = {
  ...
  scavenger:
    "bg-violet-100 text-violet-800 dark:bg-violet-900/40 dark:text-violet-300 border border-violet-300 dark:border-violet-700",
};
...
export default function UnifiedJobsPage() {
      ...
      if (opportunityIds.length > 0) {
        try {
          ...
          if (opportunitiesData) {
            opportunitiesData.forEach((o: any) => {
              quoteNumberMap[o.id] = o.quote_number ?? null;
            });
          }
        } catch (_) {}
      }

      const jobsWithCustomers = jobData.map(
        (job: any) =>
          ({
            ...job,
            ...
            quoteNumber: job.opportunity_id
              ? (quoteNumberMap[job.opportunity_id] ?? null)
              : null,
          }) as JobRow,
      );
  ...
  const filteredJobs = useMemo(() => {
    ...
    return base.filter(
      (j) =>
        ...
        (normalized !== "" &&
          normalizeId(j.job_number || "").includes(normalized)) ||
        j.quoteNumber?.toLowerCase().includes(lower) ||
        (normalized !== "" &&
          normalizeId(j.quoteNumber || "").includes(normalized)) ||
        (j.status || "").toLowerCase().includes(lower) ||
        j.poNumbers?.some((po) => po.toLowerCase().includes(lower)) ||
        (normalized !== "" &&
          j.poNumbers?.some((po) => normalizeId(po).includes(normalized))),
    );
  }, [
    jobs,
    statusFilter,
  ...
  const statusTotals = useMemo(() => {
    ...
    jobs.forEach((j) => {
      contractValueLeftToBill += j.contractValue ?? 0;
    });
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap px-3 py-4 text-sm">
                            ...
                            job.contractValue !== 0 ? (
                              ...
                              >
                                {job.contractValue < 0 ? "-" : ""}$
                                {Math.abs(job.contractValue).toLocaleString(
                                  "en-US",
                                  {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
            ...
            {!loading && filteredJobs.length > 0 && (
              <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
                <p className="text-sm text-neutral-500 dark:text-neutral-400">
                  ...
                  {filteredJobs.length !== 1 ? "s" : ""} (50 per page)
                </p>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
