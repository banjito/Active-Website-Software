...
const LiquidFilledTransformerReport: React.FC = () => {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    insulationResistance: {
      // Default voltages match screenshot
      temperature: "",
      primaryToGround: {
        testVoltage: "5000V",
        unit: "MΩ",
  ...
  const getVisualInspectionDescription = (id: string): string => {
    ...
    const descriptions: { [key: string]: string } = {
      ...
      "7.2.2.A.12":
        "Inspect bolted electrical connections for high resistance using low-resistance ohmmeter, calibrated torquewrench, or thermographic survey.",
  ...
  const loadReport = async () => {
    ...
    try {
      console.log(
        `Loading report from ${LIQUID_TRANSFORMER_TABLE} with ID: ${reportId}`,
      );
      ...
      if (error) {
        if (error.code === "PGRST116") {
          console.warn(
            `Report with ID ${reportId} not found in ${LIQUID_TRANSFORMER_TABLE}.`,
          );
      ...
      if (data) {
        ...
        if (normalized) {
          setFormData((prev) => ({
            ...
            temperature:
              normalized?.reportInfo?.temperature ?? prev.temperature,
            nameplateData: {
              ...prev.nameplateData,
              ...
              indicatorGauges: (() => {
                const ig =
                  (normalized?.reportInfo?.nameplateData
                    ?.indicatorGauges as any) || {};
                return {
                  oilLevel:
                    normalized?.reportInfo?.oilLevel ?? ig.oilLevel ?? "",
                  tankPressure:
                    normalized?.reportInfo?.tankPressure ??
                    ig.tankPressure ??
                    "",
                  oilTemperature:
                    normalized?.reportInfo?.oilTemperature ??
                    ig.oilTemperature ??
                    "",
                  windingTemperature:
                    normalized?.reportInfo?.windingTemperature ??
                    ig.windingTemperature ??
                    "",
                  oilTempRange:
                    normalized?.reportInfo?.oilTempRange ??
                    ig.oilTempRange ??
                    "",
                  windingTempRange:
                    normalized?.reportInfo?.windingTempRange ??
                    ig.windingTempRange ??
                    "",
            ...
            visualInspection:
              normalized?.visualInspection ?? prev.visualInspection,
            insulationResistance:
              normalized?.insulationResistance ?? prev.insulationResistance,
      ...
      const err = error as SupabaseError;
      console.error(
        `Error loading report from ${LIQUID_TRANSFORMER_TABLE}:`,
        err,
      );
  ...
  const handleSave = async () => {
    ...
    const wasExistingReport = Boolean(currentReportId);
    ...
    try {
      ...
      if (savedReportId) {
        console.log(`Updating ${tableName} with ID: ${savedReportId}`);
        ...
        if (result.data?.id) {
          ...
          const assetUrl = `report:/jobs/${jobId}/liquid-filled-transformer/${savedReportId}`;
      ...
      console.log(
        `${tableName} saved/updated successfully. Result:`,
        result.data,
      );
      setJustSaved(true);
      if (!wasExistingReport) {
        setIsEditing(false);
        const newId =
          savedReportId || (result as any)?.data?.id || (result as any)?.id;
        if (newId) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (siteAddress) {
        setFormData((prev) => ({
          ...prev,
          address: maskCustomerAddress(siteAddress),
        }));
      }
    };
    fetchData();
          ...
          <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-x-4 gap-y-2 print:hidden job-info-onscreen">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={
                      (formData.nameplateData &&
                        (formData as any).nameplateData?.indicatorGauges
                          ?.humidity) ||
                      ""
                    }
                    onChange={(e) => {
                  ...
                  <tbody>
                    <tr>
                      <td className="p-2 align-top border border-neutral-300 print:border-black">
                        ...
                        <div className="mt-0">
                          {formData.nameplateData.manufacturer || ""}
                        </div>
                      </td>
                      <td className="p-2 align-top border border-neutral-300 print:border-black">
                        <div className="font-semibold">Catalog Number</div>
                        <div className="mt-0">
                          {formData.nameplateData.catalogNumber || ""}
                        </div>
                      </td>
                      <td className="p-2 align-top border border-neutral-300 print:border-black">
                        <div className="font-semibold">Serial Number</div>
                        <div className="mt-0">
                          {formData.nameplateData.serialNumber || ""}
                        </div>
                      </td>
                      <td className="p-2 align-top border border-neutral-300 print:border-black">
                        <div className="font-semibold">KVA</div>
                        <div className="mt-0">
                          {formData.nameplateData.kva || ""}
                        </div>
                      </td>
                    </tr>
                    <tr>
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 text-sm text-neutral-900 dark:text-white text-center">
                        {formData.nameplateData.primary.volts || ""} /{" "}
                        {formData.nameplateData.primary.voltsSecondary || ""}
                      </td>
                      <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 text-sm text-neutral-900 dark:text-white text-center">
                        {formData.nameplateData.primary.connection === "Delta"
                          ? "☒"
                          : "☐"}{" "}
                        Delta
                      </td>
                      <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 text-sm text-neutral-900 dark:text-white text-center">
                        {formData.nameplateData.primary.connection === "Wye"
                          ? "☒"
                          : "☐"}{" "}
                        Wye
                      </td>
                      <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 text-sm text-neutral-900 dark:text-white text-center">
                        {formData.nameplateData.primary.connection ===
                        "Single Phase"
                          ? "☒"
                          : "☐"}{" "}
                        Single Phase
                      ...
                      <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 text-sm text-neutral-900 dark:text-white text-center">
                        {formData.nameplateData.primary.material === "Aluminum"
                          ? "☒"
                          : "☐"}{" "}
                        Aluminum
                      </td>
                      <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 text-sm text-neutral-900 dark:text-white text-center">
                        {formData.nameplateData.primary.material === "Copper"
                          ? "☒"
                          : "☐"}{" "}
                        Copper
                      </td>
                    ...
                    <tr>
                      ...
                      <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 text-sm text-neutral-900 dark:text-white text-center">
                        {formData.nameplateData.secondary.connection === "Delta"
                          ? "☒"
                          : "☐"}{" "}
                        Delta
                      </td>
                      <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 text-sm text-neutral-900 dark:text-white text-center">
                        {formData.nameplateData.secondary.connection === "Wye"
                          ? "☒"
                          : "☐"}{" "}
                        Wye
                      </td>
                      <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 text-sm text-neutral-900 dark:text-white text-center">
                        {formData.nameplateData.secondary.connection ===
                        "Single Phase"
                          ? "☒"
                          : "☐"}{" "}
                        Single Phase
                      ...
                      <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 text-sm text-neutral-900 dark:text-white text-center">
                        {formData.nameplateData.secondary.material ===
                        "Aluminum"
                          ? "☒"
                          : "☐"}{" "}
                        Aluminum
                      ...
                      <td className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 text-sm text-neutral-900 dark:text-white text-center">
                        {formData.nameplateData.secondary.material === "Copper"
                          ? "☒"
                          : "☐"}{" "}
                        Copper
                      </td>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 print:hidden indicator-gauges-onscreen">
                  <div>
                    ...
                    <input
                      type="text"
                      value={
                        formData.nameplateData?.indicatorGauges?.oilLevel || ""
                      }
                      onChange={(e) => {
                        const ig = {
                          ...(formData.nameplateData.indicatorGauges as any),
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={
                        formData.nameplateData?.indicatorGauges
                          ?.oilTemperature || ""
                      }
                      onChange={(e) => {
                        const ig = {
                          ...(formData.nameplateData.indicatorGauges as any),
                          oilTemperature: e.target.value,
                        };
                        handleNestedChange(
                          "nameplateData",
                          "indicatorGauges",
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={
                        formData.nameplateData?.indicatorGauges?.oilTempRange ||
                        ""
                      }
                      onChange={(e) => {
                        const ig = {
                          ...(formData.nameplateData.indicatorGauges as any),
                          oilTempRange: e.target.value,
                        };
                        handleNestedChange(
                          "nameplateData",
                          "indicatorGauges",
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={
                        formData.nameplateData?.indicatorGauges?.tankPressure ||
                        ""
                      }
                      onChange={(e) => {
                        const ig = {
                          ...(formData.nameplateData.indicatorGauges as any),
                          tankPressure: e.target.value,
                        };
                        handleNestedChange(
                          "nameplateData",
                          "indicatorGauges",
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const ig = {
                          ...(formData.nameplateData.indicatorGauges as any),
                          windingTempRange: e.target.value,
                        };
                        handleNestedChange(
                          "nameplateData",
                          "indicatorGauges",
            ...
            <section className="mb-6 visual-mechanical-inspection">
              ...
              <div className="overflow-x-auto">
                <table
                  className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700 visual-mechanical-table table-fixed"
                  style={{ width: "100%" }}
                >
                  <colgroup>
                  ...
                  <thead className="bg-neutral-50 dark:bg-dark-150">
                    <tr>
                      <th
                        style={{ width: "12%" }}
                        className="vmi-col-id px-3 py-2 text-left text-xs font-medium text-neutral-500 dark:text-white uppercase tracking-wider"
                      >
                        ID
                      </th>
                      <th
                        style={{ width: "58%" }}
                        className="vmi-col-desc px-3 py-2 text-left text-xs font-medium text-neutral-500 dark:text-white uppercase tracking-wider"
                      >
                        Description
                      </th>
                      <th
                        style={{ width: "15%" }}
                        className="vmi-col-result px-3 py-2 text-left text-xs font-medium text-neutral-500 dark:text-white uppercase tracking-wider"
                      >
                        Result
                      </th>
                      <th
                        style={{ width: "15%" }}
                        className="vmi-col-comments px-3 py-2 text-left text-xs font-medium text-neutral-500 dark:text-white uppercase tracking-wider"
                      >
                        Comments
                      </th>
                    </tr>
                    ...
                    {Object.keys(formData.visualInspection)
                      ...
                      .map((id) => (
                        <tr key={id}>
                          <td
                            style={{ width: "12%" }}
                            className="vmi-col-id px-3 py-2 whitespace-nowrap text-sm text-neutral-900 dark:text-white"
                          >
                            {id.replace("*", "")}
                          </td>
                          <td
                            style={{ width: "58%" }}
                            className="vmi-col-desc px-3 py-2 text-sm text-neutral-700 dark:text-white whitespace-normal break-words"
                          >
                            {getVisualInspectionDescription(id)}
                          </td>
                          <td
                            style={{ width: "15%" }}
                            className="vmi-col-result px-3 py-2"
                          >
                            ...
                            <div className="hidden print:block text-center">
                              {formData.visualInspection[id] || ""}
                            </div>
                          </td>
                          <td
                            style={{ width: "15%" }}
                            className="vmi-col-comments px-3 py-2"
                          >
                            <div className="print:hidden">
                              <input
                                type="text"
            ...
            >
              ...
              <textarea
                ...
                rows={4}
                readOnly={!isEditing}
                className={`mt-1 block w-full rounded-none border-neutral-300 dark:border-neutral-700 bg-neutral-50 dark:bg-dark-150 shadow-sm focus:border-brand focus:ring-brand text-neutral-900 dark:text-white ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""} print:hidden`}
              />
