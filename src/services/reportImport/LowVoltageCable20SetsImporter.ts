...
export class LowVoltageCable20SetsImporter extends BaseImporter implements ReportImporter {
  ...
  canImport(data: ReportData): boolean {
    ...
    return data.reportType?.toLowerCase().includes('lowvoltagecable') || 
           data.reportType?.toLowerCase().includes('low-voltage-cable') ||
           data.reportType?.toLowerCase().includes('12sets') ||
           data.reportType?.toLowerCase().includes('20sets');
  }
