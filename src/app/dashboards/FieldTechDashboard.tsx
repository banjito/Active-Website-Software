...
export const FieldTechDashboard: React.FC = () => {
  ...
  async function fetchRecentJobs() {
    ...
    const jobsWithCustomers = await Promise.all((data || []).map(async (job) => {
      if (!job.customer_id) return { ...job, customer: null };
      const { data: customerData, error: customerError } = await supabase
        ...
        .single();
      if (customerError) return { ...job, customer: null };
                          ...
                          <div className="p-3 sm:p-4 flex flex-col h-full">
                            <div className="flex justify-between items-start gap-2">
                              <div className="min-w-0 flex-1">
                                ...
                                <p className="text-xs sm:text-sm text-gray-600 dark:text-white mt-1 line-clamp-2">{job.customers?.company_name || job.customers?.name || 'No customer'}</p>
                              </div>
