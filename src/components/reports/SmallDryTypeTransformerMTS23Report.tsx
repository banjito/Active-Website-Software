...
const SmallDryTypeTransformerMTS23Report: React.FC = () => {
  ...
  const reportSlug = "small-dry-type-xfmr-mts23";
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    visualInspectionItems: [
      ...
      },
      {
        id: "7.2.1.1.A.5",
        description:
          "Inspect bolted electrical connections for high resistance using a low-resistance ohmmeter in accordance with Section 7.2.1.1.B.1.",
        result: "Select One",
      },
      {
        id: "7.2.1.1.A.6",
  ...
  const loadReport = async () => {
    ...
    try {
      const { data, error } = await supabase
        .schema("neta_ops")
        .from("small_dry_type_xfmr_mts23_reports")
        .select("*")
      ...
      if (data) {
        ...
        setFormData((prev) => ({
          ...
          insulationRows: reportData.insulationRows?.length
            ? reportData.insulationRows
            : prev.insulationRows,
          turnsRatio: { ...prev.turnsRatio, ...(reportData.turnsRatio || {}) },
  ...
  // The insulation section carries its own temperature on the 2023 sheet, and
  // its own correction factor: X36 = ROUND(0.1758*EXP(0.0256*X35),3), with X35
  // in degrees F. It falls back to the job information factor when left blank.
  const insulationTcf = useMemo(() => {
    const f = parseFloat(formData.insulationTemperature);
    if (!isFinite(f)) return formData.temperature.tcf || 1;
    return Math.round(0.1758 * Math.exp(0.0256 * f) * 1000) / 1000;
  }, [formData.insulationTemperature, formData.temperature.tcf]);

  // Apply temperature correction to insulation rows
  useEffect(() => {
    const tcf = insulationTcf || 1;
    setFormData((prev) => ({
      ...prev,
  ...
  // Sheet: Calculated Ratio = Primary Winding Voltage / Secondary Winding
  // Voltage, i.e. the two values shown directly above it. Reading the nameplate
  // boxes instead left the ratio stuck on the nominal tap.
  useEffect(() => {
    const primaryV = parseFloat(formData.turnsRatio.primaryWindingVoltage);
    const secondaryV = parseFloat(formData.turnsRatio.secondaryWindingVoltage);
    let calculatedRatio = "";
    ...
    // Sheet: MAX(deviations) - MIN(deviations), Pass within +/-0.5.
    const deviations = updatedRows
      .map((r) => parseFloat(r.percentDeviation))
      .filter((n) => isFinite(n));
    let differenceBetweenMR = "";
    let differenceResult: "Pass" | "Fail" | "" = "";
    if (deviations.length >= 2) {
      const diff = Math.max(...deviations) - Math.min(...deviations);
      differenceBetweenMR = diff.toFixed(4);
      differenceResult = diff <= 0.5 && diff >= -0.5 ? "Pass" : "Fail";
    }
    ...
    formData.turnsRatio.primaryWindingVoltage,
                      ...
                      <tr key={idx}>
                        ...
                        <td className="px-3 py-2">
                          <select
                            ...
                            disabled={!isEditing}
                            className={`form-select w-full min-w-[5rem] ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                          >
...
export default SmallDryTypeTransformerMTS23Report;
