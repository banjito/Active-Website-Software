...
export function JobAssignmentManagement({ portalType, division }: JobAssignmentManagementProps) {
      ...
      if (searchTerm) {
        ...
        filtered = filtered.filter(
          assignment => {
            ...
            return assignment.job?.job_number.toLowerCase().includes(lowerSearchTerm) ||
  ...
  const handleSubmitAssignment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const end = dayjs(assignmentForm.endDate || assignmentForm.assignmentDate);
      ...
      for (let i = 0; i <= days; i++) {
        ...
        const { error } = await schedulingService.saveTechnicianAssignment(assignment);
      ...
      toast({
        title: 'Success',
        description: days > 0 ? 'Assignments created for selected date range.' : 'Assignment created successfully!',
      });
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      <div className="flex items-center text-sm">
                        ...
                        <span>{formatDisplayName(assignment.user?.user_metadata?.name, assignment.user?.email)}</span>
                      </div>
