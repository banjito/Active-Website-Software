...
export default function OpportunityNotes({ opportunityId }: OpportunityNotesProps) {
  const { user } = useAuth();
  ...
  const uploadFile = async (file: File): Promise<{ url: string; name: string; type: string; size: number } | null> => {
    try {
      ...
      const { error } = await supabase.storage.from('job-documents').upload(fileName, file, { cacheControl: '3600', upsert: false });
    ...
    try {
      ...
      if (selectedFile) {
        ...
        if (!attachmentData && selectedFile) {
          toast({ title: 'Attachment failed', description: 'Note will be saved without attachment.', variant: 'destructive' });
        }
      }
  ...
  const handleEdit = async (noteId: string) => {
    ...
    try {
      ...
      if (!sessionOk) {
        toast({ title: 'Session expired', description: 'Please sign in again and try editing the note.', variant: 'destructive' });
        return;
      }
  ...
  const handleDelete = async (noteId: string) => {
    ...
    try {
      const sessionOk = await ensureValidSession();
      if (!sessionOk) {
        toast({ title: 'Session expired', description: 'Please sign in again and try deleting the note.', variant: 'destructive' });
        return;
      }
    ...
    <div className="flex flex-col min-h-[400px] max-h-[calc(100vh-320px)] bg-white dark:bg-dark-150 rounded-lg border border-gray-200 dark:border-gray-700">
      <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Add notes and see who said what and when — so you know if details still apply.
        </p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        {notes.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">Be the first to add a note to this opportunity.</p>
          </div>
        ) : (
          notes.map((note) => {
                ...
                <div className={`flex-1 max-w-[80%] ${isCurrentUser ? 'text-right' : ''}`}>
                  ...
                  <div
                    className={`rounded-lg px-4 py-3 ${
                      isCurrentUser ? 'bg-[#f26722] text-white' : 'bg-gray-100 dark:bg-dark-100 text-gray-900 dark:text-white'
                    }`}
                  >
                    {isEditing ? (
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2 justify-end">
                          <button type="button" onClick={() => { setEditingNoteId(null); setEditContent(''); }} className="p-1.5 text-gray-500 hover:text-gray-700 dark:hover:text-gray-200 bg-white dark:bg-dark-150 rounded">
                            <X className="w-4 h-4" />
                          </button>
                          <button type="button" onClick={() => handleEdit(note.id)} className="p-1.5 text-green-600 hover:text-green-700 bg-white dark:bg-dark-150 rounded">
                      ...
                      <>
                        ...
                        {note.attachment_url && (
                          <div className={`mt-2 pt-2 border-t ${isCurrentUser ? 'border-white/20' : 'border-gray-200 dark:border-gray-600'}`}>
                            <a
                            ...
                            >
                              ...
                              {note.attachment_size != null && <span className="text-xs opacity-70">({formatFileSize(note.attachment_size)})</span>}
                  ...
                  {isCurrentUser && !isEditing && (
                    <div className="flex gap-1 mt-1 justify-end">
                      <button type="button" onClick={() => { setEditingNoteId(note.id); setEditContent(note.content); }} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" title="Edit">
                        <Pencil className="w-3 h-3" />
                      </button>
                      <button type="button" onClick={() => handleDelete(note.id)} className="p-1 text-gray-400 hover:text-red-500" title="Delete">
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 px-3 py-1.5">
        {selectedFile && (
          <div className="mb-3 flex items-center gap-2 px-3 py-2 bg-gray-100 dark:bg-dark-100 rounded-lg">
            ...
            <button type="button" onClick={() => { setSelectedFile(null); if (fileInputRef.current) fileInputRef.current.value = ''; }} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
              <X className="w-4 h-4" />
            </button>
          </div>
        )}
        <form onSubmit={handleSubmit} className="flex items-center gap-2">
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} className="p-1 text-gray-500 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-dark-100 rounded shrink-0" title="Attach file">
          ...
          <button
            ...
            title="Send"
          >
            {submitting ? <div className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <Send className="w-3 h-3" />}
          </button>
        </form>
        {uploading && <p className="mt-1 text-[10px] text-gray-500 dark:text-gray-400">Uploading...</p>}
        <p className="mt-1 text-[10px] text-gray-400 dark:text-gray-500">Enter to send</p>
      </div>
    </div>
    <ProfileView
