...
const CurrentTransformerTestMTSReport: React.FC = () => {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    testEquipmentUsed: {
      megohmmeterName: '', megohmmeterSerial: '', megohmmeterAmpId: '', megohmmeterCalDate: '',
      ctRatioTestSetName: '', ctRatioTestSetSerial: '', ctRatioTestSetAmpId: '', ctRatioTestSetCalDate: ''
    },
    comments: '',
    status: 'PASS',
  });
  ...
  useEffect(() => {
    if (isPrintMode) {
      ...
      style.textContent = `
        @media print {
          .ct-ident-grid, .grid.grid-cols-3 {
          ...
          .ct-ident-grid label, .grid.grid-cols-3 label {
            width: 3rem !important;
            font-size: 8px !important;
            margin: 0 !important;
          }
          
          .ct-ident-grid input, .grid.grid-cols-3 input {
            width: 3rem !important;
            font-size: 8px !important;
          ...
          .ct-ident-grid .space-y-1 > *, .grid.grid-cols-3 .space-y-1 > * {
    ...
    try {
      ...
      if (currentReportId) {
        ...
        if (result.data) {
          ...
          // Create asset entry
          const assetData = {
            name: getAssetName(reportSlug, formData.identifier || formData.eqptLocation || 'Unnamed'),
  ...
  const handleSave = async () => {
    ...
    // Do not change ownership/job on updates; only update report_data
    const updatePayload = {
      report_data: formData,
    };
    ...
    try {
      ...
      if (currentReportId) {
        ...
        console.log('Update payload:', reportPayload);
      ...
      if (currentReportId) {
        ...
        // Stay on the report page after update
      } else {
      ...
      {[
        ...
        <div key={item.label} className="space-y-1">
          <div className="flex items-center">
            <label htmlFor={`ct-${item.topKey}`} className="text-xs font-medium text-gray-700 dark:text-white w-16">
              {item.label}:
            </label>
            <input
              type="text"
              ...
              readOnly={!isEditing}
              className={`ml-1 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded w-16 bg-white dark:bg-dark-150 text-gray-900 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150 cursor-not-allowed' : ''}`}
            />
          </div>
          <div className="flex items-center">
            <label htmlFor={`ct-${item.serialKey}`} className="text-xs font-medium text-gray-700 dark:text-white w-16">
              Serial #:
            </label>
            <input
              type="text"
    ...
    <ReportWrapper isPrintMode={isPrintMode}>
        ...
        <div className="flex gap-2 items-center">
          ...
          {currentReportId && !isEditing ? (
            ...
            <button onClick={handleSave} disabled={!isEditing} className={`px-4 py-2 text-sm text-white bg-[#f26722] rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#f26722] ${!isEditing ? 'hidden' : 'hover:bg-[#f26722]/90'}`}>
              {currentReportId ? 'Update Report' : 'Save New Report'}
            </button>
          )}
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8 print:hidden job-info-onscreen">
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={maskCustomerName(formData.customerName)} readOnly className="form-input bg-gray-50 dark:bg-dark-150" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">Address</label>
              <span className="print:inline">{maskCustomerAddress(formData.customerAddress || '')}</span>
              <textarea
                value={maskCustomerAddress(formData.customerAddress)}
                readOnly
                rows={3}
                className="form-input bg-gray-50 dark:bg-dark-150 print:hidden"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">User</label>
              <input type="text" value={formData.userName} onChange={e => handleChange('userName', e.target.value)} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">Date</label>
              <input type="date" value={formData.date} onChange={e => handleChange('date', e.target.value)} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">Identifier</label>
              <input type="text" value={formData.identifier} onChange={e => handleChange('identifier', e.target.value)} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
            </div>
          </div>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">Job #</label>
              <input type="text" value={formData.jobNumber} readOnly className="form-input bg-gray-50 dark:bg-dark-150" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">Technicians</label>
              <input type="text" value={formData.technicians} onChange={e => handleChange('technicians', e.target.value)} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-white">Temp. °F</label>
                <input type="number" value={formData.temperature.fahrenheit} onChange={e => handleFahrenheitChange(Number(e.target.value))} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-white">°C</label>
                <input type="number" value={formData.temperature.celsius} onChange={e => handleCelsiusChange(Number(e.target.value))} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-white">TCF</label>
                <input type="number" value={formData.temperature.tcf} readOnly className="form-input bg-gray-50 dark:bg-dark-150" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">Humidity</label>
                              <input type="number" value={formData.temperature.humidity || ''} onChange={e => handleChange('temperature.humidity', e.target.value === '' ? null : Number(e.target.value))} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">Substation</label>
              <input type="text" value={formData.substation} onChange={e => handleChange('substation', e.target.value)} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">Eqpt. Location</label>
              <input type="text" value={formData.eqptLocation} onChange={e => handleChange('eqptLocation', e.target.value)} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
            </div>
          </div>
        </div>
        <JobInfoPrintTable
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 print:hidden device-onscreen">
          ...
          <div className="space-y-3">
            <div><label htmlFor="deviceData.manufacturer" className="form-label inline-block w-32">Manufacturer:</label><input id="deviceData.manufacturer" type="text" value={formData.deviceData.manufacturer} onChange={(e) => handleChange('deviceData.manufacturer', e.target.value)} readOnly={!isEditing} className={`form-input w-[calc(100%-8rem)] ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="deviceData.class" className="form-label inline-block w-32">Class:</label><input id="deviceData.class" type="text" value={formData.deviceData.class} onChange={(e) => handleChange('deviceData.class', e.target.value)} readOnly={!isEditing} className={`form-input w-[calc(100%-8rem)] ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="deviceData.ctRatio" className="form-label inline-block w-32">CT Ratio:</label><input id="deviceData.ctRatio" type="text" value={formData.deviceData.ctRatio} onChange={(e) => handleChange('deviceData.ctRatio', e.target.value)} readOnly={!isEditing} className={`form-input w-[calc(100%-8rem)] ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="deviceData.catalogNumber" className="form-label inline-block w-32">Catalog Number:</label><input id="deviceData.catalogNumber" type="text" value={formData.deviceData.catalogNumber} onChange={(e) => handleChange('deviceData.catalogNumber', e.target.value)} readOnly={!isEditing} className={`form-input w-[calc(100%-8rem)] ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
          </div>
          {/* Right Column */}
          <div className="space-y-3">
            <div><label htmlFor="deviceData.voltageRating" className="form-label inline-block w-32">Voltage Rating (V):</label><input id="deviceData.voltageRating" type="text" value={formData.deviceData.voltageRating} onChange={(e) => handleChange('deviceData.voltageRating', e.target.value)} readOnly={!isEditing} className={`form-input w-[calc(100%-8rem)] ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="deviceData.polarityFacing" className="form-label inline-block w-32">Polarity Facing:</label><input id="deviceData.polarityFacing" type="text" value={formData.deviceData.polarityFacing} onChange={(e) => handleChange('deviceData.polarityFacing', e.target.value)} readOnly={!isEditing} className={`form-input w-[calc(100%-8rem)] ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="deviceData.type" className="form-label inline-block w-32">Type:</label><input id="deviceData.type" type="text" value={formData.deviceData.type} onChange={(e) => handleChange('deviceData.type', e.target.value)} readOnly={!isEditing} className={`form-input w-[calc(100%-8rem)] ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="deviceData.frequency" className="form-label inline-block w-32">Frequency:</label><input id="deviceData.frequency" type="text" value={formData.deviceData.frequency} onChange={(e) => handleChange('deviceData.frequency', e.target.value)} readOnly={!isEditing} className={`form-input w-[calc(100%-8rem)] ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
          </div>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-dark-150">
                <tr>
                  {['Identifier', 'Ratio', 
                    ...
                    <th key={header} className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">
              ...
              <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-200 dark:divide-gray-700">
                {formData.electricalTests.ratioPolarity.map((item, index) => (
                  <tr key={item.id}>
                    <td className="px-4 py-3"><input type="text" value={item.identifier} onChange={(e) => handleRatioPolarityChange(index, 'identifier', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></td>
                    <td className="px-4 py-3"><input type="text" value={item.ratio} onChange={(e) => handleRatioPolarityChange(index, 'ratio', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></td>
                    ...
                    <td className="px-4 py-3"><input type="text" value={item.pri} onChange={(e) => handleRatioPolarityChange(index, 'pri', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></td>
                    <td className="px-4 py-3"><input type="text" value={item.sec} onChange={(e) => handleRatioPolarityChange(index, 'sec', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></td>
                    <td className="px-4 py-3"><input type="text" value={item.measuredRatio} onChange={(e) => handleRatioPolarityChange(index, 'measuredRatio', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></td>
                    <td className="px-4 py-3"><input type="text" value={item.ratioDev} onChange={(e) => handleRatioPolarityChange(index, 'ratioDev', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></td>
                    <td className="px-4 py-3">
                      <select value={item.polarity} onChange={(e) => handleRatioPolarityChange(index, 'polarity', e.target.value)} disabled={!isEditing} className={`form-select w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>
        ...
        <div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border border-gray-300 dark:border-gray-600">
              <thead className="bg-gray-50 dark:bg-dark-150">
                <tr>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider whitespace-nowrap">
                    Test Voltage:&nbsp;
                    <select 
                      ...
                      disabled={!isEditing} 
                      className={`form-select inline-block w-auto p-1 text-xs ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}
                    >
                  ...
                  {['Phase 1', 'Phase 2', 'Phase 3', 'Neutral'].map(header => (
                    <th key={header} className="px-4 py-2 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">{header}</th>
                  ))}
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Units</th>
                </tr>
              </thead>
              <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-200 dark:divide-gray-700">
                <tr>
                  <td className="px-4 py-2 whitespace-nowrap text-sm font-medium text-gray-700 dark:text-white">Reading</td>
                ...
                <tr>
                  <td className="px-4 py-2 whitespace-nowrap text-sm font-medium text-gray-700 dark:text-white">Temp. Correction 20°C</td>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-4 print:hidden te-onscreen">
          <div className="grid grid-cols-4 gap-4">
            <div>
              <label htmlFor="testEquipmentUsed.megohmmeterName" className="block text-sm font-medium text-gray-700 dark:text-white">Megohmmeter</label>
              <EquipmentAutocomplete
                value={formData.testEquipmentUsed.megohmmeterName}
                onChange={(value) => handleChange('testEquipmentUsed.megohmmeterName', value)}
                onSelect={(equipment) => {
                  ...
                  handleChange('testEquipmentUsed.megohmmeterName', equipment.equipment_name);
                  handleChange('testEquipmentUsed.megohmmeterSerial', equipment.serial_number || '');
                  handleChange('testEquipmentUsed.megohmmeterAmpId', equipment.amp_id || '');
                  handleChange('testEquipmentUsed.megohmmeterCalDate', formatLocalDateShort(equipment.calibration_date));
                }}
                readOnly={!isEditing}
                className="w-full"
              />
            </div>
            <div><label htmlFor="testEquipmentUsed.megohmmeterSerial" className="block text-sm font-medium text-gray-700 dark:text-white">Serial Number</label><input id="testEquipmentUsed.megohmmeterSerial" type="text" value={formData.testEquipmentUsed.megohmmeterSerial} onChange={(e) => handleChange('testEquipmentUsed.megohmmeterSerial', e.target.value)} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="testEquipmentUsed.megohmmeterAmpId" className="block text-sm font-medium text-gray-700 dark:text-white">AMP ID</label><input id="testEquipmentUsed.megohmmeterAmpId" type="text" value={formData.testEquipmentUsed.megohmmeterAmpId} onChange={(e) => handleChange('testEquipmentUsed.megohmmeterAmpId', e.target.value)} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="testEquipmentUsed.megohmmeterCalDate" className="block text-sm font-medium text-gray-700 dark:text-white">Cal Date</label><input id="testEquipmentUsed.megohmmeterCalDate" type="text" value={formData.testEquipmentUsed.megohmmeterCalDate} onChange={(e) => handleChange('testEquipmentUsed.megohmmeterCalDate', e.target.value)} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
          </div>
          <div className="grid grid-cols-4 gap-4">
            <div>
              <label htmlFor="testEquipmentUsed.ctRatioTestSetName" className="block text-sm font-medium text-gray-700 dark:text-white">CT Ratio Test Set</label>
              <EquipmentAutocomplete
                value={formData.testEquipmentUsed.ctRatioTestSetName}
                onChange={(value) => handleChange('testEquipmentUsed.ctRatioTestSetName', value)}
                onSelect={(equipment) => {
                  const formatDate = (dateString: string | null): string => {
                    if (!dateString) return '';
                    try {
                  ...
                  handleChange('testEquipmentUsed.ctRatioTestSetName', equipment.equipment_name);
                  handleChange('testEquipmentUsed.ctRatioTestSetSerial', equipment.serial_number || '');
                  handleChange('testEquipmentUsed.ctRatioTestSetAmpId', equipment.amp_id || '');
                  handleChange('testEquipmentUsed.ctRatioTestSetCalDate', formatLocalDateShort(equipment.calibration_date));
                }}
                readOnly={!isEditing}
                className="w-full"
              />
            </div>
            <div><label htmlFor="testEquipmentUsed.ctRatioTestSetSerial" className="block text-sm font-medium text-gray-700 dark:text-white">Serial Number</label><input id="testEquipmentUsed.ctRatioTestSetSerial" type="text" value={formData.testEquipmentUsed.ctRatioTestSetSerial} onChange={(e) => handleChange('testEquipmentUsed.ctRatioTestSetSerial', e.target.value)} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="testEquipmentUsed.ctRatioTestSetAmpId" className="block text-sm font-medium text-gray-700 dark:text-white">AMP ID</label><input id="testEquipmentUsed.ctRatioTestSetAmpId" type="text" value={formData.testEquipmentUsed.ctRatioTestSetAmpId} onChange={(e) => handleChange('testEquipmentUsed.ctRatioTestSetAmpId', e.target.value)} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
            <div><label htmlFor="testEquipmentUsed.ctRatioTestSetCalDate" className="block text-sm font-medium text-gray-700 dark:text-white">Cal Date</label><input id="testEquipmentUsed.ctRatioTestSetCalDate" type="text" value={formData.testEquipmentUsed.ctRatioTestSetCalDate} onChange={(e) => handleChange('testEquipmentUsed.ctRatioTestSetCalDate', e.target.value)} readOnly={!isEditing} className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
          </div>
        </div>
          ...
          <table className="w-full table-fixed border-collapse border border-gray-200 dark:border-gray-700">
            ...
            <tbody>
              <tr>
                ...
                <td>{formData.testEquipmentUsed.megohmmeterCalDate || ''}</td>
              </tr>
              <tr>
                ...
                <td>{formData.testEquipmentUsed.ctRatioTestSetCalDate || ''}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div className="mb-6 comments-section">
        ...
        <textarea
          ...
          readOnly={!isEditing}
          rows={6}
          className={`print:hidden form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}
