...
// MTS 7.6.1 splits on breaker construction, not on the ATS 7.6.1.1.x pattern:
// molded-case and insulated-case breakers are maintained under 7.6.1.1, power
// breakers under 7.6.1.2. The blank MTS 23 sheet drives both lists off the
// Breaker Type field the same way.
const getSectionNumber = (breakerType: string): string => {
  switch (breakerType) {
    ...
    case "power":
      return "7.6.1.2";
    default:
      return "7.6.1.1"; // Default to molded case
  }
};
  ...
  switch (breakerType) {
    ...
    case "power":
      return [
        ...
        {
          id: `${sectionNumber}.A.6`,
          description: "Inspect arc chutes.",
          result: "",
        },
        {
          id: `${sectionNumber}.A.7`,
...
// The counter-reading row on the MTS 23 sheet sits beside the last checklist
// item and only exists for power breakers (7.6.1.2.A.16).
const COUNTER_READING_ITEM_ID = "7.6.1.2.A.16";
...
interface FormData {
  ...
  // Operation counter, recorded alongside the last checklist item on power
  // breakers (7.6.1.2.A.16).
  counterReadings: {
    asFound: string;
    asLeft: string;
  };
...
const LVCircuitBreakerMTS23Report: React.FC = () => {
  ...
  const reportSlug = "lv-circuit-breaker-mts23";
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    zoneInterlock: "",
    thermalMemory: "",
    // Breaker Type is blank until the technician picks one, so the sheet opens
    // on the molded-case list and swaps when they do.
    visualInspectionItems: getVisualInspectionItems("molded case"),
    counterReadings: { asFound: "", asLeft: "" },
    tripType: "",
    deviceSettings: {
      asFound: {
  ...
  const loadReport = async () => {
    ...
    try {
      const { data, error } = await supabase
        .schema("neta_ops")
        .from("lv_circuit_breaker_mts23_reports")
        .select("*")
      ...
      if (data && data.report_data) {
        ...
        setFormData((prev) => {
          ...
          return {
            ...
            counterReadings:
              reportData.counterReadings || prev.counterReadings,
  ...
  const {
    ...
    // The inspection list is built from the breaker type, so a prefilled type needs it too.
    (next, prev) =>
      next.breakerType !== prev.breakerType
        ? {
            ...next,
            visualInspectionItems: getVisualInspectionItems(next.breakerType),
          }
        : next,
  );
      ...
      */}
      ...
      >
        ...
        >
          ...
          >
            7.6.1 Low Voltage Circuit Breaker Test Sheet MTS 23
          </h1>
        </div>
        <div
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6 dark:text-white">
        ...
        <ReportHeader
          ...
          title={`${getSectionNumber(formData.breakerType)} Low Voltage Circuit Breaker Test Sheet MTS 23`}
        ...
        <div className="space-y-6">
          ...
          <div className="mb-6">
            ...
            <ReportJobInfoFields
              ...
              fieldProps={{
                ...
                eqptIdentifier: { onBlur: renameGuard.handleBlur },
              }}
            ...
            <div className="overflow-x-auto">
              <table className="w-full table-fixed border-collapse border border-neutral-300 dark:border-neutral-600">
                ...
                <tbody className="bg-white dark:bg-dark-150 divide-y divide-neutral-200 dark:divide-neutral-700">
                  ...
                  {formData.visualInspectionItems.some(
                    (item) => item.id === COUNTER_READING_ITEM_ID,
                  ) && (
                    <tr>
                      <td className="border border-neutral-300 dark:border-neutral-600 px-3 py-2 text-sm text-neutral-900 dark:text-white"></td>
                      <td
                      ...
                      >
                        <div className="flex flex-wrap items-center gap-4">
                          ...
                          <label className="flex items-center gap-2">
                            ...
                            <input
                              type="text"
                              value={formData.counterReadings.asFound}
                              onChange={(e) =>
                                handleChange(
                                  "counterReadings.asFound",
                                  e.target.value,
                                )
                              }
                              readOnly={!isEditing}
                              className={`form-input w-28 text-center ${!isEditing ? "bg-neutral-100 dark:bg-dark-150" : ""}`}
                            />
                          </label>
                          <label className="flex items-center gap-2">
                            <span>As-Left:</span>
                            <input
                              type="text"
                              value={formData.counterReadings.asLeft}
                              onChange={(e) =>
                                handleChange(
                                  "counterReadings.asLeft",
                                  e.target.value,
                                )
                              }
                              readOnly={!isEditing}
          ...
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full table-fixed border-collapse border border-neutral-300 dark:border-neutral-600">
                {/* Name/Model holds a search field with its own dropdown, so it
                    needs real width; the rest are short values. */}
                <colgroup>
                  <col style={{ width: "22%" }} />
                  <col style={{ width: "30%" }} />
                  <col style={{ width: "18%" }} />
...
export default LVCircuitBreakerMTS23Report;
