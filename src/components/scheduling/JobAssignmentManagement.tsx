...
export function JobAssignmentManagement({
  portalType,
  division,
}: JobAssignmentManagementProps) {
  const { user } = useAuth();
    ...
    if (assignments.length > 0) {
      ...
      if (searchTerm) {
        ...
        filtered = filtered.filter((assignment) => {
          const techName = formatDisplayName(
          ...
          return (
            ...
            techName.toLowerCase().includes(lowerSearchTerm)
          );
        });
      }
  ...
  const fetchJobSkillRequirements = async (jobId: string) => {
    try {
      const { data, error } = await schedulingService.getJobSkillRequirements(
        jobId,
        portalType,
      );
  ...
  const handleSubmitAssignment = async (e: React.FormEvent) => {
    ...
    if (
      !assignmentForm.technicianId ||
      !assignmentForm.jobId ||
      !assignmentForm.assignmentDate
    ) {
      toast({
        title: "Error",
    ...
    setLoading(true);
    try {
      // Support multi-day range: create one assignment per day in range
      const start = dayjs(assignmentForm.assignmentDate);
      const end = dayjs(
        assignmentForm.endDate || assignmentForm.assignmentDate,
      );
      ...
      for (let i = 0; i <= days; i++) {
        const assignment = {
          ...
          assignment_date: start.add(i, "day").format("YYYY-MM-DD"),
        ...
        const { error } =
          await schedulingService.saveTechnicianAssignment(assignment);
        if (error) throw error;
      }

      // Lightweight notification to the assigned technician (if supported by RLS)
      if (assignmentForm.technicianId) {
        try {
          await createJobNotification({
            ...
            message: `You were assigned to job on ${dayjs(assignmentForm.assignmentDate).format("MMM D")} - ${dayjs(assignmentForm.endDate || assignmentForm.assignmentDate).format("MMM D")}`,
            type: "resource_assigned",
            metadata: {
              portal_type: portalType,
              division,
              start: assignmentForm.assignmentDate,
              end: assignmentForm.endDate || assignmentForm.assignmentDate,
            },
          } as any);
        } catch (notifyErr) {
      ...
      toast({
        ...
        description:
          days > 0
            ? "Assignments created for selected date range."
      ...
      console.error("Error creating assignment(s):", error);
      toast({
        title: "Error",
        description: "Failed to create assignment(s). Please try again.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs
        value={activeTab}
        onValueChange={(value) => setActiveTab(value as AssignmentStatus)}
      >
        <TabsList>
              ...
              {filteredAssignments.map((assignment) => (
                <Card key={assignment.id} className="overflow-hidden">
                  <CardHeader className="pb-2">
                    <Badge
                      className={`${getStatusBadgeColor(assignment.status)} text-white mb-2`}
                    >
            ...
            <div>
              ...
              <Select
                ...
                options={jobs.map((job) => ({
                  label: `${job.job_number} - ${maskJobTitle(job.title)}`,
                  value: job.id,
                }))}
              />
            </div>
                    ...
                    <div key={skill.id} className="flex items-center mb-1">
                      <span className={skill.is_required ? "font-medium" : ""}>
                        ...
                        {skill.minimum_proficiency
                          ? ` (Min. Level: ${skill.minimum_proficiency})`
                          : ""}
                      </span>
                      {skill.is_required && (
