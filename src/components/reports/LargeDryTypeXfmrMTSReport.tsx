...
const LARGE_DRY_TYPE_XFMR_MTS_TABLE =
...
const calculateTCF = (celsius: number): number => {
  ...
  const closestTemp = tcfTable.reduce((prev, curr) => {
    return Math.abs(curr[0] - celsius) < Math.abs(prev[0] - celsius)
      ? curr
      : prev;
  });
...
const visualInspectionItemsConfig = [
  ...
  {
    id: "7.2.1.2.A.6",
    description: "Verify that cooling fans operate correctly.",
  },
  {
    id: "7.2.1.2.A.7",
    description:
  ...
  {
    id: "7.2.1.2.A.11",
    description: "Verify the presence of surge arresters.",
  },
];
...
const LargeDryTypeXfmrMTSReport: React.FC = () => {
    ...
    setFormData((prev) => ({
      ...
      [section]: {
        ...
        [subsection]: {
          ...
          [nestedSection]: {
            ...((prev[section] as any)[subsection][nestedSection] as any),
            [field]: value,
          },
        },
      },
  ...
  const updateCalculatedValues = (
    ...
    if (
      testId === "temperature" ||
      testId === "dielectricAbsorptionAcceptable" ||
      testId === "polarizationIndexAcceptable"
    )
      return;
    setFormData((prev) => {
      const testData = prev.insulationResistance[
        testId
      ] as typeof prev.insulationResistance.primaryToGround; // Type assertion
      if (!testData || !testData.readings) return prev; // Guard against undefined

      const tcf = prev.temperature.correctionFactor;
      const correctedHalfMinute = calculateCorrectedValue(
        testData.readings.halfMinute,
        tcf,
      );
      const correctedOneMinute = calculateCorrectedValue(
        testData.readings.oneMinute,
        tcf,
      );
      const correctedTenMinute = calculateCorrectedValue(
        testData.readings.tenMinute,
        tcf,
      );
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        visualInspectionItemsConfig.forEach((item) => {
          initialVisualInspectionState[`${item.id}_result`] =
            loadedVisualInspection[`${item.id}_result`] || "Select One";
          initialVisualInspectionState[`${item.id}_comment`] =
            loadedVisualInspection[`${item.id}_comment`] || "";
        });
        ...
        setFormData((prev) => ({
          ...
          temperature: {
            ...
            ...(data.report_info?.temperature?.fahrenheit !== undefined && {
              celsius: parseFloat(
                (
                  ((data.report_info.temperature.fahrenheit - 32) * 5) /
                  9
                ).toFixed(1),
              ),
              correctionFactor: calculateTCF(
                Math.round(
                  parseFloat(
                    (
                      ((data.report_info.temperature.fahrenheit - 32) * 5) /
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!wasExistingReport) {
        setIsEditing(false);
        const newId =
          currentReportId || (result as any)?.data?.id || (result as any)?.id;
        if (newId) {
  ...
  useEffect(() => {
    ...
    const updateCalculatedValues = (
      ...
      setFormData((prev) => {
        const testData = prev.insulationResistance[
          testId
        ] as typeof prev.insulationResistance.primaryToGround;
                    ...
                    <tr key={item.id}>
                      ...
                      <td className="px-3 py-2">
                        <div className="print:hidden">
                          <select
                            value={
                              formData.visualInspection[`${item.id}_result`] ||
                              "Select One"
                            }
                            onChange={(e) =>
...
// .form-label { @apply block text-sm font-medium text-zinc-700 dark:text-white mb-1; }
// .form-input, .form-select, .form-textarea {
//   @apply mt-1 block w-full p-2 border border-zinc-300 dark:border-zinc-600 rounded-md shadow-sm
...
//   @apply bg-zinc-100 dark:bg-dark-150 text-zinc-500 dark:text-white cursor-not-allowed;
// }
// .table-header { @apply px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-white uppercase tracking-wider bg-zinc-50 dark:bg-dark-150; }
// .table-cell { @apply px-6 py-4 whitespace-nowrap text-sm text-zinc-900 dark:text-white; }
