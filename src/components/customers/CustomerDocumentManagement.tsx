...
const CustomerDocumentManagement: React.FC<CustomerDocumentManagementProps> = ({
  customerId,
}) => {
  ...
  const [selectedFile, setSelectedFile] = useState<CustomerDocument | null>(
    null,
  );
  ...
  const loadDocuments = useCallback(async () => {
    ...
    try {
      ...
      const tags = Array.from(
        new Set(docsData.flatMap((doc) => doc.tags || [])),
      ) as string[];
      const types = Array.from(
        new Set(docsData.map((doc) => doc.type)),
      ) as string[];
  ...
  const handleViewDocument = async (document: CustomerDocument) => {
    try {
      ...
      const previewCompatible = ["pdf", "jpg", "jpeg", "png", "gif"].includes(
        fileExtension || "",
      );
  ...
  const filteredDocuments = documents.filter((doc) => {
    const matchesFolder =
      currentFolder === null || doc.folder_id === currentFolder;
        ...
        {documents.map((doc) => (
          ...
          >
            <div className="d-flex align-items-center">
              <div className="me-3 fs-4">{getFileTypeIcon(doc.type)}</div>
              <div>
      ...
      <div className="mb-4">
        <div className="d-flex justify-content-between align-items-center mb-3">
          <Breadcrumb>
            ...
            {currentFolderObj && (
              <Breadcrumb.Item active>{currentFolderObj.name}</Breadcrumb.Item>
            )}
          </Breadcrumb>
