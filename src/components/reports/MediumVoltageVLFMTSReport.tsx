...
const MediumVoltageVLFMTSReport: React.FC = () => {
  ...
  const loadJobInfo = async (currentJobId: string) => {
    ...
    try {
      ...
      let customerName = '', customerAddress = (jobData as any).site_address || '';
      ...
      <section className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 print:hidden test-eqpt-onscreen">
          {[ {label: "Ohmmeter", field: "ohmmeter"}, {label: "Serial Number", field: "ohmSerialNumber"}, {label: "AMP ID", field: "ohmAmpId"},
            ...
            <div key={idx}>
              <label className="block text-sm font-medium text-gray-700 dark:text-white">{item.label}</label>
              <input type="text" value={formData.equipment?.[item.field] || ''} onChange={(e) => handleChange('equipment', {...formData.equipment, [item.field]: e.target.value})} readOnly={!isEditMode} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/>
            </div>
          ))}
        </div>
