  ...
  const checkTableStructure = async () => {
    ...
    try {
      ...
      if (testData && testData.length > 0) {
        ...
        setMessage(
          `Available columns in customers table: ${columns.join(", ")}`,
        );
      } else {
  ...
  const trySimpleInsert = async () => {
    ...
    try {
      ...
      if (customerError) {
        ...
        if (
          customerError.message.includes("violates row-level security policy")
        ) {
          setMessage(
  ...
  const tryBasicInsert = async () => {
    ...
    try {
      ...
      if (customerError) {
        ...
        setMessage(
          `Successfully created a basic customer with ID: ${customer.id}`,
        );
  ...
  return (
    <div className="p-8">
      ...
      <div className="space-y-4">
        ...
        <div className="p-4 bg-neutral-100 rounded min-h-[100px] whitespace-pre-wrap">
          {loading ? (
            <LoadingSpinner size="md" />
          ) : (
            message || "No data yet. Click one of the buttons above."
          )}
        </div>
      </div>
    </div>
