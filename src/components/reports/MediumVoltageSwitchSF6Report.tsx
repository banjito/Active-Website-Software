...
interface EquipmentInfo { megohmmeter: { model: string; serial: string; ampId: string }; lowResOhmmeter: { model: string; serial: string; ampId: string }; hipot: { model: string; serial: string; ampId: string } }
...
const MediumVoltageSwitchSF6Report: React.FC = () => {
  ...
  const [formData, setFormData] = useState<ReportData>({
    ...
    equipment: { megohmmeter: { model: '', serial: '', ampId: '' }, lowResOhmmeter: { model: '', serial: '', ampId: '' }, hipot: { model: '', serial: '', ampId: '' } },
    comments: ''
  });
  ...
  useEffect(() => {
    const loadJobInfo = async () => {
      if (!jobId || reportId) return; // skip when loading an existing report
      try {
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-x-4 gap-y-2 print:hidden">
            <div>
              ...
              <input className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.customer} readOnly={!isEditMode} onChange={(e) => handleChange('customer', e.target.value)} />
            </div>
            <div>
              <label className="form-label">Address:</label>
              <input className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.address} readOnly={!isEditMode} onChange={(e) => handleChange('address', e.target.value)} />
            </div>
            <div>
              <label className="form-label">Job #:</label>
              <input className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.jobNumber} readOnly={!isEditMode} onChange={(e) => handleChange('jobNumber', e.target.value)} />
            </div>
            <div>
              <label className="form-label">Date:</label>
              <input type="date" className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.date} readOnly={!isEditMode} onChange={(e) => handleChange('date', e.target.value)} />
            </div>
            <div>
              <label className="form-label">Technicians:</label>
              <input className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.technicians} readOnly={!isEditMode} onChange={(e) => handleChange('technicians', e.target.value)} />
            </div>
            <div>
              <label className="form-label">Identifier:</label>
              <input className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.identifier} readOnly={!isEditMode} onChange={(e) => handleChange('identifier', e.target.value)} />
            </div>
            <div>
              <label className="form-label">Substation:</label>
              <input className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.substation} readOnly={!isEditMode} onChange={(e) => handleChange('substation', e.target.value)} />
            </div>
            <div>
              <label className="form-label">Eqpt. Location:</label>
              <input className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.eqptLocation} readOnly={!isEditMode} onChange={(e) => handleChange('eqptLocation', e.target.value)} />
            </div>
            <div className="grid grid-cols-2 gap-x-2">
              <div>
                <label className="form-label">Temp (°F):</label>
                <input type="number" className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.temperature} readOnly={!isEditMode} onChange={(e) => handleChange('temperature', Number(e.target.value))} />
              </div>
              <div>
            ...
            <div className="flex items-end">
              <div className="flex items-center mt-auto">
                ...
                <input type="number" value={Number.isFinite(celsiusTemperature) ? Number(celsiusTemperature.toFixed(0)) : 0} readOnly className="form-input text-sm w-24 bg-gray-100 dark:bg-dark-200" />
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:hidden">
            ...
            <div className="space-y-3">
              ...
              <div><label className="form-label inline-block w-48">AIC Rating (kA)</label><input className={`form-input`} value={formData.nameplate.aicRating} readOnly={!isEditMode} onChange={(e) => handleNameplateChange('aicRating', e.target.value)} /></div>
          ...
          <div className="hidden print:block">
            {(() => {
              const cells: { label: string; value: string }[] = [
                ...
                { label: 'AIC Rating (kA)', value: formData.nameplate.aicRating },
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:hidden">
            ...
            <div className="space-y-3">
              ...
              <div><label className="form-label inline-block w-48">AIC Rating (kA)</label><input className={`form-input`} value={formData.vfi.aicRating} readOnly={!isEditMode} onChange={(e) => handleVFIChange('aicRating', e.target.value)} /></div>
            </div>
          </div>
          <div className="hidden print:block">
            {(() => {
              const cells: { label: string; value: string }[] = [
                ...
                { label: 'AIC Rating (kA)', value: formData.vfi.aicRating },
              ];
              return (
                <table className="w-full border-collapse text-sm">
                  <colgroup>
        ...
        <div className="mb-6">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-fixed">
              ...
              <thead>
                <tr>
                  <th className="px-3 py-2 bg-gray-50 dark:bg-dark-200 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">NETA Section</th>
                ...
                {INSPECTION_ITEMS.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-dark-200">
                    ...
                    <td className="px-3 py-2 whitespace-nowrap">
                      <select value={formData.visualInspection[item.id]} onChange={(e) => handleInspectionChange(item.id, e.target.value as ResultOption)} disabled={!isEditMode} className={`block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-100 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`}>
                        {RESULT_OPTIONS.map(opt => (<option key={opt} value={opt}>{opt}</option>))}
                      </select>
                    </td>
                    <td className="px-3 py-2"><input type="text" className={`block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-100 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} readOnly={!isEditMode} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
                ...
                {formData.counterReadings.map((row, i) => (
                  <tr key={`${row.identifier}-${i}`} className="hover:bg-gray-50 dark:hover:bg-dark-200">
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1">
                      <input className={`form-input text-xs ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={row.identifier} readOnly={!isEditMode} onChange={(e) => handleCounterChange(i, 'identifier', e.target.value)} />
                    </td>
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1">
                      <input className={`form-input text-xs ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={row.asFound} readOnly={!isEditMode} onChange={(e) => handleCounterChange(i, 'asFound', e.target.value)} />
                    </td>
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1">
                      <input className={`form-input text-xs ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={row.asLeft} readOnly={!isEditMode} onChange={(e) => handleCounterChange(i, 'asLeft', e.target.value)} />
                    </td>
                  </tr>
                ))}
              </tbody>
                ...
                {formData.insulationMeasured.rows.map((row, i) => (
                  <tr key={row.waySection} className="hover:bg-gray-50 dark:hover:bg-dark-200">
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1 text-center">
                      <input className={`form-input text-xs text-center ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={row.waySection} readOnly={!isEditMode} onChange={(e) => handleInsulationRowChange('insulationMeasured', i, 'waySection', e.target.value)} />
                    </td>
                    {(['ag','bg','cg','ab','bc','ca','lineA','lineB','lineC'] as (keyof InsulationRow)[]).map((key) => (
                      <td key={String(key)} className="border border-gray-300 dark:border-gray-600 px-1 py-1">
                        <input className={`form-input text-xs text-center ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={(row as any)[key] as string} readOnly={!isEditMode} onChange={(e) => handleInsulationRowChange('insulationMeasured', i, key, e.target.value)} />
                      </td>
                    ))}
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1">
            ...
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {formData.insulationCorrected.rows.map((row, i) => (
                  <tr key={row.waySection} className="hover:bg-gray-50 dark:hover:bg-dark-200">
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1 text-center">
                      <input className={`form-input text-xs text-center ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={row.waySection} readOnly={!isEditMode} onChange={(e) => handleInsulationRowChange('insulationCorrected', i, 'waySection', e.target.value)} />
                    </td>
                    {(['ag','bg','cg','ab','bc','ca','lineA','lineB','lineC'] as (keyof InsulationRow)[]).map((key) => (
                      <td key={String(key)} className="border border-gray-300 dark:border-gray-600 px-1 py-1">
                        <input className={`form-input text-xs text-center ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={(row as any)[key] as string} readOnly={!isEditMode} onChange={(e) => handleInsulationRowChange('insulationCorrected', i, key, e.target.value)} />
                      </td>
                    ))}
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1">
            ...
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {formData.contactResistance.rows.map((row, i) => (
                  <tr key={row.waySection} className="hover:bg-gray-50 dark:hover:bg-dark-200">
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1 text-center">
                      <input className={`form-input text-xs text-center ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={row.waySection} readOnly={!isEditMode} onChange={(e) => handleContactRowChange(i, 'waySection', e.target.value)} />
                    </td>
                    {(['aPhase','aG','bPhase','bG','cPhase','cG'] as (keyof ContactRow)[]).map(key => (
                      <td key={String(key)} className="border border-gray-300 dark:border-gray-600 px-1 py-1">
                        <input className={`form-input text-xs text-center ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={(row as any)[key] as string} readOnly={!isEditMode} onChange={(e) => handleContactRowChange(i, key, e.target.value)} />
                      </td>
                    ))}
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1">
            ...
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {formData.dielectricWithstand.rows.map((row, i) => (
                  <tr key={row.waySection} className="hover:bg-gray-50 dark:hover:bg-dark-200">
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1 text-center">
                      <input className={`form-input text-xs text-center ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={row.waySection} readOnly={!isEditMode} onChange={(e) => handleDielectricRowChange(i, 'waySection', e.target.value)} />
                    </td>
                    {(['ag','bg','cg'] as (keyof DielectricRow)[]).map(key => (
                      <td key={String(key)} className="border border-gray-300 dark:border-gray-600 px-1 py-1">
                        <input className={`form-input text-xs text-center ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={(row as any)[key] as string} readOnly={!isEditMode} onChange={(e) => handleDielectricRowChange(i, key, e.target.value)} />
                      </td>
                    ))}
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1">
              ...
              <tbody>
                {formData.dielectricVFI.rows.map((row, i) => (
                  <tr key={`${row.vfiIdentifier}-${i}`} className="hover:bg-gray-50 dark:hover:bg-dark-200">
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1 text-center">
                      <input className={`form-input text-xs text-center ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={row.vfiIdentifier} readOnly={!isEditMode} onChange={(e) => handleDielectricVFIChange(i, 'vfiIdentifier', e.target.value)} />
                    </td>
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1"><input className={`form-input text-xs text-center ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={row.serialNumber} readOnly={!isEditMode} onChange={(e) => handleDielectricVFIChange(i, 'serialNumber', e.target.value)} /></td>
                    {(['a','b','c'] as (keyof DielectricVFIRow)[]).map(key => (
                      <td key={String(key)} className="border border-gray-300 dark:border-gray-600 px-1 py-1">
                        <input className={`form-input text-xs text-center ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={(row as any)[key] as string} readOnly={!isEditMode} onChange={(e) => handleDielectricVFIChange(i, key, e.target.value)} />
                      </td>
                    ))}
                    <td className="border border-gray-300 dark:border-gray-600 px-1 py-1">
