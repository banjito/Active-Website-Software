...
export default function EquipmentManagement({ division, portal }: EquipmentManagementProps) {
  ...
  useEffect(() => {
    async function fetchEquipment() {
      setIsLoading(true);
      try {
        const result = await equipmentService.getAllEquipment(portal, division as string);
        if (result.data) {
          setEquipment(result.data as Equipment[]);
          setFilteredEquipment(result.data as Equipment[]);
        }
      } catch (error) {
  ...
  useEffect(() => {
    const loadFilterOptions = async () => {
      try {
        ...
        const statuses = await equipmentService.getEquipmentStatuses();
