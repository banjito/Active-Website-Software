...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const maybeApplyAutoName = async (u: User | null) => {
    try {
      ...
      if (current !== derived) {
        ...
        const { error: updErr } = await supabase.auth.updateUser({ data: { name: derived } });
        if (updErr) {
  ...
  const refreshUser = async () => {
    try {
      ...
      console.log('🔄 refreshUser called', { timeSinceLastRefresh, cooldown: REFRESH_COOLDOWN_MS });
  ...
  const softRefresh = async (): Promise<boolean> => {
    try {
      console.log('🔃 softRefresh: Starting session recovery (like sign-out/sign-in but without signing out)...');
  ...
  useEffect(() => {
    ...
    const timeoutPromise = new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error('Auth session timeout')), INITIAL_SESSION_TIMEOUT_MS)
    );

    const applySessionResult = (result: { data: { session: any }; error: any }) => {
      if (!mounted) return;
      ...
      if (session?.expires_at) {
        ...
        if (diff < 0) {
          console.error('⚠️ TOKEN APPEARS EXPIRED - Check if system clock is correct!');
        }
      }
      ...
      if (sessionError) {
        ...
        const isUnrecoverableAuthError = !isRateLimitError && !isNetworkError && (
        ...
        if (isRateLimitError) {
          console.warn('Rate limit hit - waiting before retry. Do NOT clear tokens.');
          setUser(null);
        } else if (isNetworkError) {
          console.warn('Auth server unreachable (CORS/522/network). Showing login; tokens kept for retry.');
          setUser(null);
        } else if (isUnrecoverableAuthError) {
          console.error('Unrecoverable auth error detected - clearing stored auth');
          try {
    ...
    Promise.race([sessionPromise, timeoutPromise])
      ...
      .catch((err) => {
        ...
        const isRateLimitError = (err as any)?.status === 429 || errorMessage.includes('rate limit');
        ...
        if (isTimeout) {
          console.warn('Auth session timed out (Supabase may be slow or unreachable). Showing login.');
          setUser(null);
        } else if (isRateLimitError) {
          console.warn('Rate limit hit during session init - waiting before retry');
          setUser(null);
        } else if (isNetworkError) {
          console.warn('Auth server unreachable. Showing login; tokens kept for retry.');
          setUser(null);
        } else if (isUnrecoverableAuthError) {
          try {
            localStorage.removeItem('supabase.auth.token');
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === 'SIGNED_OUT' && user) {
        console.error('⚠️ SIGNED_OUT received while user exists - investigating...');
    ...
    const handleVisibilityChange = async () => {
      ...
      try {
        ...
        if (sessionError || !sessionData?.session) {
          ...
          if (!success) {
            console.warn('  ↳ softRefresh failed - user may need to sign in again');
          }
        } else {
          // Session exists, but let's verify the token is actually working
          const { error: userError } = await supabase.auth.getUser();
          if (userError) {
            console.log('  ↳ getUser failed despite session existing, triggering softRefresh...');
            await softRefresh();
          } else {
  ...
  return (
    <AuthContext.Provider value={{ user, setUser, loading, signOut, refreshUser, softRefresh }}>
      {children}
