...
export const CompensationView: React.FC<CompensationViewProps> = ({
  profileId,
  employeeName,
}) => {
  const { user } = useAuth();
  ...
  const fetchData = async () => {
    ...
    try {
      const { data: profileData } = await supabase
        ...
        .from("profiles")
        .select(
          "current_compensation_amount, current_pay_type, current_pay_frequency",
        )
        .eq("id", profileId)
        .single();
      setCurrent(
        profileData
          ? {
              current_compensation_amount:
                profileData.current_compensation_amount ?? null,
      ...
      const { data: historyData } = await supabase
        ...
        .select(
          "id, amount, pay_type, pay_frequency, effective_from, created_at",
        )
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Compensation on file for {employeeName}
          </CardDescription>
        </CardHeader>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    inputMode="decimal"
                    placeholder={
                      payTypeInput === "hourly" ? "e.g. 45.00" : "e.g. 85000"
                    }
                    value={amountInput}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Previous compensation records for {employeeName}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {history.length === 0 ? (
