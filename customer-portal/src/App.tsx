...
function RequireCustomer({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading && (!user || !isCustomer)) resetBranding();
  }, [isCustomer, loading, resetBranding, user]);
      ...
      <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4">
        <Card className="w-full max-w-md animate-scale-in shadow-lift">
          ...
          <CardContent className="space-y-4 text-sm text-muted-foreground">
            <p>
              This account isn't linked to a customer organization. If you
              received an invite, open the link in that email to finish setup.
            </p>
