        ...
        if (currentMonth >= startOfMonth(goalStart)) {
          ...
          if (currentMonth < startOfMonth(now)) {
            ...
            const elapsedDuration =
              currentMonth.getTime() - goalStart.getTime();
            const progress = Math.min(
              1,
              Math.max(0, elapsedDuration / totalDuration),
            );
          ...
          // For current and future months
            ...
            if (currentProgress >= 1) {
              ...
              const progressRate =
                timeElapsed > 0 ? currentProgress / (timeElapsed / 100) : 0;
              const totalMonths =
                (goalEnd.getFullYear() - goalStart.getFullYear()) * 12 +
                (goalEnd.getMonth() - goalStart.getMonth());
              const monthsPassed =
                (now.getFullYear() - goalStart.getFullYear()) * 12 +
              ...
              const thisMonthNum =
                (currentMonth.getFullYear() - goalStart.getFullYear()) * 12 +
              ...
              const trendProjection =
                progressRate > 0
                  ? goal.currentValue *
                    (1 +
                      ((thisMonthNum - monthsPassed) / monthsPassed) *
                        progressRate)
                  : linearProjection;

              dataPoint.projected! += Math.min(
                trendProjection,
                goal.targetValue,
              );
            }
          ...
          if (currentMonth <= startOfMonth(goalEnd)) {
            ...
            const goalEndInMonth =
              goalEnd.getMonth() === currentMonth.getMonth() &&
            ...
            if (goalEndInMonth) {
              ...
              dataPoint.target! +=
                (goal.targetValue * daysToEndInMonth) / daysInMonth;
            } else {
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-4">
          ...
          <Select
            ...
            onChange={(e) =>
              setForecastPeriod(
                e.target.value as "3months" | "6months" | "12months",
              )
            }
      ...
      <Card>
        <div className="p-6">
          ...
          {forecastData.length > 0 ? (
                ...
                >
                  ...
                  <Tooltip
                    formatter={(value, name) => {
                      if (name === "actual")
                        return [formatTooltipValue(value as number), "Actual"];
                      if (name === "projected")
                        return [
                      ...
                      if (name === "target")
                        return [formatTooltipValue(value as number), "Target"];
                      return [value, name];
                    }}
            ...
            <div className="flex justify-center items-center h-96 text-neutral-500">
        ...
        <div className="p-6">
          ...
          {forecastData.length > 0 ? (
            <div className="space-y-4">
              ...
              {(() => {
                const latestActual =
                  forecastData.find((d) => d.actual! > 0)?.actual || 0;
                const latestTarget =
                  forecastData.find((d) => d.target! > 0)?.target || 0;
                const finalProjected =
                  forecastData[forecastData.length - 1]?.projected || 0;
                const finalTarget = forecastData.reduce(
                  (sum, point) => sum + (point.target || 0),
                  0,
                );
                const percentOfTarget =
                  finalTarget > 0 ? (finalProjected / finalTarget) * 100 : 0;
                  ...
                  if (percentOfTarget >= 100) {
                    return (
                      <p className="text-green-500">
                        You're on track to exceed your revenue targets. Consider
                        setting more ambitious goals for the next cycle.
                      </p>
                    );
                  } else if (percentOfTarget >= 85) {
                    return (
                      <p className="text-amber-500">
                        You're close to your targets but may fall slightly
                        short. Focus on closing high-value deals in the pipeline
                        to bridge the gap.
                      </p>
                    );
                  } else {
                    return (
                      <p className="text-red-500">
                        You're significantly behind your revenue targets. Review
                        your sales strategy and consider adjusting your targets
                        or implementing new tactics to accelerate growth.
                      </p>
                    );
                  }
                })()}
