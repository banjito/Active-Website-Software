...
const TwelveSetsLowVoltageCableTestForm: React.FC = () => {
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      if (reportData && reportData.data) {
        ...
        // Migrate old From/To data format to new format with newlines for existing reports
        const migratedTestSets = (reportData.data.testSets || []).map(set => ({
          ...set,
          from: typeof set.from === 'string' ? set.from.replace(/-/g, '-\n') : set.from,
          to: typeof set.to === 'string' ? set.to.replace(/-/g, '-\n') : set.to,
        }));
    ...
    style.textContent = `
      ...
      @media print {
        ...
        table.electrical-tests-table td:nth-child(2) {
          width: 120px !important; /* Increased width for wrapped text */
        ...
        table.electrical-tests-table tbody td[rowspan="2"]:nth-child(2) {
          ...
          max-width: 120px !important;
          height: auto !important;
          min-height: 48px !important; /* Increased height for wrapped text */
          max-height: none !important;
          padding: 2px !important; /* Increased padding for wrapped text */
          box-sizing: border-box !important;
          overflow: visible !important;
  ...
  const handleTestSetChange = (setId: number, field: keyof Pick<TestSet, 'from' | 'to' | 'size' | 'config' | 'result'>, value: string) => {
    // Auto-wrap text at hyphens for From and To fields
    let processedValue = value;
    if ((field === 'from' || field === 'to') && value.includes('-')) {
      // Replace hyphens with hyphen + line break for automatic wrapping
    ...
    try {
        ...
        if (reportId) {
            ...
            const { data: updateData, error: updateError } = await supabase
                .schema('neta_ops')
                .from('low_voltage_cable_test_12sets')
                .update({ data: reportPayload.data, updated_at: new Date() })
                .eq('id', reportId)
                .select();
            if (updateError) {
            ...
            if (!updateData || updateData.length === 0) {
                throw new Error('No rows were updated. Report may not exist.');
            }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 print:hidden job-info-onscreen">
              {/* Column 1 */}
                ...
                <div className="mb-4">
                  ...
                  <input id="customer" name="customer" type="text" value={formData.customer} readOnly className="form-input bg-gray-100 dark:bg-dark-150" />
                </div>
                <div className="mb-4">
                  <label htmlFor="address" className="form-label inline-block w-32">Address:</label>
                  <input id="address" name="address" type="text" value={formData.address} readOnly className="form-input bg-gray-100 dark:bg-dark-150" />
                </div>
                <div className="mb-4">
