...
- `node --import ./scripts/ts-alias-loader.mjs scripts/custom-forms-regression.ts`
  (also `npm run custom-forms-regression`) runs 739 checks: adapter fidelity,
...
#### Generate from File (added alongside phase 5)

**The model drives the builder's own tools; it no longer writes documents.**
Asking for a whole template as one JSON document failed on the first real
workbook and kept failing in new ways: the answer outgrew the provider's 8192
token output ceiling, arrived truncated, then arrived whole but wrapped in
prose, then arrived as two overlapping answers when the provider ignored prefix
continuation. Each fix bought one more round.

`excel/tools.ts` exposes six authoring operations (`add_section`, `add_table`,
`add_header_row`, `set_column_text`, `add_note`, `finish`) and `excel/build.ts`
runs the conversation. The model supplies what a person reading the sheet would
supply, labels, types and source ranges; identifiers are generated by the
application, and formulas remain untouchable. This is not a smaller prompt, it
is a different contract:

- **No answer is large enough to truncate.** One call is tens of characters, so
  the output ceiling stops being a constraint rather than being worked around.
- **A bad step fails alone.** `applyToolCall` refuses what the builder cannot
  represent and returns a message written for the model to act on ("declares 1
  columns but A2:C4 is 3 wide"), which it corrects on the next step. A refusal
  costs one step, never the run.
- **Every intermediate state is a valid draft.** A run that is stopped, hits its
  budget or loses the provider still yields a form of whatever was built, marked
  INCOMPLETE in its review notes. Before this, a failure produced nothing.
- **The builder cannot be asked for something it cannot render.** The operations
  are the same authoring code the UI calls, already covered by this suite.

**Columns are named by their first data cell, not by a rectangle.** The first
real workbook (LV Molded Case Circuit Breaker ATS 25: one sheet, 305 filled
cells, 65 formulas and **417 merged ranges**) is a paper form drawn in Excel,
where eight columns a reader sees are twenty-seven columns of the raw grid.
Requiring a range whose width matched the declared column count forced the model
to pad tables with empty columns, and Device Settings came out as a 20-column
ruin. Each column now gives the cell it occupies in the first data row, the row
step is read from the workbook's own merge heights, and the sheet description
marks merged cells as `[spans 5 cols]` so the model sees the columns a person
sees.

**Any section can be rebuilt from a written instruction, in the builder.** The
sparkles control on a section opens a prompt ("four columns: Function, Setting,
Delay, I2t, with six rows"); the section is replaced and nothing else in the
form moves. It keeps its id, its place in the order and its reference code,
which formulas elsewhere may already point at. Nothing about it is specific to
an imported form, and it deliberately has no workbook: by the time someone is
looking at a section they want changed, the file it came from is long gone. It
is refused on a template using typed calculations, where replacing fields would
leave formulas pointing at fields that no longer exist.

**One section can be built again without touching the rest.** A model reading a
merged sheet will get a section wrong now and then, and rerunning the whole
workbook to fix one table throws away everything it got right. `rebuildSection`
replaces a single section and its mappings, in place, told what was there before
and what the reviewer says is wrong with it. A rebuild that produces nothing
leaves the form exactly as it was.

**`finish` is refused while a calculated cell has nowhere to go.** The first
tool-driven run of the MCCB workbook built fifteen sections and carried over
**none** of its sixty-five formulas: whole calculated columns had no field, so
every formula depending on them failed in turn. A model cannot see that from
inside its own conversation; the application can compute it exactly. `finish`
now returns the missing addresses and what they calculate, up to three times,
after which the run may end with the gaps recorded rather than being held in a
loop it cannot escape.

**Field types are read from the workbook, not guessed.** A column a model left
as text cannot carry arithmetic: the type checker rejects it and the calculation
is dropped, which is how a run can map every cell correctly and still carry
nothing over. Where every mapped cell of a column holds a number, the column
becomes numeric. A declared choice or date is never overruled.

**Formula-pattern support (September 2026).** Translation now handles `X=""`
and `X<>""`, blank `IF` outputs, `TRUE()`/`FALSE()`, `ISNUMBER`, literal text
equality, single-cell `COUNTIF` text patterns such as `"*<*"`, `"*>*"` and
`"N/A"`, and the trip-label predicate `ISNUMBER(FIND("T", cell))`. Two additive
engine functions (`isnumber` and `textmatches`) provide these tests without
changing existing calculation behavior or converting text readings into numbers.
Wildcard matching is bounded and non-recursive; FIND searches remain case-sensitive.

This is a bounded subset, not a general Excel engine. Standalone `FIND`, empty
searches, start positions other than 1, range/numeric/comparison/blank COUNTIF
criteria, mixed number/`"N/A"` results, nonzero ROUND precision and broken
`#REF!` links remain refused. COUNTIF criteria resembling logical or numeric
formats (including digits, percentages and currency symbols) are conservatively
refused. Invalid typed inputs and calculation errors remain errors even inside
`ISNUMBER`; unlike Excel, it does not turn errors into `FALSE`.

Blank numeric IF results display as missing values. The importer tracks them
through formula dependencies so downstream arithmetic cannot silently treat
Excel empty text as zero; affected calculations remain review items. Blank
saved results and missing caches remain unverified. Regression fixtures cover
filled, zero, blank and prefixed readings, trip-type changes, escaped wildcards,
and a real XLSX round-trip whose deliberately wrong cache is reported in the
non-printing notes. This does not certify the MCCB workbook or its layout.

**A formula that returns "N/A" now comes across as a blank number.** A form
cannot hold text in a numeric cell: the type checker refuses it, and refusing
cost eleven of the MCCB sheet's formulas. A literal that means "there is no
number here" (`""`, `N/A`, `NA`, `-`, en and em dashes) opposite a numeric
branch becomes a missing value, and each one is reported against its cell, so
the marker's disappearance is visible rather than silent. A text result that is
not one of those markers is still refused: `IF(x, "SEE NOTE", y)` is a mistake
worth stopping for, and `IF(x, "N/A", IF(y, "PASS", "FAIL"))` stays text,
because there "N/A" is a verdict, not a missing number.

`ROUND` accepts a literal 0 to 6 decimals rather than 0 alone, with the caveat
stated per cell: both engines round a binary double, so a value sitting exactly
on a half can differ from Excel in the last digit. `EXP` was added to the typed
engine for correction factors of the form `0.1758 * exp(0.0256 * C)`; an
overflow is an error, never infinity.

On the MCCB workbook this takes translation from 33 of 65 formulas, to 43 after
Astra's `ISNUMBER`/`COUNTIF`/`FIND`/blank-test work, to **56 of 65**. The nine
that remain contain `#REF!`: they are broken in the source file and no importer
can recover them.

The loop runs in the browser so each provider call is short, progress is visible
step by step next to the form taking shape, and it can be stopped. It is bounded
by steps, consecutive failures and wall clock, and it stops on its own when a
model repeats a failing call or stops calling tools at all.

The older single-document path (`validateExcelResponse`, continuation, range
sources) is still present and still tested; it is what `Generate from File` falls
back to conceptually, and what the notes below describe.

#### Excel import, the single-document path (superseded)
...
Parsing is not coverage: a layout can compile and agree with every formula it
kept while leaving a whole table out. Every filled cell no section covers is
counted per sheet, so a workbook that came across half-imported says so.
Headings and labels are expected to be among them, which is why this is a count
to compare against the workbook rather than a defect list.
...
`src/lib/customForms/excel/excel.regression.ts` (26 checks, in the main suite)
...
checking needs a stream from a real compressor, not one from SheetJS. The fixture
now explicitly asserts a dynamic-Huffman first block in the worksheet and checks
that its formula and saved result survive recompression and import.

Length-limited answers use **bounded continuation**, not unlimited output.
Only an explicit provider `length` finish starts another turn with the accumulated
assistant prefix (DeepSeek's beta prefix-completion endpoint). There are at most
eight turns, 1,000,000 bytes of answer text across all turns, and a 120-second
request deadline. The deadline cancels both pending provider requests and stalled
streams. The prefix still consumes provider context, so large or unusually complex
workbooks may need splitting. No live provider continuation check has been run.

Ordinary tables are requested as columns plus a row count, without `v2.body`.
Merged headers need only `v2.header`; mixed bodies use one fixed-size records
entry per uninterrupted data run. Required false-valued properties remain explicit.
This reduces layout output without changing the form schema or formula handling.

The stream reader handles fragmented UTF-8 and SSE events, LF/CRLF/CR line endings,
comments and multiline data. Malformed events, provider error events, a missing
finish reason or missing `[DONE]` are failures, never successful partial answers.
Only `stop` permits parsing and validation; valid-looking JSON cannot override an
unfinished result. Regression checks also cover continuation inside JSON escapes,
byte-budget boundaries, attempt exhaustion, cancellation and stalled connections.

JSON extraction remains a compatibility fallback for fences and prose; it is not
a substitute for provider-enforced JSON mode or application validation. Error
responses distinguish generation, interruption, timeout, size, JSON and layout
failures, with a reference ID for diagnostics. Neither model preambles nor raw
provider error bodies are echoed or logged: either can contain workbook data.

Rejections name their location. Validation aborts on the first problem, so a
cursor records the path (`sections[3] "readings" > columns[7]`) and every
message carries it: "Invalid or oversized text" alone was unactionable in a
layout of hundreds of fields. Paths hold ids and indices, never workbook text,
and the same message is logged with the request's correlation id.

Continuing from a prefix is a provider feature, not a guarantee. A provider that
ignores it answers again from the beginning, and joining that to what came
before yields two overlapping documents that parse as neither, surfacing much
later as an unexplained "the answer was not JSON". A restart is detected where
it happens (the new turn opens with the same first characters as the answer so
far) and reported as `did_not_continue`, with the partial text kept out of the
parser. A "not JSON" failure also reports how many turns it took, the offset
that would not parse, and whether the text contains more than one layout.

Continuation is **bounded recovery**, not unlimited output: each turn resends
the accumulated prefix, which still costs context, so the budget (attempts,
UTF-8 bytes, wall clock) is explicit and exhausting it is a stated failure, never
a silently accepted partial import. `parseCompletedExcelResponse` refuses an
answer that parses but whose provider never reported a clean stop.

Remaining: the edge function must be deployed for the Import Excel button to
work; the provider's real prefix-continuation behaviour is still unproven
against a live call (every streaming check here stubs the provider); and no
engineer has yet reviewed an imported draft end to end.
...
- `npm run custom-forms-regression` — 739 checks, fast, no browser;
