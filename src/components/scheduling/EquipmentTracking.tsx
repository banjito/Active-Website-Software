...
export function EquipmentTracking({ portalType, division }: EquipmentTrackingProps) {
  ...
  const fetchEquipment = async () => {
    setLoading(true);
    try {
      const { data, error } = await equipmentService.getAllEquipment(portalType, division);
  ...
  const handleEquipmentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save equipment
      const { data, error } = await equipmentService.saveEquipment({
        ...equipmentData,
        portal_type: portalType,
        division
      });
          ...
          >
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                {item.assigned_to && item.assignedTo && (
                  <div className="flex items-center text-sm">
                    <User className="h-4 w-4 mr-2 text-gray-500" />
                    <span>Assigned to: {item.assignedTo.user_metadata?.name || item.assignedTo.email}</span>
                  </div>
                )}
