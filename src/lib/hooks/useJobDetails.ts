...
export function useJobDetails(jobId: string | undefined) {
  ...
  useEffect(() => {
    ...
    async function fetchJobDetails() {
      try {
        ...
        let { data: jobData, error: netaJobError } = await supabase
          .schema('neta_ops')
          .from('jobs')
          .select(`
            id,
        ...
        if (jobData && jobData.deleted_at) {
          throw new Error('This job has been deleted and is no longer accessible.');
        }
        ...
        if (!jobData && !netaJobError) {
          console.warn('useJobDetails: Got null data with no error - possible RLS issue. Testing table access...');
          ...
          if (testError) {
            console.error('useJobDetails: Cannot access neta_ops.jobs table:', testError);
            netaJobError = testError;
          } else {
            console.log('useJobDetails: Table is accessible, job just not found');
          }
        }
        ...
        if (netaJobError || !jobData) {
          console.log('Job not found in neta_ops.jobs, trying lab_ops.lab_jobs', { 
          ...
          if (labJobError) {
            console.error(`useJobDetails: Error fetching job data from lab_ops.lab_jobs:`, {
              error: labJobError,
              code: labJobError.code,
              message: labJobError.message
            });
            // If lab_ops table doesn't exist, that's okay - just use neta_ops error
            if (labJobError.code === 'PGRST116' || labJobError.message?.includes('404') || labJobError.message?.includes('not found')) {
              // Lab table doesn't exist, use the original neta_ops error
              if (netaJobError) {
                // Check for RLS/permission errors
                if (netaJobError.code === '42501' || netaJobError.message?.includes('permission denied') || netaJobError.message?.includes('row-level security')) {
                  throw new Error(`Permission denied: Unable to access job ${jobId}. Please check your authentication and permissions.`);
                }
                if (netaJobError.code === 'PGRST116' || netaJobError.message?.includes('404')) {
                  throw new Error(`Job ${jobId} not found in database`);
                }
                throw new Error(`Error loading job: ${netaJobError.message || 'Unknown error'}`);
              }
              throw new Error(`Job ${jobId} not found in database`);
            }
            throw labJobError;
        ...
        const jobDetailsData: JobDetails = {
          ...
          progress_billing_status: (jobData as any).progress_billing_status || null,
          ...
          quickbooks_project_name: (jobData as any).quickbooks_project_name ?? null,
  ...
  const getFormattedInfoForReports = () => {
    ...
    const preferredAddress = jobDetails.site_address || jobDetails.customer?.address || 'No address provided';
    return {
  ...
  return { jobDetails, loading, error, getFormattedInfoForReports, refreshJobDetails };
} 