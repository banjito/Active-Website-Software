...
export function DivisionAnalyticsDialog({ division, isOpen, onClose }: DivisionAnalyticsDialogProps) {
  ...
  async function fetchDivisionStats() {
    ...
    try {
      ...
      const totalValue = awardedOpps.reduce((sum, opp) => sum + (opp.quoted_amount || 0), 0);
      ...
      const lastMonthValue = lastMonthAwardedOpps.reduce((sum, opp) => sum + (opp.quoted_amount || 0), 0);
      ...
      const chartData = months.map(date => {
        ...
        const monthTotalValue = monthOpportunities.reduce((sum, opp) => sum + (opp.quoted_amount || 0), 0);
        return {
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-gray-50 dark:bg-dark-150 p-4 rounded-lg">
                  ...
                  >
                    ...
                    >
                      ...
                      <ChartTooltip 
                        ...
                        formatter={(value: number) => [formatValue(value), 'Awarded Value']}
                      />
                      <Bar 
                        dataKey="value" 
                        fill="#f26722"
