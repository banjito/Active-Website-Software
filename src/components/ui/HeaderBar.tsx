...
export const HeaderBar: React.FC<HeaderBarProps> = ({ onEnterEditMode, className }) => {
  ...
  const fetchAssetsByStatus = async (status: 'ready_for_review' | 'approved' | 'issue') => {
    ...
    const jobById: Record<
      string,
      { id: string; title: string; job_number?: string; deleted_at?: string | null }
    > = {};
    (jobs || []).forEach((j) => {
      jobById[j.id] = j;
    });
  ...
  const markAllSeenInCurrentStatus = () => {
    ...
    if (detailStatus === 'needs_calibration' || detailStatus === 'equipment_out_of_cal') return;
  ...
  return (
    <>
      <div className={`bg-white dark:bg-dark-150 p-4 border-none ${className ?? ''}`}>
        <div className="flex items-center w-full min-w-0">
          ...
          >
            <div className="relative flex h-10 w-10 items-center justify-center" ref={shortcutMenuRef}>
              <button
                type="button"
                      ...
                      <div className="divide-y divide-gray-200 dark:divide-dark-200">
                        ...
                        >
                          ...
                          <div className="text-xs text-gray-600 dark:text-white">
                            ...
                            {calibrationNeedsList.length === 1 ? 'item' : 'items'}
                          </div>
                        </button>
                        <button
                        ...
                        >
                          ...
                          <div className="text-xs text-gray-600 dark:text-white">
                            ...
                            {calibrationOutList.length === 1 ? 'item' : 'items'}
                          </div>
                        </button>
                      </div>
                              ...
                              <div key={jg.jobId} className="px-4 py-3">
                                <div className="flex items-start justify-between gap-3">
                                  <div className="min-w-0">
                                    <div className="flex items-center gap-2">
                                      <button
                                        onClick={() => goToJobAssets(jg.jobId)}
                                        className="text-sm font-medium text-gray-900 dark:text-white hover:underline"
                                      >
                                        {jg.jobNumber ? `Job ${jg.jobNumber}` : 'Job'} •{' '}
