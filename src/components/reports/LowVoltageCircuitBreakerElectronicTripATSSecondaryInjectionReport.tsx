...
interface FormData {
  ...
  testEquipment: {
    ...
    lowResistanceOhmmeter: { name: string; serialNumber: string; ampId: string; calDate: string }; // Added based on image
    secondaryInjectionTestSet: { name: string; serialNumber: string; ampId: string; calDate: string }; // Added based on image
    primaryVerificationTestSet: { name: string; serialNumber: string; ampId: string; calDate: string };
  };
...
const LowVoltageCircuitBreakerElectronicTripATSSecondaryInjectionReport: React.FC = () => {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    testEquipment: {
      ...
      secondaryInjectionTestSet: { name: '', serialNumber: '', ampId: '', calDate: '' },
      primaryVerificationTestSet: { name: '', serialNumber: '', ampId: '', calDate: '' }
    },
    comments: '',
    ...
    try {
      ...
      if (generic) {
        ...
        setFormData(prev => ({
          ...
          tripUnitType: data.nameplate_data?.tripUnitType ?? prev.tripUnitType,
          ratingPlug: data.nameplate_data?.ratingPlug ?? prev.ratingPlug,
          curveNo: data.nameplate_data?.curveNo ?? prev.curveNo,
          chargeMotorVoltage: data.nameplate_data?.chargeMotorVoltage ?? prev.chargeMotorVoltage,
          operation: data.nameplate_data?.operation ?? prev.operation,
          mounting: data.nameplate_data?.mounting ?? prev.mounting,
          zoneInterlock: data.nameplate_data?.zoneInterlock ?? prev.zoneInterlock,
          ...
          testEquipment: {
            ...
            secondaryInjectionTestSet: { ...prev.testEquipment.secondaryInjectionTestSet, ...(data.test_equipment?.secondaryInjectionTestSet || {}) },
            primaryVerificationTestSet: { ...prev.testEquipment.primaryVerificationTestSet, ...(data.test_equipment?.primaryVerificationTestSet || {}) },
          },
  ...
  return (
    <ReportWrapper isPrintMode={isPrintMode}>
          ...
          <div className="mb-6 print:hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <EquipmentAutocomplete
                  value={formData.testEquipment.secondaryInjectionTestSet.name}
                  onChange={(value) => handleChange('testEquipment.secondaryInjectionTestSet.name', value)}
                  onSelect={(equipment) => {
                    const formatDate = (dateString: string | null): string => {
                      if (!dateString) return '';
                      try {
              ...
              <div><label className="form-label">Cal Date:</label><input type="text" value={formData.testEquipment.secondaryInjectionTestSet.calDate} onChange={(e) => handleChange('testEquipment.secondaryInjectionTestSet.calDate', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
              <div>
                <label className="form-label">Primary Verification Set:</label>
                <EquipmentAutocomplete
                  value={formData.testEquipment.primaryVerificationTestSet.name}
                  onChange={(value) => handleChange('testEquipment.primaryVerificationTestSet.name', value)}
                  onSelect={(equipment) => {
                    const formatDate = (dateString: string | null): string => {
                      if (!dateString) return '';
                      try {
              ...
              <div><label className="form-label">Cal Date:</label><input type="text" value={formData.testEquipment.primaryVerificationTestSet.calDate} onChange={(e) => handleChange('testEquipment.primaryVerificationTestSet.calDate', e.target.value)} readOnly={!isEditing} className={`form-input w-full ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
                    </div>
                    </div>
      ...
      `}</style>      {/* Mark Ready to Review Button */}
