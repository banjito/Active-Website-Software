...
import { FileCheck, CheckCircle, XCircle, PenTool, Download, Eye } from 'lucide-react';
import { offersService, Offer, ESignature } from '../../../services/hr/offersService';
...
export const ESignatures: React.FC = () => {
  ...
  const [signerType, setSignerType] = useState<'candidate' | 'manager' | 'hr'>('candidate');
  ...
  const openSignModal = (offer: Offer, type: 'candidate' | 'manager' | 'hr') => {
    setSelectedOffer(offer);
    setSignerType(type);
    if (type === 'candidate' && offer.candidate) {
      setSignerName(`${offer.candidate.first_name} ${offer.candidate.last_name}`);
  ...
  const handleSubmitSignature = async () => {
    ...
    try {
      const signatureInfo = {
        signer_id: signerType === 'candidate' ? selectedOffer.candidate_id : user?.id,
      ...
      await offersService.createSignature(selectedOffer.id, signerType, signatureInfo);
  ...
  const pendingSignatures = offers.filter(o => o.status === 'sent' && o.signature_status === 'pending');
  const signedOffers = offers.filter(o => o.signature_status === 'signed' || o.status === 'accepted');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">E-Signatures</h1>
        ...
        <CardContent>
          {pendingSignatures.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-white">No pending signatures</h3>
            </div>
          ) : (
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                      Sent: {offer.sent_date ? new Date(offer.sent_date).toLocaleDateString() : 'N/A'}
                    </div>
                  </div>
        ...
        <CardContent>
          {signedOffers.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-white">No signed offers</h3>
            </div>
          ) : (
            <div className="space-y-3">
              {signedOffers.map((offer) => (
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                      Signed: {offer.signed_at ? new Date(offer.signed_at).toLocaleDateString() : 'N/A'}
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button
      ...
      <Dialog open={isSignModalOpen} onOpenChange={setIsSignModalOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Sign the offer letter electronically. Draw your signature in the box below.
            </DialogDescription>
          </DialogHeader>
            ...
            {selectedOffer && (
              <>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                      {selectedOffer.offer_date 
                        ? new Date(selectedOffer.offer_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: selectedOffer.offer_letter_content || '<p>No offer content available</p>' }}
                />

                {/* Signature Section */}
                {signatures.length > 0 && (
                  <div className="mt-8 pt-6 border-t-2 border-gray-300">
                    <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Electronic Signature</h2>
                      ...
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            ...
                            <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                              Signed as: {signature.signer_type.charAt(0).toUpperCase() + signature.signer_type.slice(1)}
                            </div>
                          </div>
                        ...
                        {signature.signature_image && (
                          <div className="mt-3 border-2 border-gray-300 dark:border-gray-600 rounded-lg p-4 bg-white dark:bg-gray-900">
                            <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">Signature:</div>
                            <img
                ...
                {signatures.length === 0 && (
                  <div className="text-center py-8 text-gray-500 dark:text-gray-400 border-t-2 border-gray-300 mt-8 pt-6">
