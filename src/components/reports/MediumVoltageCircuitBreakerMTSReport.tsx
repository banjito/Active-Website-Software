...
const MediumVoltageCircuitBreakerMTSReport: React.FC = () => {
  ...
  const initialFormData: FormData = {
    ...
    testEquipment: {
      megohmmeter: {
        model: '', serialNumber: '', ampId: '', calDate: '',
      },
      lowResistanceOhmmeter: {
        model: '', serialNumber: '', ampId: '', calDate: '',
      },
    ...
    const loadReportData = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          // Normalize test equipment so model is set from equipment_name if present (e.g. from import)
          const te = payload.testEquipment || {};
          const normalizeTe = (item: any) => ({
            ...(item || {}),
            model: item?.model || item?.equipment_name || '',
          });
          const testEquipment = {
            ...(te as object),
            megohmmeter: normalizeTe(te.megohmmeter),
            lowResistanceOhmmeter: normalizeTe(te.lowResistanceOhmmeter),
            hipot: normalizeTe(te.hipot),
          };

          const newFormData = {
            ...initialFormData,
            ...payload,
            status: (payload.status as 'PASS' | 'FAIL' | 'LIMITED SERVICE') || 'PASS',
    ...
    <ReportWrapper isPrintMode={isPrintMode}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 print:hidden job-info-onscreen">
              ...
              <div>
                <div className="mb-4 flex items-center">
                  ...
                  <input id="customer" type="text" value={maskCustomerName(formData.customer)} onChange={(e) => handleInputChange(e)} name="customer" readOnly={!isEditing} className="form-input flex-1" />
                </div>
                <div className="mb-4 flex items-center">
                ...
                <div className="mb-4 flex items-center">
                  ...
                  <EquipmentAutocomplete
                    ...
                    onChange={(value) => handleInputChange({ target: { name: 'identifier', value } } as any)}
                    onSelect={(equipment) => {
                      handleInputChange({ target: { name: 'identifier', value: equipment.equipment_name } } as any);
                    }}
                    readOnly={!isEditing}
                    className="flex-1"
              ...
              <div>
                ...
                <div className="mb-4">
                  <div className="flex items-center space-x-1 mb-2">
                    ...
                    <input type="number" value={formData.temperature.celsius} readOnly className="form-input w-12 bg-gray-100 dark:bg-dark-150" />
                    <span className="text-xs">°C</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <label className="form-label inline-block w-auto">TCF:</label>
                    <input type="number" value={formData.temperature.tcf.toFixed(3)} readOnly className="form-input w-20 bg-gray-100 dark:bg-dark-150" />
                  </div>
                </div>
                <div className="mb-4 flex items-center">
      ...
      <section className="mb-6">
        ...
        <div className="overflow-x-auto">
          ...
          <table className="mt-4 border-collapse border border-gray-200 dark:border-gray-700">
            <thead>
              <tr>
                <th className="px-3 py-2 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-dark-150">
                  E-Gap
                </th>
                <th className="px-3 py-2 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-dark-150">
                  Unit Measurement
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-4 print:hidden">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-white mb-1">Test Voltage</label>
                {renderInput("dielectricWithstand.closed.testVoltage", "", "text", false, "w-full")}
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-white mb-1">Test Duration</label>
            ...
            <div className="lg:col-span-3">
              <table className="w-full table-fixed border-collapse border border-gray-300 dark:border-gray-600">
                ...
                <thead className="bg-gray-50 dark:bg-dark-150">
                  <tr>
                    <th colSpan={4} className="px-4 py-3 text-center text-sm font-medium text-gray-700 dark:text-white border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-dark-150">
                      Dielectric Withstand (Breaker In Closed Position)
                    </th>
                  </tr>
                  <tr>
      ...
      <section className="mb-6">
        ...
        <div className="print:hidden test-eqpt-onscreen">
          {/* Compact grid: one row per equipment type so all fields sit beside each other */}
          <div className="grid gap-x-3 gap-y-3 items-center" style={{ gridTemplateColumns: 'minmax(0, auto) minmax(140px, 1fr) minmax(70px, 0.9fr) minmax(64px, 0.7fr) minmax(72px, 0.8fr)' }}>
            {/* Header row (optional labels - hidden on small screens or keep for clarity) */}
            <div className="text-xs font-medium text-gray-500 dark:text-gray-400 col-span-1">&nbsp;</div>
            <div className="text-xs font-medium text-gray-500 dark:text-gray-400">Model</div>
            <div className="text-xs font-medium text-gray-500 dark:text-gray-400">Serial #</div>
            <div className="text-xs font-medium text-gray-500 dark:text-gray-400">AMP ID</div>
            <div className="text-xs font-medium text-gray-500 dark:text-gray-400">Cal Date</div>

            {/* Megohmmeter */}
            <label className="text-sm font-medium text-gray-900 dark:text-white whitespace-nowrap">Megohmmeter:</label>
            <div className="min-w-0">
              <EquipmentAutocomplete
                value={(formData.testEquipment?.megohmmeter?.model as string) || (formData.testEquipment?.megohmmeter as any)?.equipment_name || ''}
                onChange={(value) => handleInputChange({ target: { name: 'testEquipment.megohmmeter.model', value } } as any)}
                onSelect={(equipment) => {
                  setFormData(prev => ({
                    ...prev,
                    testEquipment: {
            ...
            <div className="min-w-0">{renderInput("testEquipment.megohmmeter.serialNumber", "", "text", false, "w-full min-w-0")}</div>
            <div className="min-w-0">{renderInput("testEquipment.megohmmeter.ampId", "", "text", false, "w-full min-w-0")}</div>
            <div className="min-w-0">{renderInput("testEquipment.megohmmeter.calDate", "", "text", false, "w-full min-w-0")}</div>

            {/* Low-Resistance Ohmmeter */}
            <label className="text-sm font-medium text-gray-900 dark:text-white whitespace-nowrap">Low-Res. Ohmmeter:</label>
            <div className="min-w-0">
              <EquipmentAutocomplete
                value={(formData.testEquipment?.lowResistanceOhmmeter?.model as string) || (formData.testEquipment?.lowResistanceOhmmeter as any)?.equipment_name || ''}
                onChange={(value) => handleInputChange({ target: { name: 'testEquipment.lowResistanceOhmmeter.model', value } } as any)}
                onSelect={(equipment) => {
                  setFormData(prev => ({
                    ...prev,
                    testEquipment: {
            ...
            <div className="min-w-0">{renderInput("testEquipment.lowResistanceOhmmeter.serialNumber", "", "text", false, "w-full min-w-0")}</div>
            <div className="min-w-0">{renderInput("testEquipment.lowResistanceOhmmeter.ampId", "", "text", false, "w-full min-w-0")}</div>
            <div className="min-w-0">{renderInput("testEquipment.lowResistanceOhmmeter.calDate", "", "text", false, "w-full min-w-0")}</div>

            {/* Hipot */}
            <label className="text-sm font-medium text-gray-900 dark:text-white whitespace-nowrap">Hipot:</label>
            <div className="min-w-0">
              <EquipmentAutocomplete
                value={(formData.testEquipment?.hipot?.model as string) || (formData.testEquipment?.hipot as any)?.equipment_name || ''}
                onChange={(value) => handleInputChange({ target: { name: 'testEquipment.hipot.model', value } } as any)}
                onSelect={(equipment) => {
                  setFormData(prev => ({
                    ...prev,
                    testEquipment: {
            ...
            <div className="min-w-0">{renderInput("testEquipment.hipot.serialNumber", "", "text", false, "w-full min-w-0")}</div>
            <div className="min-w-0">{renderInput("testEquipment.hipot.ampId", "", "text", false, "w-full min-w-0")}</div>
            <div className="min-w-0">{renderInput("testEquipment.hipot.calDate", "", "text", false, "w-full min-w-0")}</div>
          </div>
        </div>
            ...
            <tbody>
              <tr>
                <td className="p-2 border border-gray-300 print:border-black">
                  ...
                  <div className="text-xs">Cal Date: {formData.testEquipment.megohmmeter.calDate || ''}</div>
                </td>
                <td className="p-2 border border-gray-300 print:border-black">
                  ...
                  <div className="text-xs">Cal Date: {formData.testEquipment.lowResistanceOhmmeter.calDate || ''}</div>
                </td>
                <td className="p-2 border border-gray-300 print:border-black">
                  ...
                  <div className="text-xs">Cal Date: {formData.testEquipment.hipot.calDate || ''}</div>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      {/* Comments */}
      <section className={`mb-6 print:break-before-page comments-section print:break-inside-avoid ${!formData.comments?.trim() ? 'print:hidden' : ''}`}>
        <div className="w-full h-1 bg-[#f26722] mb-4"></div>
  ...
  style.textContent = `
    @media print {
      ...
      * { color: black !important; }
      ...
      /* Comments section specific styling for print */
      .comments-section {
      ...
      /* Ensure comments table has proper spacing */
