...
export const GoalPerformanceTable: React.FC<GoalPerformanceTableProps> = ({
    ...
    const loadGoals = async () => {
      try {
        ...
        updatedTeams.forEach((team) => {
          ...
          team.goalsOnTrack = calculateTeamGoalsByStatus(
            team.goals,
            "on-track",
          );
        ...
        setError(
          "Failed to load goal performance data. Please try again later.",
        );
  ...
  const calculateTeamGoalsByStatus = (
    teamGoals: SalesGoal[],
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-4">
          <Select
            value={currentScope}
            onChange={(e) =>
              setCurrentScope(e.target.value as "Individual" | "Team" | "All")
            }
                ...
                <TableRow key={team.teamId}>
                  ...
                  <TableCell>
                    <div className="flex items-center space-x-2">
                      <div className="w-32 bg-neutral-200 rounded-none h-2.5">
                        <div
                          className="bg-blue-600 h-2.5 rounded-none"
                  ...
                  <TableCell>
                    <div className="flex space-x-2">
                      ...
                      >
                        {team.goalsOnTrack} on track
                      </Badge>
                      <Badge
                        variant="secondary"
                        className="bg-amber-100 text-amber-800"
                      >
                        {team.goalsAtRisk} at risk
                      </Badge>
                      <Badge variant="destructive">
                        {team.goalsBehind} behind
                      </Badge>
                    </div>
                  </TableCell>
                </TableRow>
              ...
              {teams.flatMap((team) =>
                team.members.map((member) => {
                  ...
                  const memberGoals = goals.filter(
                    (g) => g.ownerId === member.id || g.scope === "Individual",
                  );
                  const avgCompletion =
                    memberGoals.length > 0
                      ? Math.round(
                          memberGoals.reduce(
                            (sum, g) =>
                              sum +
                              calculateProgress(g.currentValue, g.targetValue),
                            0,
                          ) / memberGoals.length,
                        )
                      : 0;
