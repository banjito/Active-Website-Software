...
export const CompensationHistory: React.FC = () => {
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      ...
      const { data: usersData, error: usersErr } = await supabase
        .schema("common")
        .rpc("admin_get_users");
      const allUsers: any[] = usersErr
        ? (await supabase.rpc("admin_get_users")).data || []
        : usersData || [];
      if (!allUsers?.length) {
        setEmployees([]);
        return;
      ...
      const { data: profilesData } = await supabase
        ...
        .from("profiles")
        .select(
          "id, full_name, current_compensation_amount, current_pay_type, current_pay_frequency",
        );
        ...
        .map((u: any) => {
          ...
          return {
            ...
            current_compensation_amount:
              profile?.current_compensation_amount ?? null,
  ...
  const fetchHistory = useCallback(async (profileId: string) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(
          "id, profile_id, amount, pay_type, pay_frequency, effective_from, created_at",
        )
      ...
      toast({
        title: "Error",
        description: e?.message || "Failed to load compensation history",
        variant: "destructive",
      });
      setHistory([]);
    } finally {
  ...
  const handleUpdateCompensation = async () => {
    ...
    if (!canEditCompensation) {
      toast({
        title: "Access denied",
        description: "Only HR or Admin can change pay history.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Compensation updated and history recorded.",
        variant: "success",
      });
      setSelectedEmployee({
        ...selectedEmployee,
      ...
      toast({
        title: "Error",
        description: e?.message || "Failed to update compensation",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Choose an employee to view or update their compensation and history.
          </CardDescription>
        </CardHeader>
              ...
              >
                ...
                {employees.map((emp) => (
                  <option key={emp.id} value={emp.id}>
                    ...
                    {emp.current_compensation_amount != null
                      ? `(${formatCompensation(emp.current_compensation_amount, emp.current_pay_type, emp.current_pay_frequency)})`
                      : ""}
                  </option>
                ))}
              </select>
            ...
            <div className="pt-4 border-t space-y-4">
              {!canEditCompensation ? (
                <p className="text-sm text-muted-foreground">
                  Only HR or Admin roles can change pay history. You can view
                  compensation and history below.
                </p>
              ) : (
                <>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {selectedEmployee.current_compensation_amount != null && (
                      <span className="text-sm text-muted-foreground">
                        Current:{" "}
                        {formatCompensation(
                          selectedEmployee.current_compensation_amount,
                          selectedEmployee.current_pay_type,
                          selectedEmployee.current_pay_frequency,
                        )}
                      </span>
                    )}
                  </div>
          ...
          <CardContent>
            {loadingHistory ? (
              ...
              <p className="text-muted-foreground py-4">
                No compensation history yet. Update compensation above to create
                the first record.
              </p>
            ) : (
