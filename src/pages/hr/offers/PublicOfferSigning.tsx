...
export const PublicOfferSigning: React.FC = () => {
  ...
  const fetchOfferByToken = async (signingToken: string) => {
    try {
      ...
      try {
        ...
        setAttachments(
          list.map((a) => ({ id: a.id, name: a.name, file_url: a.file_url })),
        );
      } catch {
        setAttachments([]);
      }
      ...
      if (
        offerData.signature_status === "signed" ||
  ...
  const handleDecline = async () => {
    if (!offer || !confirm("Are you sure you want to decline this offer?"))
      return;
      ...
      <div className="min-h-screen flex items-center justify-center bg-neutral-50 dark:bg-neutral-900 p-4">
        <Card className="max-w-2xl w-full">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Thank you for signing the offer letter. You will receive a
          ...
          <CardContent className="space-y-4">
            <div className="border-t pt-4">
              ...
              <p>
                <strong>Position:</strong> {offer.position_title}
              </p>
              <p>
              ...
              {offer.base_salary && (
                <p>
                  <strong>Salary:</strong> ${offer.base_salary.toLocaleString()}{" "}
                  {offer.pay_frequency && `per ${offer.pay_frequency}`}
                </p>
              )}
            </div>
