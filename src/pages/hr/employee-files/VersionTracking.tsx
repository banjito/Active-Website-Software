...
export const VersionTracking: React.FC<VersionTrackingProps> = ({ initialEmployeeId }) => {
  const { user } = useAuth();
  ...
  const fetchCertifications = async () => {
    ...
    try {
      ...
      const data = await employeeCertificationsService.fetchCertifications(filter);
  ...
  const handleSaveCert = async () => {
    ...
    try {
      ...
      if (editingCert) {
        await employeeCertificationsService.updateCertification(editingCert.id, certData);
        toast({
          title: 'Success',
  ...
  const getStatusBadge = (cert: EmployeeCertification) => {
    ...
    return (
      <span className="text-xs bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200 px-2 py-1 rounded capitalize">
      ...
      {!initialEmployeeId && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Choose a user to view and manage their certifications</CardDescription>
          </CardHeader>
          <CardContent>
            <Select
      ...
      {selectedEmployeeId && (
        <>
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                Certifications {selectedEmployee && `- ${selectedEmployee.name || selectedEmployee.email}`}
              </CardTitle>
              <CardDescription>
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm mt-2">Add a certification to get started.</p>
                </div>
              ) : (
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors ${
                          statusInfo.isExpired ? 'border-red-500 bg-red-50 dark:bg-red-950' : 
                          statusInfo.isExpiringSoon ? 'border-orange-500 bg-orange-50 dark:bg-orange-950' : ''
                        }`}
                      >
                        <div className="flex items-center gap-4 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <h3 className="font-medium truncate">{cert.cert_name}</h3>
                              {getStatusBadge(cert)}
                            </div>
                            <div className="flex items-center gap-4 mt-1 text-sm text-muted-foreground">
                              <span className="capitalize">{cert.cert_type}</span>
                              {cert.cert_number && (
      ...
      <Dialog open={isCertModalOpen} onOpenChange={setIsCertModalOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingCert ? 'Update certification details' : `Add a new certification for ${selectedEmployee?.name || selectedEmployee?.email}`}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setCertForm(prev => ({ ...prev, cert_name: e.target.value }))}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setCertForm(prev => ({ ...prev, cert_number: e.target.value }))}
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="text-sm font-medium">Certification Date *</label>
                <Input
                  type="date"
                  value={certForm.cert_date}
                  onChange={(e) => setCertForm(prev => ({ ...prev, cert_date: e.target.value }))}
                  className="mt-1"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setCertForm(prev => ({ ...prev, renewal_date: e.target.value }))}
                  className="mt-1"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5 mb-1">
                Upload the certificate PDF or image. It will be saved to this employee&apos;s documents and linked to this certification.
              </p>
              <div className="flex items-center gap-2">
                <Input
                  type="file"
