...
const LowVoltagePanelboardSmallBreakerTestATSReport: React.FC = () => {
      ...
      if (jobId) {
        try {
          const { data, error: jobError } = await supabase
            .schema('neta_ops')
            .from('jobs')
            .select('job_number, customer_id, title, site_address, customers!inner(name, company_name, address)')
            .eq('id', jobId)
            .single();
  ...
  const renderInputField = (label: string, fieldKey: string, placeholder = "", type = "text", colSpan = "md:col-span-1", isReadOnly?: boolean) => {
    ...
    return (
        ...
        <input
            ...
            className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${(!isEditing || isReadOnly) ? 'bg-gray-100 dark:bg-dark-150 cursor-not-allowed' : ''}`}
        />
        </div>
    );
  };
  ...
  return (
    <ReportWrapper isPrintMode={isPrintMode}>
      ...
      <div className={isPrintMode ? 'block px-4 space-y-4' : 'hidden print:block px-4 space-y-4'}>
        {/* Nameplate Data */}
        <div>
            <div className="w-full h-1 bg-[#f26722] mb-2"></div>
            <h2 className="text-base font-bold mb-2 text-black border-black pb-2">Nameplate Data</h2>
            <table className="w-full border-collapse border border-black mb-2">
              <thead><tr><th className="p-2 text-left border border-black" colSpan={2}>Panelboard</th></tr></thead>
              <tbody>
                <tr><td className="p-2 border border-black font-semibold text-xs">Manufacturer</td><td className="p-2 border border-black text-xs">{formData.panelboardManufacturer || ''}</td></tr>
                <tr><td className="p-2 border border-black font-semibold text-xs">Type / Cat #</td><td className="p-2 border border-black text-xs">{formData.panelboardTypeCat || ''}</td></tr>
                <tr><td className="p-2 border border-black font-semibold text-xs">Size (A)</td><td className="p-2 border border-black text-xs">{formData.panelboardSizeA || ''}</td></tr>
                <tr><td className="p-2 border border-black font-semibold text-xs">Voltage (V)</td><td className="p-2 border border-black text-xs">{formData.panelboardVoltageV || ''}</td></tr>
                <tr><td className="p-2 border border-black font-semibold text-xs">SCCR (kA)</td><td className="p-2 border border-black text-xs">{formData.panelboardSCCRkA || ''}</td></tr>
              </tbody>
            </table>
            <table className="w-full border-collapse border border-black">
              <thead><tr><th className="p-2 text-left border border-black" colSpan={2}>Main Breaker</th></tr></thead>
              <tbody>
                <tr><td className="p-2 border border-black font-semibold text-xs">Manufacturer</td><td className="p-2 border border-black text-xs">{formData.mainBreakerManufacturer || ''}</td></tr>
                <tr><td className="p-2 border border-black font-semibold text-xs">Type</td><td className="p-2 border border-black text-xs">{formData.mainBreakerType || ''}</td></tr>
                <tr><td className="p-2 border border-black font-semibold text-xs">Frame Size (A)</td><td className="p-2 border border-black text-xs">{formData.mainBreakerFrameSizeA || ''}</td></tr>
                <tr><td className="p-2 border border-black font-semibold text-xs">Rating Plug (A)</td><td className="p-2 border border-black text-xs">{formData.mainBreakerRatingPlugA || ''}</td></tr>
                <tr><td className="p-2 border border-black font-semibold text-xs">I.C. Rating (kA)</td><td className="p-2 border border-black text-xs">{formData.mainBreakerICRatingkA || ''}</td></tr>
              </tbody>
            </table>
          </div>
          ...
          <div>
            <div className="w-full h-1 bg-[#f26722] mb-2"></div>
            <h2 className="text-base font-bold mb-2 text-black border-black pb-2">Visual and Mechanical Inspection</h2>
            <table className="w-full border-collapse border border-black text-xs">
              <thead className="bg-gray-100">
                <tr>
                  <th className="p-2 text-left border border-black">NETA Section</th>
              ...
              <tbody>
                {formData.visualInspectionItems.map((item, index) => (
                  <tr key={index}>
                    <td className="p-2 border border-black">{item.netaSection}</td>
                    <td className="p-2 border border-black">{item.description}</td>
                    <td className="p-2 border border-black text-center">{item.results || ''}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div>
            <div className="w-full h-1 bg-[#f26722] mb-2"></div>
            <h2 className="text-base font-bold mb-2 text-black border-black pb-2">Test Equipment Used</h2>
            <table className="w-full border-collapse border border-black text-xs">
              <thead>
                <tr>
                  <th className="p-2 text-left border border-black">Megohmmeter</th>
                  <th className="p-2 text-left border border-black">Low-Resistance Ohmmeter</th>
                  <th className="p-2 text-left border border-black">Primary Injection Test Set</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td className="p-2 border border-black">Name: {formData.megohmmeterName}<br/>Serial #: {formData.megohmmeterSerial}<br/>AMP ID: {formData.megohmmeterAmpId}<br/>Cal Date: {formData.megohmmeterCalDate}</td>
                  <td className="p-2 border border-black">Name: {formData.lowResistanceOhmmeterName}<br/>Serial #: {formData.lowResistanceOhmmeterSerial}<br/>AMP ID: {formData.lowResistanceOhmmeterAmpId}<br/>Cal Date: {formData.lowResistanceOhmmeterCalDate}</td>
                  <td className="p-2 border border-black">Name: {formData.primaryInjectionTestSetName}<br/>Serial #: {formData.primaryInjectionTestSetSerial}<br/>AMP ID: {formData.primaryInjectionTestSetAmpId}<br/>Cal Date: {formData.primaryInjectionTestSetCalDate}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div style={{ pageBreakInside: 'auto' }}>
            <div className="w-full h-1 bg-[#f26722] mb-1" style={{ pageBreakAfter: 'avoid' }}></div>
            <h2 className="text-base font-bold mb-1 text-black pb-1" style={{ pageBreakAfter: 'avoid' }}>Electrical Tests</h2>
            <div className="flex gap-4 mb-1 text-xs" style={{ pageBreakAfter: 'avoid' }}>
            ...
            <table className="w-full border-collapse border border-black text-xs" style={{ pageBreakBefore: 'avoid' }}>
              <thead className="bg-gray-100">
                <tr>
              ...
              <tbody>
                {formData.breakers.slice(0, parseInt(formData.numberOfCircuitSpaces) || 30).map((breaker, index) => (
                  <tr key={index}>
                    <td className="border border-black p-1 text-center">{breaker.result || '-'}</td>
                    <td className="border border-black p-1 text-center">{breaker.circuitNumber}</td>
                    <td className="border border-black p-1 text-center">{breaker.poles}</td>
                    <td className="border border-black p-1">{breaker.manuf}</td>
                    <td className="border border-black p-1">{breaker.type}</td>
                    <td className="border border-black p-1 text-center">{breaker.frameA}</td>
                    <td className="border border-black p-1 text-center">{breaker.tripA}</td>
                    <td className="border border-black p-1 text-center">{breaker.ratedCurrentA}</td>
                    <td className="border border-black p-1 text-center">{breaker.testCurrentA}</td>
                    <td className="border border-black p-1 text-center">{breaker.tripToleranceMin}</td>
                    <td className="border border-black p-1 text-center">{breaker.tripToleranceMax}</td>
                    <td className="border border-black p-1 text-center">{breaker.tripTime}</td>
                    <td className="border border-black p-1 text-center">{breaker.insulationLL}</td>
                    <td className="border border-black p-1 text-center">{breaker.insulationLP}</td>
                    <td className="border border-black p-1 text-center">{breaker.insulationPP}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className={isPrintMode ? 'hidden' : 'p-6 flex justify-center bg-gray-50 dark:bg-dark-150 print:hidden'}>
        <div className="max-w-7xl w-full space-y-6">
          ...
          <div className="mb-6">
            <div className="w-full h-1 bg-[#f26722] mb-4"></div>
            <h2 className="text-xl mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 font-semibold pb-2">Nameplate Data</h2>
          ...
          <div className="mb-6">
            <div className="w-full h-1 bg-[#f26722] mb-4"></div>
            <h2 className="text-xl mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 font-semibold pb-2">Visual and Mechanical Inspection</h2>
          ...
          <div className="mb-6">
            <div className="w-full h-1 bg-[#f26722] mb-4"></div>
            <h2 className="text-xl mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 font-semibold pb-2">Test Equipment Used</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-4 test-eqpt-onscreen">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <EquipmentAutocomplete
                    ...
                    onSelect={(equipment) => {
                      ...
                      handleChange('megohmmeterSerial', equipment.serial_number || '');
                      handleChange('megohmmeterAmpId', equipment.amp_id || '');
                      handleChange('megohmmeterCalDate', formatLocalDateShort(equipment.calibration_date));
                    }}
                    readOnly={!isEditing}
                    className="flex-1"
                  />
                ...
                <div className="flex items-center space-x-2">
                  <label htmlFor="megohmmeterCalDate" className="form-label w-20 flex-shrink-0">Cal Date:</label>
                  <input id="megohmmeterCalDate" type="text" value={formData.megohmmeterCalDate} onChange={(e) => handleChange('megohmmeterCalDate', e.target.value)} readOnly={!isEditing} className="form-input flex-1" />
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <EquipmentAutocomplete
                    ...
                    onChange={(value) => handleChange('lowResistanceOhmmeterName', value)}
                    onSelect={(equipment) => {
                      const formatDate = (dateString: string | null): string => {
                        if (!dateString) return '';
                        try {
                      ...
                      handleChange('lowResistanceOhmmeterName', equipment.equipment_name);
                      handleChange('lowResistanceOhmmeterSerial', equipment.serial_number || '');
                      handleChange('lowResistanceOhmmeterAmpId', equipment.amp_id || '');
                      handleChange('lowResistanceOhmmeterCalDate', formatLocalDateShort(equipment.calibration_date));
                    }}
                    readOnly={!isEditing}
                    className="flex-1"
                  />
                ...
                <div className="flex items-center space-x-2">
                  <label htmlFor="lowResistanceOhmmeterCalDate" className="form-label w-20 flex-shrink-0">Cal Date:</label>
                  <input id="lowResistanceOhmmeterCalDate" type="text" value={formData.lowResistanceOhmmeterCalDate} onChange={(e) => handleChange('lowResistanceOhmmeterCalDate', e.target.value)} readOnly={!isEditing} className="form-input flex-1" />
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <EquipmentAutocomplete
                    ...
                    onChange={(value) => handleChange('primaryInjectionTestSetName', value)}
                    onSelect={(equipment) => {
                      const formatDate = (dateString: string | null): string => {
                        if (!dateString) return '';
                        try {
                      ...
                      handleChange('primaryInjectionTestSetName', equipment.equipment_name);
                      handleChange('primaryInjectionTestSetSerial', equipment.serial_number || '');
                      handleChange('primaryInjectionTestSetAmpId', equipment.amp_id || '');
                      handleChange('primaryInjectionTestSetCalDate', formatLocalDateShort(equipment.calibration_date));
                    }}
                    readOnly={!isEditing}
                    className="flex-1"
                  />
                ...
                <div className="flex items-center space-x-2">
                  <label htmlFor="primaryInjectionTestSetCalDate" className="form-label w-20 flex-shrink-0">Cal Date:</label>
                  <input id="primaryInjectionTestSetCalDate" type="text" value={formData.primaryInjectionTestSetCalDate} onChange={(e) => handleChange('primaryInjectionTestSetCalDate', e.target.value)} readOnly={!isEditing} className="form-input flex-1" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6 electrical-tests-section">
            <div className="w-full h-1 bg-[#f26722] mb-4"></div>
            <h2 className="text-xl mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 font-semibold pb-2">Electrical Tests</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 mb-6">
              ...
              <div className="flex items-center">
                ...
                <input 
                  ...
                  readOnly={!isEditing} 
                  className={`form-input flex-1 ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}
                />
              </div>
            </div>
                      ...
                      <td className="border border-gray-300 dark:border-gray-600 px-1 py-1">
                        <select
                          ...
                          className={`w-full text-xs rounded border-gray-300 dark:border-gray-600 
                            ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : 'bg-white dark:bg-dark-150'} 
