...
export const ShortcutService = {
  ...
  async bulkCreateShortcuts(userId: string, items: Array<{ title: string; url: string; icon?: string }>): Promise<Shortcut[]> {
    ...
    try {
      ...
      let start = existingShortcuts && existingShortcuts.length > 0 ? existingShortcuts[0].position + 1 : 0;

      const toInsert = items.map((it, idx) => ({ user_id: userId, title: it.title, url: it.url, icon: it.icon, position: start + idx }));
      const { data, error } = await supabase
        .schema('common')
        .from('user_shortcuts')
        .insert(toInsert)
