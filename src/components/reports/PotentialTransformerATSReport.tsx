...
const PotentialTransformerATSReport: React.FC = () => {
  ...
  const [formData, setFormData] = useState<ReportData>({
    ...
    equipment: {
      megohmmeter: { model: '', serial: '', ampId: '' },
      lowResOhmmeter: { model: '', serial: '', ampId: '' },
      ttrTestSet: { model: '', serial: '', ampId: '' }
    },
  ...
  const celsiusTemperature = useMemo(() => 
    Math.round(convertFahrenheitToCelsius(formData.temperature) * 10) / 10, 
  ...
  useEffect(() => {
    const updatedInsulationCorrectedRows = formData.insulationResistance.rows.map((measuredRow) => {
  ...
  useEffect(() => {
    const loadJobInfo = async () => {
      if (!jobId || reportId) return; // Skip if editing existing report
  ...
  const handleInsulationChange = (index: number, field: keyof InsulationRow, value: string) => {
    setFormData(prev => ({
      ...
      insulationResistance: {
        ...
        rows: prev.insulationResistance.rows.map((row, i) => 
  ...
  const handleInsulationCorrectedChange = (index: number, field: keyof InsulationCorrectedRow, value: string) => {
    setFormData(prev => ({
      ...
      insulationCorrected: {
        ...prev.insulationCorrected,
        rows: prev.insulationCorrected.rows.map((row, i) => 
          i === index ? { ...row, [field]: value } : row
        )
      }
    }));
  ...
  const handleTurnsRatioChange = (index: number, field: keyof TurnsRatioRow, value: string) => {
    setFormData(prev => {
      const updatedRows = prev.turnsRatio.rows.map((row, i) => {
        if (i === index) {
    ...
    <div className="p-6 flex justify-center" id="report-container">
      <div className="max-w-7xl w-full space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 print:hidden job-info-onscreen">
            <div className="space-y-4">
              <div className="flex items-center">
                ...
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.customer} onChange={(e) => handleChange('customer', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Address:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.address} onChange={(e) => handleChange('address', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">User:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.user} onChange={(e) => handleChange('user', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Date:</label>
                <input className={`form-input w-40 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.date} onChange={(e) => handleChange('date', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Identifier:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.identifier} onChange={(e) => handleChange('identifier', e.target.value)} readOnly={!isEditMode} />
              </div>
            </div>
            <div className="space-y-4">
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Job #:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.jobNumber} onChange={(e) => handleChange('jobNumber', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Technicians:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.technicians} onChange={(e) => handleChange('technicians', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Substation:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.substation} onChange={(e) => handleChange('substation', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Eqpt. Location:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.eqptLocation} onChange={(e) => handleChange('eqptLocation', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Temp:</label>
                <input type="number" className={`form-input w-20 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.temperature} onChange={(e) => handleChange('temperature', Number(e.target.value))} readOnly={!isEditMode} />
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td className="border border-black px-2 py-1 align-top">
                    ...
                    <div className="mt-1">{formData.temperature}°F / {tcf}</div>
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 print:hidden nameplate-onscreen">
            <div className="space-y-4">
              <div className="flex items-center">
                ...
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.deviceData.manufacturer} onChange={(e) => handleDeviceDataChange('manufacturer', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Catalog No:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.deviceData.catalogNo} onChange={(e) => handleDeviceDataChange('catalogNo', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Serial Number:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.deviceData.serialNumber} onChange={(e) => handleDeviceDataChange('serialNumber', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Accuracy Class:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.deviceData.accuracyClass} onChange={(e) => handleDeviceDataChange('accuracyClass', e.target.value)} readOnly={!isEditMode} />
              </div>
            </div>
            <div className="space-y-4">
              <div className="flex items-center">
                <label className="form-label inline-block w-40">Manufactured Year:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.deviceData.manufacturedYear} onChange={(e) => handleDeviceDataChange('manufacturedYear', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-40">Voltage Rating:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.deviceData.voltageRating} onChange={(e) => handleDeviceDataChange('voltageRating', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-40">Insulation Class:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.deviceData.insulationClass} onChange={(e) => handleDeviceDataChange('insulationClass', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-40">Frequency:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.deviceData.frequency} onChange={(e) => handleDeviceDataChange('frequency', e.target.value)} readOnly={!isEditMode} />
              </div>
            </div>
          </div>
        ...
        <div className="mb-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse border border-gray-300 dark:border-gray-700 print:border-black visual-mechanical-table table-fixed">
              ...
              <thead>
                <tr>
                  <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">NETA Section</th>
                  <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Description</th>
                  <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Results</th>
                  <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Comments</th>
                </tr>
              </thead>
        ...
        <div className="mb-6">
          ...
          <div className="fuse-data-onscreen grid grid-cols-1 md:grid-cols-2 gap-4 print:hidden">
            <div className="space-y-4">
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Manufacturer:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.fuseData.manufacturer} onChange={(e) => handleFuseDataChange('manufacturer', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Catalog No:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.fuseData.catalogNo} onChange={(e) => handleFuseDataChange('catalogNo', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-32">Class:</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.fuseData.class} onChange={(e) => handleFuseDataChange('class', e.target.value)} readOnly={!isEditMode} />
              </div>
            </div>
            <div className="space-y-4">
              <div className="flex items-center">
                <label className="form-label inline-block w-40">Voltage Rating (kV):</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.fuseData.voltageRating} onChange={(e) => handleFuseDataChange('voltageRating', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-40">Ampacity (A):</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.fuseData.ampacity} onChange={(e) => handleFuseDataChange('ampacity', e.target.value)} readOnly={!isEditMode} />
              </div>
              <div className="flex items-center">
                <label className="form-label inline-block w-40">I.C. Rating (kA):</label>
                <input className={`form-input flex-1 ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.fuseData.icRating} onChange={(e) => handleFuseDataChange('icRating', e.target.value)} readOnly={!isEditMode} />
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-gray-300 dark:border-gray-700 print:border-black">
                <thead>
                  <tr>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider"></th>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">As Found</th>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">As Left</th>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Units</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
          ...
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-gray-300 dark:border-gray-700 print:border-black">
                <thead>
                  <tr>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Winding Tested</th>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Test Voltage</th>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Results</th>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Units</th>
                  </tr>
                </thead>
                <tbody>
                  {formData.insulationResistance.rows.map((row, index) => (
                    <tr key={index}>
                      <td className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2">
                        <input 
                          className={`w-full border-0 bg-transparent focus:outline-none focus:ring-0 text-gray-900 dark:text-white ${!isEditMode ? 'cursor-not-allowed' : ''}`}
          ...
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-gray-300 dark:border-gray-700 print:border-black">
                ...
                <tbody>
                  {formData.insulationCorrected.rows.map((row, index) => (
                    <tr key={index}>
                      <td className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2">
                        <input 
                          className={`w-full border-0 bg-transparent focus:outline-none focus:ring-0 text-gray-900 dark:text-white ${!isEditMode ? 'cursor-not-allowed' : ''}`}
                      ...
                      <td className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2">
                        <input 
                          className={`w-full border-0 bg-transparent focus:outline-none focus:ring-0 text-gray-900 dark:text-white bg-gray-100 dark:bg-dark-200 cursor-not-allowed`}
                          value={row.results} 
                          readOnly
          ...
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-gray-300 dark:border-gray-700 print:border-black">
                <thead>
                  <tr>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Tap</th>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Primary Voltage</th>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Calculate d Ratio</th>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Measured H1-H2</th>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">% Dev.</th>
                    <th className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Pass/ Fail</th>
                  </tr>
                </thead>
                <tbody>
                  {formData.turnsRatio.rows.map((row, index) => (
                    <tr key={index}>
                      ...
                      <td className="border border-gray-300 dark:border-gray-700 print:border-black px-3 py-2">
                        <input 
                          className={`w-full border-0 bg-transparent focus:outline-none focus:ring-0 text-gray-900 dark:text-white bg-gray-100 dark:bg-dark-200 cursor-not-allowed ${row.passFail === 'Pass' ? 'text-green-600 dark:text-green-400' : row.passFail === 'Fail' ? 'text-red-600 dark:text-red-400' : ''}`}
                          value={row.passFail} 
                          readOnly
            ...
            <div className="mt-4 flex items-center">
              <span className="text-sm text-gray-700 dark:text-gray-300 mr-4">Secondary Voltage at as-found tap:</span>
              <input 
                ...
                readOnly={!isEditMode} 
              />
              <span className="ml-2 text-sm text-gray-700 dark:text-gray-300">V</span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse border border-gray-300 dark:border-gray-700 print:border-black">
              <thead>
                <tr>
                  <th className="border border-gray-300 dark:border-gray-700 print:border-black px-2 py-1 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 font-bold text-center text-gray-900 dark:text-white">Equipment</th>
                  <th className="border border-gray-300 dark:border-gray-700 print:border-black px-2 py-1 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 font-bold text-center text-gray-900 dark:text-white">Model</th>
                  <th className="border border-gray-300 dark:border-gray-700 print:border-black px-2 py-1 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 font-bold text-center text-gray-900 dark:text-white">Serial Number</th>
                  <th className="border border-gray-300 dark:border-gray-700 print:border-black px-2 py-1 bg-gray-50 dark:bg-dark-200 print:bg-gray-50 font-bold text-center text-gray-900 dark:text-white">AMP ID</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  ...
                  <td className="border border-gray-300 dark:border-gray-700 print:border-black px-2 py-1">
                    <input 
                      ...
                      readOnly={!isEditMode} 
                      onChange={(e) => handleEquipmentChange('megohmmeter', 'model', e.target.value)} 
                    />
                  </td>
                  <td className="border border-gray-300 dark:border-gray-700 print:border-black px-2 py-1">
                    <input 
                ...
                <tr>
                  ...
                  <td className="border border-gray-300 dark:border-gray-700 print:border-black px-2 py-1">
                    <input 
                      ...
                      readOnly={!isEditMode} 
                      onChange={(e) => handleEquipmentChange('lowResOhmmeter', 'model', e.target.value)} 
                    />
                  </td>
                  <td className="border border-gray-300 dark:border-gray-700 print:border-black px-2 py-1">
                    <input 
                ...
                <tr>
                  ...
                  <td className="border border-gray-300 dark:border-gray-700 print:border-black px-2 py-1">
                    <input 
                      ...
                      readOnly={!isEditMode} 
                      onChange={(e) => handleEquipmentChange('ttrTestSet', 'model', e.target.value)} 
                    />
                  </td>
                  <td className="border border-gray-300 dark:border-gray-700 print:border-black px-2 py-1">
                    <input 
        ...
        <div className="mb-6 print:mb-2">
          ...
          <textarea rows={10} className={`w-full form-textarea resize-vertical min-h-[250px] print:hidden ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} value={formData.comments} onChange={(e) => handleChange('comments', e.target.value)} readOnly={!isEditMode} />
