  ...
  const loadCoverLetters = async () => {
    try {
      ...
      if (error) {
        ...
        if (
          error.message?.includes("selected_report_ids") ||
  ...
  const loadAssets = async () => {
    try {
      ...
      console.log(
        "📦 Assets loaded:",
        assetData?.length || 0,
        "reports found (including PDF reports)",
      );
    } catch (error: any) {
      ...
      {deliverables.length === 0 ? (
        ...
        <div className="grid gap-4">
          {deliverables.map((deliverable) => {
            const reportCount = getCoverLetterReportCount(
              deliverable.cover_letter_id,
            );
            return (
              <div
                key={deliverable.id}
              ...
              {coverLetters.length === 0 ? (
                ...
                <div className="max-h-[250px] overflow-y-auto space-y-2 pr-2">
                  {coverLetters.map((coverLetter) => {
                    const reportNames = getReportNames(
                      coverLetter.selected_report_ids || [],
                    );
                    return (
                      <label
                        key={coverLetter.id}
                ...
                <ul className="mt-2 space-y-1">
                  {(() => {
                    const coverLetter = coverLetters.find(
                      (cl) => cl.id === selectedDeliverable.cover_letter_id,
                    );
            ...
            <DialogFooter>
              <div className="flex gap-2 flex-wrap">
                {selectedDeliverable.status === "draft" && (
                  <Button
                    onClick={() =>
                      handleUpdateStatus(selectedDeliverable.id, "in_review")
                    }
                ...
                {selectedDeliverable.status === "in_review" && (
                  <>
                    <Button
                      onClick={() =>
                        handleUpdateStatus(selectedDeliverable.id, "approved")
                      }
                ...
                {selectedDeliverable.status === "approved" && (
                  <Button
                    onClick={() =>
                      handleUpdateStatus(selectedDeliverable.id, "delivered")
                    }
                ...
                {selectedDeliverable.status === "draft" && (
                  <Button
                    onClick={() =>
                      handleDeleteDeliverable(selectedDeliverable.id)
                    }
                    className="bg-red-600 hover:bg-red-700 text-white"
                  >
                    Delete
