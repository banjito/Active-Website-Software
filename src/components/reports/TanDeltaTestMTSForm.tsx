        ...
        <div className="max-w-7xl w-full space-y-6">
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center gap-4 print:hidden test-params-onscreen">
              <label className="text-sm font-medium text-gray-700 dark:text-white whitespace-nowrap">System Voltage Line to Ground (kV RMS):</label>
              <input type="number" step="0.001" value={systemVoltage} onChange={(e) => setSystemVoltage(e.target.value)} disabled={!isEditing} className="mt-1 block w-32 rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white" />
            </div>
          ...
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead>
                  <tr>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Voltage Steps</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">kV</th>
                    <th colSpan={2} className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">A Phase</th>
                    <th colSpan={2} className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">B Phase</th>
                    <th colSpan={2} className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">C Phase</th>
                  </tr>
                  <tr>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150"></th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150"></th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">TD [E-3]</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Std. Dev</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">TD [E-3]</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Std. Dev</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">TD [E-3]</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Std. Dev</th>
                  ...
                  {data.map((row, index) => (
                    <tr key={index}>
                      <td className="px-3 py-2">
                        {editingData ? (
                          <input type="text" value={row.voltageLabel} onChange={(e) => handleDataChange(index, 'voltageLabel', e.target.value)} disabled={!isEditing || !editingData} className="block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white" />
                        ) : (<span className="text-gray-900 dark:text-white">{row.voltageLabel}</span>)}
                      </td>
                      <td className="px-3 py-2">
                        {editingData ? (
                          <input type="number" step="0.001" value={row.kV} onChange={(e) => handleDataChange(index, 'kV', e.target.value)} disabled={!isEditing || !editingData} className="block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white" />
                        ) : (<span className="text-gray-900 dark:text-white">{row.kV.toFixed(3)}</span>)}
                      </td>
                      <td className="px-3 py-2">{editingData ? (<input type="number" step="0.1" value={row.phaseA} onChange={(e) => handleDataChange(index, 'phaseA', e.target.value)} disabled={!isEditing || !editingData} className="block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white" />) : (<span className="text-gray-900 dark:text-white">{row.phaseA.toFixed(1)}</span>)}</td>
                      <td className="px-3 py-2">{editingData ? (<input type="number" step="0.01" value={row.phaseAStdDev === null ? '' : row.phaseAStdDev} onChange={(e) => handleDataChange(index, 'phaseAStdDev', e.target.value)} disabled={!isEditing || !editingData} className="block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white" />) : (<span className="text-gray-900 dark:text-white">{row.phaseAStdDev === null ? '' : row.phaseAStdDev.toFixed(1)}</span>)}</td>
                      <td className="px-3 py-2">{editingData ? (<input type="number" step="0.1" value={row.phaseB} onChange={(e) => handleDataChange(index, 'phaseB', e.target.value)} disabled={!isEditing || !editingData} className="block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white" />) : (<span className="text-gray-900 dark:text-white">{row.phaseB.toFixed(1)}</span>)}</td>
                      <td className="px-3 py-2">{editingData ? (<input type="number" step="0.01" value={row.phaseBStdDev === null ? '' : row.phaseBStdDev} onChange={(e) => handleDataChange(index, 'phaseBStdDev', e.target.value)} disabled={!isEditing || !editingData} className="block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white" />) : (<span className="text-gray-900 dark:text-white">{row.phaseBStdDev === null ? '' : row.phaseBStdDev.toFixed(1)}</span>)}</td>
                      <td className="px-3 py-2">{editingData ? (<input type="number" step="0.1" value={row.phaseC} onChange={(e) => handleDataChange(index, 'phaseC', e.target.value)} disabled={!isEditing || !editingData} className="block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white" />) : (<span className="text-gray-900 dark:text-white">{row.phaseC.toFixed(1)}</span>)}</td>
                      <td className="px-3 py-2">{editingData ? (<input type="number" step="0.01" value={row.phaseCStdDev === null ? '' : row.phaseCStdDev} onChange={(e) => handleDataChange(index, 'phaseCStdDev', e.target.value)} disabled={!isEditing || !editingData} className="block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white" />) : (<span className="text-gray-900 dark:text-white">{row.phaseCStdDev === null ? '' : row.phaseCStdDev.toFixed(1)}</span>)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="mb-6">
            ...
            {/* On-screen form - hidden in print */}
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 print:hidden test-eqpt-onscreen">
              <div>
                ...
                <EquipmentAutocomplete
                  ...
                  onChange={(value) => handleEquipmentChange('megohmmeterMakeModel', value)}
                  onSelect={(equip) => {
                    handleEquipmentChange('megohmmeterMakeModel', equip.equipment_name);
                    handleEquipmentChange('megohmeterSerial', equip.serial_number || '');
                    handleEquipmentChange('megohmmeterAmpId', equip.amp_id || '');
                    handleEquipmentChange('megohmmeterCalDate', formatLocalDateShort(equip.calibration_date));
                  }}
                  readOnly={!isEditing}
              ...
              <div>
                <label htmlFor="megohmmeterCalDate" className="form-label">Cal Date</label>
                <input id="megohmmeterCalDate" type="text" value={equipment.megohmmeterCalDate} onChange={(e) => handleEquipmentChange('megohmmeterCalDate', e.target.value)} disabled={!isEditing} className="form-input mt-1" />
              </div>
              <div>
                <label htmlFor="vlfHipotMakeModel" className="form-label">VLF Hipot Make/Model</label>
                <EquipmentAutocomplete
                  value={equipment.vlfHipotMakeModel}
                  onChange={(value) => handleEquipmentChange('vlfHipotMakeModel', value)}
                  onSelect={(equip) => {
                    handleEquipmentChange('vlfHipotMakeModel', equip.equipment_name);
                    handleEquipmentChange('vlfHipotSerial', equip.serial_number || '');
                    handleEquipmentChange('vlfHipotAmpId', equip.amp_id || '');
                    handleEquipmentChange('vlfHipotCalDate', formatLocalDateShort(equip.calibration_date));
                  }}
                  readOnly={!isEditing}
                  className="form-input mt-1"
                />
              ...
              <div>
                <label htmlFor="vlfHipotCalDate" className="form-label">Cal Date</label>
                <input id="vlfHipotCalDate" type="text" value={equipment.vlfHipotCalDate} onChange={(e) => handleEquipmentChange('vlfHipotCalDate', e.target.value)} disabled={!isEditing} className="form-input mt-1" />
              </div>
            </div>
              ...
              <table className="w-full border border-gray-300 print:border-black">
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="p-2 border border-gray-300 print:border-black">{equipment.megohmmeterCalDate || ''}</td>
                  </tr>
                  <tr>
                    ...
                    <td className="p-2 border border-gray-300 print:border-black">{equipment.vlfHipotCalDate || ''}</td>
                  </tr>
                </tbody>
              </table>
            </div>
