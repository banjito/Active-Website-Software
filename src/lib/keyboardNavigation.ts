...
class KeyboardNavigationManager {
  ...
  private handleGlobalKeyDown(event: KeyboardEvent) {
    ...
    // For text inputs and textareas, check if cursor movement should be handled normally
    if (currentElement instanceof HTMLInputElement || currentElement instanceof HTMLTextAreaElement) {
      const shouldAllowNormalCursorMovement = this.shouldAllowNormalCursorMovement(currentElement, event.key);
      if (shouldAllowNormalCursorMovement) {
        return; // Let the browser handle normal cursor movement
      }
    }
    ...
    switch (event.key) {
      ...
      case 'Enter':
        // For textareas, allow normal Enter behavior unless at the end
  ...
  private shouldAllowNormalCursorMovement(element: HTMLInputElement | HTMLTextAreaElement, key: string): boolean {
    const selectionStart = element.selectionStart || 0;
    const selectionEnd = element.selectionEnd || 0;
    const value = element.value || '';
    const valueLength = value.length;

    // If text is selected (not just cursor position), allow normal behavior
    ...
    if (element instanceof HTMLInputElement) {
      switch (key) {
        case 'ArrowLeft':
          // Allow normal movement if cursor is not at the beginning
          return selectionStart > 0;
        case 'ArrowRight':
          // Allow normal movement if cursor is not at the end
          return selectionStart < valueLength;
        case 'ArrowUp':
        case 'ArrowDown':
          // For single-line inputs, always navigate to other fields
          return false;
        default:
          return true;
      }
    ...
    if (element instanceof HTMLTextAreaElement) {
      ...
      const cursorPositionInLine = selectionStart - currentLineStart;
      ...
      switch (key) {
        ...
        case 'ArrowRight':
          ...
          return selectionStart < valueLength;
        case 'ArrowUp':
          // Allow normal movement if not on the first line
          return currentLineIndex > 0;
        case 'ArrowDown':
          // Allow normal movement if not on the last line
