...
export const JobTitleHistoryView: React.FC<JobTitleHistoryViewProps> = ({ profileId, employeeName }) => {
  const { user } = useAuth();
  ...
  const handleUpdateTitle = async () => {
    ...
    try {
      ...
      toast({ title: 'Error', description: e?.message || 'Failed to update title', variant: 'destructive' });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          {currentTitle ? (
            <p className="text-lg text-gray-900 dark:text-white">{currentTitle}</p>
          ) : (
            ...
            <div className="pt-4 border-t border-gray-200 dark:border-dark-200 space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={handleUpdateTitle} disabled={saving}>
                  {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Save className="h-4 w-4 mr-2" />Update title</>}
                </Button>
              </div>
            </div>
          )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Previous job titles for {employeeName}</CardDescription>
        </CardHeader>
        <CardContent>
          {history.length === 0 ? (
