...
import { CheckCircle, XCircle, Clock, AlertCircle, FileText, Play, Eye, AlertTriangle, Pencil, X, Check } from 'lucide-react';
...
type StatusCategory = 'not_started' | 'in_progress' | 'ready_for_review' | 'approved' | 'issue' | 'sent_on_time' | 'sent_late';
...
export const SubmittalTracker: React.FC<SubmittalTrackerProps> = ({
  ...
  useEffect(() => {
    ...
    reportAssets.forEach(asset => {
      ...
      const submittedAt = asset.submitted_at ? new Date(asset.submitted_at) : null;
      ...
      if (clockStart && approvedAt && clockStart !== approvedAt) {
        hoursToApproved = (approvedAt.getTime() - clockStart.getTime()) / (1000 * 60 * 60);
      }

      // Calculate hours since clock start
      if (clockStart) {
        hoursSinceSubmitted = (now.getTime() - clockStart.getTime()) / (1000 * 60 * 60);
      }

      if (asset.status === 'sent') {
        // Determine if sent on time or late based on time from clock start to SENT
        if (clockStart && sentAt) {
          hoursToSent = (sentAt.getTime() - clockStart.getTime()) / (1000 * 60 * 60);
    ...
    if (totalSent > 0) {
      setOnTimePercentage(Math.round((newCounts.sent_on_time / totalSent) * 100));
    } else {
      setOnTimePercentage(0);
    }
  }, [assets, submittalWindowHours, submittalJobType, refreshKey, dynamicAssetNames]);
  ...
  const getCategoryIcon = (category: StatusCategory) => {
    switch (category) {
      case 'not_started':
        return <FileText className="w-5 h-5 text-gray-400 dark:text-gray-500 flex-shrink-0" />;
      case 'in_progress':
        return <Play className="w-5 h-5 text-blue-500 dark:text-blue-400 flex-shrink-0" />;
      case 'ready_for_review':
        return <Eye className="w-5 h-5 text-blue-600 dark:text-blue-400 flex-shrink-0" />;
      case 'approved':
  ...
  const statusFilters: { key: StatusCategory | 'all'; label: string; count: number }[] = [
    { key: 'all', label: 'All', count: totalReports },
    { key: 'not_started', label: 'Not Started', count: counts.not_started },
    { key: 'in_progress', label: 'In Progress', count: counts.in_progress },
    { key: 'ready_for_review', label: 'Ready for Review', count: counts.ready_for_review },
  ...
  const getFilterButtonClass = (key: StatusCategory | 'all') => {
    ...
    let baseClass = 'px-3 py-2 text-sm font-medium rounded-md transition-colors ';
    ...
    if (isActive) {
      if (key === 'sent_on_time') {
        ...
        return baseClass + 'bg-white dark:bg-dark-150 text-gray-900 dark:text-white shadow-sm';
      }
    }
    return baseClass + 'text-gray-600 dark:text-white hover:text-gray-900 dark:hover:text-white';
  };
  ...
  const handleStartEditSentDate = (assetId: string, currentSentAt: Date | null) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            {getJobTypeLabel()} • {windowDays} day window (from submission to delivery)
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-6 text-center">
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Sent on time</p>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Sent late</p>
            </div>
            <div className="w-px h-12 bg-gray-200 dark:bg-gray-700"></div>
            <div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {counts.sent_on_time + counts.sent_late > 0 ? (
              <>{counts.sent_on_time} of {counts.sent_on_time + counts.sent_late} sent reports delivered within {windowDays} days of submission</>
            ) : (
              <>No reports sent yet</>
            )}
          </p>
      ...
      {/* Status Filter Tabs */}
      <div className="flex space-x-1 bg-gray-100 dark:bg-dark-150 p-1 rounded-lg overflow-x-auto">
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400 mt-1 space-y-0.5">
                        ...
                        {status.approvedAt && (
                          <p>
                            ...
                            {status.hoursToApproved !== null && status.hoursToApproved > 0 && (
                              <span className="ml-2 text-gray-400">
                                ({formatHours(status.hoursToApproved)} to approve)
                              </span>
                            )}
                          </p>
                        )}
                            ...
                            {editingAssetId === status.assetId ? (
                              ...
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                <input
                                  ...
                                  onChange={(e) => setEditingSentDate(e.target.value)}
                                  className="px-2 py-0.5 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-dark-100 text-gray-900 dark:text-white"
                                />
                                <input
                                  type="time"
                                  value={editingSentTime}
                                  onChange={(e) => setEditingSentTime(e.target.value)}
                                  className="px-2 py-0.5 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-dark-100 text-gray-900 dark:text-white"
                                />
                                <button
                              ...
                              <p className="flex items-center gap-1">
                                ...
                                {status.hoursToSent !== null && (
                                  <span className={`ml-2 font-medium ${status.hoursToSent <= status.windowHours ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                    ({formatHours(status.hoursToSent)} total)
                                  </span>
                                )}
                                {isAdmin && onUpdateSentDate && (
                                  <button
                                    onClick={() => handleStartEditSentDate(status.assetId, status.sentAt)}
                                    className="ml-2 p-0.5 text-gray-400 hover:text-[#f26722] dark:hover:text-[#f26722] transition-colors"
                        ...
                        {(status.category === 'ready_for_review' || status.category === 'approved') && status.hoursSinceSubmitted !== null && (
                          <p className={`font-medium ${status.hoursSinceSubmitted > status.windowHours ? 'text-red-500 dark:text-red-400' : 'text-orange-500 dark:text-orange-400'}`}>
                            ⏳ {formatHours(status.hoursSinceSubmitted)} elapsed
                            {status.hoursSinceSubmitted > status.windowHours && ' • PAST DEADLINE!'}
                          </p>
                        )}
                        
                        {/* Note for legacy reports without submitted_at */}
                        {status.usingFallback && (status.category === 'sent_on_time' || status.category === 'sent_late') && (
                          <p className="text-gray-400 italic">
                            * Time calculated from approval (no submission date recorded)
                          </p>
                        )}
                      </div>
                    </div>
