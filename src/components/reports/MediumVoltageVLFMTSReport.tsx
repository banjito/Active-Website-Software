...
const MediumVoltageVLFMTSReport: React.FC = () => {
  ...
  const [formData, setFormData] = useState<MediumVoltageVLFMTSReportForm>({
    ...
    equipment: {
      ...
      vlfTestSet: "",
      ohmCalDate: "", megohmCalDate: "", vlfCalDate: "",
    },
  ...
  const loadJobInfo = async (currentJobId: string) => {
    ...
    try {
      const { data: jobData, error: jobError } = await supabase.schema('neta_ops').from('jobs').select('title,job_number,customer_id,site_address').eq('id', currentJobId).single();
      ...
      setFormData(prev => ({
        ...
        reportInfo: { ...prev.reportInfo, title: maskJobTitle(jobData.title || ''), customerName: maskCustomerName(customerName) }
      }));
    } catch (error) {
    ...
    <ReportWrapper isPrintMode={isPrintMode}>
      ...
      <section className="mb-6 print:hidden job-info-section">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-x-4 gap-y-2 print:hidden job-info-onscreen">
            ...
            <div className="mb-4 flex">
              <label className="inline-block w-24 font-medium text-gray-700 dark:text-white">Customer</label>
              <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
              <input type="text" value={maskCustomerName(formData.customerName)} onChange={(e) => handleChange('customerName', e.target.value)} readOnly={!isEditMode} className={`w-full bg-transparent border-none focus:ring-0 ${!isEditMode ? 'cursor-default' : ''}`}/>
            </div></div>
            <div className="mb-4 flex">
              <label className="inline-block w-24 font-medium text-gray-700 dark:text-white">Site Address</label>
              <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
              <input type="text" value={maskCustomerAddress(formData.siteAddress)} onChange={(e) => handleChange('siteAddress', e.target.value)} readOnly={!isEditMode} className={`w-full bg-transparent border-none focus:ring-0 ${!isEditMode ? 'cursor-default' : ''}`}/>
            </div></div>
            <div className="mb-4 flex">
              <label className="inline-block w-24 font-medium text-gray-700 dark:text-white">User</label>
              <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
            ...
            <div className="mb-4 flex">
              <label className="inline-block w-24 font-medium text-gray-700 dark:text-white">Date</label>
              <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
            ...
            <div className="mb-4 flex">
              <label className="inline-block w-24 font-medium text-gray-700 dark:text-white">Identifier</label>
              <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
            ...
            <div> {/* Right Column */}
            <div className="mb-4 flex">
              <label className="inline-block w-24 font-medium text-gray-700 dark:text-white">Job #</label>
              <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
            ...
            <div className="mb-4 flex">
              <label className="inline-block w-24 font-medium text-gray-700 dark:text-white">Technicians</label>
              <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
            ...
            <div className="mb-4 flex items-center">
              <label className="inline-block w-24 font-medium text-gray-700 dark:text-white">Temp.</label>
            ...
            <div className="mb-4 flex">
              <label className="inline-block w-24 font-medium text-gray-700 dark:text-white">Humidity</label>
            ...
            <div className="mb-4 flex">
              <label className="inline-block w-24 font-medium text-gray-700 dark:text-white">Substation</label>
              <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
            ...
            <div className="mb-4 flex">
              <label className="inline-block w-24 font-medium text-gray-700 dark:text-white">Eqpt. Location</label>
              <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-3">
            {[ { label: "Tested From", field: "testedFrom", section: "cableInfo" }, { label: "", field: "", section: "" },
              ...
              <div className="flex items-center" key={idx}>
                <label className="w-1/2 text-sm font-medium text-gray-700 dark:text-white">{item.label}</label>
                <input type="text" 
            ...
            <tbody><tr>
              {[ "phaseA", "phaseB", "phaseC" ].map(phase => (
                <td className="border border-gray-300 dark:border-gray-700 px-3 py-2 whitespace-nowrap" key={phase}>
                  <input type="text" value={formData.shieldContinuity[phase]} onChange={(e) => handleChange('shieldContinuity', {...formData.shieldContinuity, [phase]: e.target.value})} readOnly={!isEditMode} className={`w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/>
                </td>
              ))}
                <td className="border border-gray-300 dark:border-gray-700 px-3 py-2 whitespace-nowrap">
                  <select value={formData.shieldContinuity.unit} onChange={(e) => handleChange('shieldContinuity', {...formData.shieldContinuity, unit: e.target.value})} disabled={!isEditMode} className={`w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>
                    {continuityUnits.map(unit => (<option key={unit.symbol} value={unit.symbol} className="dark:bg-dark-150 dark:text-white">{unit.symbol}</option>))}
                  </select>
                </td>
            </tr></tbody>
          </table>
      ...
      <section className="mb-6">
        ...
        <div className="mb-4 flex items-center">
          ...
          <select value={formData.insulationTest.testVoltage} onChange={(e) => handleChange('insulationTest', {...formData.insulationTest, testVoltage: e.target.value})} disabled={!isEditMode} className={`w-32 rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>
            {insulationTestVoltages.map(option => (<option key={option.value} value={option.value} className="dark:bg-dark-150 dark:text-white">{option.label}</option>))}
          </select>
        ...
        <div className="overflow-x-auto section-insulation-resistance">
          <table className="min-w-full border-collapse border border-gray-300 dark:border-gray-700 table-fixed">
            ...
            <thead><tr>
                ...
                <th className="border border-gray-300 dark:border-gray-700 px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Units</th>
            </tr><tr>
                <th className="border border-gray-300 dark:border-gray-700 px-2 py-2"></th>
                {Array(2).fill(null).map((_, i) => ["A-G", "B-G", "C-G"].map(phase => <th key={`${i}-${phase}`} className="border border-gray-300 dark:border-gray-700 px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">{phase}</th>))}
                <th className="border border-gray-300 dark:border-gray-700 px-2 py-2"></th>
            </tr></thead>
            <tbody>
              ...
              <tr key={test.type}>
                ...
                {[ "ag", "bg", "cg" ].map(phase => (
                  <td className="border border-gray-300 dark:border-gray-700 px-3 py-2 whitespace-nowrap" key={`${test.type}-${phase}-input`}>
                    <input type="text" value={formData.insulationTest[test.type][phase]} onChange={(e) => handleInsulationTestValueChange(test.type as 'preTest'|'postTest', phase, e.target.value)} readOnly={!isEditMode} className={`w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/>
                  </td>
                ))}
                {[ "ag", "bg", "cg" ].map(phase => (
                  <td className="border border-gray-300 dark:border-gray-700 px-3 py-2 whitespace-nowrap" key={`${test.type}-${phase}-corrected`}>
                    <input type="text" value={formData.insulationTest[`${test.type}Corrected`][phase]} readOnly={true} className="w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm dark:bg-dark-150 dark:text-white bg-gray-100 dark:bg-dark-150"/>
                  </td>
                ))}
        ...
        <div className="mt-2 flex justify-end items-center">
          ...
          <select value={formData.insulationTest.unit} onChange={(e) => handleChange('insulationTest', {...formData.insulationTest, unit: e.target.value})} disabled={!isEditMode} className={`w-32 rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>
            {insulationUnits.map(unit => (<option key={unit.symbol} value={unit.symbol} className="dark:bg-dark-150 dark:text-white">{unit.symbol}</option>))}
          </select>
        </div>
      </section>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full border-collapse border border-gray-300 dark:border-gray-700">
                ...
                {[ "phaseA", "phaseB", "phaseC" ].map(phase => (
                  ...
                  <th className="border border-gray-300 dark:border-gray-700 px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">
                    <select onChange={(e) => { const newReadings = [...formData.withstandTest.readings]; newReadings.forEach(r => { r[phase].currentUnit = e.target.value; }); handleChange('withstandTest', { readings: newReadings });}}
                      ...
                      {currentUnits.map(unit => (<option key={unit.symbol} value={unit.symbol} className="dark:bg-dark-150 dark:text-white">{unit.symbol}</option>))}
                    </select>
                  </th>
                  <th className="border border-gray-300 dark:border-gray-700 px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">nF</th>
                  </React.Fragment>
                ))}
            </tr></thead>
            <tbody>
            {formData.withstandTest?.readings.map((reading, index) => (
              <tr key={index}>
                <td className="border border-gray-300 dark:border-gray-700 px-3 py-2 whitespace-nowrap"><input type="text" value={reading.timeMinutes} onChange={(e) => handleWithstandTestChange(index, 'timeMinutes', e.target.value)} readOnly={!isEditMode} className={`w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/></td>
                <td className="border border-gray-300 dark:border-gray-700 px-3 py-2 whitespace-nowrap"><input type="text" value={reading.kVAC} onChange={(e) => handleWithstandTestChange(index, 'kVAC', e.target.value)} readOnly={!isEditMode} className={`w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/></td>
                {[ "phaseA", "phaseB", "phaseC" ].map(phase => (
                  <React.Fragment key={`${index}-${phase}`}>
                  <td className="border border-gray-300 dark:border-gray-700 px-3 py-2 whitespace-nowrap"><input type="text" value={reading[phase]?.mA || ''} onChange={(e) => handleWithstandTestChange(index, phase, e.target.value, phase, 'mA')} readOnly={!isEditMode} className={`w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/></td>
                  <td className="border border-gray-300 dark:border-gray-700 px-3 py-2 whitespace-nowrap"><input type="text" value={reading[phase]?.nF || ''} onChange={(e) => handleWithstandTestChange(index, phase, e.target.value, phase, 'nF')} readOnly={!isEditMode} className={`w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/></td>
                  </React.Fragment>
                ))}
              </tr>
            ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 print:hidden test-eqpt-onscreen">
          ...
          <div>
            ...
            <EquipmentAutocomplete
              ...
              onChange={(value) => handleChange('equipment', {...formData.equipment, ohmmeter: value})}
              onSelect={(equipment) => {
                handleChange('equipment', {
                  ...
                  ohmCalDate: formatLocalDateShort(equipment.calibration_date)
                });
              }}
              readOnly={!isEditMode}
          ...
          <div>
            ...
            <input type="text" value={formData.equipment?.ohmSerialNumber || ''} onChange={(e) => handleChange('equipment', {...formData.equipment, ohmSerialNumber: e.target.value})} readOnly={!isEditMode} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white">AMP ID</label>
            <input type="text" value={formData.equipment?.ohmAmpId || ''} onChange={(e) => handleChange('equipment', {...formData.equipment, ohmAmpId: e.target.value})} readOnly={!isEditMode} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white">Cal Date</label>
            <input type="text" value={formData.equipment?.ohmCalDate || ''} onChange={(e) => handleChange('equipment', {...formData.equipment, ohmCalDate: e.target.value})} readOnly={!isEditMode} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/>
          </div>
            ...
            <EquipmentAutocomplete
              ...
              onSelect={(equipment) => {
                handleChange('equipment', {
                  ...
                  megohmCalDate: formatLocalDateShort(equipment.calibration_date)
                });
              }}
              readOnly={!isEditMode}
              className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white">Serial Number</label>
            <input type="text" value={formData.equipment?.megohmSerialNumber || ''} onChange={(e) => handleChange('equipment', {...formData.equipment, megohmSerialNumber: e.target.value})} readOnly={!isEditMode} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white">AMP ID</label>
            <input type="text" value={formData.equipment?.megohmAmpId || ''} onChange={(e) => handleChange('equipment', {...formData.equipment, megohmAmpId: e.target.value})} readOnly={!isEditMode} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white">Cal Date</label>
            <input type="text" value={formData.equipment?.megohmCalDate || ''} onChange={(e) => handleChange('equipment', {...formData.equipment, megohmCalDate: e.target.value})} readOnly={!isEditMode} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white">Serial Number</label>
            <input type="text" value={formData.equipment?.vlfSerialNumber || ''} onChange={(e) => handleChange('equipment', {...formData.equipment, vlfSerialNumber: e.target.value})} readOnly={!isEditMode} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white">AMP ID</label>
            <input type="text" value={formData.equipment?.vlfAmpId || ''} onChange={(e) => handleChange('equipment', {...formData.equipment, vlfAmpId: e.target.value})} readOnly={!isEditMode} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-white">Cal Date</label>
            <input type="text" value={formData.equipment?.vlfCalDate || ''} onChange={(e) => handleChange('equipment', {...formData.equipment, vlfCalDate: e.target.value})} readOnly={!isEditMode} className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditMode ? 'bg-gray-100 dark:bg-dark-150' : ''}`}/>
          </div>
        </div>
          ...
          <table className="w-full border border-gray-300 print:border-black">
            ...
            <tbody>
              <tr>
                ...
                <td className="p-2 border border-gray-300 print:border-black">{formData.equipment?.ohmCalDate || ''}</td>
              </tr>
              <tr>
                ...
                <td className="p-2 border border-gray-300 print:border-black">{formData.equipment?.megohmCalDate || ''}</td>
              </tr>
              <tr>
  ...
  style.textContent = `
    ...
    input[type="number"] {
      ...
      *[class*="Back to Job"], 
      ...
      div[class*="h-1"][class*="bg-[#f26722"]][class*="print:mb-1"] {
