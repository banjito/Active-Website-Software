...
import { getSectionReferenceCode, getReferenceDescription } from '@/lib/customForms/formCellResolution';
    ...
    if (sec.fields?.length) {
      ...
      if (sec.componentType === ComponentType.JOB_INFO) {
        ...
        options.push({
          ...
          description: 'Job Details » Temperature Correction Factor (from temp/humidity)',
        });
      }
      for (const f of sec.fields) {
        if (sec.componentType === ComponentType.JOB_INFO && f.id === 'tcf') continue;
        ...
        options.push({
          ...
          friendlyRef,
          description: getReferenceDescription(friendlyRef.replace(/^\{|\}$/g, ''), allSections),
        });
      }
    }
    if (sec.columns?.length) {
      // Show at least 10 row options so users can pick any column+row; cap at 25 for dropdown size
      const numRows = Math.min(Math.max(sec.rows ?? 1, 10), sec.maxRows ?? 25, 25);
      for (let colIndex = 0; colIndex < sec.columns.length; colIndex++) {
        ...
        options.push({
          ...
          description: getReferenceDescription(friendlySame.replace(/^\{|\}$/g, ''), allSections),
        });
        for (let r = 1; r <= numRows; r++) {
          ...
          options.push({
            ...
            description: getReferenceDescription(friendlyRow.replace(/^\{|\}$/g, ''), allSections),
          });
        }
      }
    }
...
interface SectionEditorProps {
  ...
  onAppendOptionToSectionFieldsField?: (sectionId: string, fieldIndex: number) => void;
  onClose: () => void;
    ...
    <div className="w-96 bg-white dark:bg-dark-150 border-l dark:border-gray-700 flex flex-col overflow-hidden">
      ...
      <div className="p-4 border-b dark:border-gray-700 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {(onSaveAsDefault || onSaveAsNewComponent || onUpdateSavedComponent) && (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  type="button"
                  disabled={savingDefault || savingNewComponent || updatingSavedComponent}
                  className="shrink-0 flex items-center gap-1 px-2 py-1 text-xs font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded border border-gray-200 dark:border-gray-600"
                >
                  Save as Component
              ...
              <DropdownMenuContent align="start" className="min-w-[12rem]">
                {onUpdateSavedComponent && section.savedComponentId && (
                  <>
                    ...
                    >
                      ...
                      {updatingSavedComponent ? 'Updating…' : 'Update saved component'}
                    </DropdownMenuItem>
                ...
                {onSaveAsNewComponent && (
                  <>
                    ...
                    <DropdownMenuLabel className="text-[10px] font-normal text-gray-500 dark:text-gray-400">
                      Add to component library so others can drag it into their reports
                    </DropdownMenuLabel>
                  </>
                )}
                {isAdmin && onSaveAsDefault && (
                  <>
                    ...
                    >
                      <Save className="w-3.5 h-3.5" />
                      {savingDefault ? 'Saving…' : 'Save as new default component'}
                    </DropdownMenuItem>
                    <DropdownMenuLabel className="text-[10px] font-normal text-gray-500 dark:text-gray-400">
                      Override default for this component type (column and per-cell formulas)
                    </DropdownMenuLabel>
                  </>
                )}
              </DropdownMenuContent>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        {section.componentType === ComponentType.JOB_INFO && (
          <div className="rounded-lg border border-amber-300 dark:border-amber-600 bg-amber-50 dark:bg-amber-900/20 p-3">
            ...
            <p className="text-[11px] text-amber-800/90 dark:text-amber-200/90 mt-0.5">
              Temperature Correction Factor (from Temp °F / °C, Humidity above). Use this reference in any table formula.
            </p>
          </div>
        )}
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            {section.columns && section.columns.length > 0 && onToggleFormulaEditing && (
              <button
                type="button"
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded border transition-colors ${
                  isFormulaEditing
                    ...
                    : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 border-gray-200 dark:border-gray-600'
                }`}
              >
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => onUpdate({ referenceCode: e.target.value || undefined })}
              ...
              <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5">
                ...
                {'}'} ({getReferenceDescription(`${getSectionReferenceCode(section)}.C1.R2`, allSections.length ? allSections : [section])})
              </p>
              {section.componentType === ComponentType.JOB_INFO && (
                <p className="text-[10px] text-gray-600 dark:text-gray-300 mt-1 font-medium">
                  In formulas use {'{JD.TCF}'} for Temperature Correction Factor (from this section’s temp/humidity).
                </p>
              )}
            </div>
            ...
            {section.componentType === ComponentType.CONTACT_RESISTANCE && (
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => onUpdate({ showDeviation: e.target.checked })}
                  className="rounded"
                />
                <label htmlFor="showDeviation" className="text-sm text-gray-700 dark:text-gray-300">
        ...
        {section.columns && (
          <div>
            ...
            <div className="space-y-3">
              ...
              {onLinkRowCountWith && section.componentType !== ComponentType.CONDITIONAL_TABLE && (
                <div className="border-t border-gray-200 dark:border-gray-700 pt-3 mt-3">
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Link row count with
                  </label>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 mb-1.5">
                    When linked, adding or removing a row in one table updates the other so formulas stay in sync.
                  </p>
                  <select
                    value={
                      section.rowCountLinkGroupId
                        ? (allSections.find(s => s.id !== section.id && s.rowCountLinkGroupId === section.rowCountLinkGroupId)?.id ?? '')
                        : ''
                    }
                    onChange={(e) => onLinkRowCountWith(e.target.value || null)}
                    className="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-dark-150 text-gray-900 dark:text-white"
                  >
                    <option value="">None</option>
                    {allSections
                      .filter(s => s.id !== section.id && s.columns?.length && s.componentType !== ComponentType.CONDITIONAL_TABLE)
                      .map(s => (
                  ...
                  {section.rowCountLinkGroupId && (
                    <p className="text-[10px] text-green-600 dark:text-green-400 mt-1">
                      Linked with: {allSections.filter(s => s.rowCountLinkGroupId === section.rowCountLinkGroupId && s.id !== section.id).map(s => s.title || s.id).join(', ') || 'other table(s)'}
                    </p>
                  )}
                </div>
              )}
              ...
              {section.componentType !== ComponentType.CONDITIONAL_TABLE && (
                <div className="border-t border-gray-200 dark:border-gray-700 pt-3 mt-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        onUpdate({
                          aboveTableFields: [...(section.aboveTableFields || []), newField],
                        });
                      }}
                    >
                  ...
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 mb-2">
                    Optional inputs shown above the table (e.g. Test Voltage, Test Duration). Choose input type for each.
                  </p>
                  <div className="space-y-2">
                    {(section.aboveTableFields || []).map((f, idx) => (
                      <div
                        key={f.id}
                        className="p-2 rounded border border-gray-200 dark:border-gray-600 space-y-1.5 bg-gray-50/50 dark:bg-dark-100/50"
                      >
                        <div className="flex items-center gap-2">
                          <Input
                            value={f.label}
                            onChange={(e) => {
                              const next = [...(section.aboveTableFields || [])];
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              next[idx] = {
                                ...f,
                                type: newType,
                                ...(newType === FieldType.SELECT && !(f.options?.length) ? { options: [{ label: 'Option 1', value: 'Option 1' }] } : {}),
                              };
                              onUpdate({ aboveTableFields: next });
                            }}
                            className="px-2 py-1 text-xs bg-white dark:bg-dark-150 border border-gray-300 dark:border-gray-600 rounded text-gray-900 dark:text-white"
                          >
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              const next = (section.aboveTableFields || []).filter((_, i) => i !== idx);
                              onUpdate({ aboveTableFields: next });
                            }}
                        ...
                        {f.type === FieldType.SELECT && (
                          <div className="pl-1 space-y-1">
                            <span className="text-[10px] text-gray-500 dark:text-gray-400">Options:</span>
                              ...
                              <div key={oIdx} className="flex items-center gap-1">
                                <Input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    opts[oIdx] = { label: e.target.value, value: e.target.value };
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    const next = [...(section.aboveTableFields || [])];
                                    next[idx] = { ...next[idx], options: (next[idx].options || []).filter((_, i) => i !== oIdx) };
                                    onUpdate({ aboveTableFields: next });
                                  }}
                            ...
                            <button
                              ...
                              onClick={() => {
                                const next = [...(section.aboveTableFields || [])];
                                const opts = [...(next[idx].options || []), { label: `Option ${(next[idx].options?.length || 0) + 1}`, value: `Option ${(next[idx].options?.length || 0) + 1}` }];
                                next[idx] = { ...next[idx], options: opts };
                                onUpdate({ aboveTableFields: next });
                              }}
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-3 mt-3">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">
                  Margins and row height for preview and print/PDF (e.g. 0, 4px, 0.25in).
                </p>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-xs text-gray-500 dark:text-gray-400 mb-0.5">Margin top</label>
                    <Input
                      value={section.printLayout?.marginTop ?? ''}
                      onChange={(e) => onUpdate({ printLayout: { ...section.printLayout, marginTop: e.target.value || undefined } })}
                      placeholder="0"
                      className="text-xs py-1"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 dark:text-gray-400 mb-0.5">Margin bottom</label>
                    <Input
                      value={section.printLayout?.marginBottom ?? ''}
                      onChange={(e) => onUpdate({ printLayout: { ...section.printLayout, marginBottom: e.target.value || undefined } })}
                      placeholder="0"
                      className="text-xs py-1"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => onUpdate({ printLayout: { ...section.printLayout, marginLeft: e.target.value || undefined } })}
                      placeholder="0"
                      className="text-xs py-1"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => onUpdate({ printLayout: { ...section.printLayout, marginRight: e.target.value || undefined } })}
                      placeholder="0"
                      className="text-xs py-1"
                    />
                  </div>
                ...
                <div className="mt-2">
                  <label className="block text-xs text-gray-500 dark:text-gray-400 mb-0.5">Row height</label>
                  <Input
                    value={section.printLayout?.rowHeight ?? ''}
                    onChange={(e) => onUpdate({ printLayout: { ...section.printLayout, rowHeight: e.target.value || undefined } })}
                    ...
                    <div className="space-y-2">
                      {section.columns?.map((column, index) => (
                        <SortableColumnEditor
                          ...
                          onAppendOption={onAppendFieldOption ? () => onAppendFieldOption(section.id, index) : undefined}
                          onDelete={() => {
              ...
              {(section.componentType === ComponentType.CONDITIONAL_TABLE ||
                (section.columns?.length && (section.settingFields?.length || section.conditionalRows?.length))) && (
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          onUpdate({
                            settingFields: [...(section.settingFields || []), newSetting],
                          });
                        }}
                      >
                        <Plus className="w-3 h-3 mr-1" />
                    ...
                    <div className="space-y-2">
                      {(section.settingFields || []).map((sf, idx) => (
                        <div
                          key={sf.id}
                          className="p-2 rounded border border-gray-200 dark:border-gray-600 space-y-1.5"
                        >
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() => {
                                const next = (section.settingFields || []).filter((_, i) => i !== idx);
                                onUpdate({ settingFields: next });
                              }}
                            >
                          ...
                          <div className="text-[10px] text-gray-500 dark:text-gray-400">
                            Options: {sf.options.map((o) => o.label || o.value).join(', ')}
                          </div>
                          <div className="flex flex-wrap gap-1">
                            {sf.options.map((opt, oIdx) => (
                              <span
                                key={oIdx}
                                className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-gray-100 dark:bg-dark-200 text-[10px]"
                              >
                                <Input
                                  value={opt.value}
                                  onChange={(e) => {
                                    const next = [...(section.settingFields || [])];
                                    const opts = [...(next[idx].options || [])];
                                    opts[oIdx] = { ...opt, value: e.target.value, label: e.target.value };
                                    next[idx] = { ...next[idx], options: opts };
                                    onUpdate({ settingFields: next });
                                  }}
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    const next = [...(section.settingFields || [])];
                                    const opts = next[idx].options.filter((_, i) => i !== oIdx);
                                    next[idx] = { ...next[idx], options: opts };
                                    onUpdate({ settingFields: next });
                                  }}
                                >
                            ...
                            <button
                              ...
                              onClick={() => {
                                const next = [...(section.settingFields || [])];
                                const newOpt = { value: `Option ${(next[idx].options?.length || 0) + 1}`, label: `Option ${(next[idx].options?.length || 0) + 1}` };
                                next[idx] = { ...next[idx], options: [...(next[idx].options || []), newOpt] };
                                onUpdate({ settingFields: next });
                              }}
                              className="text-[10px] text-[#f26722] hover:underline"
                            >
                          ...
                          <div>
                            <label className="text-[10px] text-gray-500 dark:text-gray-400">Default: </label>
                            <select
                              value={sf.defaultValue ?? ''}
                              onChange={(e) => {
                                const next = [...(section.settingFields || [])];
                                next[idx] = { ...sf, defaultValue: e.target.value };
                                onUpdate({ settingFields: next });
                              }}
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (section.settingFields?.length) {
                            ...
                            newRow.visibleWhen = { [first.id]: first.options[0]?.value ?? '' };
                          }
                          onUpdate({
                            conditionalRows: [...(section.conditionalRows || []), newRow],
                          });
                        }}
                      >
                        <Plus className="w-3 h-3 mr-1" />
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <Input
                              value={row.label}
                              onChange={(e) => {
                                const next = [...(section.conditionalRows || [])];
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() => {
                                const next = (section.conditionalRows || []).filter((_, i) => i !== rIdx);
                                onUpdate({ conditionalRows: next });
                              }}
                            >
                              <Trash2 className="w-3 h-3 text-red-500" />
                          ...
                          {(section.settingFields || []).map((sf) => {
                            ...
                            return (
                              <div key={sf.id} className="flex flex-wrap items-center gap-1.5 text-[10px]">
                                <span className="text-gray-500 dark:text-gray-400">Show when {sf.label}:</span>
                                {sf.options.map((opt) => {
                                      ...
                                      <input
                                        ...
                                        onChange={(e) => {
                                          ...
                                          const nextList = e.target.checked
                                            ? [...currentList.filter((v) => v !== opt.value), opt.value]
                                            : currentList.filter((v) => v !== opt.value);
                                          const vw = { ...(next[rIdx].visibleWhen || {}) };
                                          if (nextList.length === 0) delete vw[sf.id];
                                          else vw[sf.id] = nextList.length === 1 ? nextList[0] : nextList;
                                          next[rIdx] = { ...next[rIdx], visibleWhen: vw };
                                          onUpdate({ conditionalRows: next });
                                        }}
                                        className="rounded"
                                      />
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                {section.componentType === ComponentType.JOB_INFO ? (
                  <div className="px-3 py-2 rounded-md bg-gray-100 dark:bg-dark-200 text-gray-700 dark:text-gray-300 text-sm border border-gray-200 dark:border-gray-600">
                ...
                <div className="space-y-2">
                  {section.fields?.map((field, index) => (
                    <FieldEditor
                      ...
                      onAppendOption={onAppendOptionToSectionFieldsField ? () => onAppendOptionToSectionFieldsField(section.id, index) : undefined}
                      onDelete={() => {
        ...
        {section.field && (
          <div>
            ...
            <FieldEditor
              ...
              onAppendOption={onAppendOptionToSectionField ? () => onAppendOptionToSectionField(section.id) : undefined}
              onDelete={() => {}}
              hideDelete
            />
          </div>
        ...
        {!section.columns && (section.fields?.length || section.field || section.checklistItems?.length) && (
          <div className="border-t border-gray-200 dark:border-gray-700 pt-3 mt-3">
...
const SortableColumnEditor: React.FC<{
  ...
  return (
    <div ref={setNodeRef} style={style}>
      <ColumnEditor dragHandleProps={{ ...attributes, ...listeners }} {...rest} />
    </div>
  );
};
...
}> = ({ column, currentSectionId, allSections, settingFields, onUpdate, onDelete, onAppendOption, dragHandleProps }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const field = column.field;
  const behavior = field.cellBehavior ?? 'user';
  const refOptions = buildFieldRefOptions(allSections /* include current section so user can pick column+row from same table */);
  ...
  const handleBehaviorChange = (next: 'user' | 'populate' | 'calculate') => {
    onUpdate({
      field: {
        ...field,
        cellBehavior: next,
        ...(next === 'user' && { populateFrom: undefined, calculation: undefined, readOnly: undefined }),
        ...(next === 'populate' && { calculation: undefined }),
        ...(next === 'calculate' && { populateFrom: undefined, readOnly: true }),
      },
    });
  };
    ...
    onUpdate({
      field: {
        ...
        populateFrom: {
          ...
          fieldId: opt.fieldId,
          ...(opt.rowIndex != null ? { rowIndex: opt.rowIndex } : { rowMode: opt.rowMode }),
        },
        readOnly: true,
      },
    });
  ...
  const getPopulateFromValue = () => {
    ...
    const opt = refOptions.find(
      (o) =>
        ...
        o.fieldId === p.fieldId &&
        (p.rowIndex != null ? o.rowIndex === p.rowIndex : o.rowMode === p.rowMode)
    );
    return opt?.refToken ?? '';
  };
      ...
      {isExpanded && (
        <div className="space-y-2 pt-2 border-t dark:border-gray-700">
          ...
          {settingFields && settingFields.length > 0 && (
            <div className="space-y-1.5 pt-1 border-t border-gray-200 dark:border-gray-600">
          ...
          {allSections.length > 0 && (
            <div className="space-y-2">
              ...
              {behavior === 'user' && (
                <div className="mt-2 space-y-2 pt-2 border-t border-gray-200 dark:border-gray-600">
                  <label className="text-xs font-medium text-gray-700 dark:text-gray-300 block">
                    Input type
                  </label>
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (newType === FieldType.SELECT && !(field.options && field.options.length)) {
                        updates.options = [{ label: 'Option 1', value: 'Option 1' }];
                      }
                      onUpdate({ field: { ...field, ...updates } });
                    }}
                    className="w-full px-2 py-1.5 text-xs bg-white dark:bg-dark-150 border border-gray-300 dark:border-gray-700 rounded-md focus:ring-2 focus:ring-[#f26722] focus:border-[#f26722] text-gray-900 dark:text-white"
                  >
                    <option value={FieldType.TEXT}>Text</option>
                    <option value={FieldType.NUMBER}>Number</option>
                    <option value={FieldType.DATE}>Date</option>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className="h-6 px-2 text-xs border border-gray-300 dark:border-gray-600 rounded hover:bg-gray-100 dark:hover:bg-dark-200 inline-flex items-center gap-1"
                        >
                      ...
                      <ul className="space-y-1 max-h-40 overflow-y-auto">
                        {(field.options || []).map((opt, idx) => (
                          <li key={`${field.id}-opt-${idx}`} className="flex items-center gap-1.5">
                            <Input
                              value={opt?.label ?? ''}
                              onChange={(e) => {
                                const next = [...(field.options || [])];
                                next[idx] = { label: e.target.value, value: e.target.value };
                                onUpdate({ field: { ...field, options: next } });
                              }}
                            ...
                            <button
                              ...
                              className="p-1 text-gray-400 hover:text-red-600 dark:hover:text-red-400 shrink-0"
                              title="Remove option"
                            >
                              <Trash2 className="w-3 h-3" />
                            </button>
                      ...
                      {(field.options || []).length === 0 && (
                        <p className="text-xs text-gray-500 dark:text-gray-400">Add options above so users can pick a value.</p>
                      )}
                    </div>
                  )}
                </div>
              ...
              {behavior === 'populate' && (
                <div className="mt-2">
                  ...
                  >
                    ...
                    {Array.from(
                      new Map(refOptions.map((o) => [o.sectionId, o.sectionTitle])).entries()
              ...
              {behavior === 'calculate' && (
                <div className="mt-2 space-y-1.5">
                  <label className="text-xs text-gray-500 dark:text-gray-400 block">
                    Formula — use refs like {'{IR.C1.R2}'} (table) or {'{JD.TCF}'} (Job Details TCF)
                  </label>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400">
                    Job Details: {'{JD.TCF}'} = Temperature Correction Factor (from temp/humidity). Add a Job Details section to the form to use it.
                  </p>
                  <Textarea
                  ...
                  <div className="flex flex-wrap gap-1">
                        ...
                        onClick={() => {
                          ...
                          onUpdate({
                            field: {
                              ...field,
                              calculation: {
                                formula: current + (current ? ' ' : '') + opt.friendlyRef,
                                dependsOn: field.calculation?.dependsOn ?? [],
                              },
                            },
                          });
                        }}
                        className="px-2 py-1 text-xs font-mono bg-gray-200 dark:bg-gray-600 rounded hover:bg-[#f26722] hover:text-white dark:hover:bg-[#e55611]"
                    ...
                    {refOptions.length > 8 && (
                      <span className="text-xs text-gray-500">+{refOptions.length - 8} more below</span>
                    )}
                  </div>
                  <select
                    className="w-full mt-1 px-2 py-1 text-xs font-mono bg-white dark:bg-dark-150 border border-gray-300 dark:border-gray-700 rounded-md"
                    onChange={(e) => {
                  ...
                  >
                    <option value="">Insert reference (pick section, column, row)…</option>
                    {Array.from(
                      new Map(refOptions.map((o) => [o.sectionId, o.sectionTitle])).entries()
                    ).map(([sectionId, sectionTitle]) => (
                      <optgroup key={sectionId} label={sectionTitle}>
...
const FieldEditor: React.FC<{
  ...
  const handleAddOption = () => {
    ...
    if (onAppendOption) {
      ...
      const currentOptions = field.options || [];
      onUpdate({ options: [...currentOptions, { label: 'New option', value: 'new-option' }] });
    }
            ...
            <div className="space-y-2 pt-1 border-t dark:border-gray-700">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="h-6 px-2 text-xs shrink-0 border border-gray-300 dark:border-gray-600 rounded hover:bg-gray-100 dark:hover:bg-dark-200 inline-flex items-center gap-1 disabled:opacity-50 disabled:pointer-events-none"
                >
                  <Plus className="w-3 h-3" />
              ...
              {(field.options || []).length === 0 && (
                <p className="text-xs text-gray-500 dark:text-gray-400">No options yet. Click Add to define dropdown choices.</p>
              )}

              {(field.options || []).length > 8 && (
                <p className="text-xs text-gray-500 dark:text-gray-400">Scroll above to see all options. You can add as many as you need.</p>
              )}
            </div>
          )}
