...
export function TechnicianScheduleManagement({
  portalType,
  division,
}: TechnicianScheduleManagementProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchTimeOffRequests = async () => {
      ...
      try {
        ...
        setError(
          "An unexpected error occurred while loading time-off requests.",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const handleTimeOffInputChange = (
    e: React.ChangeEvent<
  ...
  const handleSubmitTimeOff = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } =
        await schedulingService.createTimeOffRequest(timeOffRequest);
      ...
      if (error) {
        ...
        const { data: updatedRequests } =
          await schedulingService.getTimeOffRequests({
            technicianId: selectedTechnician,
            portalType,
            division,
          });
  ...
  const handleUpdateTimeOffStatus = async (
    requestId: string,
    ...
    try {
      const { data, error } =
        await schedulingService.updateTimeOffRequestStatus(
          requestId,
          status,
          user.id,
        );
          ...
          <form onSubmit={handleSubmitTimeOff} className="space-y-4">
            ...
            <div className="flex items-center">
              <input
                ...
                className="mr-2"
              />
              <label htmlFor="isFullDay" className="text-sm font-medium">
                Full day
              </label>
            </div>
