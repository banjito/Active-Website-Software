...
const RATED_VOLTAGE_OPTIONS = [
  '250','480','600','1000','2500','5000','8000','15000','25000','34500','46000'
];

type StatusType = 'PASS' | 'FAIL' | 'LIMITED SERVICE';

interface InsulationRow { busSection: string; ag: string; bg: string; cg: string; ab: string; bc: string; ca: string; an: string; bn: string; cn: string; }
interface ContactRow { busSection: string; aPhase: string; bPhase: string; cPhase: string; neutral: string; ground: string; }
interface DielectricRow { busSection: string; ag: string; bg: string; cg: string; result?: 'PASS' | 'FAIL' | ''; }
...
const SwitchgearSwitchboardAssembliesATS25Report: React.FC = () => {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    contactResistance: ['Section 1', 'Section 2', 'Section 3', 'Section 4', 'Section 5'].map(b => ({ busSection: b, aPhase: '', bPhase: '', cPhase: '', neutral: '', ground: '' })),
    contactUnit: 'µΩ',
    contactEvaluation: defaultBus.map(() => ({ deviation: 'N/A', criteria: '<50%', result: 'N/A' })),
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        const dVolt = data.contact_resistance?.dielectricTestVoltage || undefined;
        const dDur = data.contact_resistance?.dielectricDuration || undefined;
        const te = (data as any).test_equipment || (data as any).test_equipment_used || info.testEquipment || undefined;
        setFormData(prev => ({
          ...prev,
          ...
          nameplate: {
            ...
            phaseConfiguration: info.phaseConfiguration || prev.nameplate.phaseConfiguration,
          },
  ...
  const handleC = (c: number) => {
    ...
    const tcf = getTCF(c);
    setFormData(prev => ({ ...prev, temperature: { ...prev.temperature, celsius: c, fahrenheit: f, tcf } }));
  };
  ...
  useEffect(() => {
    ...
    setFormData(prev => ({
      ...
      tempCorrected: prev.insulationMeasured.map(row => {
        ...
        (['ag','bg','cg','ab','bc','ca','an','bn','cn'] as (keyof InsulationRow)[]).forEach(k => {
          ...
          if (!v || v.toLowerCase() === 'n/a') {
            ...
            if (isFinite(n)) {
              ...
              const formatted = corrected % 1 === 0 ? corrected.toString() : corrected.toFixed(2);
  ...
  const addContactResistanceRow = () => {
    setFormData(prev => {
      ...
      return {
        ...
        contactEvaluation: [...prev.contactEvaluation, { deviation: 'N/A', criteria: '<50%', result: 'N/A' }]
      };
    });
  };
  ...
  const removeContactResistanceRow = (index: number) => {
    ...
    const confirmed = window.confirm('Are you sure you want to remove this row? Any data in this row will be deleted.');
    if (!confirmed) return;
    setFormData(prev => ({
      ...prev,
  ...
  const removeDielectricWithstandRow = (index: number) => {
    ...
    setFormData(prev => ({
      ...prev,
      dielectricWithstand: prev.dielectricWithstand.filter((_, i) => i !== index)
    }));
  };
  ...
  useEffect(() => {
    setFormData(prev => {
      ...
      if (nRes === prev.contactNeutral.result && gRes === prev.contactGround.result) return prev;
      return { ...prev, contactNeutral: { ...prev.contactNeutral, result: nRes }, contactGround: { ...prev.contactGround, result: gRes } };
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [neutralDeviation, groundDeviation, formData.contactNeutral.criteria, formData.contactGround.criteria]);

  // Table 100.1 Criteria auto-calculation based on test voltage and unit (MΩ/GΩ)
  const computeCriteriaFromVoltage = (volts: number, unit: string): number | null => {
  ...
  useEffect(() => {
    ...
    if (val !== null) {
      const formatted = Number.isFinite(val) ? (val % 1 === 0 ? val.toString() : val.toString()) : '';
      const next = `≥ ${formatted}`;
      if (formData.criteriaValue !== next || formData.criteriaUnits !== units) {
        setFormData(prev => ({ ...prev, criteriaValue: next, criteriaUnits: units }));
      }
    }
  ...
  return (
    <ReportWrapper isPrintMode={isPrintMode}>
      ...
      <div className="print:flex hidden items-center justify-between border-b-2 border-gray-800 pb-4 mb-6">
        ...
        <div className="flex-1 text-center"><h1 className="text-2xl font-bold text-black mb-1">{reportName}</h1></div>
        ...
        <div className="max-w-7xl w-full space-y-6">
              ...
              <div>
                <div className="mb-4 flex">
                  ...
                  <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
                    <input type="text" value={maskCustomerName(formData.customerName)} readOnly className="w-full bg-transparent border-none focus:ring-0 cursor-default" />
                  </div>
                </div>
                <div className="mb-4 flex">
                  <label className="inline-block w-24 font-medium text-gray-700 dark:text-gray-300">Site Address</label>
                  <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
                    <input type="text" value={formData.customerLocation} readOnly className="w-full bg-transparent border-none focus:ring-0 cursor-default" />
                  </div>
                </div>
                <div className="mb-4 flex">
                ...
                <div className="mb-4 flex">
                  ...
                  <div className="flex-1 border-b border-gray-300 dark:border-gray-600">
                    <input type="text" value={formData.identifier} onChange={e => setFormData(p => ({ ...p, identifier: e.target.value }))} readOnly={!isEditing} placeholder="Enter an identifier for this cable" className={`w-full bg-transparent border-none focus:ring-0 ${!isEditing ? 'cursor-default' : ''}`} />
                  </div>
                </div>
              </div>
              {/* Right Column */}
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-3 gap-4 print:hidden">
              <div><label className="form-label">Manufacturer</label><input className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} value={formData.nameplate.manufacturer} onChange={e => setFormData(p => ({ ...p, nameplate: { ...p.nameplate, manufacturer: e.target.value } }))} readOnly={!isEditing} /></div>
              <div><label className="form-label">Catalog No.</label><input className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} value={formData.nameplate.catalogNumber} onChange={e => setFormData(p => ({ ...p, nameplate: { ...p.nameplate, catalogNumber: e.target.value } }))} readOnly={!isEditing} /></div>
              <div><label className="form-label">Serial Number</label><input className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} value={formData.nameplate.serialNumber} onChange={e => setFormData(p => ({ ...p, nameplate: { ...p.nameplate, serialNumber: e.target.value } }))} readOnly={!isEditing} /></div>
              <div><label className="form-label">Series</label><input className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} value={formData.nameplate.series} onChange={e => setFormData(p => ({ ...p, nameplate: { ...p.nameplate, series: e.target.value } }))} readOnly={!isEditing} /></div>
              <div><label className="form-label">Type</label><input className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} value={formData.nameplate.type} onChange={e => setFormData(p => ({ ...p, nameplate: { ...p.nameplate, type: e.target.value } }))} readOnly={!isEditing} /></div>
              <div><label className="form-label">System Voltage (V)</label><input className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} value={formData.nameplate.systemVoltage} onChange={e => setFormData(p => ({ ...p, nameplate: { ...p.nameplate, systemVoltage: e.target.value } }))} readOnly={!isEditing} /></div>
              <div>
                <label className="form-label">Rated Voltage (V)</label>
                {isEditing ? (
                  <select
                    value={formData.nameplate.ratedVoltage}
                    onChange={e => setFormData(p => ({ ...p, nameplate: { ...p.nameplate, ratedVoltage: e.target.value } }))}
              ...
              <div><label className="form-label">Rated Current (A)</label><input className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} value={formData.nameplate.ratedCurrent} onChange={e => setFormData(p => ({ ...p, nameplate: { ...p.nameplate, ratedCurrent: e.target.value } }))} readOnly={!isEditing} /></div>
              <div><label className="form-label">SCCR (kA)</label><input className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} value={formData.nameplate.aicRating} onChange={e => setFormData(p => ({ ...p, nameplate: { ...p.nameplate, aicRating: e.target.value } }))} readOnly={!isEditing} /></div>
              <div><label className="form-label">Phase Configuration</label><input className={`form-input ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} value={formData.nameplate.phaseConfiguration} onChange={e => setFormData(p => ({ ...p, nameplate: { ...p.nameplate, phaseConfiguration: e.target.value } }))} readOnly={!isEditing} /></div>
            </div>
            <NameplatePrintTable
              data={{
                manufacturer: formData.nameplate.manufacturer,
          ...
          <div className="mb-6">
            ...
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2 print:text-black print:border-black print:font-bold">Electrical - Contact Resistance Tests</h2>
              {isEditing && (
                <button
            ...
            <div className="overflow-x-auto">
              {/* Measured table */}
              <table className="min-w-[720px] divide-y divide-gray-200 dark:divide-gray-700 table-fixed">
                <colgroup>
                  <col style={{ width: '12%' }} />
                ...
                <thead>
                  <tr>
                    ...
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">A-Phase</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">B-Phase</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">C-Phase</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Neutral</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Ground</th>
                      ...
                      <td className="px-3 py-2">
                        <div className="print:hidden flex items-center gap-1">
                          <input 
                            ...
                            onChange={e => { const list = [...formData.contactResistance]; list[i] = { ...list[i], busSection: e.target.value }; setFormData(p => ({ ...p, contactResistance: list })); }} 
                            readOnly={!isEditing} 
                            className={`block flex-1 rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white text-sm ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} 
                          />
                          {isEditing && formData.contactResistance.length > 1 && (
                            <button
                        ...
                        <div className="hidden print:block text-center">{row.busSection}</div>
                      </td>
                      {(['aPhase','bPhase','cPhase','neutral','ground'] as const).map(key => (
                        <td key={key} className="px-3 py-2">
                          <div className="print:hidden"><input value={row[key]} onChange={e => { const list = [...formData.contactResistance]; list[i] = { ...list[i], [key]: e.target.value }; setFormData(p => ({ ...p, contactResistance: list })); }} readOnly={!isEditing} className={`block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
                          <div className="hidden print:block text-center">{row[key]}</div>
                        </td>
                      ))}
                      <td className="px-3 py-2"><div className="print:hidden"><select value={formData.contactUnit} onChange={e => setFormData(p => ({ ...p, contactUnit: e.target.value }))} disabled={!isEditing} className={`block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>{CONTACT_RESISTANCE_UNITS.map(u => <option key={u} value={u}>{u}</option>)}</select></div><div className="hidden print:block text-center">{formData.contactUnit}</div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 items-start max-w-[100%]">
                ...
                <div className="w-full">
                  <table className="w-full table-fixed border-collapse border border-gray-200 dark:border-gray-700">
                    ...
                    <thead>
                      <tr>
                        <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-left text-xs font-medium text-gray-700 dark:text-white">Value Deviation</th>
                        <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-700 dark:text-white">Criteria</th>
                        <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-700 dark:text-white">Results</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={idx} className="border-t border-gray-200 dark:border-gray-700">
                          ...
                          <td className="px-3 py-2">
                            <div className="print:hidden">
                              <select
                                value={ev.criteria}
                                onChange={e => { const list = [...formData.contactEvaluation]; list[idx] = { ...list[idx], criteria: e.target.value }; setFormData(p => ({ ...p, contactEvaluation: list })); }}
                                disabled={!isEditing}
                              ...
                              >
                                {['<10%','<25%','<50%','<75%','<100%'].map(c => <option key={c} value={c}>{c}</option>)}
                              </select>
                            </div>
                            <div className="hidden print:block text-center">{ev.criteria}</div>
                          </td>
                          <td className="px-3 py-2">
                            <div className="print:hidden">
                              <select
                                value={ev.result}
                                onChange={e => { const list = [...formData.contactEvaluation]; list[idx] = { ...list[idx], result: e.target.value as any }; setFormData(p => ({ ...p, contactEvaluation: list })); }}
                                disabled={!isEditing}
                                className={`w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}
                              >
                                {(['PASS','FAIL','LIMITED SERVICE','N/A'] as const).map(r => <option key={r} value={r}>{r}</option>)}
                              </select>
                            </div>
                            <div className="hidden print:block text-center">{ev.result}</div>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                    ...
                    <tbody>
                      {(['Neutral','Ground'] as const).map((label, i) => (
                        <tr key={label} className="border-t border-gray-200 dark:border-gray-700">
                          <td className="px-3 py-2">{label} : {i === 0 ? neutralDeviation : groundDeviation}</td>
                          <td className="px-3 py-2">
                            <div className="print:hidden">
                              <select
                                value={i === 0 ? formData.contactNeutral.criteria : formData.contactGround.criteria}
                                onChange={e => setFormData(p => i === 0 ? ({ ...p, contactNeutral: { ...p.contactNeutral, criteria: e.target.value } }) : ({ ...p, contactGround: { ...p.contactGround, criteria: e.target.value } }))}
                                disabled={!isEditing}
                                className={`w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}
                              >
                                {['N/A','<10%','<25%','<50%','<75%','<100%'].map(c => <option key={c} value={c}>{c}</option>)}
                              </select>
                            </div>
                            <div className="hidden print:block text-center">{i === 0 ? formData.contactNeutral.criteria : formData.contactGround.criteria}</div>
                          </td>
                          <td className="px-3 py-2">
                            <div className="print:hidden">
                              <select
                                value={i === 0 ? formData.contactNeutral.result : formData.contactGround.result}
                                onChange={e => setFormData(p => i === 0 ? ({ ...p, contactNeutral: { ...p.contactNeutral, result: e.target.value as any } }) : ({ ...p, contactGround: { ...p.contactGround, result: e.target.value as any } }))}
                                disabled={!isEditing}
                                className={`w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}
                              >
                                {(['N/A','PASS','FAIL','LIMITED SERVICE'] as const).map(r => <option key={r} value={r}>{r}</option>)}
                              </select>
                            </div>
                            <div className="hidden print:block text-center">{i === 0 ? formData.contactNeutral.result : formData.contactGround.result}</div>
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <div className="mb-6">
            <div className="w-full h-1 bg-[#f26722] mb-4"></div>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2 print:text-black print:border-black print:font-bold">Electrical - Insulation Resistance Tests</h2>
              {isEditing && (
                <button
            ...
            <div className="flex justify-between items-center mb-2">
              <div className="flex items-center gap-2">
                ...
                <input type="number" value={formData.temperature.fahrenheit} onChange={e => handleF(parseFloat(e.target.value))} readOnly={!isEditing} className={`form-input w-24 ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
                <span className="ml-3">Temp. Correction Factor:</span>
                <span className="font-semibold">{formData.temperature.tcf}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-sm">Test Voltage:</span>
                <select value={formData.insulationTestVoltage} onChange={e => setFormData(p => ({ ...p, insulationTestVoltage: e.target.value }))} disabled={!isEditing} className={`form-select ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>{INSULATION_RESISTANCE_TEST_VOLTAGES.map(v => <option key={v} value={v}>{v}</option>)}</select>
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-fixed">
                ...
                <thead>
                  <tr>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Bus Section</th>
                    {['A-G','B-G','C-G','A-B','B-C','C-A','A-N','B-N','C-N'].map(h => (<th key={h} className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">{h}</th>))}
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Units</th>
                  </tr>
                </thead>
                <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr key={i}>
                      <td className="px-3 py-2">
                        <div className="print:hidden flex items-center gap-1">
                          <input 
                            value={row.busSection} 
                            onChange={e => { const list = [...formData.insulationMeasured]; list[i] = { ...list[i], busSection: e.target.value }; const correctedList = [...formData.tempCorrected]; correctedList[i] = { ...correctedList[i], busSection: e.target.value }; setFormData(p => ({ ...p, insulationMeasured: list, tempCorrected: correctedList })); }} 
                            readOnly={!isEditing} 
                            className={`block flex-1 rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white text-sm ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} 
                          />
                          {isEditing && formData.insulationMeasured.length > 1 && (
                            <button
                      ...
                      {(['ag','bg','cg','ab','bc','ca','an','bn','cn'] as const).map(k => (
                        <td key={k} className="px-3 py-2">
                          <div className="print:hidden"><input value={row[k]} onChange={e => { const list = [...formData.insulationMeasured]; (list[i] as any)[k] = e.target.value; setFormData(p => ({ ...p, insulationMeasured: list })); }} readOnly={!isEditing} className={`block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div>
                          <div className="hidden print:block text-center">{row[k]}</div>
                        </td>
                      ))}
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-fixed">
                  ...
                  <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-200 dark:divide-gray-700">
                    {formData.tempCorrected.map((row, i) => (
                      <tr key={i}>
                        <td className="px-3 py-2"><div className="print:hidden"><input value={row.busSection} onChange={e => { const list = [...formData.tempCorrected]; list[i] = { ...list[i], busSection: e.target.value }; setFormData(p => ({ ...p, tempCorrected: list })); }} readOnly={!isEditing} className={`block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white text-sm ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div><div className="hidden print:block text-center">{row.busSection}</div></td>
                        {(['ag','bg','cg','ab','bc','ca','an','bn','cn'] as const).map(k => (
            ...
            <div className="flex items-center gap-3 mt-3">
              ...
              <input value={formData.criteriaValue} onChange={e => setFormData(p => ({ ...p, criteriaValue: e.target.value }))} readOnly={!isEditing} className={`form-input w-24 ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
              <select value={formData.criteriaUnits} onChange={e => setFormData(p => ({ ...p, criteriaUnits: e.target.value }))} disabled={!isEditing} className={`form-select w-20 ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>{INSULATION_RESISTANCE_UNITS.map(u => <option key={u} value={u}>{u}</option>)}</select>
              <span className="ml-4">Overall Result:</span>
              <select value={formData.status} onChange={e => isEditing && setFormData(p => ({ ...p, status: e.target.value as StatusType }))} disabled={!isEditing} className={`form-select w-36 ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>
                {(['PASS','FAIL','LIMITED SERVICE'] as StatusType[]).map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="mb-6">
            <div className="w-full h-1 bg-[#f26722] mb-4"></div>
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2 print:text-black print:border-black print:font-bold flex-grow">Electrical - Dielectric Withstand Tests</h2>
              <div className="flex items-center gap-2">
                <span className="text-sm">Test Voltage:</span>
                <select value={formData.dielectricTestVoltage} onChange={e => setFormData(p => ({ ...p, dielectricTestVoltage: e.target.value }))} disabled={!isEditing} className={`form-select ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>
                  {['1.6 kVAC','2.2 kVAC','14 kVAC','27 kVAC','37 kVAC','45 kVAC','60 kVAC','120 kVAC','2.3 kVDC','3.1 kVDC','20 kVDC','37.5 kVDC'].map(v => <option key={v} value={v}>{v}</option>)}
                </select>
                <span className="text-sm ml-3">Test Duration:</span>
                <input value={formData.dielectricTestDuration} onChange={e => setFormData(p => ({ ...p, dielectricTestDuration: e.target.value }))} readOnly={!isEditing} className={`form-input w-28 ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} />
                {isEditing && (
                  <button
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-fixed">
                ...
                <thead>
                  <tr>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Bus Section</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">A-G</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">B-G</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">C-G</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Units</th>
                    <th className="px-3 py-2 bg-gray-50 dark:bg-dark-150 text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Results</th>
                  </tr>
                </thead>
                <tbody className="bg-white dark:bg-dark-150 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr key={i}>
                      <td className="px-3 py-2">
                        <div className="print:hidden flex items-center gap-1">
                          <input 
                            value={row.busSection} 
                            onChange={e => { const list = [...formData.dielectricWithstand]; list[i] = { ...list[i], busSection: e.target.value }; setFormData(p => ({ ...p, dielectricWithstand: list })); }} 
                            readOnly={!isEditing} 
                            className={`block flex-1 rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white text-sm ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} 
                          />
                          {isEditing && formData.dielectricWithstand.length > 1 && (
                            <button
                      ...
                      {(['ag','bg','cg'] as const).map(k => (
                        <td key={k} className="px-3 py-2"><div className="print:hidden"><input value={row[k]} onChange={e => { const list = [...formData.dielectricWithstand]; (list[i] as any)[k] = e.target.value; setFormData(p => ({ ...p, dielectricWithstand: list })); }} readOnly={!isEditing} className={`block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`} /></div><div className="hidden print:block text-center">{row[k]}</div></td>
                      ))}
                      <td className="px-3 py-2"><div className="print:hidden"><select value={formData.dielectricUnit} onChange={e => setFormData(p => ({ ...p, dielectricUnit: e.target.value }))} disabled={!isEditing} className={`block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-[#f26722] focus:ring-[#f26722] dark:bg-dark-150 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-150' : ''}`}>{DIELECTRIC_WITHSTAND_UNITS.map(u => <option key={u} value={u}>{u}</option>)}</select></div><div className="hidden print:block text-center">{formData.dielectricUnit}</div></td>
                      <td className="px-3 py-2">
                        <div className="print:hidden">
                          <select
                            value={row.result ?? ''}
                            onChange={e => { const list = [...formData.dielectricWithstand]; (list[i] as any).result = e.target.value as 'PASS' | 'FAIL' | ''; setFormData(p => ({ ...p, dielectricWithstand: list })); }}
                            disabled={!isEditing}
                        ...
                        <div className="hidden print:block text-center">{row.result}</div>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-3 print:hidden">The same piece of equipment may be used for more than one function (e.g. resistance and insulation resistance, or megohmmeter and hipot).</p>
            <div className="grid grid-cols-4 gap-4 print:hidden">
              <div>
                ...
                <EquipmentAutocomplete
                  ...
                  onChange={value => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, lowResistanceOhmmeter: { ...p.testEquipment.lowResistanceOhmmeter, name: value } } }))}
                  onSelect={(equipment) => {
              ...
              <div><label className="form-label">Serial Number:</label><input className="form-input" value={formData.testEquipment.lowResistanceOhmmeter.serialNumber} onChange={e => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, lowResistanceOhmmeter: { ...p.testEquipment.lowResistanceOhmmeter, serialNumber: e.target.value } } }))} readOnly={!isEditing} /></div>
              <div><label className="form-label">AMP ID:</label><input className="form-input" value={formData.testEquipment.lowResistanceOhmmeter.ampId} onChange={e => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, lowResistanceOhmmeter: { ...p.testEquipment.lowResistanceOhmmeter, ampId: e.target.value } } }))} readOnly={!isEditing} /></div>
              <div><label className="form-label">Cal Date:</label><input className="form-input" value={formData.testEquipment.lowResistanceOhmmeter.calDate} onChange={e => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, lowResistanceOhmmeter: { ...p.testEquipment.lowResistanceOhmmeter, calDate: e.target.value } } }))} readOnly={!isEditing} /></div>
              <div>
              ...
              <div><label className="form-label">Serial Number:</label><input className="form-input" value={formData.testEquipment.megohmmeter.serialNumber} onChange={e => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, megohmmeter: { ...p.testEquipment.megohmmeter, serialNumber: e.target.value } } }))} readOnly={!isEditing} /></div>
              <div><label className="form-label">AMP ID:</label><input className="form-input" value={formData.testEquipment.megohmmeter.ampId} onChange={e => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, megohmmeter: { ...p.testEquipment.megohmmeter, ampId: e.target.value } } }))} readOnly={!isEditing} /></div>
              ...
              <div>
                ...
                <EquipmentAutocomplete
                  ...
                  onChange={value => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, hipot: { ...p.testEquipment.hipot, name: value } } }))}
                  onSelect={(equipment) => {
                    const formatDate = (dateString: string | null): string => {
                      if (!dateString) return '';
                      try {
              ...
              <div><label className="form-label">Serial Number:</label><input className="form-input" value={formData.testEquipment.hipot.serialNumber} onChange={e => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, hipot: { ...p.testEquipment.hipot, serialNumber: e.target.value } } }))} readOnly={!isEditing} /></div>
              <div><label className="form-label">AMP ID:</label><input className="form-input" value={formData.testEquipment.hipot.ampId} onChange={e => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, hipot: { ...p.testEquipment.hipot, ampId: e.target.value } } }))} readOnly={!isEditing} /></div>
              <div><label className="form-label">Cal Date:</label><input className="form-input" value={formData.testEquipment.hipot.calDate} onChange={e => setFormData(p => ({ ...p, testEquipment: { ...p.testEquipment, hipot: { ...p.testEquipment.hipot, calDate: e.target.value } } }))} readOnly={!isEditing} /></div>
            </div>
            ...
            <div className="hidden print:block">
              <table className="w-full table-fixed border-collapse border border-gray-300 print:border-black print-comment-table">
                ...
                <tbody>
                  <tr>
                    <td className="p-2 align-top border border-gray-300 print:border-black"><div className="font-semibold">Low Resistance Ohmmeter:</div><div>{formData.testEquipment.lowResistanceOhmmeter.name}</div></td>
                    <td className="p-2 align-top border border-gray-300 print:border-black"><div className="font-semibold">Serial Number:</div><div>{formData.testEquipment.lowResistanceOhmmeter.serialNumber}</div></td>
                    <td className="p-2 align-top border border-gray-300 print:border-black"><div className="font-semibold">AMP ID:</div><div>{formData.testEquipment.lowResistanceOhmmeter.ampId}</div></td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td className="p-2 align-top border border-gray-300 print:border-black"><div className="font-semibold">Hipot:</div><div>{formData.testEquipment.hipot.name}</div></td>
                    <td className="p-2 align-top border border-gray-300 print:border-black"><div className="font-semibold">Serial Number:</div><div>{formData.testEquipment.hipot.serialNumber}</div></td>
                    <td className="p-2 align-top border border-gray-300 print:border-black"><div className="font-semibold">AMP ID:</div><div>{formData.testEquipment.hipot.ampId}</div></td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className={`mb-6 comments-section print:break-inside-avoid ${!formData.comments?.trim() ? 'print:hidden' : ''}`}>
            ...
            {formData.comments?.trim() && (
              <div className="hidden print:block"><table className="w-full table-fixed border-collapse border border-gray-300 print:border-black print-comment-table"><tbody><tr><td className="p-2 align-top border border-gray-300 print:border-black"><div className="mt-0 whitespace-pre-wrap break-words">{formData.comments}</div></td></tr></tbody></table></div>
            )}
          </div>
        </div>
