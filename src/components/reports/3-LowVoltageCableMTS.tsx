  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      textarea {
        ...
        /* Screen: add orange top border to section headers to match other reports */
        h2.section-job-info,
        h2.section-device-data,
        h2.section-visual-mechanical,
        h2.section-fuse-data,
        h2.section-fuse-resistance,
        h2.section-insulation-ratio,
        h2.section-test-equipment,
        h2.section-comments {
      ...
      .job-info-grid input[type="number"] { -moz-appearance: textfield !important; appearance: textfield !important; }
        /* Recreate on-screen orange divider using heading top-border in print */
        .section-divider { display: none !important; }
        h2 + .grid, h2 + .overflow-x-auto, h2 + div { break-inside: avoid !important; }
        /* Add top orange rule via border on the heading to mirror on-screen dividers */
        h2.section-job-info,
        h2.section-device-data,
        h2.section-visual-mechanical,
        h2.section-fuse-data,
        ...
        /* Force horizontal compact job info layout in print */
        .job-info-grid { grid-template-columns: repeat(6, minmax(0,1fr)) !important; gap: 6px 8px !important; }
        .job-info-grid label { font-size: 8px !important; }
        .job-info-grid input { height: 14px !important; padding: 1px 2px !important; font-size: 8px !important; }
        ...
        /* Column widths for perfect alignment (percent-based, match colgroup) */
        /* Compact From/To widths in print to fit all columns on one page */
        ...
        /* Header grouping and box sizing to avoid drift */
        ...
        /* Size (2 columns) */
        table.electrical-tests-table thead tr:last-child th:nth-child(3),
        table.electrical-tests-table thead tr:last-child th:nth-child(4),
        table.electrical-tests-table td:nth-child(3),
        table.electrical-tests-table td:nth-child(4) { width: 5% !important; min-width: 5% !important; max-width: 5% !important; }
        /* RDG / 20C */
        table.electrical-tests-table thead tr:last-child th:nth-child(5),
        table.electrical-tests-table td:nth-child(5) { width: 4% !important; min-width: 4% !important; max-width: 4% !important; }
        /* Reading columns (first nine) */
        table.electrical-tests-table thead tr:last-child th:nth-child(n+6):nth-child(-n+14),
        table.electrical-tests-table td:nth-child(n+6):nth-child(-n+14) { width: 5.85% !important; min-width: 5.85% !important; max-width: 5.85% !important; }
        /* Last reading column (C-N) a touch narrower to avoid overlap */
        table.electrical-tests-table thead tr:last-child th:nth-child(15),
        table.electrical-tests-table td:nth-child(15) { width: 5.55% !important; min-width: 5.55% !important; max-width: 5.55% !important; }
        /* Cont. and Results */
        table.electrical-tests-table thead tr:first-child th:nth-child(16),
        table.electrical-tests-table thead tr:first-child th:nth-child(17),
        table.electrical-tests-table td:nth-child(16) { width: 5.25% !important; }
        table.electrical-tests-table td:nth-child(17) { width: 7.6% !important; }

        /* FINAL width lock via colgroup for absolute accuracy */
        ...
        /* Ensure our explicit divider bars render in print (brand orange) */
        .section-divider {
          ...
          <h2 className="section-job-info text-xl font-semibold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2 print:text-black print:border-black print:font-bold">Job Information</h2>
            ...
            <div className="job-info-grid grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-x-4 gap-y-2">
              ...
              <div>
                ...
                <input id="address" name="address" type="text" value={formData.address} onChange={handleChange} readOnly className={`form-input text-sm bg-gray-100 dark:bg-dark-200 cursor-not-allowed`} />
              </div>
              <div>
                <label htmlFor="jobNumber" className="form-label">Job #:</label>
                <input id="jobNumber" name="jobNumber" type="text" value={formData.jobNumber} onChange={handleChange} readOnly className={`form-input text-sm bg-gray-100 dark:bg-dark-200 cursor-not-allowed`} />
              </div>
              <div>
                <label htmlFor="date" className="form-label">Date:</label>
                <input id="date" name="date" type="date" value={formData.date} onChange={handleChange} readOnly={!isEditMode} className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} />
              </div>
              <div>
                <label htmlFor="technicians" className="form-label">Technicians:</label>
                <input id="technicians" name="technicians" type="text" value={formData.technicians} onChange={handleChange} readOnly={!isEditMode} className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} />
              </div>
              <div>
                <label htmlFor="identifier" className="form-label">Identifier:</label>
                <input id="identifier" name="identifier" type="text" value={formData.identifier} onChange={handleChange} readOnly={!isEditMode} className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} />
              </div>
              <div className="grid grid-cols-2 gap-x-2">
                <div>
                  <label htmlFor="temperature" className="form-label">Temp (°F):</label>
                  <input id="temperature" name="temperature" type="number" value={formData.temperature} onChange={handleChange} readOnly={!isEditMode} className={`form-input text-sm w-full temp-input-f ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} />
                </div>
                <div>
                  <label className="form-label">Temp (°C):</label>
                  <input type="number" value={Number.isFinite(celsiusTemperature) ? Number(celsiusTemperature.toFixed(0)) : 0} readOnly className="form-input text-sm w-full bg-gray-100 dark:bg-dark-200 temp-input-c" />
                </div>
              </div>
              ...
              <div>
                ...
                <input id="humidity" name="humidity" type="number" value={formData.humidity} onChange={handleChange} readOnly={!isEditMode} className={`form-input text-sm w-full ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} />
              </div>
              <div>
                <label htmlFor="user" className="form-label">User:</label>
                <input id="user" name="user" type="text" value={formData.user} onChange={handleChange} readOnly={!isEditMode} className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} />
              </div>
              <div>
                <label htmlFor="substation" className="form-label">Substation:</label>
                <input id="substation" name="substation" type="text" value={formData.substation} onChange={handleChange} readOnly={!isEditMode} className={`form-input text-sm ${!isEditMode ? 'bg-gray-100 dark:bg-dark-200' : ''}`} />
              </div>
              <div>
            ...
            <table className="w-full border-collapse text-sm electrical-tests-table">
                ...
                {formData.testSets.map((set, index) => (
                    ...
                    {/* Second row for 20°C corrected values */}
                      <tr className="bg-gray-50 dark:bg-dark-200 hover:bg-gray-100 dark:hover:bg-dark-300 align-middle">
