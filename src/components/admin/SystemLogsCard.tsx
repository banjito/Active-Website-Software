...
function getIcon(kind: LogKind) {
  switch (kind) {
    ...
    case 'system':
      return <Settings className="h-4 w-4 text-orange-600 dark:text-orange-300" />;
    case 'permission':
      return <Shield className="h-4 w-4 text-emerald-600 dark:text-emerald-300" />;
  }
}
...
function mapSystemChange(row: any): SystemLogItem {
  return {
    ...
    message: row.details ? JSON.stringify(row.details) : 'System setting changed',
    timestamp: row.timestamp || row.created_at,
...
export const SystemLogsCard: React.FC = () => {
  ...
  const fetchLogs = async () => {
    ...
    const sources = await Promise.allSettled([
      ...
      supabase
        ...
        .select('id,action,resource,permission_action,component,timestamp,created_at')
        .order('timestamp', { ascending: false })
        .limit(20),
    ]);
    ...
    const mappers = [mapNotification, mapRoleChange, mapSystemChange, mapPermissionChange];

    sources.forEach((result, index) => {
      if (result.status === 'rejected') {
        nextErrors.push({ source: sourceNames[index], message: getErrorMessage(result.reason) });
        return;
      }

      const { data, error } = result.value;
      if (error) {
        nextErrors.push({ source: sourceNames[index], message: getErrorMessage(error) });
        return;
      }
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs.filter((log) => {
      ...
      const matchesSearch =
        !normalizedSearch ||
        `${log.title} ${log.message} ${log.source} ${log.badge}`.toLowerCase().includes(normalizedSearch);

      return matchesFilter && matchesSearch;
    });
  }, [activeFilter, logs, searchQuery]);

  const unreadCount = logs.filter((log) => log.kind === 'notification' && !log.read).length;
        ...
        {loading ? (
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <div className="rounded-md border border-gray-200 p-4 dark:border-dark-300">
                <p className="text-xs font-medium uppercase text-gray-500 dark:text-gray-300">Events</p>
                <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-white">{logs.length}</p>
              </div>
              <div className="rounded-md border border-gray-200 p-4 dark:border-dark-300">
                <p className="text-xs font-medium uppercase text-gray-500 dark:text-gray-300">Unread</p>
                <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-white">{unreadCount}</p>
              </div>
              <div className="rounded-md border border-gray-200 p-4 dark:border-dark-300">
                <p className="text-xs font-medium uppercase text-gray-500 dark:text-gray-300">Sources</p>
                <p className="mt-1 flex items-center text-2xl font-bold text-gray-900 dark:text-white">
            ...
            <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                  ...
                  <button
                    ...
                    className={`rounded-md border px-3 py-2 text-sm font-medium ${
                      activeFilter === option.value
                        ...
                        : 'border-gray-200 bg-white text-gray-700 hover:bg-gray-50 dark:border-dark-300 dark:bg-dark-200 dark:text-gray-200 dark:hover:bg-dark-300'
                    }`}
                  >
                    {option.label}
                  </button>
              ...
              <label className="flex min-w-0 items-center gap-2 rounded-md border border-gray-200 px-3 py-2 dark:border-dark-300">
            ...
            {filteredLogs.length > 0 ? (
              <div className="overflow-hidden rounded-md border border-gray-200 dark:border-dark-300">
                {filteredLogs.map((log) => (
                  <div
                    key={log.id}
                    className="flex flex-col gap-3 border-b border-gray-100 p-4 last:border-0 dark:border-dark-300 md:flex-row md:items-start md:justify-between"
                  >
                    <div className="flex min-w-0 gap-3">
                      <div className="mt-1 rounded-md bg-gray-100 p-2 dark:bg-dark-300">{getIcon(log.kind)}</div>
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <p className="font-medium text-gray-900 dark:text-white">{log.title}</p>
                          <span className={`rounded-full border px-2 py-0.5 text-xs font-medium ${getBadgeClass(log.kind)}`}>
                            {log.badge}
                          </span>
                        </div>
                        <p className="mt-1 break-words text-sm text-gray-700 dark:text-gray-200">{log.message}</p>
                        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{log.source}</p>
                      </div>
                    </div>
                    <div className="flex shrink-0 items-center gap-1 text-sm text-gray-500 dark:text-gray-300">
              ...
              <div className="flex flex-col items-center justify-center rounded-md border border-dashed border-gray-300 p-8 text-center dark:border-dark-300">
                <Filter className="h-8 w-8 text-gray-400" />
                <p className="mt-2 font-medium text-gray-900 dark:text-white">No logs found</p>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">
                  Try another filter, or run the admin log setup SQL if sources are missing.
                </p>
              </div>
            )}
          </div>
