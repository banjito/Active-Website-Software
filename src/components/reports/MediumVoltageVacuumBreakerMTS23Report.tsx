...
const contactTimingUnits = ["ms", "sec."];
...
interface FormData {
  ...
  // Contact Timing (7.6.3: open and close speed per pole)
  contactTiming: {
    openSpeed: { p1: string; p2: string; p3: string; units: string };
    closeSpeed: { p1: string; p2: string; p3: string; units: string };
  };
...
const visualInspectionItemsList = [
  ...
  {
    id: "7.6.3.A.4",
    description: "*Perform operator analysis (first-trip) test.",
  },
  {
    id: "7.6.3.A.5",
...
const MediumVoltageVacuumBreakerMTS23Report: React.FC = () => {
  ...
  // browser so two saves racing each other upsert the same row instead of
  // inserting a second copy of the breaker onto the job.
  const reportIdRef = React.useRef<string | undefined>(
    reportId || reportIdFromUrl(),
  );
  // The asset name the job list was last told. Auto-save creates the asset on
  // the first keystroke, before the identifier is typed, so the name is
  // re-sent whenever the identifier changes rather than only once.
  ...
  const reportSlug = "medium-voltage-vacuum-breaker-mts23";
  ...
  const initialFormData: FormData = {
    ...
    contactTiming: {
      openSpeed: { p1: "", p2: "", p3: "", units: "ms" },
      closeSpeed: { p1: "", p2: "", p3: "", units: "ms" },
    },
    eGapMeasurements: {
      unitMeasurement: "",
      tolerance: "",
  ...
  const equipmentAssetIdRef = React.useRef<string | null | undefined>(
    equipmentAssetId,
  );
  useEffect(() => {
    ...
    const loadReportData = async () => {
      ...
      try {
        const { data, error } = await supabase
          .schema("neta_ops")
          .from("medium_voltage_vacuum_breaker_mts23_reports")
          .select("*")
          .eq("id", reportId)
          .single();
        ...
        if (data) {
          console.log(
            "🔍 MediumVoltageVacuumBreakerMTS23Report - Loading report data:",
          );
    ...
    const calculateCorrected = () => {
      ...
      for (const key in measured) {
        if (key !== "testVoltage" && key !== "units") {
          ...
          if (valueStr && !isNaN(parseFloat(valueStr.replace(/,/g, "")))) {
            const [int, dec] = (parseFloat(valueStr.replace(/,/g, "")) * tcf)
              .toFixed(2)
              .split(".");
            (corrected as any)[key] =
  ...
  const calculateCorrectedValue = (value: string, tcf: number): string => {
    if (value && !isNaN(parseFloat(value.replace(/,/g, "")))) {
      const [int, dec] = (parseFloat(value.replace(/,/g, "")) * tcf)
        .toFixed(2)
        .split(".");
  ...
  /**
   * Writes the report and returns its id. The Save button and auto-save both go
   * through here, and both write to the same row: the id is claimed up front and
   * kept in `reportIdRef`, so a save that starts while another is in flight
   * upserts over that row rather than inserting another copy.
   */
  const persistReport = React.useCallback(async () => {
    ...
    const rowId = reportIdRef.current as string;
    ...
    if (linkedAssetNameRef.current !== assetName) {
      const wasLinked = linkedAssetNameRef.current !== undefined;
      const reportAssetId = await ensureReportAssetLink(
        jobId,
        {
          name: assetName,
          file_url: `report:/jobs/${jobId}/${reportSlug}/${rowId}`,
          user_id: user.id,
        },
        user.id,
      );
      if (!wasLinked && equipmentAssetIdRef.current) {
        await setReportAssetEquipmentLink(
          reportAssetId,
          equipmentAssetIdRef.current,
        );
      }
    ...
    if (isNewReport) {
      setCurrentReportId(rowId);
      window.history.replaceState(
        {},
        "",
        `/jobs/${jobId}/${reportSlug}/${rowId}`,
      );
    }
    ...
    <ReportWrapper isPrintMode={isPrintMode}>
      ...
      <div className="p-6 flex justify-center">
        <div id="mvcb-mts-root" className="max-w-7xl w-full space-y-6">
          ...
          {/* Contact Timing — new on the MTS 23 sheet */}
          <section className="mb-6 section-contact-timing">
            <div className="w-full h-1 bg-brand mb-4"></div>
            <h2 className="text-xl font-semibold mb-4 text-neutral-900 dark:text-white border-b dark:border-neutral-700 pb-2 print:text-black print:border-black print:font-bold">
              Contact Timing Test
            </h2>
              ...
              <table className="w-full table-fixed border-collapse border border-neutral-200 dark:border-neutral-700">
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="px-3 py-2 border border-neutral-200 dark:border-neutral-700">
                      {renderInput("contactTiming.openSpeed.p1", "", "text", false, "w-full")}
                    </td>
                    <td className="px-3 py-2 border border-neutral-200 dark:border-neutral-700">
                      {renderInput("contactTiming.openSpeed.p2", "", "text", false, "w-full")}
                    </td>
                    <td className="px-3 py-2 border border-neutral-200 dark:border-neutral-700">
                      {renderInput("contactTiming.openSpeed.p3", "", "text", false, "w-full")}
                    </td>
                    <td className="px-3 py-2 border border-neutral-200 dark:border-neutral-700">
                      {renderSelect("contactTiming.openSpeed.units", contactTimingUnits)}
                    </td>
                  </tr>
                  <tr>
                    <td className="px-3 py-2 text-sm text-neutral-900 dark:text-white border border-neutral-200 dark:border-neutral-700">
                    ...
                    <td className="px-3 py-2 border border-neutral-200 dark:border-neutral-700">
                      {renderInput("contactTiming.closeSpeed.p1", "", "text", false, "w-full")}
                    </td>
                    <td className="px-3 py-2 border border-neutral-200 dark:border-neutral-700">
                      {renderInput("contactTiming.closeSpeed.p2", "", "text", false, "w-full")}
                    </td>
                    <td className="px-3 py-2 border border-neutral-200 dark:border-neutral-700">
                      {renderInput("contactTiming.closeSpeed.p3", "", "text", false, "w-full")}
                    </td>
                    <td className="px-3 py-2 border border-neutral-200 dark:border-neutral-700">
                      {renderSelect("contactTiming.closeSpeed.units", contactTimingUnits)}
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-4 print:hidden">
                  <div>
                    ...
                    {renderInput(
                      "dielectricWithstand.closed.testVoltage",
                      "",
                      "text",
                      false,
                      "w-full",
                  ...
                  <div>
                    ...
                    {renderInput(
                      "dielectricWithstand.closed.testDuration",
                      "",
                      "text",
                      false,
                      "w-full",
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-4 print:hidden">
                  <div>
                    ...
                    {renderInput(
                      "dielectricWithstand.open.testVoltage",
                      "",
                      "text",
                      false,
                      "w-full",
                  ...
                  <div>
                    ...
                    {renderInput(
                      "dielectricWithstand.open.testDuration",
                      "",
                      "text",
                      false,
                      "w-full",
          ...
          <section className="mb-6">
            ...
            <div className="print:hidden test-eqpt-onscreen">
              ...
              <div
                ...
                style={{
                  gridTemplateColumns:
                    "minmax(0, auto) minmax(140px, 1fr) minmax(70px, 0.9fr) minmax(64px, 0.7fr) minmax(72px, 0.8fr)",
                }}
              >
                ...
                <div className="min-w-0">
                  <EquipmentAutocomplete
                    value={
                      (formData.testEquipment?.megohmmeter?.model as string) ||
                      (formData.testEquipment?.megohmmeter as any)
                        ?.equipment_name ||
                      ""
                    }
                    onChange={(value) =>
                      handleInputChange({
                        target: {
                          name: "testEquipment.megohmmeter.model",
                          value,
                        },
                      } as any)
                    }
                  ...
                  <EquipmentAutocomplete
                    value={
                      (formData.testEquipment?.lowResistanceOhmmeter
                        ?.model as string) ||
                      (formData.testEquipment?.lowResistanceOhmmeter as any)
                        ?.equipment_name ||
                      ""
                    }
                    onChange={(value) =>
                      handleInputChange({
                        target: {
                          name: "testEquipment.lowResistanceOhmmeter.model",
                          value,
                        },
                      } as any)
                    }
                ...
                <div className="min-w-0">
                  <EquipmentAutocomplete
                    value={
                      (formData.testEquipment?.hipot?.model as string) ||
                      (formData.testEquipment?.hipot as any)?.equipment_name ||
                      ""
                    }
                    onChange={(value) =>
                      handleInputChange({
                        target: { name: "testEquipment.hipot.model", value },
                      } as any)
                    }
                    onSelect={(equipment) => {
                      setFormData((prev) => ({
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="p-2 border border-neutral-300 print:border-black">
                      ...
                      <div className="text-xs">
                        {formData.testEquipment.lowResistanceOhmmeter.model ||
                          ""}
                      </div>
                      <div className="text-xs">
                        S/N:{" "}
                    ...
                    <td className="p-2 border border-neutral-300 print:border-black">
                      ...
                      <div className="text-xs">
                        S/N: {formData.testEquipment.hipot.serialNumber || ""}
                      </div>
                      <div className="text-xs">
                        AMP ID: {formData.testEquipment.hipot.ampId || ""}
                      </div>
                      <div className="text-xs">
                        Cal Date: {formData.testEquipment.hipot.calDate || ""}
                      </div>
                    </td>
                  </tr>
                </tbody>
        ...
        <div className="mb-6 print:hidden flex justify-center">
          <button
            onClick={async () => {
              ...
              try {
                // Save the report first, and use the id that save settled on.
  ...
  style.textContent = `
    @media print {
      ...
      *:not(table):not(thead):not(tbody):not(tr):not(th):not(td) { border: none !important; }
      ...
      .bg-white, .dark\:bg-dark-150, .rounded-none, .shadow, .border, .p-6, .space-y-6 {
        background: transparent !important;
...
export default MediumVoltageVacuumBreakerMTS23Report;
