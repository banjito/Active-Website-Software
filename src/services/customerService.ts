...
export async function getCustomers(
  ...
  try {
    ...
    // Apply "starts with" filter for first letter(s) search
    if (filters?.startsWith) {
      const startsWithLike = `${filters.startsWith}%`;
      countQuery = countQuery.ilike('company_name', startsWithLike);
    }
    ...
    if (filters?.startsWith) {
      const startsWithLike = `${filters.startsWith}%`;
      query = query.ilike('company_name', startsWithLike);
    }
    ...
    console.log(`Retrieved ${data?.length || 0} customers from common.customers (total: ${totalCount})`);
